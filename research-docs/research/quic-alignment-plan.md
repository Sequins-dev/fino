...
- Bind `ngtcp2_conn_in_closing_period` / `ngtcp2_conn_in_draining_period` in `bindings.mts`.

#### P0-2. Client first-flight version + incompatible VN handling (RFC 9000 §6 / RFC 9368)
**Problem:** `normalizeVersions` defaults `['v2','v1']` and `selectWireVersion` picks the first (endpoint.mts:1292, 1642-1648) → first flight is v2. Node defaults the wire version to v1 and lets compatible VN upgrade to v2. Worse, `NGTCP2_ERR_RECV_VERSION_NEGOTIATION` (-222) is not in `bindings.mts`, so a client receiving a Version Negotiation packet falls into the generic `rc !== 0` branch (endpoint.mts:4999-5004) and emits a bogus CONNECTION_CLOSE + error instead of retrying or failing cleanly.
...
- Add `NGTCP2_ERR_RECV_VERSION_NEGOTIATION = -222` to bindings. In `_receivePacket`: on this code do **not** write CONNECTION_CLOSE; if the VN packet (already captured via `CB_RECV_VERSION_NEGOTIATION`, endpoint.mts:5491-5511) lists a mutually supported version and we haven't retried yet, recreate the native conn via `#createNative` with the new version and write `SETTINGS_ORIGINAL_VERSION` (offset exists at bindings.mts:373, never written) with the originally attempted version. Cap at one retry (RFC 9368 §2.3); otherwise fail with a typed `QuicVersionNegotiationError`.

#### P0-3. Hygiene: unbounded `#rejectedInitialCids`
Entries only added (endpoint.mts:3608) / checked (3252), cleared only on endpoint close. Convert to a size-capped LRU (reuse the `QuicAddressValidationCache` shape) or timestamp-pruned set.
...
- `js/internal/openssl.mts`: add `SSL_VERIFY_FAIL_IF_NO_PEER_CERT`, `sslGetPeerCertificate(ssl)` (SSL_get1_peer_certificate → i2d_X509 DER → X509_free); `sslGetVerifyResult` already exists (openssl.mts:2589).
- `crypto.mts`: `newServerContext(..., tlsOptions?: { verifyClient?: boolean; rejectUnauthorized?: boolean; ca? })`. With `rejectUnauthorized: false`, complete the handshake and surface the verify result instead of failing (Node tlscontext.h:204-210).
- `crypto-gnutls.mts` (straightforward subset): `gnutls_certificate_server_set_request` (REQUEST/REQUIRE), `gnutls_certificate_verify_peers3`, `gnutls_certificate_get_peers`.
- `endpoint.mts`: `QuicListenOptions += { verifyClient?, rejectUnauthorized?, ca? }`; on `QuicConnection`: `peerCertificate: Uint8Array | null` (DER) and `peerVerification: { verified, errorCode, reason } | null`, populated at `_onHandshakeCompleted` (~4679) and added to the `quic.session.handshake` topic.
...
- OpenSSL: `sslCtxLoadVerifyLocations` (exists unused, openssl.mts:2643) for file/dir; add `sslCtxAddCaCertificates` (BIO_new_mem_buf + PEM_read_bio_X509 + X509_STORE_add_cert) for in-memory PEM. When `ca` given, skip `sslCtxSetDefaultVerifyPaths` (pinned-roots semantics, same as Node). GnuTLS: `gnutls_certificate_set_x509_trust_file/_dir/_mem`. CRL deferred — document as divergence.

#### P1-4. Per-SNI server TLS contexts (OpenSSL-only initially)
- `openssl.mts`: `sslCtxSetServernameCallback` (SSL_CTX_callback_ctrl with SSL_CTRL_SET_TLSEXT_SERVERNAME_CB=53/ARG=54) + `sslSetSslCtx` (SSL_set_SSL_CTX); callback reads `SSL_get_servername`, swaps ctx, returns SSL_TLSEXT_ERR_OK/NOACK.
- `crypto.mts`: `setSNIContexts(ctx, entries)`, entries built via `newServerContext`.
- `endpoint.mts`: `QuicListenOptions.sni?: Record<string, { certificateFile, privateKeyFile, alpnProtocols?, verifyClient?, ca? }>` + `QuicListener.setSNIContexts()`; wildcard (`*.example.com`) matching like Node; free per-SNI contexts in `QuicListener.close()` (~3647). Mark OpenSSL-only in the conformance matrix.

### Phase 3 (P1): Option-surface parity (mechanical, independent)

#### P1-5. Configurable transport parameters
Currently hardcoded: `INITIAL_MAX_STREAM_DATA`/`INITIAL_MAX_DATA`/`INITIAL_MAX_STREAMS_BIDI/UNI`, `MAX_IDLE_TIMEOUT=10s`, `ACTIVE_CONNECTION_ID_LIMIT=2` (endpoint.mts:831-838, `makeTransportParams` 2199-2243). Node exposes all (transportparams.h:55-112). Add to `QuicConnectionOptions` (+ resolved options + snapshot): `maxIdleTimeoutMs`, `initialMaxData`, `initialMaxStreamDataBidiLocal/BidiRemote/Uni`, `initialMaxStreamsBidi/Uni`, `activeConnectionIdLimit` (clamp 2..8), `maxAckDelayMs`, `ackDelayExponent`, `disableActiveMigration` (server; currently always written 0 at 2213).

#### P1-6. Preferred address: both families + client policy
`preferredAddress?: { ipv4?: QuicAddress; ipv6?: QuicAddress }` (back-compat: accept bare `QuicAddress`); bind both transports in `listen()` (~2916-2935) and write both TP fields (offsets exist, bindings.mts:380-383). Client: `migration.usePreferredAddress?: boolean` (default false = ignore, Node's default) checked in `_selectPreferredAddress` (~5603).

#### P1-7. Endpoint UDP socket options
`QuicEndpointOptions.socket?: { ipv6Only?, reusePort?, receiveBufferSize?, sendBufferSize?, ttl? }` via `setsockopt` in `js/net/socket.mts` (IPV6_V6ONLY, SO_REUSEPORT, SO_RCVBUF, SO_SNDBUF, IP_TTL/IPV6_UNICAST_HOPS). Node parity: endpoint.h:135-152.

### Phase 4 (P2): Performance parity — prove with benchmarks before/after

#### P2-1. Send-path batching (sendmmsg/recvmmsg)
Today: one `sendto` + fresh `out.slice(0, n)` copy per packet (endpoint.mts:4294-4299; budget = min(quantum/maxPayload, 32)). Node batches via `uv_udp_try_send2`/sendmmsg with async fallback (endpoint.cc:615-631, 1020-1063).
- Extend `QuicDatagramTransport` with optional `sendBatch(packets)` / `recvBatch(max)`.
- `js/net/socket.mts`: bind `sendmmsg`/`recvmmsg` on Linux (mmsghdr/iovec/msghdr layouts in JS ArrayBuffers, same style as existing sockaddr encoding); Darwin fallback loops `sendto` inside `sendBatch` (still amortizes buffer prep).
...
- Read side: raise `MAX_READ_PACKETS_PER_TURN` (5, endpoint.mts:846) when recvmmsg available; one cached `now()` per burst (Node session.cc:2299).
...
- Recv: `setsockopt(IP_RECVTOS/IPV6_RECVTCLASS)`, switch to `recvmsg` + cmsg parse, populate a real `ngtcp2_pkt_info` buffer instead of `null` (endpoint.mts:4986 etc.).
...
- GnuTLS keylog (`gnutls_session_set_keylog_function`) + cipher info so `getHandshakeInfo` stops returning nulls on that backend (crypto.mts:261-268).
...
- `tests/net/fixtures/quic/packet-craft.mts` — extract VN/Initial/short-header builders currently inlined in quic.test.mts.
- `tests/net/fixtures/quic/packet-parse.mts` — minimal header parser (form, version, DCID) for `pipe.trace()` assertions.
- `tests/net/fixtures/quic/interop-harness.mts` — extract spawn/ready/collect plumbing from quic-node-interop.test.mts; scenario-parameterize `quic-node-server.mjs`/`quic-node-client.mjs`/`quic-node-fino-*.mts` (`--scenario=echo|datagram|resume|keyupdate|large|retry|alpn-mismatch`).
- `sim-harness.mts`: add `rawDatagramSocket()` + trace packet-count helpers.
...
```
tests/net/
  quic.test.mts               # extend close-code + dispose/signal tests only
  quic-streams.test.mts       # NEW: stream state machine + flow control
  quic-sim.test.mts           # stays the RFC 9000 core sim suite
  quic-sim-recovery.test.mts  # NEW: RFC 9002 loss/cc/persistent-congestion/ack-threshold
  quic-raw-packets.test.mts   # NEW: crafted packets — client VN, downgrade, expired Retry token, fuzz
  quic-node-interop.test.mts  # expanded: one it() per scenario
  fixtures/quic/{packet-craft,packet-parse,interop-harness}.mts  # NEW
  fixtures/quic/spec-fixtures.json  # evidence entries for every new Direct row
```

### B6. Conformance matrix updates
The meta-test in quic.test.mts already enforces that every `Direct` row has an `evidence` entry in `spec-fixtures.json` pointing at an existing `it(...)`. For each change: edit matrix + fixtures + test **together**.
...
2. **Phase 2** close API (P1-1, clean break incl. call-site migration) → mTLS (P1-2) → CA (P1-3) → SNI (P1-4), each layering on the previous `openssl.mts`/`crypto.mts` work, tests in the same change.
...
## Key files
- `js/internal/net/quic/endpoint.mts` — most implementation work
- `js/internal/net/quic/ngtcp2/bindings.mts` — new constants/bindings (ERR_RECV_VERSION_NEGOTIATION, in_closing/draining_period, ccerr access)
- `js/internal/net/quic/ngtcp2/crypto.mts`, `crypto-gnutls.mts`, `js/internal/openssl.mts` — TLS features
- `js/net/socket.mts` — setsockopt, sendmmsg/recvmmsg, recvmsg/sendmsg+cmsg
- `js/internal/net/simulated-provider.mts` — ECN modeling
