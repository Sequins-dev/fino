...
export function createWebAgentChatApp(options: WebAgentChatOptions): App {
  ...
  const chat = view({
    ...
    embed: ['draft'],
    actions: { send: { async handler({ state, checkpoint }, input) {
      ...
      for await (const event of stream.reader) {
        ...
        state.messages.set((current) => {
          ...
          next[next.length - 1] = {
            ...last,
            text: last.text + event.event.text
          };
          return next;
        });
        await checkpoint();
    ...
    render({ state, actions }) {
      return h('main', null, h('h1', null, 'Fino agent chat'), h('ol', null, state.messages.get().map((message) => h('li', { class: message.role }, `${message.role}: ${message.text}`))), h('form', { action: actions.send }, h('input', {
        name: 'prompt',
  ...
  const app = new App();
  const routes = app.value('cookies', cookies()).value('session', sessions({
  ...
  routes.get('/').handle(page((ctx) => h('html', null, h('head', null, h('title', null, 'Fino agent chat')), h('body', null, chat.mount(ctx), h('script', {
