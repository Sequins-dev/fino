...
import { memoryStore, type AtomicExpiringStore, type AtomicStore } from 'fino:store';
...
export interface WebAgentChatOptions {
  ...
  sessionStore?: AtomicExpiringStore;
}
/**
...
export function createWebAgentChatApp(options: WebAgentChatOptions): App {
    ...
    render({ state, actions }) {
      return h(
        ...
        h(
          ...
          state.messages
            .get()
            .map((message) => h('li', { class: message.role }, `${message.role}: ${message.text}`)),
        ),
        h(
          'form',
          { action: actions.send },
          ...
          h(
            'button',
            { disabled: state.running.get() },
            state.running.get() ? 'Thinking…' : 'Send',
          ),
        ),
      );
    },
  ...
  const routes = app
    ...
    .value(
      'session',
      sessions({
        store: options.sessionStore ?? memoryStore({ namespace: 'web-agent-chat-sessions' }),
        keys: [
          {
            id: 'current',
