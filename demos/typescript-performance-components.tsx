...
export function Panel(props: Children & {
  ...
  return <div style={{
    borderTop: `.45em solid ${color}`,
    background: '#ffffffa6',
    padding: '1.15em 1.3em',
    minWidth: 0,
    boxShadow: '0 1em 2.5em #0d1b1e12'
  }}>
    <div style={{
    color,
    fontSize: '.72em',
...
export function Layer(props: Children & {
  label: string;
  detail: string;
  tone?: Tone;
  width?: string;
  ...
  return <div style={{
    display: 'grid',
    gridTemplateColumns: props.width ?? '12em 1fr',
    alignItems: 'center',
    minHeight: '6.2em',
    background: props.tone === 'ink' ? ink : '#fff9',
    borderLeft: `.7em solid ${color}`,
    color: props.tone === 'ink' ? paper : ink,
    padding: '1em 1.35em',
    width: '100%'
  }}>
    <div style={{
    color,
    font: '900 .78em/1 "SFMono-Regular",monospace',
    letterSpacing: '.15em',
    textTransform: 'uppercase'
  }}>{props.label}</div>
    <div><div style={{ font: '800 1.5em/1.05 "Iowan Old Style",serif' }}>{props.children ?? []}</div><div style={{
    color: props.tone === 'ink' ? '#b8c4c5' : muted,
    fontSize: '.9em',
    marginTop: '.45em'
...
export function FlowNode(props: {
  ...
  return <div style={{
    ...
    borderBottom: `.4em solid ${color}`
  }}>
    <div style={{
    color,
    font: '900 .7em/1 "SFMono-Regular",monospace',
    letterSpacing: '.14em'
  }}>{props.number}</div>
    <div style={{
    font: '800 1.35em/1.08 "Iowan Old Style",serif',
    margin: '.65em 0 .45em'
  }}>{props.title}</div>
    <div style={{
    color: '#bdc8ca',
...
export function Metric(props: {
  ...
  return <div style={{
    ...
    <div style={{
    color,
    font: '800 4em/.9 "Iowan Old Style",serif',
    letterSpacing: '-.055em'
  }}>{props.value}</div>
    <div><div style={{
    fontSize: '1.05em',
    fontWeight: 900
  }}>{props.label}</div>{props.note ? <div style={{
    color: '#aebbbc',
    fontSize: '.78em',
    marginTop: '.3em',
    lineHeight: 1.3
  }}>{props.note}</div> : null}</div>
  </div>;
}
export function Evidence(props: {
  ...
  return <div style={{
    ...
    color: paper,
    padding: '1.15em 1.3em',
    borderLeft: `.55em solid ${color}`
  }}>
    <div style={{
    color,
    font: '800 3em/.9 "Iowan Old Style",serif',
    letterSpacing: '-.05em'
  }}>{props.value}</div>
    <div><div style={{
    fontSize: '1.15em',
    fontWeight: 900
  }}>{props.title}</div><div style={{
    color: '#afbdbe',
    fontSize: '.82em',
    lineHeight: 1.35,
    marginTop: '.35em'
  }}><span style={{ color: coral }}>{props.before}</span> → <span style={{ color: cyan }}>{props.after}</span></div></div>
  </div>;
}
export function Callout(props: Children & {
  ...
  return <div style={{
    ...
    font: '900 .66em/1 "SFMono-Regular",monospace',
    letterSpacing: '.12em',
    textTransform: 'uppercase'
...
export function CodeLabel(props: Children): VNode {
  return <div style={{
    ...
    padding: '.55em .8em',
    width: 'fit-content',
    font: '900 .65em/1 "SFMono-Regular",monospace',
    letterSpacing: '.12em',
    textTransform: 'uppercase'
  }}>{props.children ?? []}</div>;
}
export function Decision(props: {
  ...
  return <div style={{
    ...
    gap: '.8em',
    borderTop: `.125em solid ${color}`,
    padding: '.75em .1em',
    fontSize: '1em',
    lineHeight: 1.3
  }}><div style={{ color: muted }}>{props.when}</div><div style={{
    color,
    fontWeight: 900
  }}>{props.choose}</div><div>{props.reason}</div></div>;
}
