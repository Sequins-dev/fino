...
const marker = /(?:readiness timeout trace: |# readiness trace: )(\{.*)/;
const line = text.split('\n').findLast((line) => marker.test(line));
const snapshot = JSON.parse(line ? marker.exec(line)![1]! : text);
...
console.log(
  JSON.stringify(
    {
      ...analyzeReadiness(snapshot),
      nativeWork: snapshot.nativeWork,
      wakeSources: snapshot.wakeSources,
      wakeSourcesDropped: snapshot.wakeSourcesDropped,
      pool: snapshot.pool,
      realms,
    },
    null,
