...
type ValidatorFn = (value: unknown, path: string) => {
  value: unknown;
  issues: ValidationIssue[];
};
type Refinement = {
...
function compileSchema(schema: JsonSchema): ValidatorFn {
  ...
  const prefixSchemas = Array.isArray(schema.prefixItems) ? schema.prefixItems.map((item) => compileSchema(schemaOf(item))) : Array.isArray(schema.items) ? schema.items.map((item) => compileSchema(schemaOf(item))) : null;
  const additionalPropertyValidator = isRecord(schema.additionalProperties) ? compileSchema(schema.additionalProperties) : null;
...
export class SchemaBuilder<T = unknown> {
  ...
  */
  constructor(schema: JsonSchema, optional = false) {
    this.schema = schema;
    if (optional) Object.defineProperty(this.schema, optionalSymbol, {
      value: true,
      configurable: true
    });
  }
