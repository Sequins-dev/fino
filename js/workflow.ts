...
export class InMemoryWorkflowStore implements WorkflowStore {
  ...
  */
  async list(filter: {
    ...
    const runs = [...this.#runs.values()].filter((state) => filter.workflowId === undefined || state.workflowId === filter.workflowId).filter((state) => filter.status === undefined || state.status === filter.status).sort((a, b) => b.updatedAt - a.updatedAt);
...
> {
  ...
  constructor(workflow: Workflow<In, Out>, store: WorkflowStore, state: WorkflowState, signal: AbortSignal | undefined, checkpoint: (state: WorkflowState) => Promise<void>) {
    this.#workflow = workflow;
    this.#store = store;
    this.#state = state;
    this.#signal = signal;
  ...
  async #call<
    In2,
    Out2
  >(activity: Activity<In2, Out2>, input: In2, retry?: WorkflowRetryOptions): Promise<Out2> {
  ...
  async #sleep(id: string, duration: number | string | Date): Promise<void> {
    ...
    const dueAt = existing?.type === 'timer' && existing.stepIndex === index ? existing.dueAt : toDueAt(duration);
    if (Date.now() < dueAt) {
      this.#state = {
        ...this.#state,
        status: 'waiting',
  ...
  async #waitForSignal<T>(name: string, opts?: {
    timeout?: number | string | Date;
  }): Promise<T> {
    const index = this.#callIndex++;
    const completed = this.#state.steps[index];
...
function toDueAt(value: number | string | Date): number {
  ...
  const unit = match[2];
  const scale = unit === 'ms' ? 1 : unit === 's' ? 1e3 : unit === 'm' ? 6e4 : unit === 'h' ? 36e5 : 864e5;
