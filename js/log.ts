...
export class Logger {
  ...
  */
  child(options: Fields | (Partial<LoggerOptions> & {
    ...
    const childName = typeof childOptions.name === 'string' && childOptions.name.length > 0 ? `${this.#name}.${childOptions.name}` : this.#name;
    delete childOptions.name;
    const childLevel = typeof childOptions.level === 'string' ? normalizeLevel(childOptions.level) : this.#level;
    delete childOptions.level;
    const childContext = childOptions.context && typeof childOptions.context === 'object' ? cloneFields(childOptions.context) : childOptions;
    return new Logger({
      name: childName,
      level: childLevel,
      context: {
