...
export class Logger {
  ...
  child(
    ...
    const childName =
      typeof childOptions.name === 'string' && childOptions.name.length > 0
        ? `${this.#name}.${childOptions.name}`
        : this.#name;
    delete childOptions.name;
    const childLevel =
      typeof childOptions.level === 'string' ? normalizeLevel(childOptions.level) : this.#level;
    delete childOptions.level;
    const childContext =
      childOptions.context && typeof childOptions.context === 'object'
...
function formatText(record: LogRecord): string {
  const parts = [record.timestamp, record.level.toUpperCase(), record.logger, record.message];
  const extra = {
    ...record.context,
    ...record.fields,
  };
...
export function createJsonSink(options: WriteSinkOptions = {}) {
  const write =
    options.write ??
    ((line: string, record: LogRecord) => {
      writeLine(
        record.level === 'warn' || record.level === 'error' || record.level === 'fatal' ? 2 : 1,
        line,
      );
    });
