...
const DOCS_CSS = `body{font-family:system-ui,sans-serif;max-width:1120px;margin:40px auto;padding:0 20px;line-height:1.5;color:#1f2328}a{color:#0969da;text-decoration:none}a:hover{text-decoration:underline}main{display:block}pre{background:#f6f8fa;border:1px solid #d0d7de;border-radius:6px;padding:12px;overflow:auto}code{font-family:ui-monospace,Menlo,monospace;white-space:pre-wrap}.tok-keyword{color:#cf222e}.tok-string{color:#0a3069}.tok-number{color:#0550ae}.tok-comment{color:#6e7781}.tok-regexp{color:#8250df}.tok-type{color:#953800}.tag{color:#57606a}.module-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;list-style:none;padding:0}.module-card{border:1px solid #d0d7de;border-radius:6px;padding:12px}.muted{color:#57606a}.member{border-left:3px solid #d0d7de;padding-left:12px}`;
...
const MODULE_PAGE_TEMPLATE = `<p><a href="{{rootHref}}">API Modules</a></p>
<h1 id="{{id}}">{{name}}</h1>
...
const INDEX_PAGE_TEMPLATE = `<h1>API Modules</h1>
<p class="muted">{{moduleCount}} modules documented.</p>
<ul class="module-list">
{{#modules}}
<li class="module-card">
<a href="{{href}}"><strong>{{name}}</strong></a>
<p class="muted">{{exportCount}} exports</p>
{{#summary}}<p>{{{summaryHtml}}}</p>{{/summary}}
</li>
{{/modules}}
</ul>
`;
...
async function extractModuleFromSource(path: string, includePrivate: boolean): Promise<ModuleDoc> {
  ...
  const moduleDoc: ModuleDoc = {
    ...
    doc: firstJsdocBefore(comments, body[0]?.start ?? Number.MAX_SAFE_INTEGER, source) ?? emptyDoc(),
    exports,
  };
...
function collectNamedExport(exports: DocExport[], statement: AstNode, comments: ParseComment[], locals: Map<string, DocExport>, source: string, includePrivate: boolean): void {
  if (statement.declaration) {
    if (statement.declaration.type === 'VariableDeclaration') {
      exports.push(...collectVariableExports(statement.declaration, statement, comments, source, true));
      return;
    }
  ...
  for (const specifier of statement.specifiers ?? []) {
    ...
    item.signature = item.signature.startsWith('export ') ? item.signature : `export ${item.signature}`;
    item.signatures = item.signatures.map((signature) => signature.startsWith('export ') ? signature : `export ${signature}`);
    exports.push(item);
  }
}
...
function collectDeclarationExport(declaration: AstNode | undefined, owner: AstNode, comments: ParseComment[], source: string, includePrivate: boolean, forcedName?: string): DocExport | undefined {
  ...
  if (declaration.type === 'TSEnumDeclaration') {
    return exportDoc(name, 'enum', statementSignature(owner, declaration, source), doc, locationFor(source, owner.start ?? declaration.start));
  }
  return undefined;
}

function collectVariableExports(declaration: AstNode, owner: AstNode, comments: ParseComment[], source: string, exported: boolean): DocExport[] {
  const out: DocExport[] = [];
  ...
  for (const declarator of declaration.declarations ?? []) {
    ...
    const prefix = exported ? `export ${kind} ` : `${kind} `;
    const body = sourceSlice(source, declarator.id?.start ?? declarator.start, end).trim().replace(/=$/, '').trim();
    out.push(exportDoc(name, kind, cleanSignature(prefix + body), doc, locationFor(source, owner.start ?? declaration.start)));
  }
  return out;
}
...
function renderModuleHtml(api: ApiDoc, moduleDoc: ModuleDoc, title: string): string {
  ...
  const contentHtml = renderTemplate(MODULE_PAGE_TEMPLATE, {
    ...htmlModule,
    rootHref: relativeRootHref(htmlModule.href),
  });
...
function renderIndexHtml(api: ApiDoc, title: string): string {
  const modules = api.modules.map((moduleDoc) => toHtmlModule(api, moduleDoc)).sort((a, b) => compareAscii(a.name, b.name));
...
function toHtmlModule(api: ApiDoc, moduleDoc: ModuleDoc): HtmlModule {
  const href = `${moduleDoc.name}.html`;
  const ctx = { api, module: moduleDoc };
...
function symbolHref(symbol: FlatSymbol): string {
  return `${moduleHref(symbol.module)}#${symbolAnchor(symbol)}`;
}
...
function buildLinkResolver(api: ApiDoc, moduleDoc: ModuleDoc): (href: string, label: string) => string | undefined {
  return (href) => {
    ...
    if (!pathPart && fragment) {
      ...
      return symbol ? symbolHref(symbol) : href;
    }
    const targetModule = resolveSourceModule(api, moduleDoc, pathPart);
    if (!targetModule) return href;
    if (!fragment) return moduleHref(targetModule);
    const symbol = findSymbolInModule(api, targetModule, fragment);
    return symbol ? symbolHref(symbol) : `${moduleHref(targetModule)}#${slug(fragment)}`;
  };
}
...
function signatureIdentifierHref(name: string, ctx: HtmlRenderContext): string | undefined {
  ...
  if (local) return symbolHref(local);
...
function relativeRootHref(moduleHref: string): string {
  const depth = moduleHref.split('/').length - 1;
...
async function extractApi(files: string[], includePrivate: boolean = false): Promise<ApiDoc> {
  ...
  for (const file of files) modules.push(await extractModuleFromSource(String(file), includePrivate));
...
async function writeSqliteIndex(api: ApiDoc, dbPath: string): Promise<string> {
  const sqlite = await import('../sqlite.mts');
...
async function populateDocsIndex(db: DocsDatabase, api: ApiDoc): Promise<void> {
  await db.exec(DOCS_INDEX_SCHEMA);
  for (const moduleDoc of api.modules) {
...
async function runBuildCommand(ctx: CommandContext): Promise<string> {
  ...
  for (const moduleDoc of api.modules) {
    ...
    if (format === 'html' || format === 'both') {
      const htmlPath = `${outDir}/${moduleDoc.name}.html`;
  ...
  if (format === 'html' || format === 'both') {
    ...
    await fs.writeFile(indexPath, renderIndexHtml(api, title));
...
async function runDocTestCommand(ctx: CommandContext): Promise<string> {
  ...
  const { run } = await import('../test/test.mts');
  await run({});
