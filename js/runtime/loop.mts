/**
 * fino:runtime/loop — global event loop singleton.
 *
...
interface LoopEvent {
  ...
  flags:   number;
  fflags?: number;
  data?:   number;  // EVFILT_READ: bytes available to read (kqueue); 0 on io_uring
  res?:    number;
}
...
// ---------------------------------------------------------------------------
// Runtime hooks — exported for _main.mts to drive the event loop
// ---------------------------------------------------------------------------

/**
 * Poll the event loop for I/O events and dispatch them. Called by `_main.mts`
 ...
 * `_main.mts` uses this to decide whether to exit the run loop.
 */
...
export function spin<T>(promise: Promise<T>, options?: SpinOptions): T {
  ...
  promise.then(
    function onSpinResolved(v) { result = v; settled = true; },
    function onSpinRejected(e) { error = e; hasError = true; settled = true; },
  );

  let onAbort: (() => void) | undefined;
  if (signal) {
    onAbort = function onSpinAborted() { error = signal.reason; hasError = true; settled = true; };
