...
// XmlParser
// ---------------------------------------------------------------------------
const MAX_ENTITY_EXPANSION = 1e6;
const MAX_DEPTH = 500;
...
class XmlParser {
  ...
  parseDocument(): XmlDocument {
    ...
    while (!sc.done) {
      ...
      } else if (next === '!' && sc.peekCode(1) === 45 && sc.peekCode(2) === 45) {
  ...
  *events(): Generator<XmlEvent, void> {
    ...
    while (!sc.done) {
      ...
      if (sc.match('<')) {
        const next = sc.peek();
        if (next === '?') {
          this.#parsePI(firstPi);
          firstPi = false;
          continue;
        }
        if (next === '!' && sc.peekCode(1) === 45) {
          this.#parseComment();
          firstPi = false;
          continue;
        }
        if (sc.peek(8) === '!DOCTYPE') {
          this.#parseDoctype();
          firstPi = false;
          continue;
        }
  ...
  *#childEvents(parentName: string): Generator<XmlEvent, void> {
  ...
  #parseStartTag() {
    ...
    const name = sc.eatWhile((c) => c !== 32 && c !== 9 && c !== 10 && c !== 47 && c !== 62);
  ...
  #parseEndTagName(): string {
    const sc = this.#sc;
    const useNs = this.#opts.namespaces !== false;
    const closeName = sc.eatWhile((c) => c !== 32 && c !== 9 && c !== 10 && c !== 13 && c !== 62);
  ...
  #parsePI(allowXmlDeclaration = false): XmlPI {
    const sc = this.#sc;
    sc.expect('?');
    const target = sc.eatWhile((c) => c !== 32 && c !== 9 && c !== 10 && c !== 63 && c !== 62);
    ...
    sc.skipWhitespace();
    let data = '';
    while (!sc.done && !(sc.peekCode() === 63 && sc.peekCode(1) === 62)) {
  ...
  #parseDoctype(): XmlDoctype {
    ...
    while (!sc.done) {
      ...
      if (s.endsWith('<!ENTITY')) {
        ...
        sc.skipWhitespace();
        const ename = sc.eatWhile((c) => c !== 32 && c !== 9 && c !== 10);
