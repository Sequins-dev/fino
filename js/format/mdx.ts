...
function validateMdx(source: string): MdxDiagnostic[] {
  ...
  for (let index = 0; index < source.length;) {
    ...
    if (!fenced && source[index] === '{') {
      ...
      if (end < 0) {
        ...
        diagnostics.push(diagnostic('Unclosed MDX expression.', place.line, place.column, index, 'expression-unclosed'));
        break;
      }
      index = end + 1;
      continue;
    ...
    if (!fenced && source[index] === '<') {
      ...
      if (end < 0) {
        const place = lineAt(source, index);
        diagnostics.push(diagnostic('Unclosed MDX JSX tag.', place.line, place.column, index, 'jsx-tag-unclosed'));
        break;
      }
  ...
  if (diagnostics.length === 0 && open) {
    ...
    diagnostics.push(diagnostic(`Missing closing JSX tag for <${open.name}>.`, place.line, place.column, open.offset, 'jsx-tag-unclosed'));
  }
  return diagnostics;
}
...
function protectMdx(source: string): {
  ...
  for (let index = 0; index < source.length;) {
    ...
    if (end >= index) {
      ...
      tokens.push(token.replace(/^(<\/?)(Notes|Head|Header|Footer|Steps)(?=[\s>])/u, '$1_components.$2'));
      markdown += marker;
      index = end + 1;
    } else {
...
function renderNode(node: MarkdownNode, document: MarkdownDocument, tight = false): string {
  ...
  if (node.kind === 'heading') return `<_components.h${node.level}>${inline(node.text, document)}</_components.h${node.level}>`;
  if (node.kind === 'thematicBreak') return '<_components.hr />';
  if (node.kind === 'blockquote') return `<_components.blockquote>${node.nodes.map((child) => renderNode(child, document)).join('')}</_components.blockquote>`;
  ...
  const head = node.header.map((cell, index) => {
  ...
  const rows = node.rows.map((row) => `<_components.tr>${node.header.map((_, index) => {
    const align = node.align[index] ? ` align={${JSON.stringify(node.align[index])}}` : '';
...
function generateTsx(source: SourceDocument): string {
  ...
  const sections = source.slides.map((slide, index) => {
