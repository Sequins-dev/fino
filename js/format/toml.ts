...
class TomlParser {
  ...
  #parseValue(): TomlValue {
    ...
    if (ch === 't') {
      if (sc.match('true')) return true;
      throw sc.error('expected true');
    }
    if (ch === 'f') {
      if (sc.match('false')) return false;
      throw sc.error('expected false');
    }
    if (ch === 'i') {
      if (sc.match('inf')) return Infinity;
      throw sc.error('expected inf');
    }
    if (ch === 'n') {
      if (sc.match('nan')) return NaN;
      throw sc.error('expected nan');
    }
    if (ch === '+') {
      if (sc.peekCode(1) === 105) {
        sc.match('+inf');
        return Infinity;
      }
      if (sc.peekCode(1) === 110) {
        sc.match('+nan');
        return NaN;
      }
    }
  ...
  #parseTime(raw: string): TomlLocalTime {
    ...
    const ms = fracStr ? Math.round(Number(`0.${fracStr}`) * 1e3) : 0;
