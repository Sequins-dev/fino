...
export function retriever(memory: Memory, defaults: Omit<MemoryQuery, 'text'> = {}): Retriever {
  return { async retrieve(text: string, opts: Omit<MemoryQuery, 'text'> = {}): Promise<RecallHit[]> {
...
export class SqliteMemory implements Memory {
  ...
  static async open(opts: SqliteMemoryOptions): Promise<SqliteMemory> {
    ...
    await db.exec(`CREATE INDEX IF NOT EXISTS idx_messages_thread ON messages(thread_id, created_at)`);
    ...
    if (semantic) {
      try {
        await db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS chunk_vectors USING vec0(embedding float[${dim}])`);
      } catch {
        semantic = false;
      }
    }
  ...
  async append(msg: Omit<MemoryMessage, 'id' | 'createdAt' | 'threadId'>): Promise<MemoryMessage> {
    ...
    const stmt = this.#db.prepare(`INSERT INTO messages(id, thread_id, role, content, created_at) VALUES(?, ?, ?, ?, ?)`);
    try {
  ...
  async recall(query: MemoryQuery = {}): Promise<RecalledContext> {
    ...
    const scopeId = query.scope === 'resource' ? this.#resourceId ?? this.#threadId : this.#threadId;
    const knnStmt = this.#db.prepare(`SELECT rowid, distance FROM chunk_vectors WHERE embedding MATCH ? AND k = ? ORDER BY distance`);
    ...
    const chunksStmt = this.#db.prepare(`SELECT rowid, id, text, metadata, scope_id FROM chunks WHERE rowid IN (${placeholders})`);
    ...
    const distByRowid = new Map<bigint, number>(knnRows.map((r) => [r.rowid as bigint, r.distance as number]));
    const recalled: RecallHit[] = chunkRows.filter((r) => r.scope_id === scopeId).map((r) => ({
      row: r,
      metadata: r.metadata !== null ? JSON.parse(r.metadata as string) as Record<string, unknown> : undefined
    })).filter(({ metadata }) => metadataMatches(metadata, query.filter?.metadata)).sort((a, b) => {
      const da = distByRowid.get(a.row.rowid as bigint) ?? Infinity;
      const db = distByRowid.get(b.row.rowid as bigint) ?? Infinity;
      return da - db;
    }).slice(0, topK).map(({ row, metadata }) => ({
      id: row.id as string,
    ...
    try {
      for (const doc of docs) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          const result = await insertChunk.run(id, scopeId, chunks[i], doc.metadata !== undefined ? JSON.stringify(doc.metadata) : null, blob);
