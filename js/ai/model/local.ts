...
const LLAMA_SYMBOLS = {
  ...
  llama_tokenize: {
    parameters: ['pointer', 'buffer', 'i32', 'buffer', 'i32', 'bool', 'bool'],
    result: 'i32',
  },
  llama_token_to_piece: {
    parameters: ['pointer', 'i32', 'buffer', 'i32', 'i32', 'bool'],
    result: 'i32',
  },
  llama_vocab_is_eog: {
...
class FfiLlamaBindings implements LocalLlamaBindings {
  ...
  async open(path: string, options: LocalOpenOptions): Promise<ArrayBuffer> {
    ...
    const model = (await this.#lib.symbols.llama_model_load_from_file(
      cstring(path),
      modelParams,
    )) as ArrayBuffer | null;
    if (model === null) throw new Error('llama.cpp model open failed');
    const ctx = (await this.#lib.symbols.llama_init_from_model(
      model,
      contextParams,
    )) as ArrayBuffer | null;
    if (ctx === null) {
  ...
  #tokenize(
    ...
    for (;;) {
      ...
      const rc = Number(
        this.#lib.symbols.llama_tokenize(
          vocab,
          bytes,
          bytes.byteLength,
          tokenBytes,
  ...
  #tokenToPiece(vocab: ArrayBuffer, token: number): string {
    ...
    for (;;) {
      ...
      const rc = Number(
        this.#lib.symbols.llama_token_to_piece(vocab, token, out, capacity, 0, true),
      );
  ...
  #createSampler(options: LocalGenerateOptions): ArrayBuffer {
    ...
    if (temperature <= 0) {
      this.#lib.symbols.llama_sampler_chain_add(
        chain,
        this.#lib.symbols.llama_sampler_init_greedy(),
      );
      return chain;
    }
    ...
    if (topK > 0) {
      this.#lib.symbols.llama_sampler_chain_add(
        chain,
        this.#lib.symbols.llama_sampler_init_top_k(topK),
      );
    }
    ...
    if (topP > 0 && topP < 1) {
      this.#lib.symbols.llama_sampler_chain_add(
        chain,
        this.#lib.symbols.llama_sampler_init_top_p(topP, 1),
      );
    }
    this.#lib.symbols.llama_sampler_chain_add(
      chain,
      this.#lib.symbols.llama_sampler_init_temp(temperature),
    );
    this.#lib.symbols.llama_sampler_chain_add(
      chain,
      this.#lib.symbols.llama_sampler_init_dist(LLAMA_DEFAULT_SEED),
    );
    return chain;
  }
...
function candidateLibraryPaths(): string[] {
  ...
  if (os === 'darwin') {
    ...
    paths.push(
      ...
      '/usr/lib/aarch64-linux-gnu/libllama.so',
    );
  } else {
...
function candidateGgmlPaths(llamaPath: string): string[] {
  ...
  if (slash >= 0)
    paths.push(
      `${llamaPath.slice(0, slash)}/libggml.dylib`,
      `${llamaPath.slice(0, slash)}/libggml.so`,
    );
  if (os === 'darwin') {
    ...
    paths.push(
      ...
      '/usr/lib/aarch64-linux-gnu/libggml.so',
    );
  }
  return [...new Set(paths)];
}
...
function requireBindings(opts: LocalModelOptions): LocalLlamaBindings {
  ...
  if (opts.libraryPath) {
    ...
    throw new LocalModelLibraryError(`Unable to load libllama at ${opts.libraryPath}`, {
...
async function resolveModelPath(opts: LocalModelOptions): Promise<string> {
  ...
  const cachePath = join(
    opts.cacheDir ?? DEFAULT_CACHE_DIR,
    source.repo.replace(/\//g, '__'),
    revision,
    source.file,
  ).toString();
  ...
  return content
    .map((part) => {
      ...
      if (part.type === 'tool_result')
        return typeof part.content === 'string' ? part.content : contentText(part.content);
      throw new LocalModelUnsupportedError(
        `Local llama.cpp models only support text content parts; got ${part.type}`,
      );
    })
    .join('');
}
function renderPrompt(req: GenerateRequest): string {
  ...
  if (req.system != null) {
    const sys =
      typeof req.system === 'string' ? req.system : req.system.map((p) => p.text).join('');
...
function assertSupported(req: GenerateRequest): void {
  ...
  if (req.responseFormat != null) {
    throw new LocalModelUnsupportedError(
      'Local llama.cpp models do not support native response formats yet.',
    );
  }
}
class LocalLlamaModel implements Model {
  ...
  #handle: unknown;
  #defaults: Required<
    Pick<LocalModelOptions, 'maxTokens' | 'contextSize' | 'threads' | 'batchSize' | 'gpuLayers'>
  > & {
    temperature?: number;
    topP?: number;
    topK?: number;
