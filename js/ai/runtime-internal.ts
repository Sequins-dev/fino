...
function addUsage(a: Usage, b: Usage): Usage {
  return {
    ...
    ...(a.cacheCreationInputTokens != null || b.cacheCreationInputTokens != null
      ...
      : {}),
    ...(a.localCacheReadInputTokens != null || b.localCacheReadInputTokens != null
      ? {
          localCacheReadInputTokens:
            (a.localCacheReadInputTokens ?? 0) + (b.localCacheReadInputTokens ?? 0),
        }
      : {}),
    ...(a.localCacheReadOutputTokens != null || b.localCacheReadOutputTokens != null
      ? {
          localCacheReadOutputTokens:
            (a.localCacheReadOutputTokens ?? 0) + (b.localCacheReadOutputTokens ?? 0),
        }
      : {}),
  };
}
...
export class AgentRuntime {
    ...
    for (const activeModel of models) {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        if (attempt > 0) {
          ...
          const retryAfterMs =
            lastErr instanceof ModelError && lastErr.retryAfterMs != null
  ...
  > {
    ...
    const view = await this.#historyStrategy.onRead(
      historyReadCtx(this.#model, this.#budgetTokens, state.signal),
    );
    ...
    if (this.#guardrails?.input) {
      ...
      if (gr.action === 'block')
        throw new GuardrailError(gr.reason ?? 'Input blocked by guardrail', gr.reason);
    ...
    try {
      return await runWithActiveSpan(stepSpan, async () => {
        ...
        if (this.#guardrails?.output && turnText) {
          ...
          if (gr.action === 'block')
            throw new GuardrailError(gr.reason ?? 'Output blocked by guardrail', gr.reason);
        ...
        const newMessages: ModelMessage[] = [
          ...effectiveMessages,
          {
            role: 'assistant',
            content:
              assistantContent.length === 1 && turnText && !toolUseParts.length
                ? turnText
                : assistantContent,
          },
        ];
        ...
        if (turn.usage.localCacheReadInputTokens != null) {
          stepSpan.setAttribute(
            'gen_ai.usage.local_cache_read.input_tokens',
            turn.usage.localCacheReadInputTokens,
          );
        }
        if (turn.usage.localCacheReadOutputTokens != null) {
          stepSpan.setAttribute(
            'gen_ai.usage.local_cache_read.output_tokens',
            turn.usage.localCacheReadOutputTokens,
          );
        }
        ...
        const { toolResults, suspend } = await this.#runTools(
          toolUseParts,
          newMessages,
          state,
          runId,
        ...
        try {
          ...
          if (result.isError) {
            toolSpan.recordException?.(
              new Error(typeof result.content === 'string' ? result.content : 'tool error'),
            );
  ...
  #hasPendingToolCall(messages: ModelMessage[], request: ToolApprovalRequest): boolean {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        return !next.content.some(
          (part) => part.type === 'tool_result' && part.toolCallId === request.toolCallId,
        );
      }
    }
    return false;
        ...
        return runContext.runWithValue(runState, async () => {
          ...
          while (true) {
            ...
            if (capture) {
              ...
              if (r.suspend)
                throw new Error('Structured output run suspended before respond was called');
              if (capture.retried)
                throw new Error('Structured output validation failed after repair');
              capture.retried = true;
            } else if (
              r.suspend ||
              r.done ||
              this.#stopWhen.some((c) => c(state, { stopReason: r.stopReason }))
            ) {
              break;
            }
          }
          ...
          if (state.usage.localCacheReadInputTokens != null) {
            runSpan.setAttribute(
              'gen_ai.usage.local_cache_read.input_tokens',
              state.usage.localCacheReadInputTokens,
            );
          }
          if (state.usage.localCacheReadOutputTokens != null) {
            runSpan.setAttribute(
              'gen_ai.usage.local_cache_read.output_tokens',
              state.usage.localCacheReadOutputTokens,
            );
          }
