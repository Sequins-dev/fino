...
export class Scanner {
  ...
  get line(): number {
    ...
    if (!this.#lineColValid)
      throw new Error(
        'Scanner.line: position tracking invalidated by byte op; restore a snapshot to re-synchronize',
      );
    return this.#line;
  }
  /**
  ...
  get column(): number {
    ...
    if (!this.#lineColValid)
      throw new Error(
        'Scanner.column: position tracking invalidated by byte op; restore a snapshot to re-synchronize',
      );
    return this.#col;
  }
  // BYTE OPS
  ...
  eatUntilByte(c: number, max?: number): Uint8Array {
    ...
    const limit =
      max !== undefined ? Math.min(this.#buf.length, this.#offset + max) : this.#buf.length;
  ...
  readLineCRLF(): string {
    ...
    while (this.#offset < this.#buf.length) {
      const b = this.#buf[this.#offset]!;
      if (b === 10)
        throw this.error('expected CRLF line ending, got bare LF', {
          offset: this.#offset,
          line: this.#line,
          column: this.#col,
        });
  ...
  expectToken(
    ...
    const ok = options?.caseInsensitive
      ...
      : actual === expected;
    if (!ok)
      throw this.error(`expected ${options?.name ?? 'token'} '${expected}', got '${actual}'`);
  }
  /**
  ...
  readStrictInt(options?: {
    ...
    if (options?.min !== undefined && value < options.min)
      throw this.error(`invalid ${name}: ${value} < ${options.min}`);
    if (options?.max !== undefined && value > options.max)
      throw this.error(`invalid ${name}: ${value} > ${options.max}`);
    return value;
  }
  /**
  ...
  error(
    ...
    const toMark: ScannerMark | undefined =
      span !== undefined && 'from' in span ? span.to : undefined;
  ...
  #readNum(
    ...
    if (this.#offset + byteLen > this.#buf.length)
      throw this.error(
        `unexpected end of input (need ${byteLen} bytes, got ${this.#buf.length - this.#offset})`,
      );
    const o = this.#offset;
    this.#offset += byteLen;
    if (this.#encoding !== null) this.#lineColValid = false;
  ...
  #readField(name: string, byteLen: number, fn: () => number | bigint): number | bigint {
    if (this.#offset + byteLen > this.#buf.length) {
      throw this.error(
        `unexpected end of input while reading ${name} (need ${byteLen} bytes, got ${this.#buf.length - this.#offset})`,
      );
    }
    return fn();
  }
  ...
  #peekCpAt(offset: number): [number, number] {
    ...
    // UTF-8 decode
    if (b < 128) return [b, 1];
    if ((b & 224) === 192 && offset + 1 < this.#buf.length)
      return [((b & 31) << 6) | (this.#buf[offset + 1]! & 63), 2];
    if ((b & 240) === 224 && offset + 2 < this.#buf.length)
      return [
        ((b & 15) << 12) | ((this.#buf[offset + 1]! & 63) << 6) | (this.#buf[offset + 2]! & 63),
        3,
      ];
    if ((b & 248) === 240 && offset + 3 < this.#buf.length)
      return [
        ((b & 7) << 18) |
          ((this.#buf[offset + 1]! & 63) << 12) |
          ((this.#buf[offset + 2]! & 63) << 6) |
          (this.#buf[offset + 3]! & 63),
        4,
      ];
    return [65533, 1];
  }
