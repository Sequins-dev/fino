...
function parseZip(bytes: Uint8Array): LoadedArchiveEntry[] {
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    const matchesName = localNameBytes.byteLength === nameBytes.byteLength && localNameBytes.every((byte, index) => byte === nameBytes[index]);
    if (localFlags !== flags || localMethod !== method || localCrc !== crc || localCompressedSize !== compressedSize || localSize !== size || !matchesName) {
    ...
    const kind: ArchiveKind = name.endsWith('/') || (externalAttrs >>> 16 & 61440) === 16384 ? 'directory' : 'file';
    entries.push({
      ...
      mode: externalAttrs >>> 16 & 65535 || (kind === 'directory' ? DEFAULT_DIR_MODE : DEFAULT_MODE),
      method,
      crc32: crc,
      loader: async () => {
        ...
        if (method === ZIP_METHOD_STORE) {
          if (size > MAX_DECOMPRESSED_BYTES) {
            throw new Error(`Archive entry '${name}' is ${size} bytes, ` + `exceeding the ${MAX_DECOMPRESSED_BYTES}-byte limit`);
          }
        ...
        if (method === ZIP_METHOD_DEFLATE) {
          ...
          if (decompressed.byteLength > MAX_DECOMPRESSED_BYTES) {
            throw new Error(`Archive entry '${name}' decompressed to ${decompressed.byteLength} bytes, ` + `exceeding the ${MAX_DECOMPRESSED_BYTES}-byte limit`);
          }
...
function parseTar(bytes: Uint8Array): LoadedArchiveEntry[] {
  ...
  while (offset + 512 <= bytes.byteLength) {
    ...
    if (kind === 'file' && size > MAX_DECOMPRESSED_BYTES) {
      throw new Error(`Tar entry '${fullName}' is ${size} bytes, exceeding the ${MAX_DECOMPRESSED_BYTES}-byte limit`);
    }
...
async function serializeTar(entries: LoadedArchiveEntry[]): Promise<Uint8Array> {
  ...
  for (const entry of entries) {
    ...
    writeOctal(header, 136, 12, entry.mtime ? Math.floor(new Date(entry.mtime).getTime() / 1e3) : Math.floor(Date.now() / 1e3));
