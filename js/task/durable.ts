...
*/
export interface DurableTaskOptions<
  Input = unknown,
  Output = unknown
> extends Omit<TaskOptions<Input, Output>, 'run'> {
  /**
...
function _composeContext(taskCtx: TaskContext, wctx: WorkflowContext): DurableTaskContext {
  return {
    ...
    ...taskCtx.optionProvided !== undefined ? { optionProvided: taskCtx.optionProvided.bind(taskCtx) } : {},
    ...
    suspend: () => {
      throw new Error('suspend() is not available in durable tasks; use ctx.waitForSignal() for durable pauses');
    }
  };
}
...
> extends Task<Input, Output> {
  ...
  */
  async start(rawInput: Input, opts: DurableRunOptions & {
    ...
    const runId = opts.runId ?? `${this.name}-run-${Date.now().toString(36)}-${_nextDurableRunId++}`;
  ...
  */
  #resolveStore(override?: WorkflowStore): Promise<WorkflowStore> {
    ...
    if (this.#storeInstance === undefined) {
      ...
      this.#storeInstance = Promise.resolve(typeof source === 'function' ? source() : source ?? new InMemoryWorkflowStore());
    }
  ...
  * @internal
  */
  async #startOrResume(wf: Workflow<Input, Output>, store: WorkflowStore, runId: string | undefined, input: Input, key: string | undefined, signal: AbortSignal | undefined): Promise<WorkflowResult<Output>> {
    if (runId !== undefined) {
  ...
  */
  async #driveToCompletion(input: Input, taskCtx: TaskContext): Promise<Output> {
    ...
    let result = await this.#startOrResume(wf, store, taskCtx.runId, input, extras?.key, taskCtx.signal);
  ...
  * @internal
  */
  async #driveOnce(store: WorkflowStore, runId: string, input: unknown, opts: DurableRunOptions, allowStart: boolean): Promise<DurableRunHandle> {
  ...
  * @internal
  */
  #awaitSignalDelivery(runId: string, timeoutAt: number | undefined, signal?: AbortSignal): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      if (signal?.aborted) {
        reject(new Error('Durable task run aborted'));
        return;
