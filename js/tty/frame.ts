 ...
 *
 * ANSI parsing recognizes the SGR subset of
 * [ECMA-48](https://ecma-international.org/publications-and-standards/standards/ecma-48/)
 * emitted by `fino:tty/style`; other terminal control sequences are discarded.
 * Cell widths follow the terminal-oriented approximation documented by the
 * exported width helpers rather than promising font-dependent visual layout.
 *
 * ```ts
...
import {
  charWidth as measureCharWidth,
  graphemes as splitGraphemes,
  clusterWidth as measureClusterWidth,
  stringWidth as measureStringWidth,
} from 'internal:tty/width';

/** Return the terminal cell width of the code point `cp`. */
export function charWidth(cp: number): 0 | 1 | 2 {
  return measureCharWidth(cp);
}

/**
 * Split `text` into approximate terminal grapheme clusters.
 *
 * Combining marks, emoji modifiers, regional-indicator pairs, and common ZWJ
 * sequences stay together. This is intentionally not a complete Unicode text
 * segmentation implementation because terminal rendering varies by emulator.
 */
...
export function clusterWidth(cluster: string): 0 | 1 | 2 {
  return measureClusterWidth(cluster);
}
...
export function frameToScreen(
  ...
  for (let i = 0; i < frame.rows.length; i++) {
    ...
    // Erase before writing: if a row fills the terminal, a trailing erase can
    // consume the final cell while the cursor has a pending wrap.
