...
* ```
*/
import { readLine, stdinIsTTY, stdoutIsTTY, writeStdout, writeStderr } from '../tty.ts';
/**
  ...
  */
  async text(options: TextPromptOptions): Promise<string> {
    ...
    while (true) {
      ...
      const value = response == null || response.trim().length === 0 ? options.defaultValue ?? '' : response.trim();
