 ...
 *
 * SGR encoding and decoding implement the color and text-attribute subset of
 * [ECMA-48](https://ecma-international.org/publications-and-standards/standards/ecma-48/)
 * used by modern terminals. Other control functions remain outside this style
 * data model.
 *
 * ```ts
 ...
 *
 * Keeping detection pure lets callers pass an injected environment value and
 * reuse the same policy without coupling rendering code to process state.
 *
 * ```ts
 * import { supportsTruecolor } from 'fino:tty/style';
 *
 * supportsTruecolor('truecolor'); // true
 * supportsTruecolor(undefined); // false
 * ```
 */
...
function nearestCubeStep(value: number): number {
  ...
  for (let i = 0; i < CUBE_STEPS.length; i++) {
    const distance = Math.abs(CUBE_STEPS[i]! - value);
 ...
 *
 * Both the 6×6×6 color cube and grayscale ramp are considered. Centralizing
 * this fallback avoids each color-producing component implementing its own
 * approximation.
 *
 * ```ts
 ...
 * nearestAnsi256(128, 128, 128); // 244
 * ```
 */
export function nearestAnsi256(r: number, g: number, b: number): number {
  const red = nearestCubeStep(r);
  const green = nearestCubeStep(g);
  const blue = nearestCubeStep(b);
  const cubeRed = CUBE_STEPS[red]!;
  const cubeGreen = CUBE_STEPS[green]!;
  const cubeBlue = CUBE_STEPS[blue]!;
  const cubeDistance = (r - cubeRed) ** 2 + (g - cubeGreen) ** 2 + (b - cubeBlue) ** 2;
