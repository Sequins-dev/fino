...
function encodeTypeTable(b: Builder, type: DataType): number {
  ...
  switch (t.kind) {
    ...
    default:
      throw new ArrowError(`cannot encode Arrow type ${type.kind}`);
  }
}
 ...
 * ```
 */
export function recordBatchMessage(
  length: number,
  nodes: FieldNode[],
  buffers: BufferRegion[],
  variadicCounts: number[],
 ...
 * ```
 */
export function dictionaryBatchMessage(
  id: number,
  isDelta: boolean,
  length: number,
  nodes: FieldNode[],
...
function decodeBaseType(typeTag: number, table: FbTable | null, children: Field[]): DataType {
  switch (typeTag) {
    ...
    case Type.Decimal:
      return decimal(table!.i32(0, 0), table!.i32(1, 0), table!.i32(2, 128) as 32 | 64 | 128 | 256);
    case Type.Date:
      return table!.i16(0, 1) === 0 ? date32() : date64();
    case Type.Time:
      return table!.i32(1, 32) === 32 ? time32(table!.i16(0, 1)) : time64(table!.i16(0, 1));
    case Type.Timestamp:
      return timestamp(table!.i16(0, 0), table!.string(1));
    case Type.Interval:
      return interval(table!.i16(0, 0));
    case Type.Duration:
      return duration(table!.i16(0, 1));
    case Type.Utf8:
      return utf8();
    case Type.LargeUtf8:
      return largeUtf8();
    ...
    case Type.FixedSizeBinary:
      return fixedSizeBinary(table!.i32(0, 0));
    case Type.List:
    ...
    case Type.FixedSizeList:
      return fixedSizeList(table!.i32(0, 0), children[0]!);
    case Type.Struct:
      return struct(children);
    case Type.Map:
      return map(children[0]!, table!.bool(0, false));
    case Type.Union: {
    ...
    default:
      throw new ArrowError(`unsupported Arrow type tag ${typeTag}`);
  }
}
...
function decodeField(table: FbTable): Field {
  ...
  if (childrenVec)
    for (let i = 0; i < childrenVec.length; i++) children.push(decodeField(childrenVec.table(i)));
...
export function decodeSchema(table: FbTable): Schema {
  ...
  const fields: Field[] = [];
  if (fieldsVec)
    for (let i = 0; i < fieldsVec.length; i++) fields.push(decodeField(fieldsVec.table(i)));
...
function decodeRecordBatchTable(rb: FbTable): RecordBatchHeader {
  ...
  if (variadicVec)
    for (let i = 0; i < variadicVec.length; i++)
      variadicBufferCounts.push(Number(variadicVec.i64(i)));
  return {
    length,
    nodes,
    buffers,
