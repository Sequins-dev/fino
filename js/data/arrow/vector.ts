  ...
  protected getValue(i: number): unknown {
    ...
    switch (t.kind) {
      case 'int':
        switch (t.bitWidth) {
          case 8:
            return t.signed ? this.#dv.getInt8(pos) : this.#dv.getUint8(pos);
          case 16:
            return t.signed ? this.#dv.getInt16(pos, true) : this.#dv.getUint16(pos, true);
          case 32:
            return t.signed ? this.#dv.getInt32(pos, true) : this.#dv.getUint32(pos, true);
          case 64:
            return t.signed ? this.#dv.getBigInt64(pos, true) : this.#dv.getBigUint64(pos, true);
        }
        return 0;
      case 'float':
      ...
      case 'date':
        return t.unit === DateUnit.DAY
          ? this.#dv.getInt32(pos, true)
          : this.#dv.getBigInt64(pos, true);
      case 'time':
        return t.bitWidth === 32 ? this.#dv.getInt32(pos, true) : this.#dv.getBigInt64(pos, true);
      case 'timestamp':
      case 'duration':
        return this.#dv.getBigInt64(pos, true);
      case 'interval':
        if (t.unit === IntervalUnit.YEAR_MONTH) return this.#dv.getInt32(pos, true);
        if (t.unit === IntervalUnit.DAY_TIME)
          return {
...
class VarBinaryVector extends Vector {
  ...
  #offsetAt(i: number): number {
    return this.#large
      ? Number(this.#offsets.getBigInt64(i * 8, true))
      : this.#offsets.getInt32(i * 4, true);
  }
  protected getValue(i: number): unknown {
...
class ListViewVector extends Vector {
  ...
  protected getValue(i: number): unknown {
    const child = this.children[0]!;
    const start = this.#large
      ? Number(this.#offsets.getBigInt64(i * 8, true))
      : this.#offsets.getInt32(i * 4, true);
    const size = this.#large
      ? Number(this.#sizes.getBigInt64(i * 8, true))
      : this.#sizes.getInt32(i * 4, true);
...
class UnionVector extends Vector {
  ...
  constructor(data: VectorData) {
    ...
    this.#offsets =
      this.#dense && data.valueOffsets
        ? new DataView(
            data.valueOffsets.buffer,
            data.valueOffsets.byteOffset,
            data.valueOffsets.byteLength,
          )
        : null;
...
export function makeVector(data: VectorData): Vector {
  switch (data.type.kind) {
    case 'null':
      return new NullVector(data);
    case 'bool':
      return new BoolVector(data);
    case 'int':
    case 'float':
    case 'date':
    case 'time':
    ...
    case 'duration':
    case 'interval':
      return new PrimitiveVector(data);
    case 'decimal':
      return new DecimalVector(data);
    case 'fixedsizebinary':
      return new FixedSizeBinaryVector(data);
    case 'utf8':
    case 'largeutf8':
    case 'binary':
    case 'largebinary':
      return new VarBinaryVector(data);
    case 'utf8view':
    case 'binaryview':
      return new VarBinaryViewVector(data);
    case 'list':
    case 'largelist':
      return new ListVector(data);
    case 'listview':
    case 'largelistview':
      return new ListViewVector(data);
    case 'fixedsizelist':
      return new FixedSizeListVector(data);
    case 'struct':
      return new StructVector(data);
    case 'map':
      return new MapVector(data);
    case 'union':
      return new UnionVector(data);
    case 'dictionary':
      return new DictionaryVector(data);
    case 'runendencoded':
      return new RunEndEncodedVector(data);
  }
}
  ...
  for (const v of values) {
    ...
    switch (typeof v) {
      ...
      case 'object':
        ...
        return struct(
          Object.keys(v as object).map(
            (k) => new Field(k, inferType([(v as Record<string, unknown>)[k]]), true),
          ),
        );
    }
  }
...
export function vectorFromArray(values: unknown[], type?: DataType): Vector {
  ...
  switch (dt.kind) {
    ...
    case 'struct': {
      const children = dt.children.map((f) =>
        vectorFromArray(
          values.map((v) => (v == null ? null : ((v as Record<string, unknown>)[f.name] ?? null))),
          f.type,
        ),
      );
      return makeVector({
    ...
    default:
      throw new ArrowError(
        `vectorFromArray does not support building ${dt.kind}; use makeVector with raw buffers`,
      );
  }
}
      ...
      switch (type.bitWidth) {
        ...
        case 64:
          type.signed
            ...
            : dv.setBigUint64(pos, BigInt(v as number | bigint), true);
          return;
      }
      return;
    case 'float':
