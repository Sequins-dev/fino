...
export class RecordBatch {
  ...
  constructor(schema: Schema, columns: Vector[]) {
    if (schema.fields.length !== columns.length) {
      throw new ArrowError(
        `schema has ${schema.fields.length} fields but ${columns.length} columns were provided`,
      );
    }
    const numRows = columns.length > 0 ? columns[0]!.length : 0;
    for (const col of columns) {
      if (col.length !== numRows)
        throw new ArrowError('all columns in a record batch must have the same length');
    }
    this.schema = schema;
  ...
  static from(
    ...
    if ('schema' in input && 'columns' in input)
      return new RecordBatch(input.schema, input.columns);
    const fields: Field[] = [];
  ...
  *[Symbol.iterator](): Iterator<Record<string, unknown>> {
  ...
  slice(begin = 0, end: number = this.numRows): RecordBatch {
    return new RecordBatch(
      this.schema,
      this.columns.map((c) => c.slice(begin, end)),
    );
  }
}
