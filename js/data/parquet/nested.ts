...
function buildNode(field: Field, parentDef: number, parentRep: number, path: string[]): Node {
  ...
  if (type.kind === 'struct') {
    ...
    node.children = type.children.map((f) =>
      buildNode(f, defLevel, parentRep, [...path, field.name]),
    );
    return node;
  }
  if (type.kind === 'list' || type.kind === 'largelist' || type.kind === 'fixedsizelist') {
    ...
    node.children = [buildNode(type.child, groupDef + 1, repLevel, [...path, field.name, 'list'])];
    return node;
  }
  if (type.kind === 'map') {
    const groupDef = parentDef + (nullable ? 1 : 0);
    ...
    node.children = [
      buildNode(type.child, groupDef + 1, repLevel, [...path, field.name, 'key_value']),
    ];
    return node;
  }
  // leaf
...
export function schemaElementsToNodes(elements: SchemaElement[]): {
  ...
  const nodes: Node[] = [];
  for (let c = 0; c < (root.numChildren ?? 0); c++)
    nodes.push(parseNode(elements, cursor, 0, 0, []));
  return {
...
function parseNode(
  ...
  const isMap =
    el.convertedType === ConvertedType.MAP ||
    el.convertedType === ConvertedType.MAP_KEY_VALUE ||
    el.logicalType?.kind === 'map';
  if (isList) {
    ...
    void repeated;
    const element = parseNode(elements, cursor, groupDef + 1, repLevel, [...path, el.name, 'list']);
  ...
  if (isMap) {
    ...
    for (let k = 0; k < kvChildren; k++)
      entryChildren.push(
        parseNode(elements, cursor, groupDef + 1, repLevel, [...path, el.name, 'key_value']),
      );
    const entriesStruct = new Field(
      'entries',
      structType(entryChildren.map((n) => n.field)),
      false,
    );
  ...
  for (let k = 0; k < numChildren; k++)
    children.push(parseNode(elements, cursor, defLevel, parentRep, [...path, el.name]));
...
function emitAbsent(
  ...
  repLevel: number,
  defLevel: number,
  streams: Map<ColumnDescriptor, LeafStream>,
): void {
  if (node.kind === 'leaf') {
  ...
  switch (node.kind) {
    ...
    case 'struct': {
      ...
      for (const child of node.children)
        shredValue(child, obj[child.name] ?? null, repLevel, node.defLevel, streams);
      return;
    }
    case 'list': {
      if (value === null || value === undefined) {
...
function assembleValue(
  node: Node,
  cursors: Map<ColumnDescriptor, LeafCursor>,
  repFloor: number,
): unknown {
  ...
  switch (node.kind) {
    case 'leaf':
      return vectorFromArray(
        values.map((v) => (v === null || v === undefined ? null : node.decode!(v))),
        node.field.type,
      );
    case 'struct': {
      ...
      const children = node.children.map((child) =>
        buildNested(
          ...
          values.map((v) =>
            v === null || v === undefined
              ? null
              : ((v as Record<string, unknown>)[child.name] ?? null),
          ),
        ),
      );
      return makeVector({
    ...
    case 'map': {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        if (Array.isArray(entries))
          for (const [k, v] of entries as [unknown, unknown][])
            entryObjs.push({
              key: k,
              value: v,
            });
