...
export function arrowTypeToParquet(type: DataType): ParquetTypeInfo {
  switch (type.kind) {
    ...
    case 'decimal': {
      const bytesToBig = (v: unknown) =>
        typeof v === 'bigint'
          ? v
          : typeof v === 'number'
            ? BigInt(v)
            : decimalBytesToBigInt(v as Uint8Array);
      if (type.bitWidth === 32)
        return {
  ...
  switch (el.type) {
    ...
    case PType.INT32:
      if (isDecimal)
        return {
          type: decimal(
            el.precision ?? logical?.decimal?.precision ?? 9,
            el.scale ?? logical?.decimal?.scale ?? 0,
            32,
          ),
          decode: (v) => BigInt(v as number),
          encode: (v) => Number(v as bigint),
        };
      if (logical?.kind === 'date' || converted === ConvertedType.DATE)
        return {
          type: date32(),
          decode: identity,
          encode: identity,
        };
      if (logical?.kind === 'time' || converted === ConvertedType.TIME_MILLIS)
        return {
          type: time32(TimeUnit.MILLISECOND),
          decode: identity,
          encode: identity,
        };
      if (logical?.kind === 'integer')
        return {
    ...
    case PType.INT64:
      if (isDecimal)
        return {
          type: decimal(
            el.precision ?? logical?.decimal?.precision ?? 18,
            el.scale ?? logical?.decimal?.scale ?? 0,
            64,
          ),
          decode: identity,
      ...
      if (logical?.kind === 'time')
        return {
          type: time64(
            logical.time!.unit.unit === TimeUnitId.NANOS
              ? TimeUnit.NANOSECOND
      ...
      if (converted === ConvertedType.TIME_MICROS)
        return {
      ...
      if (converted === ConvertedType.UINT_64)
        return {
          type: uint64(),
          decode: identity,
          encode: identity,
    ...
    case PType.FIXED_LEN_BYTE_ARRAY: {
      ...
      if (isDecimal)
        return {
          type: decimal(
            el.precision ?? logical?.decimal?.precision ?? 38,
            el.scale ?? logical?.decimal?.scale ?? 0,
            len > 16 ? 256 : 128,
          ),
          decode: (v) => decimalBytesToBigInt(v as Uint8Array),
    ...
    default:
      throw new ParquetError(`Parquet physical type ${el.type} is not supported`);
  }
}
...
function timestampFromUnit(unitId: number, isUtc: boolean): DataType {
  const unit =
    unitId === TimeUnitId.MILLIS
      ? TimeUnit.MILLISECOND
      : unitId === TimeUnitId.NANOS
        ? TimeUnit.NANOSECOND
        : TimeUnit.MICROSECOND;
...
export function parquetSchemaToArrow(elements: SchemaElement[]): {
  ...
  for (let c = 0; c < numChildren; c++) {
    ...
    if ((el.numChildren ?? 0) > 0)
      throw new ParquetError(`nested column '${el.name}' requires the nested reader`);
