...
export class Resolver {
            ...
            fetch: async (fetchName, fetchType) => {
              try {
                ...
                if (
                  ...
                  throw err;
                const dnssecErr: DnsError = new Error(
                  `dnssec: validation fetch failed for '${fetchName}'`,
                );
  ...
  async #sendTcpQuery(
    ...
    const fd = sock.socket(
      server.family === 'ipv6' ? sock.AF_INET6 : sock.AF_INET,
      sock.SOCK_STREAM,
      0,
    );
    try {
