...
**Routing and middleware** — use `fino:net/http/app`. It provides `App`, `Router`, Koa-style middleware, URLPattern route builders, body and schema helpers, OpenAPI generation, cookie and session producers, and a `listen()` shortcut that wires the app into `serve()`. See the [routing guide](./http/routing.md).
