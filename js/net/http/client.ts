...
import type { WebSocketConnectOptions } from '../../globals/websocket.ts';
...
import { QuicEndpoint } from '../quic.ts';
import type { QuicConnection } from '../quic.ts';
...
export class HttpSession {
  ...
  */
  ...
  async _request(input: string | URL, init: HttpRequestInit = {}): Promise<HttpResponse> {
    ...
    const response = this.protocol === 'h3' ? await this.#h3Request(url, method, headers, init) : await runtimeFetch(url.href, {
      method,
      headers,
      body: init.body,
    ...
    const connection: HttpConnectionInfo = this.protocol === 'h3' && this.#h3Transport !== null ? this.#h3Transport.connection : {
      id: nextConnectionId(),
      protocol,
...
export class HttpClient {
  ...
  async request(input: string | URL | Request, init: HttpRequestInit = {}): Promise<HttpResponse> {
    this.#assertOpen();
    const url = input instanceof Request ? resolveHttpUrl(this.#baseUrl, input.url) : resolveHttpUrl(this.#baseUrl, input);
