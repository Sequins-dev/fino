...
export class WebSocketError extends DOMException {
  ...
  constructor(message = '', init: WebSocketErrorInit = {}) {
    ...
    const closeCode =
      init.closeCode === undefined || init.closeCode === null
        ? reason === ''
          ? null
          : 1e3
...
export class WebSocketConnection extends EventTarget implements ConnectionTakeover {
  ...
  static accept(
    ...
    if (upgrade !== 'websocket') {
      throw Object.assign(new Error('Missing or invalid Upgrade: websocket header'), {
        name: 'SyntaxError',
      });
    }
    ...
    const negotiatedExtensions = _perMessageDeflateResponse(
  ...
  static connect(url: string | URL, opts: WebSocketConnectOptions = {}): WebSocketConnection {
    ...
    if (parsed.protocol !== 'ws:' && parsed.protocol !== 'wss:') {
      throw Object.assign(new Error('WebSocket URL must use ws: or wss: scheme'), {
        name: 'SyntaxError',
      });
    }
    if (parsed.hash !== '') {
      throw Object.assign(new Error('WebSocket URL must not have a fragment identifier'), {
        name: 'SyntaxError',
      });
    }
    // Normalize protocols
    ...
    for (const p of protocols) {
      if (protoSet.has(p)) {
        throw Object.assign(new Error('Duplicate WebSocket subprotocol: ' + p), {
          name: 'SyntaxError',
        });
      }
      protoSet.add(p);
  ...
  async #doConnect(
    ...
    try {
      ...
      if (serverProto && !protocols.includes(serverProto)) {
        throw new Error(
          'WebSocket handshake failed: server offered unrecognized protocol: ' + serverProto,
        );
      }
      const serverExt = (respHeaders.get('sec-websocket-extensions') ?? '').trim();
      if (serverExt) {
        throw new Error(
          'WebSocket handshake failed: server offered unsupported extensions: ' + serverExt,
        );
      }
      ...
      while (true) {
        ...
        if (payloadLen > this.#maxPayloadSize) {
          await this.#failProtocol(
            1009,
            'Payload exceeds maxPayloadSize (' + this.#maxPayloadSize + ' bytes)',
          );
          break;
        }
        // Read mask key
        ...
        // Read payload
        const payload =
          payloadLen > 0 ? await _readExactly(reader, buf, payloadLen) : new Uint8Array(0);
        ...
        if (fin) {
          ...
          if (compressed) {
            ...
            if (assembled.byteLength > this.#maxPayloadSize) {
              await this.#failProtocol(
                1009,
                'Inflated payload exceeds maxPayloadSize (' + this.#maxPayloadSize + ' bytes)',
              );
              break;
            }
          }
...
export class WebSocket extends EventTarget {
  ...
  send(data: string | ArrayBuffer | ArrayBufferView | Blob): void {
    ...
    if (this.#conn.readyState === CONNECTING) {
      throw Object.assign(new Error('WebSocket is not yet open: buffering not supported'), {
