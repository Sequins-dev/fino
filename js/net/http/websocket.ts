...
*
* Implements RFC 6455 in two composable layers: `WebSocketConnection`, the
* protocol engine that server handlers and advanced clients drive directly,
* and `WebSocket`, the WHATWG facade installed as a global for browser-style
* client code. Both layers are exported from `fino:net/http/websocket`.
*
*
...
*/
import { DOMException, btoa } from '../../globals/encoding.ts';
import { encodeUtf8 } from 'internal:encoding';
import { digest } from '../../internal/openssl.ts';
import { Headers, _headerTokenList, _parseHeaders, _parseResponseLine } from './index.ts';
...
import { crypto } from '../../globals/crypto.ts';
import { ZlibRawMessageInflater } from '../../internal/compress/zlib.ts';
import type { BytesReader, BytesWriter } from '../../internal/stream.ts';
import type { IPv4Address, IPv6Address } from '../socket.ts';
...
function _validReceivedCloseCode(code: number): boolean {
  if (code < 1e3 || code > 4999) return false;
...
export class WebSocketError extends DOMException {
  ...
  */
  constructor(message = '', init: WebSocketErrorInit = {}) {
    ...
    const closeCode = init.closeCode === undefined || init.closeCode === null ? reason === '' ? null : 1e3 : Number(init.closeCode);
...
/**
* Encode a single WebSocket frame from an opcode and raw payload bytes.
*
* `mask` selects the wire form: true for client→server frames (masked with a
* fresh random 4-byte key), false for server→client frames. `fin` defaults to
* true for single-frame messages; pass false for fragment continuation. `rsv`
* carries extension bits (e.g. RSV1 for permessage-deflate).
*/
function _encodeFrame(opcode: number, payload: Uint8Array, mask: boolean, fin: boolean = true, rsv: number = 0): Uint8Array {
...
*/
async function _readUpgradeResponse(reader: BytesReader, preamble: _Buf): Promise<{
  status: number;
  ...
  while (true) {
    ...
    for (let i = searchFrom; i + 3 < assembled.byteLength; i++) {
      if (assembled[i] === CR && assembled[i + 1] === LF && assembled[i + 2] === CR && assembled[i + 3] === LF) {
        headerEnd = i + 4;
        break;
      }
    }
...
* 'error' (ErrorEvent), 'ping', 'pong'. Instances are also async iterable —
* `for await (const message of conn)` yields `WebSocketMessage` values until
* the connection closes — and support `await using` for scoped cleanup.
*
...
export class WebSocketConnection extends EventTarget implements ConnectionTakeover {
  ...
  * @internal
  */
  #handshakeBytes: Uint8Array | null = null;
  /**
  ...
  */
  async close(code: number = 1e3, reason: string = ''): Promise<void> {
    ...
    await Promise.race([this.#closePromise, closeTimer.then(() => {
      if (this.#readyState !== CLOSED) this.#teardown();
    })]);
    closeTimer.cancel();
  }
  /**
  * Run the close handshake when leaving an `await using` scope.
  *
  * Equivalent to `close()` with the default code 1000 and no reason.
  *
  * ```ts no_run
  * await using conn = WebSocketConnection.connect('wss://example.com/ws');
  * ```
  */
      ...
      next(): Promise<IteratorResult<WebSocketMessage>> {
        if (self.#msgQueue.length > 0) {
          return Promise.resolve({
            ...
            value: self.#msgQueue.shift()!
          });
        }
        if (self.#readyState === CLOSED) {
          return Promise.resolve({
            done: true,
            value: (undefined as unknown) as WebSocketMessage
          });
        }
  ...
  static accept(req: {
    ...
    if (opts.selectProtocol) {
      ...
      if (offered.length > 0 && !offered.includes(opts.protocol)) {
        throw Object.assign(new Error('Requested protocol "' + opts.protocol + '" not offered by client'), { name: 'SyntaxError' });
      }
  ...
  */
  static connect(url: string | URL, opts: WebSocketConnectOptions = {}): WebSocketConnection {
    ...
    if (parsed.protocol !== 'ws:' && parsed.protocol !== 'wss:') {
      throw Object.assign(new Error('WebSocket URL must use ws: or wss: scheme'), { name: 'SyntaxError' });
    }
    if (parsed.hash !== '') {
      throw Object.assign(new Error('WebSocket URL must not have a fragment identifier'), { name: 'SyntaxError' });
    }
    // Normalize protocols
  ...
  * @internal
  */
  async #doConnect(parsed: InstanceType<typeof URL>, isWss: boolean, protocols: string[], opts: WebSocketConnectOptions): Promise<void> {
    const hostname = parsed.hostname;
    const port = parsed.port ? parseInt(parsed.port, 10) : isWss ? 443 : 80;
  ...
  /**
  * Transition to OPEN, dispatch 'open', and start the frame read pump.
  *
  * This implementation detail is included when documentation is built with
  * `--include-private`. It describes state or helper behavior used by the
  * owning module rather than a stable application-facing contract. Prefer the
  ...
  */
  async #readPump(): Promise<void> {
    ...
    try {
      while (true) {
        ...
        const lenCode = header[1]! & 127;
        const rsv1 = (rsv & 64) !== 0;
        const unexpectedRsv = rsv & 48;
        if (unexpectedRsv !== 0) {
        ...
        if (lenCode <= 125) {
          ...
          payloadLen = ext[0]! * 72057594037927940 + ext[1]! * 281474976710656 + ext[2]! * 1099511627776 + ext[3]! * 4294967296 + ext[4]! * 16777216 + ext[5]! * 65536 + ext[6]! * 256 + ext[7]!;
        }
        ...
        // Read payload
        const payload = payloadLen > 0 ? await _readExactly(reader, buf, payloadLen) : new Uint8Array(0);
  ...
  // ---------------------------------------------------------------------------
  /**
  * Send a close frame and set the closeSent flag (used for protocol violations).
  *
  * This implementation detail is included when documentation is built with
  * `--include-private`. It describes state or helper behavior used by the
  * owning module rather than a stable application-facing contract. Prefer the
...
export class WebSocket extends EventTarget {
  ...
  */
  get bufferedAmount(): number {
    return this.#conn.bufferedAmount;
  }
  ...
  */
  send(data: string | ArrayBuffer | ArrayBufferView | Blob): void {
    ...
    if (this.#conn.readyState === CONNECTING) {
      throw Object.assign(new Error('WebSocket is not yet open: buffering not supported'), { name: 'InvalidStateError' });
    }
  ...
  */
  ...
  /**
  * Start the close handshake when leaving a `using` scope.
  *
  * Equivalent to `close()` with the default code 1000 and no reason; the
  * handshake completes in the background after the scope exits.
  *
  * ```ts no_run
  * using ws = new WebSocket('wss://example.com/ws');
  * ```
  */
  [Symbol.dispose](): void {
    this.close();
...
_makePrototypeMembersEnumerable(CloseEvent.prototype, [
  'code',
  'reason',
  'wasClean'
]);
