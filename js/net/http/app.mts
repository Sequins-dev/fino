...
import { join, normalize } from '../../file/path.mts';
import { v4 as uuidv4 } from '../../uuid.mts';
import { parseCookieHeader, serializeCookie, type CookieOptions } from '../../security/cookie.mts';
...
type StackItem =
  | { type: 'middleware'; fn: Middleware; meta?: OperationMeta }
  | { type: 'producer'; name: string; fn: Producer; meta?: OperationMeta };
...
function cloneSchema(schema: unknown): unknown {
  if (schema && typeof (schema as { toJSON?: unknown }).toJSON === 'function') return (schema as { toJSON(): unknown }).toJSON();
  return schema;
}
...
function cloneMeta(meta: OperationMeta): OperationMeta {
  return {
    ...
    parameters: meta.parameters?.map((p) => ({ ...p, schema: cloneSchema(p.schema) })),
...
function cloneResponses(responses: Record<string, OpenApiResponse>): Record<string, OpenApiResponse> {
  ...
  for (const status of Object.keys(responses)) {
    ...
    out[status] = response.content === undefined ? { description: response.description } : { description: response.description, content };
  }
  return out;
}
...
function mergeMeta(base: OperationMeta, next: OperationMeta): OperationMeta {
  ...
  if (next.parameters !== undefined) {
    ...
    for (const parameter of next.parameters) {
      ...
      out.parameters.push({ ...parameter, schema: cloneSchema(parameter.schema) });
    }
  }
...
function forkState(state: BuildState): BuildState {
  return { stack: state.stack.slice(), slots: new Set(state.slots), meta: cloneMeta(state.meta) };
}
...
export function defineMiddleware<T extends Middleware>(fn: T, meta: OperationMeta = {}): T {
  Object.defineProperty(fn, middlewareMeta, { value: cloneMeta(meta), configurable: true });
  return fn;
}
...
class BuilderBase<TSelf> {
  ...
  constructor(state?: BuildState) {
    this._state = state ?? { stack: [], slots: new Set(), meta: {} };
  }
  ...
  use(...middleware: Middleware[]): TSelf {
    for (const fn of middleware) this._state.stack.push({ type: 'middleware', fn, meta: metadataOf(fn) });
  ...
  value(name: string, producer: Producer): TSelf {
    ...
    this._state.stack.push({ type: 'producer', name, fn: producer, meta: metadataOf(producer) });
    return this as unknown as TSelf;
  }
...
export class App extends BuilderBase<App> {
   ...
   * ```
   */
  get(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('GET', path, stack); }
   ...
   * ```
   */
  post(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('POST', path, stack); }
   ...
   * ```
   */
  put(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('PUT', path, stack); }
   ...
   * ```
   */
  patch(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('PATCH', path, stack); }
   ...
   * ```
   */
  delete(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('DELETE', path, stack); }
   ...
   * ```
   */
  head(path: string, ...stack: Array<Middleware | Handler>): this | MethodBuilder { return this.#direct('HEAD', path, stack); }
  ...
  openapi(options: OpenApiOptions): Record<string, unknown> {
    ...
    const doc: Record<string, unknown> = {
      openapi: '3.1.0',
      info: { title: options.title ?? this.#name, version: options.version },
      paths,
    };
...
export class Router extends BuilderBase<Router> {
   ...
   * ```
   */
  get(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('GET', stack); return this; }
   ...
   * ```
   */
  post(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('POST', stack); return this; }
   ...
   * ```
   */
  put(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('PUT', stack); return this; }
   ...
   * ```
   */
  patch(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('PATCH', stack); return this; }
   ...
   * ```
   */
  delete(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('DELETE', stack); return this; }
   ...
   * ```
   */
  head(path: string, ...stack: Array<Middleware | Handler>): this { this.route(path)._method('HEAD', stack); return this; }
...
export class RouteBuilder extends BuilderBase<RouteBuilder> {
   ...
   * ```
   */
  get(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('GET', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  post(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('POST', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  put(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('PUT', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  patch(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('PATCH', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  delete(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('DELETE', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  head(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('HEAD', stack); return stack.length > 0 ? this : builder; }
   ...
   * ```
   */
  options(...stack: Array<Middleware | Handler>): MethodBuilder | RouteBuilder { const builder = this._method('OPTIONS', stack); return stack.length > 0 ? this : builder; }
...
export const body = {
  ...
  form(): Producer {
    return defineProducer(async (ctx) => {
      const req = ctx.request as Request & { formData?: () => Promise<unknown> };
...
export class CookieJar {
  ...
  delete(name: string, options: CookieOptions = {}): void {
    ...
    this.#out.push(serializeCookie(name, '', { ...options, expires: new Date(0), maxAge: 0 }));
  }
...
export function memorySessionStore(): SessionStore {
  ...
  return {
    get(id) {
      ...
      return session === undefined ? null : { id: session.id, isNew: false, data: { ...session.data } };
    },
    set(id, session) {
      map.set(id, { id, isNew: false, data: { ...session.data } });
    },
    delete(id) {
      map.delete(id);
    },
 ...
 * ```
 */
export function sessions(opts: { store: SessionStore; cookie?: string; cookieOptions?: CookieOptions }): Producer {
  ...
  return defineProducer(async (ctx) => {
    ...
    if (session === null) session = { id: uuidv4().toString(), data: {}, isNew: true };
    ...
    ctx.__sessionApply = async (res: Response) => {
      ...
      if (session!.isNew || existing === undefined) jar.set(cookie, session!.id, { path: '/', httpOnly: true, ...opts.cookieOptions });
    };
    return session;
  });
}
 ...
 * ```
 */
export function errorHandler(opts: { expose?: boolean } = {}): Middleware {
  return defineMiddleware(async (_ctx, next) => {
    try {
      return await next();
    } catch (err) {
 ...
 * ```
 */
export function staticFiles(root: string, opts: { index?: string; prefix?: string } = {}): Middleware {
