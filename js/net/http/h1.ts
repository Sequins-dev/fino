...
* @internal
*/
import { Arena, serializeResponse, serializeRequest, parseResponse, connectionParser, buildWireResponse, _buildResponseHead, _concat, Headers, Response } from './index.ts';
...
import type { ServerDriver, ClientDriver, ServerDriverOptions, ClientDriverOptions, ServerHandler, CancelSignal } from './driver.ts';
...
const _101 = new _TextEncoder().encode('HTTP/1.1 101 Switching Protocols\r\nConnection: Upgrade\r\nUpgrade: h2c\r\n\r\n');
...
class _H2cUpgradeTakeover implements ConnectionTakeover {
  ...
  constructor(handler: ServerHandler, opts: ServerDriverOptions, req: Request, settingsPayload: Uint8Array, headRequest: boolean) {
  ...
  async _takeOver(reader: BytesReader, writer: BytesWriter): Promise<void> {
    ...
    await h2Driver.runFromUpgrade(reader as any, writer, this.#handler, this.#opts, this.#req, this.#settingsPayload, this.#headRequest);
  }
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function _shouldKeepAlive(reqHeaders: {
...
async function _writeResponse(writer: BytesWriter, prepared: PreparedResponse, arena: Arena, vecs: Uint8Array[]): Promise<void> {
...
async function _writeResponseBatch(writer: BytesWriter, batch: PreparedResponse[], arena: Arena, vecs: Uint8Array[]): Promise<void> {
  let vecCount = 0;
  let totalBytes = 0;
