...
export class WebTransportDatagramDuplexStream {
  ...
  */
  constructor(send: DatagramSink, maxDatagramSize = 65535) {
    ...
    this.#readable = new ReadableStream<Uint8Array>({ start: (controller) => {
  ...
  */
  createWritable(_options: {
    ...
    return new WritableStream<Uint8Array>({ write: async (chunk) => {
...
export class WebTransport {
  ...
  */
  constructor(url: string | URL, options: WebTransportOptions = {}, connect = true) {
    ...
    this.#incomingBidirectionalStreams = new ReadableStream<WebTransportBidirectionalStream>({ start: (controller) => {
      this.#incomingBidirectionalController = controller;
    } });
    this.#incomingUnidirectionalStreams = new ReadableStream<WebTransportReceiveStream>({ start: (controller) => {
  ...
  */
  ...
  #adopt(other: WebTransport): void {
    ...
    other.ready.then(() => this.#ready.resolve(undefined), (error) => this.#ready.reject(error));
    other.closed.then((info) => this.#finishClose(info), (error) => this.#fail(error));
    other.draining.then(() => this.#draining.resolve(undefined), (error) => this.#draining.reject(error));
  }
...
* @internal
*/
export function _fromHttp3WebTransport(url: string | URL, init: Http3WebTransportInit): WebTransport {
...
* @internal
*/
export function _acceptIncomingQuicWebTransportStream(transport: WebTransport, stream: any, firstChunk: Uint8Array): void {
...
function wrapReceiveStream(reader: {
  read(): Promise<Uint8Array | null>;
}, firstChunk?: Uint8Array): WebTransportReceiveStream {
  let bytesRead = 0;
