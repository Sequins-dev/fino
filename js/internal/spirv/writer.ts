 ...
 * doubling and return an owned copy from `finish()`.
 *
 * Words are written in host order. `vkCreateShaderModule` takes a `uint32_t*`
 * and reads it in host order too, so no byte swapping is needed for that use.
 * Consumers persisting or transporting the words own the byte-order contract.
 *
