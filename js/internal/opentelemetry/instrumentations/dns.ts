...
export class DnsInstrumentation {
  ...
  enable(sdk: OtelSdkLike): Disposable {
    const onStart = topic<RuntimeDnsEvent>(otelRuntimeTopic('dns', 'lookup', 'start')).subscribe(
      (event) => {
    ...
    const onEnd = topic<RuntimeDnsEvent>(otelRuntimeTopic('dns', 'lookup', 'end')).subscribe(
      (event) => finish(event, { code: 'OK' }),
    );
    const onError = topic<RuntimeDnsEvent>(otelRuntimeTopic('dns', 'lookup', 'error')).subscribe(
      (event) =>
        finish(event, {
          code: 'ERROR',
