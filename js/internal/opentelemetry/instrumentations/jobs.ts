...
export class JobsInstrumentation {
  ...
  enable(sdk: OtelSdkLike): Disposable {
    const onStart = topic<JobsRuntimeEvent>(otelRuntimeTopic('jobs', 'job', 'start')).subscribe(
      (event) => {
    ...
    const onEnd = topic<JobsRuntimeEvent>(otelRuntimeTopic('jobs', 'job', 'end')).subscribe(
      (event) => {
        if (!isTracerProviderContextEnabled()) return;
        if (event.jobId === undefined) return;
