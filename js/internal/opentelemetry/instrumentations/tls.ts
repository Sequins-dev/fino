...
export class TlsInstrumentation {
  ...
  enable(sdk: OtelSdkLike): Disposable {
    const onStart = topic<RuntimeTlsEvent>(otelRuntimeTopic('tls', 'handshake', 'start')).subscribe(
      (event) => {
    ...
    const onEnd = topic<RuntimeTlsEvent>(otelRuntimeTopic('tls', 'handshake', 'end')).subscribe(
      (event) => finish(event, { code: 'OK' }),
    );
    const onError = topic<RuntimeTlsEvent>(otelRuntimeTopic('tls', 'handshake', 'error')).subscribe(
      (event) =>
        finish(event, {
          code: 'ERROR',
