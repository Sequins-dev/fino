...
export class TraceTopicInstrumentation {
  ...
  enable(sdk: OtelSdkLike): Disposable {
    ...
    const onAttribute = subscribeMatching<
      SpanRecord & {
        attributeKey?: string;
    ...
    const onEvent = subscribeMatching<
      SpanRecord & {
    ...
    const onLink = subscribeMatching<
      SpanRecord & {
        link?: SpanLinkRecord;
      }
    >(
    ...
    const onStatus = subscribeMatching<SpanRecord>(
      (name) => isScopedTraceTopic(name, 'status'),
      (event) => {
        const span = active.get(event.spanId);
        if (span) span.status = event.status ?? null;
      },
    );
    const onRename = subscribeMatching<
      SpanRecord & {
    ...
    const onEnd = subscribeMatching<SpanRecord>(
      (name) => isScopedTraceTopic(name, 'end'),
      (span) => {
