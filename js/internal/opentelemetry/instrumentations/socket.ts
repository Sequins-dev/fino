...
export class SocketInstrumentation {
  ...
  enable(sdk: OtelSdkLike): Disposable {
    const onStart = topic<RuntimeSocketEvent>(
      otelRuntimeTopic('socket', 'connect', 'start'),
    ).subscribe((event) => {
    ...
    const onEnd = topic<RuntimeSocketEvent>(otelRuntimeTopic('socket', 'connect', 'end')).subscribe(
      (event) => finish(event, { code: 'OK' }),
    );
    const onError = topic<RuntimeSocketEvent>(
      otelRuntimeTopic('socket', 'connect', 'error'),
    ).subscribe((event) =>
      finish(event, {
        code: 'ERROR',
