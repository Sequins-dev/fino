...
function normalizeOtlpOptions(
  ...
  return {
    ...
    onPartialSuccess:
      typeof options.onPartialSuccess === 'function' ? options.onPartialSuccess : null,
...
function withoutProviderContexts<R>(fn: () => R): R {
  return runWithoutTracerProvider(() =>
    runWithoutLoggerProvider(() => runWithoutMeterProvider(fn)),
  );
}
function encodeCompressedBody(
  body: Uint8Array,
  compression: CompressionKind,
): {
  body: Uint8Array;
  ...
  if (compression === 'br' && brotliAvailable)
    return {
...
function parsePartialSuccessText(text: string): PartialSuccessResult | null {
  ...
  const partial = (parsed.partialSuccess || parsed.partial_success) as
...
async function postOtlp(
  ...
  while (attempt < cfg.retry.maxAttempts) {
    ...
    cfg.onError?.(
      new Error(
        `OTLP ${path} export failed with status ${response.status}${responseText ? ` body=${responseText}` : ''}`,
      ),
    );
...
function jsonResource(resource?: Resource): Record<string, unknown> | undefined {
  ...
  if (resource.droppedAttributesCount > 0)
    out.droppedAttributesCount = resource.droppedAttributesCount;
...
function groupByResourceAndScopeJson<
  TRecord extends {
    resource?: Resource;
    scope?: ScopeInfo;
  },
            ...
            ...(span.events?.length
              ? {
                  events: span.events.map((event) => ({
                    timeUnixNano: String(
                      event.timeUnixNano || span.endTimeUnixNano || span.startTimeUnixNano || 0,
                    ),
      ...
      scopeLogs: [
        {
          ...
          logRecords: group.items.map((log) => {
            ...
            const flags =
              log.flags ?? (log.traceFlags !== undefined ? log.traceFlags & 255 : undefined);
            return {
              timeUnixNano: String(log.timeUnixNano || nowUnixNano()),
              observedTimeUnixNano: String(
                log.observedTimeUnixNano || log.timeUnixNano || nowUnixNano(),
              ),
...
function jsonExemplar(exemplar: ExemplarRecord): Record<string, unknown> {
  ...
  if (exemplar.filteredAttributes)
    out.filteredAttributes = jsonAttributes(exemplar.filteredAttributes);
  ...
  else if (typeof exemplar.value === 'number' && Number.isInteger(exemplar.value))
    out.asInt = String(exemplar.value);
...
function jsonMetric(metric: MetricRecord): Record<string, unknown> {
  ...
  const kind = String(
    normalizeMetricKind(metric.aggregationKind || metric.kind || ''),
  ).toLowerCase();
        ...
        dataPoints: [
          {
            ...
            ...(metric.bucketCounts
              ? { bucketCounts: metric.bucketCounts.map((v) => String(v)) }
              : {}),
            ...
            ...(metric.positive
              ? {
                  positive: {
                    ...
                    ...(metric.positive.bucketCounts
                      ? { bucketCounts: metric.positive.bucketCounts.map((v) => String(v)) }
                      : {}),
                  },
                }
              : {}),
            ...(metric.negative
              ? {
                  negative: {
                    ...
                    ...(metric.negative.bucketCounts
                      ? { bucketCounts: metric.negative.bucketCounts.map((v) => String(v)) }
                      : {}),
                  },
                }
              : {}),
        ...
        dataPoints: [
          {
            ...
            ...(metric.quantileValues
              ? {
                  quantileValues: metric.quantileValues.map((v) => ({
