...
* ```
*/
export function normalizeScope(name: string, version?: string, schemaUrl?: string | null, attributes?: Attributes, droppedAttributesCount?: number): ScopeInfo {
  return {
...
export class Resource {
  ...
  */
  constructor(attributes: Attributes = {}, options: ResourceOptions = {}) {
    ...
    this.#entityRefs = Array.isArray(options.entityRefs) ? options.entityRefs.map((ref) => ({
...
export class Baggage {
  ...
  */
  toString(): string {
    return [...this.#entries.entries()].map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join(',');
  }
  /**
...
export function runWithActiveContext<R>(context: ActiveTelemetryContext, fn: () => R): R {
  const baggage = context?.baggage instanceof Baggage ? context.baggage : context?.baggage ? new Baggage(context.baggage) : undefined;
  ...
  return activeTelemetryContext.runWithValue(normalized, () => normalized.baggage instanceof Baggage ? activeBaggageContext.runWithValue(normalized.baggage, fn) : fn());
}
/**
...
const _pendingRequestContexts = new Map<string, {
...
* ```
*/
export function limitAttributeEntries(attributes: Attributes, limits: {
  attributeCountLimit?: number;
  attributeValueLengthLimit?: number;
}): {
  attrs: Attributes;
...
* ```
*/
export function otelRuntimeEvent<TPayload extends Record<string, unknown>>(domain: string, operation: string, phase: string, payload: TPayload = {} as TPayload): TPayload & {
  schemaVersion: number;
  topic: string;
  family: string;
  domain: string;
  ...
  return {
    ...
    operation,
    phase,
    correlationId: payload.requestId || payload.lookupId || payload.connectId || payload.handshakeId || payload.spanId || null,
    ...payload
  };
}
