...
export class Logger {
  ...
  emitRecord(builder: LogRecordBuilder | Partial<LogRecord>): void {
    ...
    if (record.observedTimeUnixNano !== undefined)
      finalRecord.observedTimeUnixNano = record.observedTimeUnixNano;
    if (record.droppedAttributesCount !== undefined)
      finalRecord.droppedAttributesCount = record.droppedAttributesCount;
...
export function applyLogLimits(
  ...
  const hasLimit =
    Number.isFinite(limits.attributeCountLimit ?? Infinity) ||
    Number.isFinite(limits.attributeValueLengthLimit ?? Infinity);
