...
export async function handleEval({ id, code, port }: EvalRequest): Promise<void> {
  ...
  const evaluateCode = (returnByValue: boolean) =>
    (evaluate as (code: string, opts: { replMode: boolean; awaitPromise: boolean; returnByValue: boolean; sourceName: string }) => Promise<string>)(
      code,
      { replMode: true, awaitPromise: true, returnByValue, sourceName },
    );
  ...
  if (exceptionDetails) {
    ...
    const message = (exc?.['description'] as string | undefined) ??
      (exceptionDetails['text'] as string | undefined) ??
