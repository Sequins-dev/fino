...
* @internal
*/
import { createWorkload, dispatchWorkload, driveResidentWorkload, completeHostOperation, terminateWorkload, workloadOwner, workloadWakeFd } from 'internal:scheduler-native';
/**
...
/**
* A scheduled tenant isolate.
*
* `resident` keeps readiness and I/O inside the isolate. When
* `processReadiness` is also true, scalar readiness registration is delegated
* to the process TypeScript reactor so the exited isolate can move between
* worker threads. Callers normally use the helpers in
* `internal:scheduler/readiness` rather than selecting these flags directly.
*/
export class Isolate {
  #handle: number;
  constructor(entryPath: string, resident = false, processReadiness = false) {
    this.#handle = createWorkload(entryPath, !resident, processReadiness);
  }
  /**
