...
export async function runFormat(options: RunFormatOptions): Promise<string> {
  ...
  if (options.check && changed.length > 0) {
    ...
    throw new Error(
      `fino fmt: ${changed.length} file${changed.length === 1 ? '' : 's'} would reformat`,
    );
  }
  if (options.check)
    return files.length === 0
      ? 'fino fmt: no source files found'
      : `fino fmt: ${files.length} file${files.length === 1 ? '' : 's'} checked`;
  return changed.length === 0
    ? 'fino fmt: no changes'
    : `fino fmt: formatted ${changed.length} file${changed.length === 1 ? '' : 's'}`;
}
