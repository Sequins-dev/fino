...
function _normalizeDigestAlgorithm(algorithm: string): DigestAlgorithm {
  switch (algorithm.toLowerCase()) {
    ...
    case 'sha-256':
    case 'sha-384':
    case 'sha-512': return algorithm.toLowerCase() as DigestAlgorithm;
...
function _normalizeCipherAlgorithm(algorithm: string): CipherAlgorithm {
  switch (algorithm) {
    ...
    case 'aes-256-gcm':
    case 'aes-128-cbc':
    case 'aes-256-cbc': return algorithm;
...
export function hmac(algorithm: string, key: Uint8Array | ArrayBuffer, data: Uint8Array | ArrayBuffer): Uint8Array {
  ...
  const result = lib.symbols.HMAC(md, keyArr, keyArr.byteLength, dataArr, dataArr.byteLength, outBuf, outlenBuf);
...
export function pbkdf2(password: Uint8Array, salt: Uint8Array, iterations: number, hashAlg: string, keyLength: number): Uint8Array {
  ...
  const rc = lib.symbols.PKCS5_PBKDF2_HMAC(password, password.byteLength, salt, salt.byteLength, iterations, md, keyLength, out);
...
export function evpPkeyImportSpki(der: Uint8Array): {
  ...
  if (!info || !namedCurve) {
    ...
    throw new Error(`SPKI header does not match any supported EC curve (P-256/P-384/P-521). ` + `Expected ${knownSizes} bytes, got ${der.length}.`);
  }
...
export function sslGetVerifyResult(ssl: object): {
  ...
  const reason = code === 0 ? null : readCStr(lib.symbols.X509_verify_cert_error_string(code) as ArrayBuffer | null) || null;
  return {
    code,
    reason
  };
...
export function sslExportKeyingMaterial(ssl: object, label: string, context: Uint8Array, length: number): ArrayBuffer {
  ...
  const rc = lib.symbols.SSL_export_keying_material(ssl, out, out.byteLength, labelBytes, labelBytes.byteLength, context, context.byteLength, 1) as number;
...
export function sslCtxSetAlpnServerProtos(ctx: object, protocols: string[]): object {
  ...
  const cb = new FfiCallback({
    ...
    result: 'i32'
  }, (_ssl: ArrayBuffer, out: ArrayBuffer, outlen: ArrayBuffer, inPtr: ArrayBuffer, inlen: number | bigint, _arg: ArrayBuffer): number => {
