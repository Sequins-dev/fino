...
const zstdPaths = isDarwin
  ? ['/opt/homebrew/lib/libzstd.dylib', '/usr/local/lib/libzstd.dylib', 'libzstd.dylib']
...
const zstdSymbols = {
  ...
  ZSTD_compress: {
    parameters: ['buffer', 'usize', 'buffer', 'usize', 'i32'],
    result: 'usize',
  },
  ZSTD_isError: {
    parameters: ['usize'],
...
export function zstdCompress(data: ByteInput, opts?: ZstdCompressionOptions): Uint8Array {
  ...
  const written = checkZstd(
    lib,
    BigInt(
      lib.symbols.ZSTD_compress(out, bound, u8, u8.byteLength, opts?.level ?? ZSTD_DEFAULT_LEVEL),
    ),
    'ZSTD_compress',
  );
...
export class ZstdCompressor extends ZstdCodec {
  ...
  constructor(opts?: ZstdCompressionOptions) {
    ...
    checkZstd(
      lib,
      BigInt(
        lib.symbols.ZSTD_CCtx_setParameter(
          cctx,
          ZSTD_C_COMPRESSION_LEVEL,
          opts?.level ?? ZSTD_DEFAULT_LEVEL,
        ),
      ),
      'ZSTD_CCtx_setParameter',
    );
  ...
  write(chunk: ByteInput): Uint8Array[] {
    ...
    while (this.inPos < this.inSize) {
      ...
      checkZstd(
        this.#lib,
        BigInt(
          this.#lib.symbols.ZSTD_compressStream2(
            this.#cctx,
            this.outStruct,
            this.inStruct,
            ZSTD_E_CONTINUE,
...
export class ZstdDecompressor extends ZstdCodec {
  ...
  write(chunk: ByteInput): Uint8Array[] {
    ...
    while (this.inPos < this.inSize) {
      ...
      checkZstd(
        this.#lib,
        BigInt(this.#lib.symbols.ZSTD_decompressStream(this.#dctx, this.outStruct, this.inStruct)),
        'ZSTD_decompressStream',
      );
      this.takeOut(parts);
    }
