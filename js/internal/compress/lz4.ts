...
const lz4Paths = isDarwin
  ? ['/opt/homebrew/lib/liblz4.dylib', '/usr/local/lib/liblz4.dylib', 'liblz4.dylib']
...
const lz4Symbols = {
  ...
  LZ4F_compressUpdate: {
    parameters: ['pointer', 'buffer', 'usize', 'buffer', 'usize', 'pointer'],
    result: 'usize',
  },
  LZ4F_compressEnd: {
    parameters: ['pointer', 'buffer', 'usize', 'pointer'],
...
export class Lz4Compressor implements CompressionTransform {
  ...
  constructor(_opts?: Lz4CompressionOptions) {
    ...
    check(
      this.#lib,
      BigInt(this.#lib.symbols.LZ4F_createCompressionContext(ctxBuf, LZ4F_VERSION)),
      'LZ4F_createCompressionContext',
    );
  ...
  #begin(): Uint8Array[] {
    ...
    const n = Number(
      check(
        this.#lib,
        BigInt(this.#lib.symbols.LZ4F_compressBegin(this.#cctx, header, header.byteLength, null)),
        'LZ4F_compressBegin',
      ),
    );
  ...
  write(chunk: ByteInput): Uint8Array[] {
    ...
    const n = Number(
      check(
        this.#lib,
        BigInt(
          this.#lib.symbols.LZ4F_compressUpdate(this.#cctx, out, bound, u8, u8.byteLength, null),
        ),
        'LZ4F_compressUpdate',
      ),
    );
  ...
  finish(): Uint8Array[] {
    ...
    try {
      ...
      const n = Number(
        check(
          this.#lib,
          BigInt(this.#lib.symbols.LZ4F_compressEnd(this.#cctx, out, bound, null)),
          'LZ4F_compressEnd',
        ),
      );
      if (n > 0) parts.push(out.subarray(0, n).slice());
...
export class Lz4Decompressor implements CompressionTransform {
  ...
  constructor() {
    ...
    check(
      this.#lib,
      BigInt(this.#lib.symbols.LZ4F_createDecompressionContext(ctxBuf, LZ4F_VERSION)),
      'LZ4F_createDecompressionContext',
    );
    this.#dctx = ctxBuf;
  }
  ...
  write(chunk: ByteInput): Uint8Array[] {
    ...
    while (offset < u8.byteLength || u8.byteLength === 0) {
      ...
      check(
        this.#lib,
        BigInt(
          this.#lib.symbols.LZ4F_decompress(
            this.#dctx,
            this.#out,
            this.#dstSizeBuf,
            remaining,
