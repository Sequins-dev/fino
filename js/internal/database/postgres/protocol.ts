 ...
 * ```
 */
export function encodeSaslInitialResponse(
  mechanism: string,
...
export function encodeParse(statement: string, query: string, typeOids: number[] = []): Uint8Array {
  return frontend(
    'P',
    concat([cstring(statement), cstring(query), i16(typeOids.length), ...typeOids.map(i32)]),
  );
}
/**
...
export function encodeBind(
  portal: string,
  statement: string,
  values: Array<string | Uint8Array | null>,
): Uint8Array {
...
class Reader {
  ...
  i16(): number {
    const value = new DataView(this.bytes.buffer, this.bytes.byteOffset + this.offset, 2).getInt16(
      0,
      false,
    );
    this.offset += 2;
  ...
  i32(): number {
    const value = new DataView(this.bytes.buffer, this.bytes.byteOffset + this.offset, 4).getInt32(
      0,
      false,
    );
    this.offset += 4;
...
export function decodeBackendMessage(frame: Uint8Array): BackendMessage {
  ...
  switch (tag) {
    ...
    case 'S':
      return { type: 'ParameterStatus', name: reader.cstring(), value: reader.cstring() };
    case 'Z':
      return { type: 'ReadyForQuery', status: String.fromCharCode(reader.u8()) };
    case '1':
    ...
    case 'E':
      return { type: 'ErrorResponse', fields: decodeFields(reader) };
    case 'N':
      return { type: 'NoticeResponse', fields: decodeFields(reader) };
    case 'A':
      return {
    ...
    case 'G':
      return decodeCopy('CopyInResponse', reader);
    case 'H':
      return decodeCopy('CopyOutResponse', reader);
    case 'W':
      return decodeCopy('CopyBothResponse', reader);
    case 'd':
      return { type: 'CopyData', data: reader.bytesN(frame.byteLength - 5) };
    default:
      return { type: 'Unknown', tag, body: reader.bytesN(frame.byteLength - 5) };
  }
}
