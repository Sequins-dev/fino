...
export class Event {
  ...
  /**
   * Always false for events constructed or dispatched by this JS runtime.
   *
   * ```typescript no_run
   ...
   *
   * Because there is no DOM tree, the path is [target] after dispatch and an
   * empty array before dispatch.
   *
   * ```typescript no_run
   * const event = new Event('x');
   * event.composedPath(); // []
