...
* workloads instead send scalar registrations to a permanently entered
* TypeScript reactor realm that owns the process backend. Each task captures
* its realm owner in scalar user data. Native routing returns readiness to that
* owner, then this module resolves the realm-local promise and leaves the
* actual read/write and buffer ownership in the workload's TypeScript.
*
*
* ## API
*
...
import { pollSharedReactor, registerProcessReadiness, registerSharedReadiness, routeSharedLoopEvent, takeSharedLoopEvents } from 'internal:scheduler-native';
// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const _delegatesReadiness = (globalThis as {
  __finoSchedulerDelegatesReadiness?: boolean;
}).__finoSchedulerDelegatesReadiness === true;
const _nativeReadinessRegistration = (globalThis as {
  __finoNativeReadinessRegistration?: boolean;
}).__finoNativeReadinessRegistration === true;
const _processReadiness = (globalThis as {
  __finoProcessReadiness?: boolean;
}).__finoProcessReadiness === true;
let _raw: object | undefined = _delegatesReadiness || _nativeReadinessRegistration ? undefined : backend.create() as object;
function rawBackend(): object {
  return _raw ??= backend.create() as object;
}
type SchedulerHostOperation = (operation: 'readable' | 'writable' | 'removeRead' | 'removeWrite', args: {
  fd: number;
}) => Promise<unknown>;
...
const _workloadOwner = (globalThis as {
...
function _dispatch(ev: LoopEvent): void {
  ...
  if (owner !== _workloadOwner) {
    routeSharedLoopEvent(owner, JSON.stringify({
      ...ev,
      ident: localId,
      udata: token,
      routed: true
...
* @internal
*/
export function _dispatchNativeEvent(ident: number, filter: number, flags: number, fflags: number, data: number, res: number, udata: number): void {
  _dispatch({
    ident,
    filter,
    flags,
...
export function tick(timeoutMs: number | null): number {
  ...
  const events = _nativeReadinessRegistration ? [] : _wait(rawBackend(), routed.length > 0 ? 0 : timeoutMs);
...
export function readable(fd: number): Promise<number> {
  ...
  return new Promise(function onReadable(resolve) {
    ...
    if (_nativeReadinessRegistration) {
      (_processReadiness ? registerProcessReadiness : registerSharedReadiness)(fd, EVFILT_READ, EV_ADD_ENABLE_ONESHOT, 0, 0, token);
    } else {
...
export function writable(fd: number): Promise<void> {
  ...
  return new Promise(function onWritable(resolve) {
    ...
    const token = taskToken(fd);
    if (_nativeReadinessRegistration) {
      (_processReadiness ? registerProcessReadiness : registerSharedReadiness)(fd, EVFILT_WRITE, EV_ADD_ENABLE_ONESHOT, 0, 0, token);
    } else {
...
export function removeRead(fd: number): void {
  ...
  _reads.delete(fd);
  if (_nativeReadinessRegistration) {
    (_processReadiness ? registerProcessReadiness : registerSharedReadiness)(fd, EVFILT_READ, EV_DELETE, 0, 0, 0);
  } else {
...
export function removeWrite(fd: number): void {
  ...
  _writes.delete(fd);
  if (_nativeReadinessRegistration) {
    (_processReadiness ? registerProcessReadiness : registerSharedReadiness)(fd, EVFILT_WRITE, EV_DELETE, 0, 0, 0);
  } else {
