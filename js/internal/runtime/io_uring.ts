    ...
    if (userData.kind === USER_DATA_TIMER) {
      ...
      if (loop.signalFdToSig.has(ident)) {
        submitSqe(
          ...
          0,
          0,
          packUserData(USER_DATA_SIGNAL, ident),
          POLLIN,
        );
      }
...
export function wait(loop: IoUringLoop, timeoutMs: number | null = null): CqeEvent[] {
  ...
  if (timeoutMs !== null) {
    ...
    submitSqe(
      ...
      1,
      0,
      packUserData(USER_DATA_WAIT_TIMER, waitTimerId),
      0,
    );
  }
  submitPending(loop);
...
export function asyncOpen(
  ...
  submitSqe(
    ...
    mode,
    0,
    packUserData(USER_DATA_FILE, userData),
    flags,
  );
}
/**
