...
export function networkAllowsNetwork(policy: NetworkPolicy | undefined): boolean {
  ...
  const anyAllow = (rules: NetworkRule[] | undefined): boolean =>
...
export function planSeccomp(policy: SandboxPolicy): SeccompPlan {
  ...
  if (syscalls?.mode === 'allowlist') {
    ...
    plan = {
      defaultAction: 'allow',
      rules: syscalls.names.map((syscall) => ({ syscall, action: 'errno' as const })),
    };
  } else {
  ...
  if (
    !networkAllowsNetwork(policy.network) &&
 ...
 * ```
 */
export function checkInitialCommand(
