 ...
 *
 * Reference: [Metal Shading Language specification](https://developer.apple.com/metal/Metal-Shading-Language-Specification.pdf).
 * Emits source only; device allocation, compilation and execution are separate layers.
 *
 * @internal
 *
 * This module is re-exported through `internal:kernel/msl`; import from there.
 */
...
export function lowerToMSL(ir: KernelIR, options: MslOptions = {}): string {
  ...
  function expr(e: Expr): string {
    switch (e.k) {
      ...
      case 'bin': {
        if (e.op === 'mod' && ['f32', 'f16', 'bf16'].includes(typeOf(e.a, env).scalar)) {
          return `fmod(${expr(e.a)}, ${expr(e.b)})`;
        }
