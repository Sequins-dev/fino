 ...
 *
 * References: [SPIR-V specification](https://registry.khronos.org/SPIR-V/specs/unified1/SPIRV.html).
 * This lowering emits the Vulkan 1.1 shader environment; it does not execute kernels.
 *
 * @internal
 *
 * This module is re-exported through `internal:kernel/spirv`; import from there.
 */
import {
  BuiltIn,
  Capability,
...
export interface SpirvOptions {
  /** Target limits; defaults are 1024 threads and 128 parameter bytes. Drivers must supply their device limits. */
  limits?: { maxWorkgroupThreads?: number; maxParamBytes?: number };
...
export function lowerToSPIRV(ir: KernelIR, options: SpirvOptions = {}): Uint32Array {
  ...
  const maxThreads = limits.maxWorkgroupThreads ?? 1024;
  const maxParams = limits.maxParamBytes ?? 128;
  if (
    !Number.isSafeInteger(maxThreads) ||
    maxThreads < 1 ||
    ir.wg.reduce((a, b) => a * b, 1) > maxThreads
  )
    throw new Error('kernel exceeds target workgroup limit');
  if (!Number.isSafeInteger(maxParams) || maxParams < 0 || ir.params.length * 4 > maxParams)
    throw new Error('kernel exceeds target parameter limit');
