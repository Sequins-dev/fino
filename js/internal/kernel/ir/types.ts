 ...
 * expresses typed arithmetic and structured memory operations. Higher layers
 * compose these into numerical kernels without adding target syntax.
 *
 ...
 *   kernel snapshot in this initial subset. Target-specific specialization
 *   and dynamic storage are outside this contract.
 *
 * @internal
 *
 ...
 * field at a 4-byte offset. Target lowerings enforce their own parameter-block limits. There are no 64-bit fields:
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    if (
      ...
      (!Number.isInteger(value) ||
        value < (p.type === 'u32' ? 0 : -2147483648) ||
        value > (p.type === 'u32' ? 4294967295 : 2147483647))
    )
      throw new Error(`parameter '${p.name}' is outside ${p.type} range`);
...
export function validateKernel(ir: KernelIR): void {
  if (ir.wg.length !== 3 || ir.wg.some((n) => !Number.isSafeInteger(n) || n < 1)) {
    throw new Error('workgroup dimensions must be positive safe integers');
  }
  const names = new Set<string>();
  const declare = (name: string) => {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) throw new Error(`invalid identifier '${name}'`);
    if (names.has(name)) throw new Error(`duplicate declaration '${name}'`);
    names.add(name);
  };
  declare(ir.name);
  for (const b of ir.buffers) declare(b.name);
  for (const p of ir.params) declare(p.name);
  for (const s of ir.shared) {
    declare(s.name);
    if (!Number.isSafeInteger(s.length) || s.length < 1)
      throw new Error('shared array length must be a positive safe integer');
  }
  validateBody(ir);
}
