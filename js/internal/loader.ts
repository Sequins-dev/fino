...
const packageMap: PackageMap | null = packageMapJson
  ? (JSON.parse(packageMapJson) as PackageMap)
  : null;
...
function resolveWithPackageMap(specifier: string, referrerDir: string | null): string | null {
  ...
  if (!packageId) {
    throw new Error(
      `Cannot resolve package '${specifier}': no package-map entry. Run 'fino install'.`,
    );
  }
  const pkg = packageMap.packages?.[packageId];
  if (!pkg) {
    throw new Error(
      `Cannot resolve package '${specifier}': missing package-map record for '${packageId}'`,
    );
  }
...
function resolve(specifier: string, referrerDir: string | null, root: string): string {
  ...
  for (const ext of ['.ts', '.tsx', '.mts', '.mdx', '.jsx', '.mjs', '.js', '.json']) {
