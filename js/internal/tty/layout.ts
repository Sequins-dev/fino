...
function placeFlex(node: LayoutNode, spec: BoxSpec, innerW: number, innerH: number): Placed[] {
  ...
  for (const line of lines) {
    ...
    if (free > 0 && growTotal > 0) {
      ...
      // Growing children are flexible in both directions when no child has a
      // dedicated shrink weight. This keeps a grow-based row inside its frame
      // without changing explicit shrink policy when one is present.
      const useShrinkWeights = shrinkTotal > 0;
      let weightLeft = useShrinkWeights ? shrinkTotal : growTotal;
      let deficit = -free;
      if (weightLeft > 0) {
        for (const item of line) {
          const weight = useShrinkWeights ? item.cl.shrink : item.cl.grow;
