...
import { nodePaintOrder, nodeRect } from 'internal:tty/layout';
...
function parentNode(node: TerminalNode): TerminalNode | null {
  return node.parent && 'parent' in node.parent ? node.parent : null;
}

function closest(
  node: TerminalNode | null,
  predicate: (candidate: TerminalNode) => boolean,
): TerminalNode | null {
  let current = node;
  while (current) {
...
function isActivatable(node: TerminalNode): boolean {
  return node.props.disabled !== true && typeof node.props.onClick === 'function';
}
...
export class TuiDispatcher {
  ...
  /** Drop transient state that no longer points at an enabled mounted node. */
  reconcile(): void {
    if (this.#focused && !isFocusable(this.#focused)) this.#setFocus(null);
    if (this.#pressed && !isActivatable(this.#pressed)) this.#pressed = null;
  }
  ...
  hitNode(x: number, y: number): TerminalNode | null {
    ...
    for (const top of this.#root.children) {
      walk(top, (node) => {
        const order = nodePaintOrder(node) ?? -1;
        if (order > bestOrder && contains(node, x, y)) {
          best = node;
          bestOrder = order;
        }
      });
  ...
  #activationTarget(node: TerminalNode | null): TerminalNode | null {
    return closest(node, isActivatable);
  }

  #activate(node: TerminalNode | null): boolean {
    const target = this.#activationTarget(node);
  ...
  #dispatchMouse(event: TuiMouseEventLike): boolean {
    ...
    if (event.action === 'press') {
      this.#pressed = this.#activationTarget(target);
      const focusable = closest(target, isFocusable);
    ...
    if (event.action === 'release') {
      ...
      this.#pressed = null;
      if (pressed && this.#activationTarget(target) === pressed) {
