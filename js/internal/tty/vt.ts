...
const ATTRS = new Set([1, 2, 3, 4, 7, 9]);
...
export class Terminal {
  ...
  #applySgr(params: string): void {
    ...
    while (i < parts.length) {
      ...
      if (p === 0) {
        ...
        if (mode === 5 && i + 2 < parts.length) {
          ...
          color = [p, 2, parts[i + 2]!, parts[i + 3]!, parts[i + 4]!];
          i += 4;
        } else {
          break;
        }
    ...
    const out = [...this.#attrs].sort((a, b) => a - b);
    if (this.#fg) out.push(...this.#fg);
    if (this.#bg) out.push(...this.#bg);
    this.#sgr = out.length === 0 ? EMPTY_SGR : Object.freeze(out);
  }
