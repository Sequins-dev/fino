...
export class JobsStore {
    ...
    return {
      save: (state: WorkflowState): Promise<void> =>
        this.#op(async () => {
          ...
          const stmt =
            db.prepare(`INSERT INTO workflow_runs(run_id, workflow_id, status, state, updated_at)
  ...
  async #insertJobRaw(job: InsertJob): Promise<{
    ...
    const stmt = this.#db
      .prepare(`INSERT INTO jobs (id, queue, task, input, status, priority, run_at, attempts, max_attempts, backoff, timeout_ms, dedupe_key, schedule_id, created_at, updated_at)
    ...
    try {
      changes = (
        await stmt.run({
          ...
          backoff:
            job.backoff === null || job.backoff === undefined ? null : JSON.stringify(job.backoff),
    ...
    if (changes === 0) {
      ...
      if (existing === null) {
        throw new Error(
          `jobs insert conflicted but no active job found for key "${job.dedupeKey}"`,
        );
      }
      return {
        job: existing,
  ...
  async #findActiveByDedupeRaw(queue: string, dedupeKey: string): Promise<JobRecord | null> {
    const stmt = this.#db.prepare(
      `SELECT * FROM jobs WHERE queue = :queue AND dedupe_key = :dedupe_key AND status IN ${ACTIVE_STATUSES} LIMIT 1`,
    );
    try {
  ...
  async #claimReadyRaw(
    ...
    const taskFilter =
      tasks === undefined ? '' : ` AND task IN (${tasks.map(() => '?').join(',')})`;
  ...
  async #sweepLeasesRaw(now: number): Promise<JobRecord[]> {
    return this.#db.transaction(async () => {
      const expired = this.#db.prepare(
        `SELECT * FROM jobs WHERE status IN ('claimed','running') AND claimed_until IS NOT NULL AND claimed_until <= :now`,
      );
          ...
          await this.#update(
            job.id,
            {
              ...
              error: JSON.stringify({
                message: `lease expired after attempt ${job.attempts}/${job.maxAttempts}`,
              }),
              finished_at: now,
            },
            now,
  ...
  async #heartbeatRaw(
    ...
    const stmt = this.#db.prepare(
      `UPDATE jobs SET claimed_until = ?, updated_at = ? WHERE claimed_by = ? AND status IN ('claimed','running') AND id IN (${placeholders})`,
    );
    try {
  ...
  cancel(id: string): Promise<boolean> {
    return this.#op(async () => {
      const stmt = this.#db.prepare(
        `UPDATE jobs SET status = 'cancelled', claimed_by = NULL, claimed_until = NULL, finished_at = :now, updated_at = :now WHERE id = :id AND status IN ${ACTIVE_STATUSES}`,
      );
      try {
  ...
  retry(id: string): Promise<boolean> {
    return this.#op(async () => {
      const stmt = this.#db.prepare(
        `UPDATE jobs SET status = 'pending', attempts = 0, run_at = :now, error = NULL, finished_at = NULL, updated_at = :now WHERE id = :id AND status IN ('dead','error','cancelled')`,
      );
      try {
        const { changes } = await stmt.run({
          id,
  ...
  wake(id: string): Promise<boolean> {
    return this.#op(async () => {
      const stmt = this.#db.prepare(
        `UPDATE jobs SET status = 'pending', run_at = :now, updated_at = :now WHERE id = :id AND status = 'waiting'`,
      );
      try {
        const { changes } = await stmt.run({
          id,
  ...
  async #queueStatsRaw(queue?: string): Promise<QueueStats> {
    ...
    const stmt = this.#db.prepare(
      `SELECT status, COUNT(*) AS count, MIN(CASE WHEN status = 'pending' THEN run_at ELSE NULL END) AS oldest_pending_at FROM jobs ${where} GROUP BY status`,
    );
    try {
      for (const row of await stmt.all(params)) {
        ...
        else if (status in stats && status !== 'oldestPendingAt')
          (stats as Record<string, number | null>)[status] = count;
        if (row.oldest_pending_at !== null && row.oldest_pending_at !== undefined) {
          ...
          stats.oldestPendingAt =
            stats.oldestPendingAt === null ? oldest : Math.min(stats.oldestPendingAt, oldest);
        }
      }
    } finally {
      stmt.finalize();
  ...
  async #listJobsRaw(filter: {
    ...
    const stmt = this.#db.prepare(
      `SELECT * FROM jobs ${where.length > 0 ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY created_at DESC, id DESC LIMIT :limit OFFSET :offset`,
    );
    try {
  ...
  async #upsertScheduleRaw(schedule: {
    ...
    const now = Date.now();
    const stmt = this.#db
      .prepare(`INSERT INTO schedules (id, task, input, queue, spec, enabled, overlap, catchup, retry, next_run_at, created_at, updated_at)
  ...
  dueSchedules(now = Date.now()): Promise<ScheduleRecord[]> {
    return this.#op(async () => {
      const stmt = this.#db.prepare(
        'SELECT * FROM schedules WHERE enabled = 1 AND next_run_at <= :now ORDER BY next_run_at ASC',
      );
      try {
  ...
  async #advanceScheduleRaw(
    ...
    const stmt =
      firing !== undefined
        ? this.#db.prepare(
            'UPDATE schedules SET next_run_at = :next_run_at, last_run_at = :last_run_at, last_job_id = :last_job_id, updated_at = :now WHERE id = :id',
          )
        : this.#db.prepare(
            'UPDATE schedules SET next_run_at = :next_run_at, updated_at = :now WHERE id = :id',
          );
    try {
      await stmt.run(
        firing !== undefined
