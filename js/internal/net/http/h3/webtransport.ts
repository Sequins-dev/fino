...
function toBigInt(value: bigint | number): bigint {
  ...
  if (!Number.isSafeInteger(value))
    throw new RangeError('QUIC varint number must be a safe integer');
  return BigInt(value);
}
/**
...
export function webTransportSettingsEnabled(settings: ReadonlyMap<number, number>): boolean {
  return (
    settings.get(SETTINGS_WT_ENABLED) === 1 &&
 ...
 * ```
 */
export function encodeH3SettingsPayload(
  settings: ReadonlyMap<number, number | bigint>,
): Uint8Array {
...
export function decodeH3SettingsFrame(frame: Uint8Array): Map<number, number | bigint> {
  ...
  if (type.value !== BigInt(H3_SETTINGS_FRAME_TYPE))
    throw new RangeError('HTTP/3 frame is not SETTINGS');
 ...
 * ```
 */
export function readWebTransportSettings(
  controlStreamBytes: Uint8Array,
  ...
  return decodeH3SettingsPayload(
    controlStreamBytes.slice(located.payloadStart, located.payloadEnd),
  );
}
/**
...
export function injectWebTransportSettings(controlStreamBytes: Uint8Array): Uint8Array {
  ...
  const settings = decodeH3SettingsPayload(
    controlStreamBytes.slice(located.payloadStart, located.payloadEnd),
  );
 ...
 * ```
 */
export function encodeHttpDatagram(
  ...
  if (streamId < 0n || (streamId & 3n) !== 0n) {
    throw new RangeError(
      'HTTP Datagram session stream id must be a client-initiated bidirectional stream',
    );
  }
 ...
 * ```
 */
export function encodeWebTransportStreamPrefix(
  ...
  if (streamId < 0n || (streamId & 3n) !== 0n) {
    throw new RangeError(
      'WebTransport session stream id must be a client-initiated bidirectional stream',
    );
  }
  const type =
    kind === 'bidirectional' ? WEBTRANSPORT_BIDI_STREAM_TYPE : WEBTRANSPORT_UNI_STREAM_TYPE;
