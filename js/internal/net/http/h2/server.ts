...
function _readFrameStreamId(bytes: Uint8Array, off: number): number {
  return (bytes[off + 5]! & 127) << 24 | bytes[off + 6]! << 16 | bytes[off + 7]! << 8 | bytes[off + 8]!;
}
function _buildRawFrame(type: number, flags: number, streamId: number, payload: Uint8Array): Uint8Array {
...
class H2ServerFrameValidator {
  ...
  constructor(private readonly maxConcurrent: number, expectPreface: boolean, upgradeStream1 = false, optionalPreface = false) {
  ...
  #validateFrame(type: number, flags: number, streamId: number, length: number, payloadOff: number): H2RawAction | 'ignore' | null {
    if (this.#continuationStream !== 0) {
    ...
    switch (type) {
      case NGHTTP2_FRAME_TYPE_DATA: return this.#validateData(flags, streamId, length, payloadOff);
      case NGHTTP2_FRAME_TYPE_HEADERS: return this.#validateHeaders(flags, streamId);
      case NGHTTP2_FRAME_TYPE_PRIORITY: return this.#validatePriority(streamId, length, payloadOff);
      case NGHTTP2_FRAME_TYPE_RST_STREAM: return this.#validateRstStream(streamId, length);
      case NGHTTP2_FRAME_TYPE_SETTINGS: return this.#validateSettings(flags, streamId, length, payloadOff);
      case NGHTTP2_FRAME_TYPE_PUSH_PROMISE: return {
        kind: 'goaway',
        errorCode: NGHTTP2_PROTOCOL_ERROR
      };
      case NGHTTP2_FRAME_TYPE_PING: return this.#validatePing(flags, streamId, length);
      case NGHTTP2_FRAME_TYPE_WINDOW_UPDATE: return this.#validateWindowUpdate(streamId, length, payloadOff);
  ...
  #validatePriority(streamId: number, length: number, payloadOff: number): H2RawAction | null {
    ...
    const dependency = (this.#buffer[payloadOff]! & 127) << 24 | this.#buffer[payloadOff + 1]! << 16 | this.#buffer[payloadOff + 2]! << 8 | this.#buffer[payloadOff + 3]!;
    if (dependency === streamId) return {
      kind: 'rst',
      streamId,
      errorCode: NGHTTP2_PROTOCOL_ERROR
    };
  ...
  #validateSettings(flags: number, streamId: number, length: number, payloadOff: number): H2RawAction | null {
    ...
    if ((flags & 1) !== 0) {
      ...
      for (let off = payloadOff; off < payloadOff + length; off += 6) {
        ...
        if (id === 4) {
          const value = (this.#buffer[off + 2]! << 24 | this.#buffer[off + 3]! << 16 | this.#buffer[off + 4]! << 8 | this.#buffer[off + 5]!) >>> 0;
  ...
  #validateWindowUpdate(streamId: number, length: number, payloadOff: number): H2RawAction | null {
    ...
    const increment = (this.#buffer[payloadOff]! & 127) << 24 | this.#buffer[payloadOff + 1]! << 16 | this.#buffer[payloadOff + 2]! << 8 | this.#buffer[payloadOff + 3]!;
    if (increment === 0) {
      if (streamId === 0) return {
        kind: 'goaway',
        errorCode: NGHTTP2_PROTOCOL_ERROR
...
function _makeCtx(writer: BytesWriter, handler: ServerHandler, maxConcurrent: number): H2ServerCtx {
  ...
  async function dispatchStream(stream: H2ServerStream): Promise<void> {
    ...
    const reqBody = stream.method === 'GET' || stream.method === 'HEAD' ? null : stream.body as any;
  ...
  function drainStreams(): void {
    ...
    for (const [, s] of streams) {
      s.cancelled = true;
      s.body.error(new HttpStreamError('transport', `H2 stream ${s.streamId} drained before request body completed`, { streamId: s.streamId }));
      if (s.triggerDispatch !== null) {
        s.bodyDone = true;
        s.triggerDispatch();
        s.triggerDispatch = null;
...
// Shared recv loop
// ---------------------------------------------------------------------------
async function _recvLoop(reader: BufferedBytesReader, writer: BytesWriter, session: Nghttp2Session, inFlight: Set<Promise<void>>, drainWrite: () => Promise<void>, drainStreams: () => void, cancelStream: (streamId: number) => void, goawayReceived: () => boolean, maxConcurrent: number, expectPreface: boolean, upgradeStream1 = false, optionalPreface = false): Promise<void> {
  const validator = new H2ServerFrameValidator(maxConcurrent, expectPreface, upgradeStream1, optionalPreface);
...
export class H2ServerDriver implements ServerDriver {
  ...
  * ```
  */
  async run(reader: BufferedBytesReader, writer: BytesWriter, handler: ServerHandler, opts: ServerDriverOptions): Promise<void> {
    const ctx = _makeCtx(writer, handler, opts.maxConcurrent);
    const session = Nghttp2Session.createServer(ctx.callbacks);
    ctx.setSession(session);
    session.submitSettings([[NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS, opts.maxConcurrent], [NGHTTP2_SETTINGS_MAX_HEADER_LIST_SIZE, 65536]]);
    await ctx.drainWrite();
    await _recvLoop(reader, writer, session, ctx.inFlight, ctx.drainWrite, ctx.drainStreams, ctx.cancelStream, ctx.goawayReceived, opts.maxConcurrent, true);
  }
  /**
  ...
  * ```
  */
  async runFromUpgrade(reader: BufferedBytesReader, writer: BytesWriter, handler: ServerHandler, opts: ServerDriverOptions, initialReq: Request, settingsPayload: Uint8Array, headRequest: boolean): Promise<void> {
    const ctx = _makeCtx(writer, handler, opts.maxConcurrent);
    const session = Nghttp2Session.createServer(ctx.callbacks);
    ctx.setSession(session);
    ...
    await _recvLoop(reader, writer, session, ctx.inFlight, ctx.drainWrite, ctx.drainStreams, ctx.cancelStream, ctx.goawayReceived, opts.maxConcurrent, false, true, true);
  }
}
