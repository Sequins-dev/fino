...
export class QuicStream extends EventTarget {
  ...
  resetAt(errorCode: number, finalSize: number | bigint): void {
    ...
    if (reliableSize < 0n)
      throw new RangeError('QUIC stream resetAt finalSize must be non-negative');
  ...
  #assertConnectionOpen(): void {
    if (
      this.#connection[quicConnectionInternals.isClosedForInternalUse]() ||
      ptrAddress(this.#connection.nativeHandle) === 0n
    ) {
  ...
  [quicStreamInternals.pushIncoming](offset: number, data: Uint8Array, fin: boolean): void {
    ...
    this.#stats.maxOffsetReceived = Math.max(
      this.#stats.maxOffsetReceived,
  ...
  #insertIncomingSegment(offset: number, data: Uint8Array): number {
    ...
    for (const segment of segments) {
      ...
      const coveredUntil =
        last === undefined ? this.#incomingOffset : last.offset + last.data.byteLength;
      if (segmentEnd <= coveredUntil) continue;
      const appendOffset = Math.max(segmentStart, coveredUntil);
      normalized.push({
        offset: appendOffset,
        data:
          appendOffset === segment.offset
            ? segment.data
            : segment.data.subarray(appendOffset - segment.offset),
      });
    }
  ...
  [quicStreamInternals.resetFromConnection](errorCode: number): void {
    ...
    deferAfterNativeCallback(() =>
      this.dispatchEvent(
        new QuicStreamResetEvent('reset', {
          error,
          errorCode,
        }),
      ),
  ...
  [quicStreamInternals.blockedFromConnection](): void {
    deferAfterNativeCallback(() =>
      this.dispatchEvent(
        new QuicStreamBlockedEvent('blocked', {
          stream: this,
          connection: this.#connection,
          streamId: this.id,
        }),
  ...
  [quicStreamInternals.stopSendingFromConnection](errorCode: number): void {
    ...
    deferAfterNativeCallback(() =>
      this.dispatchEvent(new QuicStopSendingEvent('stopsending', { errorCode })),
    );
    this.#maybeClose();
  }
  ...
  [quicStreamInternals.closeFromConnection](error?: Error): void {
    if (
      error !== undefined &&
      (this.#incomingFinOffset === null || this.#incomingOffset < this.#incomingFinOffset)
    ) {
