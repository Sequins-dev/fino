...
type ResolvedRetryOptions = { enabled: false } | { enabled: true; tokenSecret?: Uint8Array };
type ResolvedPreferredAddressOptions = { ipv4?: QuicAddress; ipv6?: QuicAddress };
type ResolvedMigrationOptions = { enabled: boolean; preferredAddress?: ResolvedPreferredAddressOptions; usePreferredAddress: boolean };
...
export interface QuicDatagramTransport {
  ...
  recvNow(maxBytes: number): { data: Uint8Array; addr: QuicAddress; ecn?: number } | null;
  /** Read up to `maxPackets` packets if available, or return an empty array. */
  recvBatch?(maxPackets: number, maxBytes: number): Array<{ data: Uint8Array; addr: QuicAddress; ecn?: number }>;
  ...
  sendBatch?(packets: Array<{ data: Uint8Array; dest: QuicAddress; ecn?: number }>): { sent: number; errno: number | null };
...
export interface QuicDatagramTransportFactory {
  ...
  bind(address: QuicAddress, options?: { ecn?: boolean }): Promise<QuicDatagramTransport>;
}
...
type PreferredAddressParams = {
  ipv4?: PreferredAddressEntry & { address: QuicAddress };
  ipv6?: PreferredAddressEntry & { address: QuicAddress };
};

const STREAM_DATA_FLAG_FIN = 0x01;
const TLS_ALERT_NO_APPLICATION_PROTOCOL = 120;
const NGTCP2_CRYPTO_TOKEN_MAGIC_RETRY2 = 0xb7;
...
const NGTCP2_QLOG_WRITE_FLAG_FIN = 0x01;
const NGTCP2_ENCRYPTION_LEVEL_1RTT = 2;
const NGTCP2_MILLISECONDS = 1_000_000n;
const NGTCP2_SECONDS = 1_000_000_000n;
...
const CONNECTION_DRAINING_TIMEOUT_MS = 3_000;
...
const NGTCP2_CONNECTION_REFUSED = 0x02;
const VERSION_NEGOTIATION_GREASE = 0x0a0a0a0a;
...
class RealQuicDatagramTransport implements QuicDatagramTransport {
  ...
  recvNow(maxBytes: number): { data: Uint8Array; addr: QuicAddress } | null {
    ...
    return received.ecn === undefined ? { data: received.data, addr } : { data: received.data, addr, ecn: received.ecn };
  }
  ...
  recvBatch(maxPackets: number, maxBytes: number): Array<{ data: Uint8Array; addr: QuicAddress; ecn?: number }> {
    ...
    if (received !== null) {
      ...
      return received.flatMap((packet) => {
        ...
        return [packet.ecn === undefined ? { data: packet.data, addr } : { data: packet.data, addr, ecn: packet.ecn }];
      });
    }
    const packets: Array<{ data: Uint8Array; addr: QuicAddress; ecn?: number }> = [];
  ...
  sendBatch(packets: Array<{ data: Uint8Array; dest: QuicAddress; ecn?: number }>): { sent: number; errno: number | null } {
    ...
    for (const packet of packets) {
      ...
      if (rc < 0) return { sent, errno: rc };
      sent++;
    }
    return { sent, errno: null };
  }
...
function normalizeSocketOptions(input: QuicSocketOptions | undefined): Required<Pick<QuicSocketOptions, never>> & QuicSocketOptions {
  ...
  return {
    ...
    ...(input.receiveBufferSize === undefined ? {} : { receiveBufferSize: normalizeSocketOptionInteger(input.receiveBufferSize, 1, 0x7fffffff, 'receiveBufferSize') }),
    ...(input.sendBufferSize === undefined ? {} : { sendBufferSize: normalizeSocketOptionInteger(input.sendBufferSize, 1, 0x7fffffff, 'sendBufferSize') }),
...
class RealQuicDatagramTransportFactory implements QuicDatagramTransportFactory {
  ...
  async bind(address: QuicAddress, options: { ecn?: boolean } = {}): Promise<QuicDatagramTransport> {
...
class ByteQueue {
  #chunks: Uint8Array[] = [];
  #waiters: (QueueResolver<Uint8Array | null> & { maxBytes: number; cleanup(): void })[] = [];
  #closed = false;
  ...
  read(maxBytes = 65536, signal?: AbortSignal | null): Promise<Uint8Array | null> {
    ...
    return new Promise((resolve, reject) => {
      let cleanup = () => {};
      const waiter = { resolve, reject, maxBytes, cleanup: () => cleanup() };
      const onAbort = () => {
...
function normalizeRetry(input: QuicRetryOptions | undefined, base?: ResolvedQuicOptions): ResolvedRetryOptions {
  ...
  return input.tokenSecret === undefined
    ? { enabled: true }
    : { enabled: true, tokenSecret: input.tokenSecret.slice() };
}
...
function normalizeDatagrams(input: QuicDatagramOptions | undefined, base?: ResolvedQuicOptions): ResolvedDatagramOptions {
  ...
  if (!Number.isInteger(maxPending) || maxPending < 0 || maxPending > 0xffff) {
  ...
  return { enabled: true, maxFrameSize, maxPending, dropPolicy, maxSendAttempts };
}
...
function normalizeMigration(input: QuicMigrationOptions | undefined, base?: ResolvedQuicOptions): ResolvedMigrationOptions {
  if (input === undefined) return base?.migration ?? { enabled: false, usePreferredAddress: false };
...
function normalizeRateLimit(input: QuicRateLimitOptions | undefined, base: ResolvedRateLimitOptions | undefined, defaults: ResolvedRateLimitOptions, name: string): ResolvedRateLimitOptions {
  ...
  if (input === false) return { rate: 0, burst: 0 };
...
function normalizeConnection(input: QuicConnectionOptions | undefined, base?: ResolvedQuicOptions): ResolvedConnectionOptions {
  ...
  return {
    ...
    unacknowledgedPacketThreshold: BigInt(normalizeLimit(
      input?.unacknowledgedPacketThreshold,
      baseConnection === undefined ? undefined : Number(baseConnection.unacknowledgedPacketThreshold),
      0,
    ...
    activeConnectionIdLimit: BigInt(normalizeClampedInteger(
    ...
    ackDelayExponent: BigInt(normalizeInteger(
...
function normalizeTransport(input: QuicTransportOptions | undefined, base?: ResolvedQuicOptions): ResolvedTransportOptions {
  ...
  const allow = input?.sourceAddress?.allow === undefined
    ? baseTransport?.sourceAddress.allow ?? null
    : normalizeAddressSet(input.sourceAddress.allow);
  const deny = input?.sourceAddress?.deny === undefined
  ...
  return {
    ...
    retryRateLimit: normalizeRateLimit(
      ...
      { rate: DEFAULT_RETRY_RATE, burst: DEFAULT_RETRY_BURST },
      'retryRateLimit',
    ),
    versionNegotiationRateLimit: normalizeRateLimit(
      input?.versionNegotiationRateLimit,
      baseTransport?.versionNegotiationRateLimit,
      { rate: DEFAULT_VERSION_NEGOTIATION_RATE, burst: DEFAULT_VERSION_NEGOTIATION_BURST },
    ...
    statelessResetRateLimit: normalizeRateLimit(
      ...
      { rate: DEFAULT_STATELESS_RESET_RATE, burst: DEFAULT_STATELESS_RESET_BURST },
    ...
    immediateCloseRateLimit: normalizeRateLimit(
      ...
      { rate: DEFAULT_IMMEDIATE_CLOSE_RATE, burst: DEFAULT_IMMEDIATE_CLOSE_BURST },
    ...
    sessionCreationRateLimit: normalizeRateLimit(
      ...
      { rate: DEFAULT_SESSION_CREATION_RATE, burst: DEFAULT_SESSION_CREATION_BURST },
...
function freezeRateLimitSnapshot(input: ResolvedRateLimitOptions): QuicResolvedRateLimitOptions {
  return Object.freeze({ rate: input.rate, burst: input.burst });
}
...
function longHeaderVersion(packet: Uint8Array): number | null {
  if (packet.byteLength < 5 || (packet[0]! & 0x80) === 0) return null;
...
class QuicTokenBucket {
  ...
  consume(timestamp = now(this.runtime)): boolean {
    const elapsedSeconds = Number(timestamp - this.#lastTimestamp) / 1_000_000_000;
...
function generateRegularToken(secret: Uint8Array, remoteAddress: QuicAddress, runtime: QuicRuntime): Uint8Array | null {
  ...
  const tokenLen = Number(cryptoSym!.ngtcp2_crypto_generate_regular_token(
    token,
    secret,
    secret.byteLength,
...
function cidBytes(cid: ArrayBuffer | null): Uint8Array {
  ...
  const bytes = cid.byteLength >= NGTCP2_CID_SIZE
    ? new Uint8Array(cid)
...
function readPacketVarint(packet: Uint8Array, offset: number): { value: number; offset: number } | null {
  ...
  if (length === 1) return { value: first & 0x3f, offset: offset + 1 };
  if (length === 2) return { value: ((first & 0x3f) << 8) | packet[offset + 1], offset: offset + 2 };
  if (length === 4) {
    return {
      value: ((first & 0x3f) * 0x1000000) + (packet[offset + 1] << 16) + (packet[offset + 2] << 8) + packet[offset + 3],
      offset: offset + 4,
    };
  }
  return null;
...
function parseInitialTokenHeader(packet: Uint8Array): { version: number; dcid: ArrayBuffer; scid: ArrayBuffer; token: Uint8Array } | null {
  if (packet.byteLength < 7 || (packet[0] & 0x80) === 0 || (packet[0] & 0x30) !== 0) return null;
  ...
  return { version, dcid, scid, token };
}

function packetHeaderFromParsedInitial(parsed: { version: number; dcid: ArrayBuffer; scid: ArrayBuffer; token: Uint8Array }): ArrayBuffer {
...
function makePath(localAddress: QuicAddress, remoteAddress: QuicAddress, fd = 0): NativePath {
  ...
  return { path, local, remote, userData, fd };
}
...
function pathSnapshotFromNative(path: ArrayBuffer | null, fallbackFd: number): PathSnapshot | null {
  ...
  return { localAddress, remoteAddress, fd: fdFromPath(copy, fallbackFd) };
}
...
function pathSnapshotsDiffer(a: PathSnapshot | null, b: PathSnapshot | null): boolean {
  ...
  return a.fd !== b.fd
    || addressKey(a.localAddress) !== addressKey(b.localAddress)
    || addressKey(a.remoteAddress) !== addressKey(b.remoteAddress);
}
...
function preferredAddressFromNative(paddr: ArrayBuffer | null, family: 'ipv4' | 'ipv6'): QuicAddress | null {
  ...
  const presentOffset = family === 'ipv4'
    ? TP_PREFERRED_ADDR_IPV4_PRESENT
    : TP_PREFERRED_ADDR_IPV6_PRESENT;
...
function transportParameterSnapshot(params: ArrayBuffer | null): QuicTransportParameterSnapshot | null {
  ...
  const preferred = Pointer.readU8(params, TP_PREFERRED_ADDR_PRESENT) === 0
    ? null
  ...
  const statelessResetToken = Pointer.readU8(params, TP_STATELESS_RESET_TOKEN_PRESENT) === 0
    ? null
    : Pointer.copyFrom(Pointer.offset(params, TP_STATELESS_RESET_TOKEN), NGTCP2_STATELESS_RESET_TOKENLEN) as Uint8Array;
  return Object.freeze({
...
function qlogOutputPath(basePath: string | undefined, connectionId: string): { path: string; directory?: string } {
  if (basePath === undefined || basePath.length === 0) return { path: `${connectionId}.sqlog` };
  if (basePath.endsWith('/')) return { directory: basePath.slice(0, -1), path: `${basePath}${connectionId}.sqlog` };
  const last = basePath.slice(basePath.lastIndexOf('/') + 1);
  if (last.includes('.')) return { path: basePath };
  return { directory: basePath, path: `${basePath}/${connectionId}.sqlog` };
}
...
function appendKeylogLine(options: QuicKeylogOptions, line: string): void {
  ...
  const fd = Number(fileLib.symbols.open(fileCstr(options.path), O_WRONLY | O_CREAT | O_APPEND, 0o600));
  if (fd < 0) return;
  try {
    fileLib.symbols.fchmod(fd, 0o600);
...
function createServerTlsContext(
  certificateFile: string,
  privateKeyFile: string,
  alpnProtocols: string[],
  ...
  return newServerContext(
    ...
    { ...tlsOptions, groups: tlsOptions.groups ?? options.tlsGroups },
  );
}
...
function qlogWriteCallbackPointer(): ArrayBuffer {
  if (_qlogWriteCallback === null) {
    _qlogWriteCallback = new FfiCallback(
      { parameters: ['pointer', 'u32', 'pointer', 'usize'], result: 'void' },
...
function makeSettings(options: ResolvedQuicOptions, runtime: QuicRuntime, token: Uint8Array | null = null, tokenType = NGTCP2_TOKEN_TYPE_UNKNOWN, originalVersion = 0): ArrayBuffer {
  ...
  writeU64(settings, SETTINGS_MAX_TX_UDP_PAYLOAD_SIZE, BigInt(
...
function ensureCallbackTable(): ArrayBuffer {
  ...
  const getConn = new FfiCallback({ parameters: ['pointer'], result: 'pointer' }, (connRef: ArrayBuffer) => withNativeCallback(() => {
    const userData = Pointer.readPointer(connRef, 8) as ArrayBuffer | null;
    return connectionFromUserData(userData)?.nativeHandle ?? null;
  }));

  const handshakeCompleted = new FfiCallback({ parameters: ['pointer', 'pointer'], result: 'i32' }, (_conn: ArrayBuffer, userData: ArrayBuffer | null) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onHandshakeCompleted();
    return 0;
  }));

  const handshakeConfirmed = new FfiCallback({ parameters: ['pointer', 'pointer'], result: 'i32' }, (_conn: ArrayBuffer, userData: ArrayBuffer | null) => withNativeCallback(() => {
  ...
  const recvDatagram = new FfiCallback(
    { parameters: ['pointer', 'u32', 'pointer', 'usize', 'pointer'], result: 'i32' },
  ...
  const streamOpen = new FfiCallback({ parameters: ['pointer', 'i64', 'pointer'], result: 'i32' }, (_conn: ArrayBuffer, streamId: bigint, userData: ArrayBuffer | null) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onRemoteStreamOpen(Number(streamId));
    return 0;
  }));

  const streamClose = new FfiCallback({ parameters: ['pointer', 'u32', 'i64', 'u64', 'pointer', 'pointer'], result: 'i32' }, (_conn, _flags, streamId, _appCode, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onStreamClose(Number(streamId));
    return 0;
  }));

  const streamReset = new FfiCallback({ parameters: ['pointer', 'i64', 'u64', 'u64', 'pointer', 'pointer'], result: 'i32' }, (_conn, streamId, _finalSize, appCode, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onStreamReset(Number(streamId), Number(appCode));
    return 0;
  }));

  const acked = new FfiCallback({ parameters: ['pointer', 'i64', 'u64', 'u64', 'pointer', 'pointer'], result: 'i32' }, (_conn, streamId, offset, datalen, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onAckedStreamDataOffset(Number(streamId), Number(offset), Number(datalen));
    return 0;
  }));
  const extendStreamsBidi = new FfiCallback({ parameters: ['pointer', 'u64', 'pointer'], result: 'i32' }, (_conn, _maxStreams, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onLocalStreamCredit('bidirectional');
    return 0;
  }));
  const extendStreamsUni = new FfiCallback({ parameters: ['pointer', 'u64', 'pointer'], result: 'i32' }, (_conn, _maxStreams, userData) => withNativeCallback(() => {
  ...
  const stopSending = new FfiCallback({ parameters: ['pointer', 'i64', 'u64', 'pointer', 'pointer'], result: 'i32' }, (_conn, streamId, appCode, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onStreamStopSending(Number(streamId), Number(appCode));
    return 0;
  }));
  const rand = new FfiCallback({ parameters: ['pointer', 'usize', 'pointer'], result: 'void' }, (dest: ArrayBuffer | null, len: bigint) => withNativeCallback(() => {
    if (dest !== null) Pointer.copyTo(dest, randomBytes(Number(len)));
  }));
  const getPathChallengeData = new FfiCallback({ parameters: ['pointer', 'pointer', 'pointer'], result: 'i32' }, (_conn, data) => withNativeCallback(() => {
    if (data === null) return NGTCP2_ERR_CALLBACK_FAILURE;
    Pointer.copyTo(data, randomBytes(8));
    return 0;
  }));
  const getNewConnectionId = new FfiCallback({ parameters: ['pointer', 'pointer', 'pointer', 'usize', 'pointer'], result: 'i32' }, (_conn, cid, token, cidlen, userData) => withNativeCallback(() => {
  ...
  const removeConnectionId = new FfiCallback({ parameters: ['pointer', 'pointer', 'pointer'], result: 'i32' }, (_conn, cid, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._unregisterIssuedCid(cid);
    return 0;
  }));
  const dcidStatus = new FfiCallback(
    { parameters: ['pointer', 'i32', 'u64', 'pointer', 'pointer', 'pointer'], result: 'i32' },
  ...
  const recvNewToken = new FfiCallback(
    { parameters: ['pointer', 'pointer', 'usize', 'pointer'], result: 'i32' },
  ...
  const ackDatagram = new FfiCallback({ parameters: ['pointer', 'u64', 'pointer'], result: 'i32' }, (_conn, dgramId, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onDatagramStatus(Number(dgramId), 'ack');
    return 0;
  }));
  const lostDatagram = new FfiCallback({ parameters: ['pointer', 'u64', 'pointer'], result: 'i32' }, (_conn, dgramId, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onDatagramStatus(Number(dgramId), 'lost');
    return 0;
  }));
  const recvKey = new FfiCallback({ parameters: ['pointer', 'i32', 'pointer'], result: 'i32' }, (_conn, level, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onKeyInstalled(level);
    return 0;
  }));
  const recvVersionNegotiation = new FfiCallback(
    { parameters: ['pointer', 'pointer', 'pointer', 'usize', 'pointer'], result: 'i32' },
    (_conn, hd, sv, nsv, userData) => withNativeCallback(() => {
      connectionFromUserData(userData)?._onVersionNegotiation(hd, sv, Number(nsv));
      return 0;
    }),
  );
  const earlyDataRejected = new FfiCallback({ parameters: ['pointer', 'pointer'], result: 'i32' }, (_conn, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onEarlyDataRejected();
    return 0;
  }));
  const recvStatelessReset = new FfiCallback({ parameters: ['pointer', 'pointer', 'pointer'], result: 'i32' }, (_conn, _sr, userData) => withNativeCallback(() => {
    connectionFromUserData(userData)?._onStatelessReset();
    return 0;
  }));
  const beginPathValidation = new FfiCallback(
    { parameters: ['pointer', 'u32', 'pointer', 'pointer', 'pointer'], result: 'i32' },
  ...
  const pathValidation = new FfiCallback(
    { parameters: ['pointer', 'u32', 'pointer', 'pointer', 'i32', 'pointer'], result: 'i32' },
...
export class QuicEndpoint extends EventTarget {
  ...
  constructor(options: QuicEndpointOptions = {}, internals: QuicEndpointInternals = {}) {
    ...
    this.#transportFactory = internals.transportFactory ?? (options.socket === undefined
      ? realQuicDatagramTransportFactory
      : new RealQuicDatagramTransportFactory(options.socket));
    this.#clientBindAddress = internals.clientBindAddress === undefined
      ? undefined
    ...
    publishQuicTopic('quic.endpoint.created', { endpoint: this, options: this.#options });
  }
  ...
  setBusy(busy: boolean): void {
    ...
    if (previousBusy !== nextBusy) {
      publishQuicTopic('quic.endpoint.busy.change', { endpoint: this, busy: nextBusy });
    }
  }
   ...
   * @internal
   */
  _inspectAddressValidationStats(): { retrySent: number; retryTokenAccepted: number; addressTokenAccepted: number } {
    return {
  ...
  async _bindTransport(address: QuicAddress, options: { ecn?: boolean } = {}): Promise<QuicDatagramTransport> {
  ...
  async listen(options: QuicListenOptions = {}): Promise<QuicListener> {
    ...
    try {
      ...
      if (options.sni !== undefined) {
        for (const [servername, sni] of Object.entries(options.sni)) {
          sniContexts.set(servername, createServerTlsContext(
            sni.certificateFile,
            sni.privateKeyFile,
    ...
    listener._start();
    publishQuicTopic('quic.endpoint.listen', { endpoint: this, listener, address: bound, options: listenerOptions });
    return listener;
  }
  ...
  async connect(options: QuicConnectOptions): Promise<QuicConnection> {
    ...
    const bindAddress = this.#clientBindAddress ?? (remoteAddress.family === 'ipv6'
      ? { family: 'ipv6', ip: '::', port: 0 }
      : { family: 'ipv4', ip: '0.0.0.0', port: 0 });
    ...
    const ctx = newClientContext(
      ...
      resolvedOptions.keylog === false ? undefined : (line) => appendKeylogLine(resolvedOptions.keylog, line),
      {
    ...
    const earlyDataMax = resolvedOptions.earlyData === false ? 0 : resolvedOptions.earlyData.maxBytes ?? 0xffffffff;
    ...
    if (sessionState !== undefined && sessionState !== null) {
      if (sessionState.expiresAt !== undefined && sessionState.expiresAt <= Date.now()) {
        ...
        if (sessionState.ticket !== undefined && resolvedOptions.earlyData !== false) {
          attemptedEarlyData = true;
          const storedEarlyDataMax = resolvedOptions.earlyData === false
            ? 0
            : sessionState.earlyDataMax ?? resolvedOptions.earlyData.maxBytes ?? 0xffffffff;
          if (sessionState.version !== undefined && !resolvedOptions.versions.includes(sessionState.version)) {
    ...
    const earlyTransportParametersAccepted = resolvedOptions.earlyData !== false
      && resumedSession
      && sessionEarlyDataMax > 0
      && rememberedTransportParameters !== undefined
      && connection._setEarlyTransportParameters(rememberedTransportParameters);
    const earlyDataReady = earlyTransportParametersAccepted;
    if (attemptedEarlyData && !earlyDataReady && earlyDataRejectReason === null) {
      earlyDataRejectReason = rememberedTransportParameters === undefined || !earlyTransportParametersAccepted
    ...
    publishQuicTopic('quic.endpoint.connect', { endpoint: this, connection, address: remoteAddress, options: resolvedOptions });
  ...
  async close(): Promise<void> {
    if (this.#closed) return;
    publishQuicTopic('quic.endpoint.closing', { endpoint: this, stats: this.stats });
    this.#closed = true;
    ...
    publishQuicTopic('quic.endpoint.closed', { endpoint: this, stats: this.stats });
    this.#dispatch(new Event('close'));
  }

  async closeGracefully(options: QuicCloseOptions = {}): Promise<void> {
    if (this.#closed) return;
    publishQuicTopic('quic.endpoint.closing', { endpoint: this, stats: this.stats, graceful: true });
    this.#closed = true;
    this.#destroyedAt = Date.now();
    for (const listener of this.#listeners.slice()) await listener.close();
    ...
    this.#transports.clear();
    this.#acceptQueue.close(new Error('QUIC endpoint is closed'));
    publishQuicTopic('quic.endpoint.closed', { endpoint: this, stats: this.stats, graceful: true });
    this.#dispatch(new Event('close'));
  }
  ...
  _track(connection: QuicConnection): void {
    ...
    publishQuicTopic(
      connection._roleForStats() === 'server' ? 'quic.session.created.server' : 'quic.session.created.client',
      { endpoint: this, connection, address: connection.remoteAddress },
    );
    ...
    connection.addEventListener('error', (event: any) => {
      ...
      publishQuicTopic('quic.endpoint.error', { endpoint: this, error, connection });
  ...
  #connectingServerConnection(listener: QuicListener, remoteAddress: QuicAddress): QuicConnection | null {
    ...
    for (const connection of this.#connections) {
      if (
        ...
        || !connection._matchesServerConnection(listener, remoteAddress)
  ...
  _handleDatagram(listener: QuicListener | null, transportOrFd: QuicDatagramTransport | number, localAddress: QuicAddress, packet: Uint8Array, remoteAddress: QuicAddress, packetEcn?: number): void {
    ...
    if (existing) {
      ...
      if (
        ...
        && existing._roleForStats() === 'server'
        && existing._matchesServerConnection(listener, remoteAddress)
        && existing._wireVersionForRouting() !== routedVersion
        ...
        this.#acceptInitial(listener, transport, localAddress, remoteAddress, packet, decoded, packetEcn);
        return;
      }
      existing._receivePacket(packet, remoteAddress, localAddress, transport, packetEcn);
      return;
    }

    if ((packet[0] & 0x80) === 0 && this.#handleStatelessReset(packet)) return;
    ...
    if (connecting !== null && connecting._wireVersionForRouting() === version) {
      connecting._receivePacket(packet, remoteAddress, localAddress, transport, packetEcn);
      return;
    }
  ...
  #handleStatelessReset(packet: Uint8Array): boolean {
    if (packet.byteLength < 17 || (packet[0] & 0x80) !== 0) return false;
  ...
  #writeStatelessReset(listener: QuicListener, transport: QuicDatagramTransport, remoteAddress: QuicAddress, dcid: Uint8Array, sourcePacketLength: number): void {
    ...
    const n = ngtcp2PktWriteStatelessReset(
      out,
      packetLength,
      token,
      random,
  ...
  #writeImmediateConnectionCloseFromInitial(transport: QuicDatagramTransport, remoteAddress: QuicAddress, packet: Uint8Array): void {
    ...
    const n = Number(cryptoSym!.ngtcp2_crypto_write_connection_close(
      out,
      out.byteLength,
      parsed.version,
  ...
  #writeVersionNegotiation(listener: QuicListener, transport: QuicDatagramTransport, remoteAddress: QuicAddress, decoded: ArrayBuffer): void {
    ...
    const out = new Uint8Array(NGTCP2_MAX_UDP_PAYLOAD_SIZE);
    const n = Number(ngtcp2Sym!.ngtcp2_pkt_write_version_negotiation(
      out,
      out.byteLength,
      randomBytes(1)[0],
      clientScid,
  ...
  #writeRetryFromParts(listener: QuicListener, transport: QuicDatagramTransport, remoteAddress: QuicAddress, version: number, clientScid: ArrayBuffer, originalDcid: ArrayBuffer, maxPacketLength: number): void {
    ...
    const tokenLen = Number(cryptoSym!.ngtcp2_crypto_generate_retry_token2(
      token,
      listener.retryTokenSecret,
    ...
    const n = Number(cryptoSym!.ngtcp2_crypto_write_retry(
      out,
      out.byteLength,
      version,
  ...
  #validateRetryToken(listener: QuicListener, remoteAddress: QuicAddress, hd: ArrayBuffer): { originalDcid: ArrayBuffer; retryScid: ArrayBuffer; token: Uint8Array; tokenType: number } | null {
  ...
  #validateAddressToken(listener: QuicListener, remoteAddress: QuicAddress, hd: ArrayBuffer): { token: Uint8Array; tokenType: number } | null {
    const tokenLen = Number(readU64(hd, PKT_HD_TOKENLEN));
    if (tokenLen === 0) return null;
    const token = copyFromPtr(ptrField(hd, PKT_HD_TOKEN), tokenLen);
  ...
  #acceptInitial(listener: QuicListener, transport: QuicDatagramTransport, localAddress: QuicAddress, remoteAddress: QuicAddress, packet: Uint8Array, decoded: ArrayBuffer, packetEcn?: number): void {
    const parsedInitial = parseInitialTokenHeader(packet);
    if (parsedInitial !== null && parsedInitial.token.byteLength > 0 && !isRetryToken(parsedInitial.token)) {
      const parsedHd = packetHeaderFromParsedInitial(parsedInitial);
      const addressInfo = this.#addressValidation.peek(remoteAddress);
      const remoteAddressValidated = addressInfo?.validated === true;
      const retry = parsedInitial.token.byteLength <= 64
        ? null
    ...
    const earlyDataMax = listener.options.earlyData === false ? 0 : 0xffffffff;
    ...
    this._track(connection);
    const rc = connection._receivePacket(packet, remoteAddress, localAddress, transport, packetEcn);
...
export class QuicListener {
  ...
  constructor(endpoint: QuicEndpoint, address: QuicAddress, alpnProtocols: string[], transports: QuicDatagramTransport[], ctx: QuicTlsContext, options: ResolvedQuicOptions, sniContexts: Map<string, QuicTlsContext> = new Map()) {
    ...
    this._ctx = ctx;
    this.#retryTokenSecret = options.retry.enabled && options.retry.tokenSecret !== undefined
      ? options.retry.tokenSecret.slice()
      : randomBytes(32);
  }
  ...
  setSNIContexts(entries: Record<string, QuicSNIContextOptions>): void {
    ...
    try {
      for (const [servername, sni] of Object.entries(entries)) {
        newContexts.set(servername, createServerTlsContext(
          sni.certificateFile,
          sni.privateKeyFile,
          sni.alpnProtocols?.slice() ?? this.alpnProtocols,
          this.options,
          {
...
export class QuicConnection extends EventTarget {
  ...
  #lastDatagramEvent: { data: Uint8Array; earlyData: boolean } | null = null;
  #nextDatagramId = 1n;
  ...
  get localTransportParameters(): QuicTransportParameterSnapshot | null {
    ...
    return this.#withInitialSourceConnectionId(
      transportParameterSnapshot(ngtcp2Sym!.ngtcp2_conn_get_local_transport_params(this.#conn) as ArrayBuffer | null),
  ...
  get remoteTransportParameters(): QuicTransportParameterSnapshot | null {
    if (ptrAddress(this.#conn) === 0n) return null;
    return this.#withInitialSourceConnectionId(
      transportParameterSnapshot(ngtcp2Sym!.ngtcp2_conn_get_remote_transport_params(this.#conn) as ArrayBuffer | null),
   ...
   * @internal
   */
  _inspectLastDatagramEvent(): { data: Uint8Array; earlyData: boolean } | null {
   ...
   * @internal
   */
  _inspectSendState(): { pendingWriteCount: number; pendingWriteBytes: number; outstandingStreamBytes: number } {
  ...
  #tryOpenLocalStream(direction: 'bidirectional' | 'unidirectional'): QuicStream | number {
    const out = new ArrayBuffer(8);
    const rc = direction === 'bidirectional'
  ...
  #waitForLocalStreamCredit(direction: 'bidirectional' | 'unidirectional'): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.#localStreamCreditWaiters[direction].push({ resolve, reject });
    });
  }
  ...
  async sendDatagram(data: QuicDatagramSource, encoding: QuicDatagramEncoding = 'utf8'): Promise<number> {
    ...
    this.#pendingDatagrams.push({ id, data: copy, attempts: 0, earlyData });
  ...
  #writePendingDatagram(outPath: NativePath, out: Uint8Array, ts: bigint, pktInfo: ArrayBuffer | null): number {
    for (;;) {
      ...
      const n = Number(ngtcp2Sym!.ngtcp2_conn_write_datagram_versioned(
        this.#conn,
  ...
  #queueWrittenPacket(batch: PendingSendPacket[], n: number, outPath: NativePath, fallbackRemoteAddress: QuicAddress, out: Uint8Array, ts: bigint, pktInfo: ArrayBuffer | null): void {
    ...
    batch.push({ fd: output.fd, data: out.subarray(0, n), remoteAddress: output.remoteAddress, ecn: packetInfoEcn(pktInfo) });
  ...
  async #close(errorCode: number, reason: string, sendConnectionClose: boolean, closeError?: Error, type: 'transport' | 'application' = 'application'): Promise<void> {
    ...
    if (this.#closeInfo === null) {
      this.#closeInfo = { errorCode, reason, type, remote: closeError !== undefined };
    }
    const canSendConnectionClose = sendConnectionClose
      && ptrAddress(this.#conn) !== 0n
      && !this.#connectionCloseSent;
    publishQuicTopic('quic.session.closing', { connection: this, errorCode, reason });
    this.#state = 'closing';
    if (this.#stats.closingAt === null) this.#stats.closingAt = Date.now();
    ...
    publishQuicTopic('quic.session.closed', { connection: this, error: closeError, stats: this.stats, closeInfo: this.closeInfo });
    this.#dispatch(new Event('close'));
  }
  ...
  #computeDrainingRetentionMs(): number {
    ...
    return Math.max(1, Math.ceil(Number(retentionNs) / 1_000_000));
  }
  ...
  #closeFromTransport(errorCode = 0, reason = '', closeError?: Error, type: 'transport' | 'application' = 'transport', remote = closeError !== undefined): void {
    ...
    try {
      if (remote && this.#closeInfo === null) {
        this.#closeInfo = { errorCode, reason, type, remote: true };
      }
  ...
  _scheduleEarlyDataEvent(accepted: boolean, reason: string): void {
    this.#runtime.defer(() => {
      this.#runtime.setTimer(0, () => {
        if (this.#closed) return;
        this.dispatchEvent(new QuicEarlyDataEvent('earlydata', { accepted, rejected: !accepted, reason }));
      });
    });
  }
  ...
  async #saveSessionTicket(ticket: Uint8Array, emitEvent = true): Promise<void> {
    ...
    if (emitEvent) {
      ...
      publishQuicTopic('quic.session.ticket', { connection: this, ticket: ticketCopy });
    }
    ...
    const earlyDataMax = this.#options.earlyData === false ? 0 : this.#options.earlyData.maxBytes ?? 0xffffffff;
    const version = this.version;
    const existing = await store.load(key) ?? {};
    await store.save(key, transportParameters === null
      ? { ...existing, ticket: ticketCopy, earlyDataMax, version }
      : { ...existing, ticket: ticketCopy, transportParameters, earlyDataMax, version });
  }
  ...
  _initClient(rememberedVersion = 0): void {
    ...
    const initialVersion = rememberedVersion !== 0
  ...
  #createNative(dcid: ArrayBuffer, scid: ArrayBuffer, originalDcid: ArrayBuffer | null, version: number, client: boolean, originalVersion = 0): void {
    ...
    const preferredAddress = !client && this.#options.migration.preferredAddress !== undefined
      ? this.#makePreferredAddressParams()
      : null;
    const statelessResetToken = !client && this.#listener !== null
    ...
    const keepAliveTimeout = this.#options.connection.keepAliveTimeout > 0n
  ...
  #scheduleHandshakeTimeout(): void {
    ...
    const timeoutMs = Math.max(1, Number(this.#options.connection.handshakeTimeout / 1_000_000n));
  ...
  #waitHandshakeConfirmed(): Promise<void> {
    ...
    this.#startDeferredHandshake();
    this._driveWrites();
    return new Promise((resolve, reject) => this.#handshakeConfirmedWaiters.push({ resolve, reject }));
  }
  ...
  #fail(error: Error): void {
    ...
    publishQuicTopic('quic.session.error', { connection: this, error });
  ...
  #writeBufferSize(): number {
    ...
    return maxPayload > 0
      ? Math.min(65536, Math.max(NGTCP2_MAX_UDP_PAYLOAD_SIZE, maxPayload))
      : NGTCP2_MAX_UDP_PAYLOAD_SIZE;
  }
  ...
  #outputFromPath(path: ArrayBuffer, fallbackRemoteAddress: QuicAddress, fallbackFd: number = this.#fd): { fd: number; localAddress: QuicAddress; remoteAddress: QuicAddress } {
    ...
    return { fd, localAddress, remoteAddress };
  }
  ...
  #syncActivePathSnapshot(snapshot: PathSnapshot | null): boolean {
    ...
    const changed = fd !== this.#fd
      || addressKey(localAddress) !== addressKey(this.#activeLocalAddress)
      || addressKey(remoteAddress) !== addressKey(this.remoteAddress);
  ...
  #sendPacket(fd: number, data: Uint8Array, remoteAddress: QuicAddress): boolean {
    return this.#blockedSend.length === 0 && this.#flushPacketBatch([{ fd, data, remoteAddress }]);
  }
  ...
  #flushPacketBatch(packets: PendingSendPacket[]): boolean {
    for (let index = 0; index < packets.length;) {
      ...
      const result = transport.sendBatch === undefined
        ? this.#sendPacketChunkFallback(transport, chunk)
        : transport.sendBatch(chunk.map((packet) => ({ data: packet.data, dest: packet.remoteAddress, ecn: packet.ecn })));
  ...
  #sendPacketChunkFallback(transport: QuicDatagramTransport, packets: PendingSendPacket[]): { sent: number; errno: number | null } {
    let sent = 0;
    for (const packet of packets) {
  ...
  #writeConnectionClose(liberr: number, remoteAddress: QuicAddress): void {
    ...
    const n = Number(ngtcp2Sym!.ngtcp2_conn_write_connection_close_versioned(
      this.#conn,
      Pointer.of(outPath.path),
      NGTCP2_PKT_INFO_VERSION,
      null,
  ...
  _receivePacket(packet: Uint8Array, remoteAddress: QuicAddress, localAddress: QuicAddress = this.localAddress, transport: QuicDatagramTransport | null = this.#transportById(this.#fd), packetEcn?: number): number {
    ...
    const packetPath = this.#retainPath(localAddress, remoteAddress, fd);
  ...
  _driveWrites(remoteAddress: QuicAddress = this.remoteAddress): void {
    ...
    try {
      ...
      for (; packets < packetBudget; packets++) {
        ...
        const writeNoStreamData = (): number => Number(ngtcp2Sym!.ngtcp2_conn_writev_stream_versioned(
          this.#conn,
          Pointer.of(outPath.path),
          NGTCP2_PKT_INFO_VERSION,
          pktInfoPtr,
        ...
        const isCoalescingRetry = (code: number): boolean => code === NGTCP2_ERR_WRITE_MORE
        ...
        if (this.#pendingWrites.length > 0) {
          ...
          for (;;) {
            ...
            const flags = (pending.fin ? NGTCP2_WRITE_STREAM_FLAG_FIN : 0)
              | NGTCP2_WRITE_STREAM_FLAG_MORE;
            coalescing = true;
  ...
  #scheduleTimer(): void {
    ...
    let delayMs = Math.max(1, Number(deltaNs / 1_000_000n));
  ...
  #nextStreamIdleDelayMs(current: bigint): number | null {
    ...
    return Math.max(1, Number((nextDue - current) / 1_000_000n));
  }
  ...
  _onDatagram(data: Uint8Array, earlyData: boolean): void {
    ...
    this.#lastDatagramEvent = { data: data.slice(), earlyData: receivedEarlyData };
    ...
    this.#dispatch(new QuicDatagramEvent('datagram', { data, earlyData: receivedEarlyData }));
  }
  ...
  _onDatagramStatus(id: number, status: QuicDatagramStatus): void {
    ...
    publishQuicTopic('quic.session.receive.datagram.status', { connection: this, id, status });
    this.#dispatch(new QuicDatagramStatusEvent('datagramstatus', { id, status }));
    const eventType = status === 'ack'
      ? 'datagramack'
      : status === 'lost'
        ? 'datagramlost'
        : 'datagramabandoned';
    this.#dispatch(new QuicDatagramStatusEvent(eventType, { id, status }));
  }
  ...
  #publishVersionNegotiation(wireVersion: number, requestedWireVersions: number[], supportedWireVersions = this.#options.versions.map(versionToWire)): void {
    publishQuicTopic('quic.session.version.negotiation', {
      connection: this,
      version: wireVersion === NGTCP2_PROTO_VER_V1 || wireVersion === NGTCP2_PROTO_VER_V2
  ...
  _onNewToken(token: Uint8Array): void {
    ...
    if (this.#options.sessionStore !== undefined) {
      ...
      void Promise.resolve(store.load(key)).then((existing) => {
        return store.save(key, { ...(existing ?? {}), addressToken });
      }).catch((error) => {
    ...
    this.#dispatch(new QuicNewTokenEvent('newtoken', { token: eventToken, address: this.remoteAddress }));
    publishQuicTopic('quic.session.new.token', { connection: this, token: eventToken, address: this.remoteAddress });
  }
  ...
  #openQlogFd(): number {
    ...
    if (output.directory !== undefined && output.directory.length > 0) {
      fileLib.symbols.mkdir(fileCstr(output.directory), 0o755);
    }
    const fd = Number(fileLib.symbols.open(fileCstr(output.path), O_WRONLY | O_CREAT | O_TRUNC, 0o644));
    if (fd < 0) throw new Error(`qlog open failed: ${fd}`);
    fileLib.symbols.fchmod(fd, 0o644);
    this.#qlogFd = fd;
    return fd;
  }
  ...
  _onEarlyDataRejected(): void {
    ...
    this.#dispatch(new QuicEarlyDataEvent('earlydata', { accepted: false, rejected: true, reason: 'rejected-by-peer' }));
  }
  ...
  _onPathValidationFinished(path: ArrayBuffer | null, fallbackPath: ArrayBuffer | null, result: number, flags: number): void {
    ...
    deferAfterNativeCallback(() => {
      ...
      publishQuicTopic('quic.session.path.validation', { connection: this, ...init });
  ...
  #forgetActivePathValidation(path: PathSnapshot | null, previousPath: PathSnapshot | null, flags: number): void {
    ...
    const index = this.#activePathValidations.findIndex((validation) => (
...
export class QuicStream extends EventTarget {
  ...
  _resetFromConnection(errorCode: number): void {
    ...
    publishQuicTopic('quic.stream.reset', { stream: this, connection: this.#connection, error, errorCode });
    deferAfterNativeCallback(() => this.dispatchEvent(new QuicStreamResetEvent('reset', { error, errorCode })));
    if (this.#readStopped) {
      this.#maybeClose();
      return;
    }
  ...
  #finish(error?: Error): void {
    ...
    publishQuicTopic('quic.stream.closed', { stream: this, connection: this.#connection, error, stats: this.stats });
