...
export type DnssecCache = {
  entries: Map<string, { value: DnskeyInput[] | null; expiresAt: number }>;
  maxEntries?: number;
};
...
function dnskeyRdata(flags: number, algorithm: number, publicKeyBase64: string): Uint8Array {
  return concatBytes([writeU16(flags), new Uint8Array([3, algorithm]), base64Decode(publicKeyBase64)]);
}
...
function dnssecError(message: string): Error & { code: string } {
  const err = new Error(`dnssec: ${message}`) as Error & { code: string };
  err.code = 'EDNSSEC';
  return err;
}
...
export async function digestDnskey(ownerName: string, dnskeyRdata: Uint8Array, digestType: number): Promise<Uint8Array> {
  ...
  switch (digestType) {
    ...
    case 2: hash = 'SHA-256'; break;
    case 4: hash = 'SHA-384'; break;
...
function parseRsaDnskeyPublicKey(publicKey: Uint8Array): { exponent: Uint8Array; modulus: Uint8Array } {
...
function dnskeyPublicData(dnskeyRdata: Uint8Array): { flags: number; protocol: number; algorithm: number; publicKey: Uint8Array } {
...
async function importDnskey(dnskeyRdata: Uint8Array): Promise<{ key: CryptoKey; verifyAlgorithm: { name: string; hash?: string } }> {
  ...
  if (dnskey.algorithm === 13 || dnskey.algorithm === 14) {
    ...
    return { key, verifyAlgorithm: { name: 'ECDSA', hash } };
  }
  ...
  if (dnskey.algorithm === 15) {
    ...
    return { key, verifyAlgorithm: { name: 'Ed25519' } };
  }
...
export async function validateSignedResponse(
  ...
  const signatures = response.answers
    .filter((record) => record.type === 46 && sameName(record.name, rrset[0]!.name) && isRrsigData(record.data))
...
function signaturesFor(response: DnssecResponse, name: string, type: number): RrsigData[] {
  return [...response.answers, ...response.authorities]
    .filter((record) => record.type === QTYPE_RRSIG && sameName(record.name, name) && isRrsigData(record.data))
    .map((record) => record.data as RrsigData)
...
function nsecTypes(record: DnssecRecord): number[] {
  const data = record.data as { types?: number[] } | undefined;
...
function nsec3Data(record: DnssecRecord): {
  ...
  if (typeof data?.hashAlgorithm !== 'number'
    ...
    || !Array.isArray(data.types)) return null;
  return {
...
function nsecNextName(record: DnssecRecord): string | null {
  const data = record.data as { nextDomainName?: string } | undefined;
...
function cacheSet(cache: DnssecCache | undefined, key: string, value: DnskeyInput[] | null, expiresAt: number): void {
  ...
  cache.entries.set(key, { value, expiresAt });
...
async function hasValidatedDsNegativeProof(
  ...
  for (const record of nsec3s) {
    ...
    if ((data.flags & 0x01) !== 0 && await nsec3RecordCoversName(record, name)) return true;
  }
  return false;
}
...
async function getTrustedKeys(
  ...
  state: { fetches: number; depth: number },
  ...
  if (directAnchors.length > 0) {
    if (normalizedZone === '.') {
      try {
        ...
        // Fall back to the trust anchors themselves for deterministic fixtures
        // and for environments that block root DNSKEY bootstrap fetches.
      }
    }
  ...
  for (const key of dnskeys) {
    for (const ds of dsRecords) {
      const data = ds.data as { keyTag?: number; algorithm?: number; digestType?: number; digest?: Uint8Array } | undefined;
...
export async function validateDnssecResponse(
  ...
  async function validateDenial(): Promise<void> {
    ...
    for (const signerName of signerNames) {
      const keys = await getTrustedKeys(signerName, options, cache, { fetches: 0, depth: 0 });
      if (keys === null) return;
  ...
  async function hasInsecureAncestor(): Promise<boolean> {
    for (const zone of ancestorZones(qname)) {
      if (zone === '.') continue;
      const keys = await getTrustedKeys(zone, options, cache, { fetches: 0, depth: 0 });
  ...
  for (const candidate of candidates) {
    ...
    for (const sig of sigs) {
      const keys = await getTrustedKeys(sig.signerName, options, cache, { fetches: 0, depth: 0 });
      if (keys === null) return;
