...
function resolveLinkOptions(
  input: SimulatedLinkOptions | undefined,
  base: RequiredLinkOptions = DEFAULT_LINK,
): RequiredLinkOptions {
  return {
    ...
    bandwidthBytesPerMs:
      input?.bandwidthBytesPerMs === undefined
    ...
    duplicateRate:
      input?.duplicateRate === undefined ? base.duplicateRate : clampRate(input.duplicateRate),
    corruptionRate:
      input?.corruptionRate === undefined ? base.corruptionRate : clampRate(input.corruptionRate),
...
class SimulatedDatagramSocket implements DatagramSocket {
  ...
  send(data: Uint8Array, dest: SocketAddress, ecn?: number): Promise<number> {
    ...
    if (!(data instanceof Uint8Array))
      return Promise.reject(new TypeError('Simulated datagram send data must be a Uint8Array'));
  ...
  sendNow(data: Uint8Array, dest: SocketAddress, ecn?: number): number {
    ...
    if (!(data instanceof Uint8Array))
      throw new TypeError('Simulated datagram send data must be a Uint8Array');
    this.#provider._sendDatagram(this.address, data, dest, ecn);
  ...
  recv(maxBytes = 65536): Promise<{
    ...
    if (!Number.isInteger(maxBytes) || maxBytes < 0)
      return Promise.reject(new RangeError('maxBytes must be a non-negative integer'));
...
export class SimulatedNetworkProvider extends NetworkProvider {
  ...
  advance(ms: number): number {
    if (!Number.isFinite(ms) || ms < 0)
      throw new RangeError('advance(ms) requires a non-negative finite number');
    this.#clock += ms;
    return this.#clock;
  }
  /**
  ...
  dropNextDatagrams(
    ...
    if (!Number.isInteger(count) || count < 0)
      throw new RangeError('dropNextDatagrams(count) requires a non-negative integer');
    if (count === 0) return;
  ...
  corruptNextDatagrams(
    ...
    if (!Number.isInteger(count) || count < 0)
      throw new RangeError('corruptNextDatagrams(count) requires a non-negative integer');
    if (count === 0) return;
  ...
  async connect(addr: SocketAddress, _opts?: ConnectOptions): Promise<Connection> {
    ...
    const server = new SimulatedConnection(
      addr,
      localAddress,
      pair.serverInbound,
      pair.clientInbound,
  ...
  async datagram(addr: SocketAddress): Promise<DatagramSocket> {
    ...
    const key = addressKey(bound);
    if (this.#datagrams.has(key))
      throw new Error(`Simulated datagram socket already bound at ${key}`);
  ...
  #allocateAddress<T extends Map<string, unknown>>(
    addr: SocketAddress,
    occupied: T,
  ): SocketAddress {
