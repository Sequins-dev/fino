 ...
 * @internal
 */
export type SimulatedNetworkTraceEvent =
...
function resolveLinkOptions(input: SimulatedLinkOptions | undefined, base: RequiredLinkOptions = DEFAULT_LINK): RequiredLinkOptions {
  return {
    ...
    bandwidthBytesPerMs: input?.bandwidthBytesPerMs === undefined
      ? base.bandwidthBytesPerMs
      : Math.max(0, input.bandwidthBytesPerMs),
    ...
    corruptByte: input?.corruptByte === undefined ? base.corruptByte : input.corruptByte & 0xff,
...
function copyAddress<T extends SocketAddress>(addr: T): T {
  if (addr.family === 'unix') return { family: 'unix', path: addr.path } as T;
  return { family: addr.family, ip: addr.ip, port: addr.port } as T;
}
...
function ensureIpAddress(addr: SocketAddress, operation: string): Extract<SocketAddress, { family: 'ipv4' | 'ipv6' }> {
...
class SimulatedStreamReader implements AsyncIterable<Uint8Array> {
  ...
  read(input?: number | { maxBytes?: number }): Promise<Uint8Array | null> {
    if (this.#closed) return Promise.resolve(null);
...
class SimulatedConnection implements Connection {
  ...
  split(): [any, any] {
    ...
    this.#split = true;
    return [
      new SimulatedStreamReader(this.#inbound),
      new SimulatedStreamWriter(this.#outbound),
    ];
  }
...
class SimulatedDatagramSocket implements DatagramSocket {
  ...
  #waiters: Array<QueueResolver<DatagramPacket> & { maxBytes: number }> = [];
  ...
  recv(maxBytes = 65536): Promise<{ data: Uint8Array; addr: SocketAddress }> {
    if (this.#closed) return Promise.reject(new Error('Simulated datagram socket is closed'));
    ...
    return new Promise((resolve, reject) => {
      this.#waiters.push({ resolve, reject, maxBytes });
    });
  }

  recvNow(maxBytes = 65536): { data: Uint8Array; addr: SocketAddress } | null {
  ...
  _enqueue(data: Uint8Array, addr: SocketAddress, ecn?: number): void {
    if (this.#closed) return;
    const packet = { data: cloneBytes(data), addr: copyAddress(addr), ecn };
    const waiter = this.#waiters.shift();
    if (waiter) {
      waiter.resolve({
...
export class SimulatedNetworkProvider extends NetworkProvider {
   ...
   */
  dropNextDatagrams(count = 1, filters: { from?: SocketAddress; to?: SocketAddress } = {}): void {
   ...
   */
  corruptNextDatagrams(count = 1, filters: { from?: SocketAddress; to?: SocketAddress; corruptByte?: number } = {}): void {
    ...
    this.#scriptedCorruptions.push({
      ...
      ...(filters.from === undefined ? {} : { fromKey: addressKey(filters.from) }),
      ...(filters.to === undefined ? {} : { toKey: addressKey(filters.to) }),
      ...(filters.corruptByte === undefined ? {} : { corruptByte: filters.corruptByte & 0xff }),
    });
  }
  ...
  listen(addr: SocketAddress, _opts?: ListenOptions): Listener {
    ...
    this.#trace({ type: 'stream:listen', at: this.#clock, address: copyAddress(bound) });
    return listener;
  }
  ...
  _sendDatagram(localAddress: SocketAddress, data: Uint8Array, dest: SocketAddress, ecn?: number): void {
    ...
    const payload = scriptedCorruptByte === undefined
      ? this.#maybeCorrupt(data, options)
      : this.#corruptPayload(data, scriptedCorruptByte);
  ...
  #allocateAddress<T extends Map<string, unknown>>(addr: SocketAddress, occupied: T): SocketAddress {
    ...
    for (;;) {
      const candidate = { family: addr.family, ip: addr.ip, port: this.#nextEphemeralPort++ } as SocketAddress;
  ...
  #allocateStreamLocal(remote: SocketAddress): SocketAddress {
    if (remote.family === 'unix') {
      return { family: 'unix', path: `${remote.path}.client.${this.#nextEphemeralPort++}` };
    }
    const ip = remote.family === 'ipv6' ? '::' : '0.0.0.0';
    return this.#allocateAddress({ family: remote.family, ip, port: 0 }, new Map());
  }
  ...
  #link(from: SocketAddress, to: SocketAddress): LinkPair {
    ...
    if (state === undefined) {
      state = { options: this.#defaultLink, queued: 0 };
  ...
  #dueAt(options: RequiredLinkOptions, bytes: number): number {
    const jitter = options.jitterMs === 0
      ? 0
      : (this.#rng.next() * 2 - 1) * options.jitterMs;
    const bandwidthDelay = options.bandwidthBytesPerMs === Infinity || options.bandwidthBytesPerMs <= 0
      ? 0
  ...
  #maybeCorrupt(data: Uint8Array, options: RequiredLinkOptions): Uint8Array {
    ...
    if (out.byteLength > 0 && this.#rng.chance(options.corruptionRate)) {
      out[0] = options.corruptByte ?? (out[0]! ^ 0xff);
    }
    return out;
  }
  ...
  #corruptPayload(data: Uint8Array, corruptByte: number | null): Uint8Array {
    const out = cloneBytes(data);
    if (out.byteLength > 0) out[0] = corruptByte ?? (out[0]! ^ 0xff);
    return out;
  }
  ...
  #schedule(
    ...
    dueAt: number,
    releaseQueueSlot: boolean,
    traceType: 'datagram:queued' | 'datagram:duplicated',
    ecn?: number,
  ): void {
    this.#scheduled.push({
      id: this.#nextEventId++,
