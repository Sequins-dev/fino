...
const BLOCKED_FETCH_PORTS = new Set([
  ...
  995, 1719, 1720, 1723, 2049, 3659, 4045, 4190, 5060, 5061, 6e3, 6566, 6665, 6666, 6667, 6668,
...
function _fetchBlobURL(
  url: string,
  method: string,
  headers: Headers,
  capturedBlob: ReturnType<Request['_getBlobURLObject']> = null,
): Response {
  if (method !== 'GET') {
...
class H3PoolEntry {
  ...
  constructor(
    ...
    target: AltSvcEntry,
    tls?: FetchInit['tls'],
    handshakeTimeoutMs: number | null = null,
  ) {
    this.#origin = origin;
    this.#target = target;
    this.#tls = tls;
    ...
    this.#ready = (async () => {
      ...
      try {
        const conn = await endpoint.connect({
          ...
          ...(this.#handshakeTimeoutMs !== null
            ? { connection: { handshakeTimeoutMs: this.#handshakeTimeoutMs } }
            : {}),
        ...
        if (
          ...
          error.message === 'QUIC connection is closed'
        ) {
          throw new Error(
            `QUIC connection is closed (${closeInfo.type} ${closeInfo.errorCode}${closeInfo.reason ? `: ${closeInfo.reason}` : ''})`,
          );
        }
        throw error;
      }
...
async function _singleFetchH3(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `QUIC connection is closed during ${stage} (${closeInfo.type} ${closeInfo.errorCode}${closeInfo.reason ? `: ${closeInfo.reason}` : ''})`,
      );
    }
...
async function _singleFetch(
  ...
  if (isHttps) {
    ...
    try {
      ...
      topic(otelRuntimeTopic('socket', 'connect', 'error')).publish(
        otelRuntimeEvent('socket', 'connect', 'error', {
          connectId,
          requestId: runtime.requestId,
          hop: runtime.hop,
          host: address,
      ...
      topic(otelRuntimeTopic('tls', 'handshake', 'error')).publish(
        otelRuntimeEvent('tls', 'handshake', 'error', {
          handshakeId,
          requestId: runtime.requestId,
          hop: runtime.hop,
          hostname,
...
export async function fetch(input: string | Request, init?: FetchInit): Promise<Response> {
  ...
  if (init?.referrer !== 'no-referrer') {
    ...
    const referrer =
      typeof init?.referrer === 'string' && init.referrer !== 'about:client'
        ? init.referrer
        : undefined;
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    if (REDIRECT_STATUSES.has(status)) {
      ...
      if (!location) {
        ...
        return _buildFinalResponseWithIntegrity(
          response,
          sock,
          currentUrl,
          redirected,
