...
*
*/
import { btoa, DOMException, encodeUtf8, decodeUtf8, TextDecoder, _registerBlobCloneHelper } from './encoding.ts';
import { Event, EventTarget } from './eventtarget.ts';
import { ReadableStream } from './webstreams.ts';
// ---------------------------------------------------------------------------
// Internal
// ---------------------------------------------------------------------------
...
function _toWebIdlLongLong(value: unknown): number {
  ...
  if (fraction < .5) return sign * floor;
  if (fraction > .5) return sign * (floor + 1);
...
export class Blob {
  ...
  */
  slice(start?: number, end?: number, contentType?: string): Blob {
    ...
    else s = Math.min(s, size);
    if (e < 0) e = Math.max(size + e, 0);
    else e = Math.min(e, size);
...
function _decodeFileReaderText(bytes: Uint8Array, blob: Blob, label?: string): string {
  ...
  if (encoding === undefined) {
    ...
    else if (bytes[0] === 255 && bytes[1] === 254) encoding = 'utf-16le';
    else encoding = 'utf-8';
  }
...
_makePrototypeMembersEnumerable(File.prototype, ['name', 'lastModified']);
