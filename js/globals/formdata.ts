 ...
 * @internal
 */
export async function _serializeFormData(
  fd: FormData,
  boundary?: string,
): Promise<{
  contentType: string;
  ...
  for (const [name, value] of fd) {
    ...
    if (typeof value === 'string') {
      parts.push(
        encodeUtf8(`Content-Disposition: form-data; name="${_escapeParameter(name)}"\r\n\r\n`),
      );
      ...
      parts.push(
        encodeUtf8(
          `Content-Disposition: form-data; name="${_escapeParameter(name)}"; filename="${_escapeParameter(value.name)}"\r\nContent-Type: ${type}\r\n\r\n`,
        ),
      );
