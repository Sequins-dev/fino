...
function _digestAlgorithm(name: string): string {
  switch (name.toUpperCase()) {
    ...
    case 'SHA-512':
      return 'sha-512';
    default:
      throw _webCryptoError('NotSupportedError', 'Unsupported hash algorithm: ' + name);
  }
}
function _cipherAlgorithm(name: string, keyLength: number): string {
  ...
  switch (name.toUpperCase()) {
    ...
    default:
      throw _webCryptoError('NotSupportedError', 'Unsupported cipher algorithm: ' + name);
  }
}
// ---------------------------------------------------------------------------
...
function _deriveBitsByteLength(length: number | null | undefined): number {
  if (!Number.isInteger(length) || length === null || length < 0 || length % 8 !== 0) {
    throw _webCryptoError(
      'OperationError',
      'deriveBits: length must be a non-negative multiple of 8',
    );
  }
  return length / 8;
}
...
function _rsaJwkAlg(algName: string, hashName: string): string {
  if (algName === 'RSA-OAEP')
    return `RSA-OAEP${hashName === 'SHA-256' ? '-256' : hashName === 'SHA-384' ? '-384' : '-512'}`;
  if (algName === 'RSA-PSS')
    return hashName === 'SHA-256' ? 'PS256' : hashName === 'SHA-384' ? 'PS384' : 'PS512';
  /* RSASSA-PKCS1-V1_5 */ return hashName === 'SHA-256'
    ? 'RS256'
    : hashName === 'SHA-384'
      ? 'RS384'
      : 'RS512';
}
function _symmetricJwkAlg(
  algName: string,
  keyBytes: number,
  hash?:
    | string
...
function _validateJwkKeyOps(jwkOps: unknown, requestedUsages: readonly KeyUsage[]): void {
  ...
  for (const usage of requestedUsages) {
    if (!jwkOps.includes(usage)) {
      throw _webCryptoError(
        'InvalidAccessError',
        `importKey: requested usage "${usage}" is not allowed by JWK key_ops`,
      );
    }
  }
}
...
function _derSigToRaw(der: Uint8Array, coordSize: number): Uint8Array {
  ...
  off++;
  const rLen = der[off++]!;
  if (off + rLen > der.length)
    throw new Error('Invalid ECDSA DER signature: r length overruns buffer');
  const rBytes = der.subarray(off, off + rLen);
  off += rLen;
  if (der[off] !== 2) throw new Error('Invalid ECDSA DER signature: expected INTEGER for s');
  off++;
  const sLen = der[off++]!;
  if (off + sLen > der.length)
    throw new Error('Invalid ECDSA DER signature: s length overruns buffer');
...
const subtle: SubtleCrypto = {
  ...
  async sign(
    ...
    if (_isEd25519Algorithm(alg.name)) {
      if (key.type !== 'private')
        throw _webCryptoError('InvalidAccessError', 'Ed25519 sign requires a private key');
      if (!key.usages.includes('sign'))
        throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow sign');
    ...
    if (alg.name === 'RSA-PSS') {
      ...
      return _toArrayBuffer(
        openssl.rsaPssSign(_pkeyPtr(key), hashAlg, saltLen, _toUint8Array(data)),
      );
    }
    if (alg.name === 'RSASSA-PKCS1-V1_5') {
      if (key.algorithm.name !== 'RSASSA-PKCS1-V1_5' || key.type !== 'private') {
        throw _webCryptoError(
          'InvalidAccessError',
          'RSASSA-PKCS1-v1_5 sign requires an RSASSA-PKCS1-v1_5 private key',
        );
      }
      if (!key.usages.includes('sign'))
        throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow sign');
    ...
    if (alg.name !== 'HMAC')
      throw _webCryptoError('NotSupportedError', 'sign: unsupported algorithm "' + alg.name + '"');
  ...
  async verify(
    ...
    if (_isEd25519Algorithm(alg.name)) {
      if (key.type !== 'public')
        throw _webCryptoError('InvalidAccessError', 'Ed25519 verify requires a public key');
      if (!key.usages.includes('verify'))
        throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow verify');
    ...
    if (alg.name === 'RSA-PSS') {
      if (key.algorithm.name !== 'RSA-PSS' || key.type !== 'public') {
        throw _webCryptoError(
          'InvalidAccessError',
          'RSA-PSS verify requires an RSA-PSS public key',
        );
      }
      if (!key.usages.includes('verify'))
        throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow verify');
      const hashName = _hashName(_requiredHash(key.algorithm.hash, 'RSA-PSS hash'));
      const hashAlg = _digestAlgorithm(hashName);
      return openssl.rsaPssVerify(
        _pkeyPtr(key),
        hashAlg,
        _toUint8Array(signature),
        _toUint8Array(data),
      );
    }
    if (alg.name === 'RSASSA-PKCS1-V1_5') {
      if (key.algorithm.name !== 'RSASSA-PKCS1-V1_5' || key.type !== 'public') {
        throw _webCryptoError(
          'InvalidAccessError',
          'RSASSA-PKCS1-v1_5 verify requires an RSASSA-PKCS1-v1_5 public key',
        );
      }
      if (!key.usages.includes('verify'))
        throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow verify');
    ...
    if (alg.name !== 'HMAC')
      throw _webCryptoError(
        'NotSupportedError',
        'verify: unsupported algorithm "' + alg.name + '"',
      );
    if (key.algorithm.name !== 'HMAC' || key.type !== 'secret') {
  ...
  async encrypt(
    ...
    if (!key.usages.includes('encrypt'))
      throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow encrypt');
    if (alg.name === 'RSA-OAEP') {
      if (key.algorithm.name !== 'RSA-OAEP' || key.type !== 'public') {
        throw _webCryptoError(
          'InvalidAccessError',
          'RSA-OAEP encrypt requires an RSA-OAEP public key',
        );
      }
      ...
      return _toArrayBuffer(
        openssl.rsaOaepEncrypt(_pkeyPtr(key), hashAlg, label, _toUint8Array(data)),
      );
    }
    if (alg.name !== 'AES-GCM' && alg.name !== 'AES-CBC') {
      throw _webCryptoError(
        'NotSupportedError',
        'encrypt: unsupported algorithm "' + alg.name + '"',
      );
    }
    if (key.algorithm.name !== alg.name || key.type !== 'secret') {
      throw _webCryptoError(
        'InvalidAccessError',
        `${alg.name} encrypt requires a matching secret key`,
      );
    }
  ...
  async decrypt(
    ...
    if (!key.usages.includes('decrypt'))
      throw _webCryptoError('InvalidAccessError', 'CryptoKey does not allow decrypt');
    if (alg.name === 'RSA-OAEP') {
      if (key.algorithm.name !== 'RSA-OAEP' || key.type !== 'private') {
        throw _webCryptoError(
          'InvalidAccessError',
          'RSA-OAEP decrypt requires an RSA-OAEP private key',
        );
      }
      const hashName = _hashName(_requiredHash(key.algorithm.hash, 'RSA-OAEP hash'));
      const hashAlg = _digestAlgorithm(hashName);
      const label = alg.label ? _toUint8Array(alg.label as BufferSource) : null;
      return _toArrayBuffer(
        openssl.rsaOaepDecrypt(_pkeyPtr(key), hashAlg, label, _toUint8Array(data)),
      );
    }
    if (alg.name !== 'AES-GCM' && alg.name !== 'AES-CBC') {
      throw _webCryptoError(
        'NotSupportedError',
        'decrypt: unsupported algorithm "' + alg.name + '"',
      );
    }
    if (key.algorithm.name !== alg.name || key.type !== 'secret') {
      throw _webCryptoError(
        'InvalidAccessError',
        `${alg.name} decrypt requires a matching secret key`,
      );
    }
    const keyBytes = _keyData(key);
    const cipherAlg = _cipherAlgorithm(alg.name, keyBytes.byteLength);
    ...
    if (cipherAlg.endsWith('-gcm')) {
      ...
      if (buf.byteLength < tagLength)
        throw _webCryptoError('OperationError', 'AES-GCM: data too short (no tag)');
  ...
  async importKey(
    ...
    if (format === 'jwk') {
      ...
      if (
        (alg.name === 'RSA-OAEP' || alg.name === 'RSA-PSS' || alg.name === 'RSASSA-PKCS1-V1_5') &&
        jwk.kty !== 'RSA'
      ) {
      ...
      if (jwk.kty === 'OKP') {
        if (!_isEd25519Algorithm(alg.name))
          throw _webCryptoError('DataError', 'importKey: OKP JWK requires Ed25519 algorithm');
        if (jwk.crv !== 'Ed25519')
          throw _webCryptoError('DataError', 'importKey: OKP JWK crv must be "Ed25519"');
        if (typeof jwk.x !== 'string')
          throw _webCryptoError('DataError', 'importKey: Ed25519 JWK requires "x" field');
        const x = _base64urlDecode(jwk.x);
        if (x.byteLength !== 32)
          throw _webCryptoError('DataError', 'importKey: Ed25519 JWK x must be 32 bytes');
        let pkey: object;
        try {
          pkey =
            typeof jwk.d === 'string'
          ...
          throw _webCryptoError(
            'DataError',
            err instanceof Error ? err.message : 'Malformed Ed25519 JWK',
          );
        }
      ...
      if (jwk.kty === 'EC') {
        ...
        let pkey: object;
        try {
          pkey = openssl.evpPkeyImportEcJwk(
            crv,
            padTo(x),
            padTo(y),
            d !== undefined ? padTo(d) : undefined,
          );
        } catch (err) {
          throw _webCryptoError(
            'DataError',
            err instanceof Error ? err.message : 'Malformed EC JWK',
          );
        }
      ...
      if (jwk.kty === 'RSA') {
        ...
        try {
          ...
          throw _webCryptoError(
            'DataError',
            err instanceof Error ? err.message : 'Malformed RSA JWK',
          );
        }
        const hashName = _hashName(
          (
            alg as {
              hash?:
                | string
      ...
      if (jwk.kty !== 'oct')
        throw _webCryptoError(
          'DataError',
          `importKey: unsupported JWK kty "${jwk.kty}" (supported: "oct", "EC", "RSA", "OKP")`,
        );
      if (typeof jwk.k !== 'string')
        throw _webCryptoError('DataError', 'importKey: JWK missing "k" field');
      ...
      if (jwk.alg !== undefined && jwk.alg !== expectedAlg) {
        throw _webCryptoError(
          'DataError',
          `importKey: JWK alg "${jwk.alg}" does not match ${expectedAlg}`,
        );
      }
    ...
    if (format === 'pkcs8') {
      ...
      try {
        ...
        throw _webCryptoError(
          'DataError',
          err instanceof Error ? err.message : 'Malformed PKCS#8 key data',
        );
      }
      if (_isEd25519Algorithm(alg.name)) {
        return new CryptoKey(
    ...
    if (format === 'spki') {
      ...
      if (_isEd25519Algorithm(alg.name)) {
        ...
        try {
          ...
          throw _webCryptoError(
            'DataError',
            err instanceof Error ? err.message : 'Malformed Ed25519 SPKI key data',
          );
        }
        return new CryptoKey(
          'public',
      ...
      if (alg.name === 'RSA-OAEP' || alg.name === 'RSA-PSS' || alg.name === 'RSASSA-PKCS1-V1_5') {
        ...
        try {
          ...
          throw _webCryptoError(
            'DataError',
            err instanceof Error ? err.message : 'Malformed RSA SPKI key data',
          );
        }
        return new CryptoKey(
          'public',
      ...
      if (alg.name !== 'ECDSA' && alg.name !== 'ECDH') {
        throw _webCryptoError(
          'NotSupportedError',
          'importKey: "spki" format only supported for EC algorithms (ECDSA, ECDH)',
        );
      }
      const derBytes = _toUint8Array(keyData);
      let pkey: object;
      ...
      try {
        ...
        throw _webCryptoError(
          'DataError',
          err instanceof Error ? err.message : 'Malformed SPKI key data',
        );
      }
      ...
      if (expectedCurve && expectedCurve !== namedCurve) {
        ...
        throw _webCryptoError(
          'DataError',
          `importKey: SPKI curve (${namedCurve}) does not match algorithm.namedCurve (${expectedCurve})`,
        );
      }
      return new CryptoKey(
        'public',
    ...
    if (format !== 'raw')
      throw _webCryptoError(
        'NotSupportedError',
        `importKey: unsupported format "${format}"; supported: "raw", "spki", "jwk"`,
      );
    const bytes = _toUint8Array(keyData);
    if (_isEd25519Algorithm(alg.name)) {
      if (bytes.byteLength !== 32)
        throw _webCryptoError('DataError', 'Ed25519 raw public key must be 32 bytes');
      return new CryptoKey(
        'public',
        extractable,
        { name: 'Ed25519' },
  ...
  async exportKey(format: KeyFormat, key: CryptoKey): Promise<ArrayBuffer | object> {
    ...
    if (format === 'jwk') {
      if (!key.extractable)
        throw _webCryptoError('InvalidAccessError', 'CryptoKey is not extractable');
      ...
      if (algName === 'RSA-OAEP' || algName === 'RSA-PSS' || algName === 'RSASSA-PKCS1-V1_5') {
        ...
        const { n, e, d, p, q, dp, dq, qi } = _rsaParsePkcs8(
          openssl.evpPkeyExportPkcs8(_pkeyPtr(key)),
        );
        return {
          kty: 'RSA',
          alg: jwkAlg,
      ...
      if (algName === 'HMAC') {
        ...
        throw _webCryptoError(
          'NotSupportedError',
          `exportKey: JWK not supported for algorithm ${algName}`,
        );
      }
      return {
        kty: 'oct',
    ...
    if (format === 'pkcs8') {
      if (!key.extractable)
        throw _webCryptoError('InvalidAccessError', 'CryptoKey is not extractable');
      if (key.type !== 'private')
        throw _webCryptoError('InvalidAccessError', 'exportKey: pkcs8 requires a private key');
    ...
    if (format === 'spki') {
      ...
      if (algName !== 'ECDSA' && algName !== 'ECDH') {
        throw _webCryptoError(
          'NotSupportedError',
          'exportKey: "spki" format only supported for EC and RSA algorithms',
        );
      }
      const namedCurve = key.algorithm.namedCurve ?? 'P-256';
    ...
    if (format !== 'raw')
      throw _webCryptoError(
        'NotSupportedError',
        `exportKey: unsupported format "${format}"; supported: "raw", "pkcs8", "spki", "jwk"`,
      );
    if (!key.extractable)
      throw _webCryptoError('InvalidAccessError', 'CryptoKey is not extractable');
    if (key.algorithm.name === 'Ed25519') {
      if (key.type !== 'public')
        throw _webCryptoError('InvalidAccessError', 'exportKey: raw Ed25519 requires a public key');
  ...
  > {
    ...
    if (_isEd25519Algorithm(alg.name)) {
      ...
      let pkeyPub: object;
      try {
        pkeyPub = openssl.evpPkeyImportRawPublicEd25519(
          openssl.evpPkeyExportRawPublicEd25519(pkeyFull),
        );
      } catch (err) {
    ...
    if (alg.name === 'ECDH') {
      ...
      const privateKey = new CryptoKey(
        ...
        extractable,
        algoDescriptor,
        privateUsages.length ? privateUsages : ['deriveKey', 'deriveBits'],
        null,
        pkeyFull,
      );
    ...
    if (alg.name === 'ECDSA') {
      ...
      const privateKey = new CryptoKey(
        ...
        extractable,
        algoDescriptor,
        privateUsages.length ? privateUsages : ['sign'],
        null,
        pkeyFull,
      );
      const publicKey = new CryptoKey(
        ...
        extractable,
        algoDescriptor,
        publicUsages.length ? publicUsages : ['verify'],
        null,
        pkeyPub,
      );
      return {
    ...
    if (alg.name === 'RSA-OAEP' || alg.name === 'RSA-PSS' || alg.name === 'RSASSA-PKCS1-V1_5') {
      ...
      const privateUsages = keyUsages.filter(
        (u) => u === 'decrypt' || u === 'sign' || u === 'unwrapKey',
      );
      const publicUsages = keyUsages.filter(
        (u) => u === 'encrypt' || u === 'verify' || u === 'wrapKey',
      );
      const privateKey = new CryptoKey(
        'private',
        extractable,
    ...
    if (alg.name === 'HMAC') {
      ...
      if (defaultLength === undefined)
        throw _webCryptoError('NotSupportedError', `Unsupported HMAC digest: ${digestName}`);
  ...
  async deriveBits(
    ...
    if (alg.name === 'ECDH') {
      if (!baseKey.usages.includes('deriveBits') && !baseKey.usages.includes('deriveKey')) {
        throw _webCryptoError(
          'InvalidAccessError',
          'CryptoKey does not allow deriveBits/deriveKey',
        );
      }
      if (baseKey.type !== 'private')
        throw _webCryptoError('InvalidAccessError', 'ECDH deriveBits requires a private key');
      const publicKeyParam = alg.public;
      if (!publicKeyParam || !(publicKeyParam instanceof CryptoKey)) {
        throw _webCryptoError(
          'DataError',
          'ECDH deriveBits: algorithm.public must be an ECDH public CryptoKey',
        );
      }
      if (publicKeyParam.type !== 'public')
        throw _webCryptoError(
          'InvalidAccessError',
          'ECDH deriveBits: algorithm.public must be a public key',
        );
      ...
      if (privCurve !== pubCurve) {
        throw _webCryptoError(
          'InvalidAccessError',
          `ECDH deriveBits: key curves do not match (${privCurve} vs ${pubCurve})`,
        );
      }
      ...
      if (!Number.isInteger(requestedBits) || requestedBits < 0) {
        throw _webCryptoError(
          'OperationError',
          'ECDH deriveBits: length must be a non-negative integer',
        );
      }
      ...
      if (requestedBytes > secret.byteLength) {
        throw _webCryptoError(
          'OperationError',
          `ECDH deriveBits: requested ${requestedBytes} bytes but shared secret is only ${secret.byteLength}`,
        );
      }
    ...
    if (alg.name === 'PBKDF2') {
      ...
      if (iterations === undefined)
        throw _webCryptoError('DataError', 'PBKDF2 iterations are required');
  ...
  async deriveKey(
    ...
    if (derivedAlg.name === 'HMAC') {
      ...
      throw _webCryptoError(
        'NotSupportedError',
        'deriveKey: unsupported derivedKeyType: ' + derivedAlg.name,
      );
    }
  ...
  async wrapKey(
    ...
    _checkCryptoAvailable();
    if (!key.extractable)
      throw _webCryptoError('InvalidAccessError', 'wrapKey: key is not extractable');
    if (!wrappingKey.usages.includes('wrapKey'))
      throw _webCryptoError('InvalidAccessError', 'wrappingKey does not allow wrapKey');
    ...
    if (alg.name !== 'AES-GCM' && alg.name !== 'AES-CBC') {
      throw _webCryptoError(
        'NotSupportedError',
        'wrapKey: unsupported algorithm "' + alg.name + '"',
      );
    }
    if (wrappingKey.algorithm.name !== alg.name || wrappingKey.type !== 'secret') {
      throw _webCryptoError(
        'InvalidAccessError',
        `${alg.name} wrapKey requires a matching secret key`,
      );
    }
    ...
    const { ciphertext, tag } = openssl.cipherEncrypt(
      cipherAlg,
      wrappingKeyBytes,
      iv,
      keyBytes,
  ...
  async unwrapKey(
    ...
    if (!unwrappingKey.usages.includes('unwrapKey'))
      throw _webCryptoError('InvalidAccessError', 'unwrappingKey does not allow unwrapKey');
    ...
    if (alg.name !== 'AES-GCM' && alg.name !== 'AES-CBC') {
      throw _webCryptoError(
        'NotSupportedError',
        'unwrapKey: unsupported algorithm "' + alg.name + '"',
      );
    }
    if (unwrappingKey.algorithm.name !== alg.name || unwrappingKey.type !== 'secret') {
      throw _webCryptoError(
        'InvalidAccessError',
        `${alg.name} unwrapKey requires a matching secret key`,
      );
    }
    ...
    try {
      decrypted = _toArrayBuffer(
        openssl.cipherDecrypt(cipherAlg, unwrappingKeyBytes, iv, ct, tag, aad),
      );
    } catch (err) {
      throw _webCryptoError(
        'OperationError',
        err instanceof Error ? err.message : 'unwrapKey operation failed',
      );
    }
    if (format === 'raw') {
...
export const crypto: Crypto = {
  ...
  getRandomValues<T extends ArrayBufferView>(typedArray: T): T {
    ...
    if (
      ...
      throw _webCryptoError(
        'TypeMismatchError',
        'getRandomValues: argument must be an integer typed array',
      );
    }
