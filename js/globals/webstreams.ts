...
function _streamReadResult(done: boolean, value: any): {
  done: boolean;
  value: any;
} {
...
function _publicReadResult(done: boolean, value: any): {
  done: boolean;
  value: any;
} {
  const result = {
...
function _extractStrategy(strategy: QueuingStrategyLike | null | undefined, defaultHWM: number): {
  highWaterMark: number;
...
function _rsMarkClosed(s: ReadableStreamState): void {
  ...
  s.closedResolve?.();
  if (s.pendingReads) {
    while (s.pendingReads.length > 0) s.pendingReads.shift()!.resolve(_streamReadResult(true, undefined));
  }
...
function _rsPullIfNeeded(s: ReadableStreamState): void {
  ...
  s.pulling = true;
  s.pullAgain = false;
  Promise.resolve().then(function rsCallPull() {
...
function _rsNextChunk(s: ReadableStreamState): Promise<{
  ...
  if (s.kind === 'iterable') {
    ...
    return iter.next().then(function rsIterNext(r): {
      done: boolean;
      value: any;
    } {
...
export class ReadableByteStreamController {
  ...
  */
  enqueue(chunk: ArrayBuffer | ArrayBufferView) {
    ...
    if (s.state !== 'readable') throw new TypeError('Stream is not readable');
    if (s.closeRequested) throw new TypeError('Cannot enqueue after close()');
    if (ArrayBuffer.isView(chunk) && chunk.byteLength === 0 || chunk instanceof ArrayBuffer && chunk.byteLength === 0) {
    ...
    while (offset < bytes.length && s.pendingByob!.length > 0) {
      ...
      const toCopy = Math.min(available, needed);
      new Uint8Array(desc.view.buffer, desc.view.byteOffset + desc.bytesFilled, toCopy).set(bytes.subarray(offset, offset + toCopy));
...
export class ReadableStream {
  ...
  */
  constructor(underlyingSource?: any, queuingStrategy?: QueuingStrategyLike) {
    ...
    startResult.then(function rsOnStarted() {
      s.started = true;
      _rsPullIfNeeded(s);
  ...
  */
  static from(asyncIterable: AsyncIterable<any> | Iterable<any>) {
    ...
    if (asyncIterable == null || typeof source[Symbol.asyncIterator] !== 'function' && typeof source[Symbol.iterator] !== 'function') {
  ...
  */
  getReader(options?: {
    ...
    if (mode === 'byob') {
      if (!s.isByteStream) throw new TypeError('getReader({mode:"byob"}) requires a byte stream (type:"bytes")');
  ...
  values(options?: {
    ...
    return {
      ...
      return(value: unknown) {
        s.locked = false;
        if (preventCancel) return Promise.resolve({
          done: true,
          value
        });
  ...
  * ```
  */
  async pipeTo(destination: WritableStream, options?: {
...
export class ReadableStreamBYOBReader {
  ...
  * ```
  */
  read(view: ArrayBufferView, options?: {
    min?: number;
  }) {
    const br = _br.get(this);
    ...
    if (s.queue!.length > 0) {
      ...
      if (filled !== null) return Promise.resolve({
        done: false,
        value: filled
      });
    }
...
function _wsAbort(s: WritableStreamState, reason: unknown): Promise<void> {
  ...
  if (s.kind === 'sink' && s.underlyingSink?.abort) {
    return Promise.resolve().then(function wsCallAbort() {
...
function _wsDoSinkWrite(s: WritableStreamState, chunk: any, size: number, deferAfterWrite = false): Promise<void> {
  ...
  const settled = deferAfterWrite ? Promise.resolve().then(function wsDeferAfterWrite() {
    return p;
  }) : p;
...
export class WritableStream {
  ...
  */
  constructor(underlyingSink?: any, queuingStrategy?: QueuingStrategyLike) {
    ...
    const startResult = s.underlyingSink.start ? Promise.resolve(s.underlyingSink.start(controller)) : Promise.resolve();
...
export class WritableStreamDefaultWriter {
  ...
  */
  close() {
    const ww = _ww.get(this);
    if (!ww) return Promise.reject(new TypeError('Writer is released'));
    return _wsCloseInternal(ww.wsState).then(function wwClosed() {
      ww.closedResolve?.();
  ...
  constructor(transformer?: any, writableStrategy?: QueuingStrategyLike, readableStrategy?: QueuingStrategyLike) {
    ...
    this.#writable = new WritableStream({
      ...
      close() {
        ...
        return Promise.resolve(result).then(function tsCloseChannel() {
          channel.close();
    ...
    if (transformer?.start) {
      try {
        Promise.resolve(transformer.start.call(transformer, controller)).catch(function transformStartFailed(e) {
          controller.error(e);
        });
      } catch (e) {
        controller.error(e);
