...
// ---------------------------------------------------------------------------
const _rs = new WeakMap<ReadableStream, ReadableStreamState>();
const _ws = new WeakMap<WritableStream, WritableStreamState>();
const _rr = new WeakMap<ReadableStreamDefaultReader, ReadableReaderState>();
const _br = new WeakMap<ReadableStreamBYOBReader, ByobReaderState>();
const _ww = new WeakMap<WritableStreamDefaultWriter, WritableWriterState>();
const _rc = new WeakMap<ReadableStreamDefaultController, ReadableStreamState>();
const _rbc = new WeakMap<ReadableByteStreamController, ReadableStreamState>();
const _wc = new WeakMap<WritableStreamDefaultController, WritableStreamState>();
const _tc = new WeakMap<TransformStreamDefaultController, Channel>();
const _byobRequestCtorToken = {};
function _streamReadResult(
  done: boolean,
  value: any,
): {
  done: boolean;
...
function _byteStreamViewOnBuffer(
  view: ArrayBufferView,
  buffer: ArrayBuffer,
  ...
  return {
    enqueue(value: any) {
      ...
      if (pending) {
        const p = pending;
        pending = null;
        p.resolve(_streamReadResult(false, value));
    ...
    close() {
      ...
      if (pending) {
        const p = pending;
        pending = null;
        p.resolve(_streamReadResult(true, undefined));
      }
    },
    ...
    [Symbol.asyncIterator]() {
      return {
        next() {
          ...
          return new Promise(function parkRead(resolve, reject) {
            pending = {
              resolve,
              reject,
            };
...
function _validateQueueSize(size: unknown): number {
  ...
  if (!Number.isFinite(n) || n < 0)
    throw new RangeError('Queue size must be a finite, non-negative number');
  return n;
}
function _extractStrategy(
  strategy: QueuingStrategyLike | null | undefined,
  defaultHWM: number,
): {
  highWaterMark: number;
...
function _extractSinkMethod(
  sink: any,
  name: 'start' | 'write' | 'close' | 'abort',
): Function | undefined {
...
// ---------------------------------------------------------------------------
function _rsMakeState(
  kind: ReadableStreamState['kind'],
  extra: Partial<ReadableStreamState>,
): ReadableStreamState {
  let closedResolve: (() => void) | undefined;
  let closedReject: ((e: unknown) => void) | undefined;
  const closedPromise = new Promise<void>(function captureRsClosed(res, rej) {
    closedResolve = res;
    closedReject = rej;
  });
  return {
...
function _rsPullIfNeeded(s: ReadableStreamState): void {
  ...
  s.pullAgain = false;
  Promise.resolve()
    .then(function rsCallPull() {
      return s.underlyingSource.pull ? s.underlyingSource.pull(s.controller) : undefined;
    })
  ...
  if (!s._iter) {
    ...
    if (asyncIterator) {
      ...
      s._iter = {
        ...
        ...(syncIter.return
          ? { return: (v: unknown) => Promise.resolve(syncIter.return!(v)) }
          : {}),
      };
    } else {
...
function _rsNextChunk(s: ReadableStreamState): Promise<{
  done: boolean;
  value: any;
}> {
  s.disturbed = true;
...
function _rsByobFillFromQueue(
  s: ReadableStreamState,
  view: ArrayBufferView,
  min: number,
): Uint8Array | null {
  ...
  if (bytesFilled < min) return null;
  if (s.closeRequested && s.queue!.length === 0) {
    _rsMarkClosed(s);
  } else {
    _rsPullIfNeeded(s);
...
export class ReadableByteStreamController {
  ...
  get byobRequest() {
    ...
    if (!desc.request)
      desc.request = new ReadableStreamBYOBRequest(_byobRequestCtorToken, this, desc);
    return desc.request;
  }
  /**
  ...
  enqueue(chunk: ArrayBuffer | ArrayBufferView) {
    ...
    while (offset < bytes.length && s.pendingByob!.length > 0) {
      ...
      const toCopy = Math.min(available, needed);
      new Uint8Array(desc.view.buffer, desc.view.byteOffset + desc.bytesFilled, toCopy).set(
  ...
  _byobRespondWithNewView(view: ArrayBufferView) {
    ...
    if (s.closeRequested || s.state === 'closed') {
      if (view.byteOffset !== current.byteOffset)
        throw new RangeError('view byteOffset must match the BYOB request');
      ...
      if (view.byteLength > current.byteLength)
        throw new RangeError('view byteLength must not exceed the BYOB request');
    }
...
export class ReadableStream {
  ...
  constructor(underlyingSource?: any, queuingStrategy?: QueuingStrategyLike) {
    ...
    if (isByteStream) {
      s = _rsMakeState('source', {
        ...
        autoAllocateChunkSize:
          src.autoAllocateChunkSize === undefined ? undefined : Number(src.autoAllocateChunkSize),
        controller: null,
      });
      controller = new ReadableByteStreamController(s);
      s.controller = controller;
      _rs.set(this, s);
      startResult = src.start ? Promise.resolve(src.start(controller)) : Promise.resolve();
    } else {
      s = _rsMakeState('source', {
        underlyingSource: src,
        queue: [],
    ...
    startResult
      .then(function rsOnStarted() {
        s.started = true;
        _rsPullIfNeeded(s);
      })
      .catch(function rsStartError(e) {
        controller.error(e);
      });
  }
  /**
  ...
  values(options?: { preventCancel?: boolean }) {
    ...
    const iter = s.kind === 'iterable' ? _rsIterableIterator(s) : null;
    return {
      next() {
        const read = iter ? iter.next() : _rsNextChunk(s);
        return read.then(
          function rsAsyncIterNext(r) {
            if (r.done) {
              s.locked = false;
              _rsMarkClosed(s);
            }
  ...
  async pipeTo(
    ...
    if (signal) {
      abortPromise = new Promise<never>(function captureAbortReject(_, reject) {
        abortReject = reject;
      });
      abortListener = function onAbortPipe() {
    ...
    const pump = async () => {
      ...
      const destinationErrorPromise = () =>
        _wsErrorPromise(dst).catch(function wrapDestinationError(reason) {
  ...
  pipeThrough(
    ...
    if (!_rs.get(readable)) throw new TypeError('transform.readable must be a ReadableStream');
    ...
    Promise.resolve(_rsPipeToMethod.call(this, writable, pipeOptions)).catch(
      function swallowPipeError() {},
    );
    return readable;
  }
  /**
  ...
  tee() {
        ...
        [Symbol.asyncIterator]() {
          ...
          return {
            ...
            return(value: unknown) {
              ...
              return iter.return
                ? iter.return(value)
                : Promise.resolve(_streamReadResult(true, value));
            },
            [Symbol.asyncIterator]() {
              return this;
            },
    ...
    feed();
    return [
      ReadableStream.from(makeTeeIterable(ch1, 0)),
      ReadableStream.from(makeTeeIterable(ch2, 1)),
    ];
  }
}
...
export class ReadableStreamDefaultReader {
  ...
  constructor(stream: ReadableStream) {
    ...
    let closedResolve: (() => void) | undefined;
    let closedReject: ((e: unknown) => void) | undefined;
    const closedPromise = new Promise<void>(function captureRrClosed(res, rej) {
      closedResolve = res;
      closedReject = rej;
    });
    _rr.set(this, {
  ...
  cancel(reason: unknown) {
    const rr = _rr.get(this);
    if (!rr) return Promise.reject(new TypeError('Reader is released'));
    return _rsCancel(rr.rsState, reason).then(function rrCancelled() {
      rr.closedResolve?.();
    });
  }
  /**
...
export class ReadableStreamBYOBReader {
  ...
  constructor(stream: ReadableStream) {
    ...
    if (s.locked) throw new TypeError('ReadableStream is already locked to a reader');
    if (!s.isByteStream)
      throw new TypeError('ReadableStreamBYOBReader requires a byte stream (type:"bytes")');
    let closedResolve: (() => void) | undefined;
    let closedReject: ((e: unknown) => void) | undefined;
    const closedPromise = new Promise<void>(function captureBrClosed(res, rej) {
      closedResolve = res;
      closedReject = rej;
    });
    _br.set(this, {
  ...
  read(
    ...
    if (min > view.byteLength)
      return Promise.reject(new RangeError('min must be <= view.byteLength'));
    const s = br.rsState;
  ...
  cancel(reason: unknown) {
    const br = _br.get(this);
    if (!br) return Promise.reject(new TypeError('Reader is released'));
    return _rsCancel(br.rsState, reason).then(function brCancelled() {
      br.closedResolve?.();
    });
  }
  /**
...
// ---------------------------------------------------------------------------
function _wsMakeState(
  kind: WritableStreamState['kind'],
  extra: Partial<WritableStreamState>,
): WritableStreamState {
  let closedResolve: (() => void) | undefined;
  let closedReject: ((e: unknown) => void) | undefined;
  const closedPromise = new Promise<void>(function captureWsClosed(res, rej) {
    closedResolve = res;
    closedReject = rej;
  });
  return {
...
function _wsMakeReadyPending(s: WritableStreamState): void {
  if (s.readyResolve !== null) return;
  s.readyPromise = new Promise(function captureWsReady(resolve, reject) {
...
function _wsAbort(s: WritableStreamState, reason: unknown): Promise<void> {
  ...
  if (s.kind === 'sink' && s.underlyingSink?.abort) {
    return Promise.resolve()
      .then(function wsCallAbort() {
        return s.underlyingSink.abort(reason);
      })
      .catch(function wsAbortFailed(e) {
        throw e;
      });
  }
  return Promise.resolve();
}
function _wsWriteInternal(
  s: WritableStreamState,
  chunk: any,
...
function _wsDoSinkWrite(
  s: WritableStreamState,
  chunk: any,
  size: number,
  deferAfterWrite = false,
): Promise<void> {
  let writeResult: unknown;
  try {
    writeResult = s.underlyingSink.write ? s.underlyingSink.write(chunk, s.controller) : undefined;
  } catch (e) {
    writeResult = Promise.reject(e);
  }
  const p = Promise.resolve(writeResult);
  const settled = deferAfterWrite
    ? Promise.resolve().then(function wsDeferAfterWrite() {
        return p;
      })
    : p;
  return settled
    .then(function wsAfterWrite() {
      s.queueTotalSize! -= size;
      if (s.queueTotalSize! < 0) s.queueTotalSize = 0;
...
async function _wsCloseInternal(s: WritableStreamState): Promise<void> {
  ...
  if (s.kind === 'writer') {
    ...
    try {
      await Promise.resolve().then(function wsCallClose() {
        return s.underlyingSink.close();
      });
    } catch (e) {
      _wsError(s, e);
      throw e;
...
export class WritableStreamDefaultController {
   ...
   */
  get abortReason() {
    return this.#abortCtrl.signal.reason;
  }
  /**
...
export class WritableStream {
  ...
  constructor(underlyingSink?: any, queuingStrategy?: QueuingStrategyLike) {
    ...
    const sinkAlgorithms: WritableSinkAlgorithms = {
      ...
      ...(writeMethod
        ? { write: (chunk, controller) => writeMethod.call(sink, chunk, controller) }
        : {}),
    ...
    const startResult = s.underlyingSink.start
      ...
      : Promise.resolve();
    startResult.catch(function wsStartError(e) {
      controller.error(e);
    });
  }
  /**
...
export class WritableStreamDefaultWriter {
  ...
  constructor(stream: WritableStream) {
    ...
    let closedResolve: (() => void) | undefined;
    let closedReject: ((e: unknown) => void) | undefined;
    const closedPromise = new Promise<void>(function captureWwClosed(res, rej) {
      closedResolve = res;
      closedReject = rej;
    });
    _ww.set(this, {
  ...
  close() {
    ...
    return _wsCloseInternal(ww.wsState)
      .then(function wwClosed() {
        ww.closedResolve?.();
      })
      .catch(function wwCloseFailed(e) {
        ww.closedReject?.(e);
        throw e;
      });
  }
...
export class TransformStreamDefaultController {
   ...
   * @internal
   */
  _setReadableState(rsState: ReadableStreamState | undefined) {
    this.#rsState = rsState ?? null;
  }
  /**
...
export class TransformStream {
  ...
  constructor(
    transformer?: any,
    writableStrategy?: QueuingStrategyLike,
    readableStrategy?: QueuingStrategyLike,
  ) {
    ...
    const runCancel = (reason: unknown): Promise<void> => {
      ...
      cancelPromise = Promise.resolve(result)
        .then(function transformCancelDone() {
    ...
    this.#writable = new WritableStream(
      {
        ...
        write(chunk: any) {
          ...
          return wait.then(function transformAfterBackpressure() {
    ...
    this.#readable = new ReadableStream(
      {
        pull(ctrl: ReadableStreamDefaultController) {
          ...
          return chIter.next().then(function tsChannelPull({
            done,
            value,
    ...
    if (transformer?.start) {
      try {
        Promise.resolve(transformer.start.call(transformer, controller)).catch(
          function transformStartFailed(e) {
            controller.error(e);
          },
        );
      } catch (e) {
