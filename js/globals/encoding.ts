...
export function btoa(data: string): string {
  ...
  for (let i = 0; i < str.length; i++) {
    if (str.charCodeAt(i) > 255) {
      throw new TypeError('btoa: The string to be encoded contains characters outside of the Latin1 range.');
    }
  }
  let out = '';
...
type BlobCloneHelper = {
  ...
  BlobCtor: new (parts: Iterable<unknown>, opts?: {
    type?: string;
  }) => object;
  FileCtor: new (parts: Iterable<unknown>, name: string, opts?: {
    type?: string;
    lastModified?: number;
  }) => object;
...
* @internal
*/
export function _structuredCloneWithTransferMap<T>(value: T, options?: {
  transfer?: ArrayBuffer[];
...
function _clone(value: unknown, seen: WeakMap<object, unknown>, transferSet: Set<ArrayBuffer> | null = null, transferMap?: WeakMap<object, unknown>): unknown {
  ...
  if (_blobCloneHelper && _blobCloneHelper.isBlob(value)) {
    ...
    const blobClone = h.isFile(value) ? new h.FileCtor([bytes], h.getName(value), {
  ...
  */
  decode(input?: ArrayBuffer | ArrayBufferView | null, options?: {
    ...
    if (this.#encoding === 'utf-16le' || this.#encoding === 'utf-16be') {
      try {
        const result = decodeUtf16(bytes, this.#encoding === 'utf-16le', this.#fatal, shouldStripBom, streaming, this.#pendingUtf16Lead);
