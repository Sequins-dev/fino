...
export type HeaderMatcher =
  | string
  | RegExp
  | ((value: string | null, call: MockFetchCall) => boolean);
/**
...
export type BodyMatcher =
  ...
  | Uint8Array
  | RegExp
  | ((body: Uint8Array, text: string, call: MockFetchCall) => boolean);
/**
...
function _isAbortSignal(value: unknown): value is {
  aborted: boolean;
} {
...
async function _withScopedFetch<T>(scope: MockFetchScope, fn: () => T | Promise<T>): Promise<T> {
...
export class MockFetchExpectation {
  ...
  reply(
    ...
    this.#expectation.response = new Response(body as never, {
      ...init,
      status,
    });
    return this;
...
export class MockFetchScope {
  ...
  patch(input: string | URL): MockFetchExpectation {
    return this.#expect('PATCH', input);
  }
  /**
  ...
  delete(input: string | URL): MockFetchExpectation {
    return this.#expect('DELETE', input);
  }
  /**
  ...
  options(input: string | URL): MockFetchExpectation {
    return this.#expect('OPTIONS', input);
  }
  /**
  ...
  async _dispatch(input: FetchInput, init?: MockFetchInit): Promise<Response> {
    ...
    if (expectation.method && call.method !== expectation.method) {
      throw new Error(
        `fetch call #${call.callIndex} method mismatch: expected ${expectation.method}, got ${call.method}`,
      );
    }
    if (expectation.url && call.url.href !== expectation.url) {
      throw new Error(
        `fetch call #${call.callIndex} url mismatch: expected ${expectation.url}, got ${call.url.href}`,
      );
    }
   ...
   */
  verify(): void {
    const remaining = this.#expectations.reduce(
      (sum, expectation) => sum + expectation.remaining,
      0,
    );
    if (remaining > 0) {
