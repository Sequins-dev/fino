 ...
 */
export type TaskHandler<Input, Output = unknown> = (
  input: Input,
  ctx: TaskContext,
 ...
 */
export interface TaskOptions<Input = unknown, Output = unknown> {
...
function assertOutputMode(
  taskName: string,
  supported: TaskOutputMode,
  requested: TaskRequestedOutputMode,
): void {
  if (supported === 'both' || supported === requested) return;
  throw new Error(`Task "${taskName}" does not support ${requested} output`);
}
function normalizeRequestedOutputMode(
  mode: TaskOutputMode,
  requested?: TaskRequestedOutputMode,
 ...
 */
export class Task<Input = unknown, Output = unknown> {
  /** Stable task name used by CLI, tool, and child lookup surfaces. */
  ...
  constructor(options: TaskOptions<Input, Output>) {
    ...
    this.inputSchema =
      options.inputSchema === undefined ? undefined : normalizeSchema(options.inputSchema);
  ...
  #validate(rawInput: unknown): Input {
    ...
    const summary = parsed.issues
      .map(
        (issue: { path: string; message: string }) => `${issue.path || '<root>'}: ${issue.message}`,
      )
      .join('\n');
  ...
  async run(rawInput: Input, options: TaskRunOptions = {}): Promise<Output> {
    ...
    if (options.writer !== undefined && options.writer.mode !== requested) {
      throw new Error(
        `Task "${this.name}" writer mode ${options.writer.mode} does not match requested ${requested} output`,
      );
    }
    ...
    return await this.#execute(input, {
      ...
      optionProvided: options.providedOptions
        ? (key) => options.providedOptions!.has(key)
        : undefined,
    });
  }
  /**
  ...
  parse(argv: string[], options: TaskParseOptions = {}): Output | Promise<Output> {
    ...
    const requested =
      options.outputMode ?? global.outputMode ?? normalizeRequestedOutputMode(this.outputMode);
  ...
  #toArgvCommand(
    options: Omit<TaskRunOptions, 'writer'> & {
      writer?: TaskOutputWriter;
    },
  ): ArgvCommand {
    ...
    const config: CommandConfig = {
      ...
      commands: (this.#allChildren as Array<Task | ArgvCommand>).map((child) =>
        child instanceof Task ? child.#toArgvCommand(options) : child,
      ),
      run: (ctx) =>
        this.run(contextInput(this, ctx) as Input, {
          ...options,
          prompt: ctx.prompt,
  ...
  async invoke(
    ...
    try {
      ...
      if (err instanceof Error && (err.name === 'AbortError' || err.name === 'SuspendSignal'))
        throw err;
  ...
  worker(): (call: unknown) => Promise<unknown> {
    ...
    return async (call: unknown) => {
      ...
      dispatcher ??= import('internal:jobs/runner').then((mod) =>
        (
          mod as {
 ...
 */
export function task<Input = unknown, Output = unknown>(
  options: TaskOptions<Input, Output>,
): Task<Input, Output> {
