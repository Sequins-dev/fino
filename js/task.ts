...
function makeDefaultWriter(mode: TaskRequestedOutputMode): TaskOutputWriter {
  if (mode === 'json') return { mode: 'json', writeJson: () => undefined };
  return { mode: 'text', writeText: () => undefined };
}
...
function stripGlobalJson(argv: string[]): { argv: string[]; outputMode?: TaskRequestedOutputMode } {
  const out: string[] = [];
...
export class Task<Input = unknown, Output = unknown> {
  ...
  constructor(options: TaskOptions<Input, Output>) {
    ...
    this.parameters = this.inputSchema ?? { type: 'object', properties: {} };
  ...
  async #execute(input: Input, ctx: TaskContext): Promise<Output> {
    ...
    try {
      ...
      return await Promise.race([
        this.#run(input, { ...ctx, signal: controller.signal }),
        timeoutPromise,
      ]);
    } finally {
  ...
  #toArgvCommand(options: Omit<TaskRunOptions, 'writer'> & { writer?: TaskOutputWriter }): ArgvCommand {
  ...
  async invoke(
    ...
    if (result !== null && typeof result === 'object' && 'content' in result) {
      return result as { content: string | ContentPart[]; isError?: boolean };
    }
