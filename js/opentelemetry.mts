 ...
 * | --- | --- | --- |
 * | Traces | Fino-native tracer/provider/span model with W3C context fields, span events, links, status, limits, and in-memory or OTLP export. | `tests/opentelemetry.test.mts` |
 * | Logs | Log records, severities, attributes, resource/scope metadata, active trace correlation, and export through SDK processors. | `tests/opentelemetry.test.mts` |
 * | Metrics | Counters, up-down counters, histograms, gauges, observable instruments, views, exemplars, and aggregation export. | `tests/opentelemetry.test.mts` |
 * | Resources | Default `unknown_service`, user attributes, merging, schema URL, and resource metadata on exported signals. | `tests/opentelemetry.test.mts` |
 * | W3C Trace Context | `traceparent` and `tracestate` inject/extract propagation for active telemetry contexts. | `tests/opentelemetry.test.mts` |
 * | W3C Baggage | Baggage item parsing, mutation, serialization, and carrier propagation. | `tests/opentelemetry.test.mts` |
 * | OTLP/HTTP JSON | JSON request shape for trace, metric, and log signal export over OTLP/HTTP. | `tests/opentelemetry-http.integration.test.mts` |
 * | Retry | Configurable retry attempts, delay, backoff, jitter, and retryable HTTP failure handling. | `tests/opentelemetry-http.integration.test.mts` |
 * | Compression | OTLP/HTTP JSON gzip request compression. | `tests/opentelemetry-http.integration.test.mts` |
 * | Partial success | OTLP partial-success responses are surfaced without treating the export as a transport failure. | `tests/opentelemetry-http.integration.test.mts` |
