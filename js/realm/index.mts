...
import { transpile as transpileTypeScript } from '../format/typescript.mts';
...
export class FacadeHandle {
   ...
   * @internal
   */
  _streamNames() { return [...this.#streams.keys()]; }
  /**
   ...
   * @internal
   */
  _sinkNames()   { return [...this.#sinks.keys()]; }
}
...
export class Realm<F extends RealmFn = RealmFn> {
  ...
  static fromSource<F extends RealmFn = RealmFn>(
    ...
    if ((options as RealmSourceOptions & { watch?: boolean }).watch !== undefined) {
      throw new Error('fino:realm — watch is not supported for source entrypoints');
    }

    const specifier = options.specifier ?? `fino:realm/source/${_nextSourceRealmId++}.mts`;
    const transpiled = transpileTypeScript(source, { filename: specifier, sourceType: 'ts' });
    if (!transpiled.ok) {
    ...
    if (options.overrides) {
      ...
      if (options.blocked) {
        for (const spec of options.blocked) rules.push({ pattern: spec, directive: 'block' });
      }
    }

    rules.push({
      pattern: specifier,
      directive: { type: 'source', code: transpiled.code, source_map: options.sourceMap ?? transpiled.map ?? '' },
    });
  ...
  constructor(opts: RealmOptions) {
    ...
    if (repl) {
      rules.push({ pattern: 'internal:repl-handler', directive: 'inherit' });
    }
   ...
   * import { Realm } from 'fino:realm';
   *
   * using realm = new Realm({ entry: './worker.mts' });
