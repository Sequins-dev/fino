...
function estimatedCompletionMs(w: PoolWorker): number {
  ...
  const queuePressure =
    w.submissionRate > 0 ? w.submissionRate / Math.max(w.completionRate, .001) : 1;
...
export class RealmPool<F extends RealmFn = RealmFn> {
  ...
  #spawnWorker(slotIndex: number): PoolWorker {
    ...
    realm.port.addEventListener('message', (ev) => {
      ...
      worker.avgLatencyMs =
        worker.completedCount === 0 ? latencyMs : ema(worker.avgLatencyMs, latencyMs);
      ...
      if (msg.__pool_result) {
        if (call.startPublished || _topicPoolCallEnd.hasSubscribers) {
          _topicPoolCallEnd.publish(
            otelRuntimeEvent('realm_pool', 'call', 'end', {
              correlationId,
        ...
        if (this.#exclusive) {
          ...
          if (idle !== undefined) {
            ...
            if (queueStartPublished) {
              _topicPoolQueue.publish(
                otelRuntimeEvent('realm_pool', 'queue', 'start', {
                  correlationId,
  ...
  #dispatch(
    ...
    if (startPublished) {
      _topicPoolCall.publish(
        otelRuntimeEvent('realm_pool', 'call', 'start', {
          correlationId,
          poolSize: this.size,
  ...
  #pumpQueue(): void {
    while (this.#queue.length > 0) {
      ...
      if (queued.queueStartPublished || _topicPoolQueueEnd.hasSubscribers) {
        _topicPoolQueueEnd.publish(
          otelRuntimeEvent('realm_pool', 'queue', 'end', {
  ...
  async close(): Promise<void> {
    ...
    const drainResult = await Promise.race([
      ...
      new Promise<'timeout'>((res) => setTimeout(() => res('timeout'), this.#closeTimeout)),
    ]);
