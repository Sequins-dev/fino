...
function parseTarget(
  input: string | URL | PostgresOptions,
...
function socketAddress(target: ConnectionTarget): Address {
  if (target.host.startsWith('/'))
    return { family: 'unix', path: `${target.host}/.s.PGSQL.${target.port}` };
  return target.host.includes(':')
    ? { family: 'ipv6', ip: target.host, port: target.port }
    : { family: 'ipv4', ip: target.host, port: target.port };
}
async function resolveSocketAddress(target: ConnectionTarget): Promise<Address> {
  ...
  return resolved.family === 6
    ? { family: 'ipv6', ip: resolved.address, port: target.port }
    : { family: 'ipv4', ip: resolved.address, port: target.port };
}
...
async function* toAsyncChunks(
  source: Iterable<Uint8Array | string> | AsyncIterable<Uint8Array | string>,
...
export class PostgresStatement {
  ...
  run(...params: DbValue[]): Promise<PostgresQueryResult> {
    return this.#db
      ._execute(this.#sql, params, this.#name)
      .then((result) => ({ changes: result.changes, command: result.command }));
  }
  /**
...
export class PostgresDatabase {
   ...
   */
  static async open(
    target: string | URL | PostgresOptions,
    options: PostgresOptions = {},
  ...
  async #connect(target: ConnectionTarget): Promise<void> {
    ...
    if (target.tls !== 'disable') {
      ...
      if (response[0] === 0x53) {
        ...
        target.tls === 'require' ||
        target.tls === 'verify-ca' ||
        target.tls === 'verify-full'
      ) {
        socket.close();
  ...
  async #handleAuth(message: Extract<BackendMessage, { type: 'Authentication' }>): Promise<void> {
    ...
    if (code === 5) {
      await this.#writer.write(
        encodePasswordMessage(
          md5Password(password, this.#target.user, message.data.subarray(0, 4)),
        ),
      );
      return this.#writer.flush();
    }
    if (code === 10) {
      ...
      await this.#writer.write(
        encodeSaslInitialResponse('SCRAM-SHA-256', scram.initialResponse(this.#target.user)),
      );
      await this.#writer.flush();
      const challenge = decodeBackendMessage(await readFrame(this.#reader));
      if (challenge.type !== 'Authentication' || challenge.code !== 11)
        throw new Error('postgres: expected SASL challenge');
      await this.#writer.write(
        encodeSaslResponse(await scram.finalMessage(dec.decode(challenge.data))),
      );
      await this.#writer.flush();
      const final = decodeBackendMessage(await readFrame(this.#reader));
      if (final.type !== 'Authentication' || final.code !== 12)
        throw new Error('postgres: expected SASL final');
    ...
    await this._serialize(async () => {
      ...
      while (true) {
        ...
        if (message.type === 'ErrorResponse')
          throw new Error(message.fields.M ?? 'postgres query failed');
  ...
  async _execute(
    ...
    params: DbValue[] = [],
    statement = '',
  ): Promise<{ rows: DbRow[]; changes: number; command?: string }> {
      ...
      while (true) {
        ...
        if (message.type === 'RowDescription') fields = message.fields;
          ...
          for (let index = 0; index < fields.length; index++)
            row[fields[index]!.name] = decodeValue(fields[index]!, message.values[index] ?? null);
          rows.push(row);
   ...
   * ```
   */
  async listen(channel: string): Promise<
    AsyncIterable<PostgresNotification> & {
      close(): Promise<void>;
  ...
  async copyFrom(
    ...
    return this._serialize(async () => {
      ...
      while (true) {
        ...
        if (message.type === 'ErrorResponse')
          throw new Error(message.fields.M ?? 'postgres copy failed');
      }
      try {
        for await (const chunk of toAsyncChunks(source))
          await this.#writer.write(encodeCopyData(chunk));
...
export class PostgresPool {
  ...
  constructor(
    target: string | URL | PostgresOptions,
    options: PostgresOptions & { max?: number } = {},
  ) {
    this.#target = target;
  ...
  async close(): Promise<void> {
    ...
    for (const waiter of this.#waiters.splice(0))
      waiter.reject(new Error('postgres pool is closed'));
    this.#total = 0;
  }
}
