...
function parseTsOrThrow(source: string, filename: string, lineNum: number, original: string): void {
  ...
  if (!parsed.ok)
    throw new MigrationParseError(
      parsed.errors[0]?.message ?? 'invalid TypeScript syntax',
      original,
      lineNum,
    );
}
  ...
  for (let i = 0; i < param.length; i++) {
    ...
    if (ch === '<' || ch === '{' || ch === '[' || ch === '(') depth++;
      ...
      if (!/^[A-Za-z_$][\w$]*$/.test(name))
        throw new MigrationParseError(
          `invalid parameter name: ${JSON.stringify(name)}`,
          source,
          lineNum,
        );
      if (!type)
        throw new MigrationParseError(
          `missing type for parameter: ${JSON.stringify(name)}`,
          source,
          lineNum,
        );
      return { name, type };
...
function parseSignature(
  ...
  source: string,
  lineNum: number,
): { params: SqlParamIR[]; returnType: string } {
  parseTsOrThrow(`function ${name}${raw} {}`, `${source}.ts`, lineNum, source);
  const close = raw.lastIndexOf(')');
  const params = splitParams(raw.slice(1, close)).map((param) =>
    splitParam(param, source, lineNum),
  );
...
function validateSqlLine(sqlLine: string, source: string, lineNum: number): void {
  ...
  if (opening !== closing)
    throw new MigrationParseError('unbalanced placeholder braces', source, lineNum);
  if (/\{\{!?\s*\}\}/.test(sqlLine))
    throw new MigrationParseError('empty placeholder', source, lineNum);
}
...
function pathParts(path: string): Array<string | number> {
  ...
  path.replace(
    /([A-Za-z_$][\w$]*)|\[(\d+|'[^']+'|"[^"]+")\]/g,
    (_, ident: string | undefined, bracket: string | undefined) => {
...
function readPath(values: Record<string, unknown>, path: string): unknown {
  ...
  for (const part of pathParts(path)) {
    if (cur === null || cur === undefined || !(part in Object(cur)))
      throw new Error(`Missing SQL placeholder value: ${path}`);
...
function renderFunctionBody(
  sqlText: string,
  params: SqlParamIR[],
  functions: Record<string, (...args: unknown[]) => string>,
  args: unknown[],
  escapeFn: (value: unknown) => unknown,
): string {
  ...
  return sqlText
    .replace(/\{\{!\s*([^}]+?)\s*\}\}/g, (_, rawPath: string) => {
      ...
      if (
        ...
        !path.includes('[') &&
        functions[path] &&
        params.every((p) => p.name !== path)
      )
        return String(functions[path]());
      return String(readPath(values, path) ?? '');
    })
    .replace(/\{\{\s*([^}]+?)\s*\}\}/g, (_, rawPath: string) =>
      String(escapeFn(readPath(values, rawPath.trim()))),
    );
}
 ...
 * ```
 */
export function compileSqlModule(
  ir: SqlModuleIR,
  ...
  for (const fn of ir.functions) {
    functions[fn.name] = (...args: unknown[]) =>
      renderFunctionBody(fn.sql, fn.params, functions, args, escapeFn);
...
export function toSqlModuleSource(ir: SqlModuleIR): string {
  ...
  out.push(
    '  path.replace(/([A-Za-z_$][\\w$]*)|\\[(\\d+|\\\'[^\\\']+\\\'|"[^"]+")\\]/g, (_: string, ident: string | undefined, bracket: string | undefined) => {',
  );
  ...
  for (const fn of ir.functions) {
    ...
    if (fn.description.length > 0)
      out.push('/**', ...fn.description.map((line) => ` * ${line}`), ' */');
    out.push(`export function ${fn.name}(${params}): ${fn.returnType} {`);
    out.push(
      `  const __values: Record<string, unknown> = { ${fn.params.map((p) => p.name).join(', ')} };`,
    );
    out.push(
      `  return ${jsString(fn.sql)}.replace(/\\{\\{!\\s*([^}]+?)\\s*\\}\\}/g, (_, p) => String(__read(__values, String(p).trim()) ?? "")).replace(/\\{\\{\\s*([^}]+?)\\s*\\}\\}/g, (_, p) => String(__escape(__read(__values, String(p).trim())))) as ${fn.returnType};`,
    );
    out.push('}');
  }
  out.push('');
