...
function isFragment(value: unknown): value is SqlFragment {
  return (
    typeof value === 'object' &&
    value !== null &&
    (value as { kind?: unknown }).kind === 'sql' &&
...
function appendFragment(parts: SqlPart[], values: DbValue[], fragment: SqlFragment): void {
  ...
  for (const match of rendered.matchAll(/\$(\d+)/g)) {
    if (match.index! > cursor)
      parts.push({ kind: 'text', text: rendered.slice(cursor, match.index) });
...
function quoteIdentifier(name: string): string {
  if (!/^[A-Za-z_][A-Za-z0-9_ ]*$/.test(name))
    throw new TypeError(`Invalid SQL identifier: ${name}`);
  ...
  return {
    ...
    bind(params: DbParams[]): DbValue[] {
      ...
      return tokens.map((token) => {
        ...
        if (!named || !(token.name in named))
          throw new Error(`Missing SQL parameter :${token.name}`);
...
class GenericDatabase implements DatabaseConnection {
  ...
  prepare(query: SqlInput): DatabaseStatement {
    const rendered = renderInput(query, this.driver);
    const plan =
      this.driver === 'postgres' && typeof query === 'string'
        ? postgresPlaceholderPlan(rendered.text)
        : null;
...
export class Database {
  ...
  static async open(
    ...
    if (isPostgresTarget(value))
      return new GenericDatabase('postgres', await PostgresDatabase.open(value, options as any));
    return new GenericDatabase(
      'sqlite',
      await sqlite.Database.open(sqlitePathFromTarget(value), options as sqlite.DatabaseOptions),
    );
  }
}
/**
