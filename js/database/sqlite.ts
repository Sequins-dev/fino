...
export class Statement {
  ...
  */
  async #compile(): Promise<ArrayBuffer> {
    ...
    const rc = await s.sqlite3_prepare_v2(this.#db.ptr, sqlBuf, -1, Pointer.of(ppStmt), null) as number;
    if (rc !== SQLITE_OK) {
  ...
  */
  #resolveParams(ptr: ArrayBuffer, params: SqlValue[]): void {
    if (params.length === 1 && params[0] !== null && params[0] !== undefined && typeof params[0] === 'object' && !(params[0] instanceof Uint8Array)) {
...
export class Database {
  ...
  */
  static async open(path: string, opts: DatabaseOptions = {}): Promise<Database> {
    ...
    const rc = await s.sqlite3_open_v2(pathBuf, Pointer.of(ppDb), flags, vfs.nameCstrPointer) as number;
  ...
  */
  #probeVectors(): boolean {
    ...
    const envPath = typeof process !== 'undefined' ? (process as {
      env?: Record<string, string>;
    }).env?.['FINO_SQLITE_VEC_PATH'] : undefined;
    ...
    for (const p of candidates) {
      try {
        ...
        const rc2 = s.sqlite3_load_extension(this.#ptr, Pointer.of(cstr(p)), null, Pointer.of(ppErr)) as number;
  ...
  */
  loadExtension(path: string, entryPoint?: string): void {
    ...
    const rc = s.sqlite3_load_extension(this.#ptr, Pointer.of(cstr(path)), entryPoint ? Pointer.of(cstr(entryPoint)) : null, Pointer.of(ppErr)) as number;
    if (rc !== SQLITE_OK) {
