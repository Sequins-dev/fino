 ...
 */
import { createDefaultPrompt, PromptSession } from '../tty/prompt.ts';
type OptionDefault =
  | boolean
  | string
  | number
  ...
  | ((
      ...
      | number
      | Array<string | number>
      | Promise<boolean | string | number | Array<string | number>>);
...
export class Command {
  ...
  constructor(config: CommandConfig = {}) {
    ...
    for (const child of config.commands ?? [])
      this.#registerChild(child instanceof Command ? child : new Command(child));
  }
  /**
  ...
  parse(argv: string[], options: ParseOptions = {}): unknown {
    ...
    if (finalInvocation === undefined || finalParsed === undefined)
      throw new Error('Command parse failed');
    ...
    if (isPromiseLike(resolved)) {
      return resolved.then(() =>
        finalInvocation.command.run(createContext(finalInvocation, invocationChain, prompt)),
      );
    }
  ...
  usage(programName?: string): string {
    ...
    if (segments.length > 0) {
      const visibleSegments =
        programName !== undefined && segments[0] === programName ? segments.slice(1) : segments;
  ...
  help(programName?: string): string {
    ...
    if (this.#options.length > 0) {
      ...
      for (const def of this.#options)
        lines.push(
          `  ${formatFlags(def)}${formatChoices(def.choices)}${formatDescription(def.description)}`,
        );
    }
    if (this.#positionals.length > 0) {
      ...
      for (const def of this.#positionals) {
        lines.push(
          `  ${def.name} (${def.type}${def.multiple ? '[]' : ''})${formatChoices(def.choices)}${formatDescription(def.description)}`,
        );
      }
    }
    if (this.#children.length > 0) {
      ...
      for (const child of this.#children)
        lines.push(`  ${child.name}${formatDescription(child.description)}`);
    }
    return lines.join('\n');
  }
  /**
  ...
  #parseInto(state: ParseState, path: string[]): ParsedNode[] {
    ...
    while (state.index < state.argv.length) {
      ...
      if (!stopOptions && child !== undefined) {
        if (!parsed.helpRequested)
          this.#finalizeParsedNode(parsed, this.#formatPath(path), {
            allowMissingPositionals: true,
          });
        state.index++;
    ...
    if (!parsed.helpRequested)
      this.#finalizeParsedNode(parsed, this.#formatPath(path), { allowMissingPositionals: false });
    return [parsed];
  }
  /**
  ...
  #consumeLongOption(state: ParseState, parsed: ParsedNode, path: string[]): boolean {
    ...
    if (negated) {
      if (def.type !== 'boolean')
        throw new Error(`Option "--no-${flag.slice(3)}" can only be used with boolean options`);
    ...
    if (def.type === 'boolean') {
      assignOptionValue(
        ...
        def,
        value === null ? true : coerceOptionValue(def, value, formatPath(path), formatOption(def)),
      );
      return true;
    }
    const raw =
    ...
    assignOptionValue(
      ...
      parsed.providedOptions,
      def,
      coerceOptionValue(def, raw, formatPath(path), formatOption(def)),
    );
    return true;
  }
  /**
    ...
    for (const invocation of chain) {
      for (const def of invocation.command.#options) {
        ...
        if (typeof defaultValue === 'function') {
          schedule(async () => {
            invocation.options[def.key] = await defaultValue(
              createContext(invocation, chain, prompt),
            );
          });
        } else {
          ...
          if (
            ...
            (def.multiple && Array.isArray(currentValue) && currentValue.length === 0)
          ) {
      ...
      schedule(() => {
        validateRequiredOptions(
          ...
          invocation.options,
          invocation.command.#formatPath(invocation.path),
        );
      });
    }
...
function takeNextValue(state: ParseState, def: OptionDefinition, formattedPath: string): string {
  if (state.index >= state.argv.length)
    throw new Error(`Missing value for option "${formatOption(def)}" on ${formattedPath}`);
...
function coercePositionalValue(
  def: PositionalDefinition,
  raw: string,
  path: string,
): string | number {
...
function validateRequiredOptions(
  ...
  for (const def of defs) {
    ...
    if (def.required && missing)
      throw new Error(`Missing required option "${formatOption(def)}" on ${formattedPath}`);
  }
}
function buildInvocationChain(parsedChain: ParsedNode[]): CommandInvocation[] {
  ...
  for (const parsed of parsedChain) {
    const invocation: CommandInvocation = new CommandInvocation(
      parsed.command,
      parent,
      parsed.args,
      parsed.options,
