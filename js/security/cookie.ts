...
import {
  ...
  toBytes,
  utf8,
  type BufferLike as SecurityBufferLike,
...
export function serializeCookie(name: string, value: string, options: CookieOptions = {}): string {
  ...
  if (options.domain !== undefined && /[\r\n]/.test(options.domain))
    throw new Error('Invalid cookie Domain attribute');
  if (options.path !== undefined && /[\r\n]/.test(options.path))
    throw new Error('Invalid cookie Path attribute');
  if (options.expires !== undefined && !Number.isFinite(options.expires.getTime()))
    throw new Error('Invalid cookie Expires attribute');
  if (options.maxAge !== undefined && !Number.isFinite(options.maxAge))
    throw new Error('Invalid cookie Max-Age attribute');
  if (
