...
function keyMatchesPurpose(
  ...
  if (purpose?.key_ops && Array.isArray(key.key_ops)) {
    for (const op of purpose.key_ops) {
      ...
      if (op === 'verify' && key.kty !== 'oct' && (key.key_ops as unknown[]).includes('sign'))
        continue;
      return false;
    }
  }
...
function validateClaims(payload: Record<string, unknown>, options: JwtVerifyOptions): void {
  ...
  if (jwtIds !== null && !jwtIds.includes(String(payload.jti ?? '')))
    throw new Error('JWT jti mismatch');
...
export async function jwtSign(
  ...
  let signature = new Uint8Array(
    (await crypto.subtle.sign(signAlgorithm(alg), cryptoKey, toBytes(signingInput))) as ArrayBuffer,
  );
...
export async function jwtVerify(
  ...
  const ok = await crypto.subtle.verify(
    signAlgorithm(alg as JwtAlgorithm),
    cryptoKey,
    signature,
    toBytes(`${parts[0]}.${parts[1]}`),
  );
 ...
 * ```
 */
export async function jwtEncrypt(
  payload: Record<string, unknown>,
  key: JsonWebKeyLike,
  ...
  if (options.algorithm === 'dir') {
    if (key.kty !== 'oct' || typeof key.k !== 'string')
      throw new Error('dir JWE requires an oct JWK');
    ...
    encryptedKey = new Uint8Array(
      (await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, publicKey, cek)) as ArrayBuffer,
    );
  }
...
export async function jwtDecrypt(token: string, keys: JwtKeyInput): Promise<JwtResult> {
  ...
  try {
    ...
    if (cek.byteLength !== cekLength(enc))
      throw new Error('JWE content encryption key length mismatch');
