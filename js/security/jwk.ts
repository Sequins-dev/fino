...
function algorithmForJwk(
  ...
  if (jwk.kty === 'oct') {
    if (
      ...
      return {
        name: 'AES-GCM',
        length: base64urlDecode(String(jwk.k ?? '')).byteLength * 8,
      };
    }
    return {
      name: 'HMAC',
  ...
  if (jwk.kty === 'RSA') {
    ...
    if (alg?.startsWith('RSA-OAEP'))
      return {
        name: 'RSA-OAEP',
        hash: alg === 'RSA-OAEP' ? 'SHA-1' : hashForAlg(alg),
      };
    return {
  ...
  if (jwk.kty === 'EC')
    return {
      name: 'ECDSA',
      namedCurve: String(jwk.crv ?? 'P-256'),
    };
  throw new Error(`Unsupported JWK kty: ${String(jwk.kty)}`);
}
function generationAlgorithm(
  options: GenerateJwkOptions,
): AlgorithmIdentifier | RsaHashedKeyGenParams | EcKeyGenParams | HmacKeyGenParams {
  if (options.kty === 'oct')
    return {
      name: 'HMAC',
      hash: hashForAlg(options.alg),
      length: options.length ?? 256,
    };
  if (options.kty === 'RSA') {
    const alg = options.alg?.startsWith('PS')
      ? 'RSA-PSS'
      : options.alg?.startsWith('RSA-OAEP')
        ? 'RSA-OAEP'
        : 'RSASSA-PKCS1-v1_5';
    return {
      name: alg,
...
function defaultUsages(options: GenerateJwkOptions): KeyUsage[] {
  if (options.kty === 'oct' && (options.alg === 'dir' || options.alg?.startsWith('A')))
...
export async function generateJwk(options: GenerateJwkOptions): Promise<JsonWebKeyLike> {
  ...
  const generated = await crypto.subtle.generateKey(
    generationAlgorithm(options),
    true,
    defaultUsages(options),
  );
...
export async function exportPublicJwk(key: JsonWebKeyLike | CryptoKey): Promise<JsonWebKeyLike> {
  const jwk =
    key instanceof CryptoKey
      ? ((await crypto.subtle.exportKey('jwk', key)) as JsonWebKeyLike)
      : { ...key };
...
export async function jwkThumbprint(jwk: JsonWebKeyLike): Promise<string> {
  ...
  for (const field of fields) {
    if (jwk[field] === undefined)
      throw new Error(`JWK thumbprint missing required member: ${field}`);
...
export function selectJwk(
  ...
  return keys.find((key) => {
    ...
    if (selector.alg !== undefined && key.alg !== undefined && key.alg !== selector.alg)
      return false;
    if (selector.kty !== undefined && key.kty !== selector.kty) return false;
    if (selector.use !== undefined && key.use !== undefined && key.use !== selector.use)
      return false;
