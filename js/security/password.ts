 ...
 * ```
 */
import { pbkdf2 } from '../internal/openssl.ts';
import {
  base64urlDecode,
  base64urlEncode,
  timingSafeEqual,
...
export function hashPassword(password: string, options: HashPasswordOptions = {}): string {
  ...
  const iterations = options.iterations ?? 21e4;
  const saltLength = options.saltLength ?? 16;
  const keyLength = options.keyLength ?? 32;
  if (!Number.isSafeInteger(iterations) || iterations < 1)
    throw new RangeError('iterations must be a positive integer');
  if (!Number.isSafeInteger(saltLength) || saltLength < 1)
    throw new RangeError('saltLength must be a positive integer');
  if (!Number.isSafeInteger(keyLength) || keyLength < 1)
    throw new RangeError('keyLength must be a positive integer');
