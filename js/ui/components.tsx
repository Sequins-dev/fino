...
// Charts
//
// `BarChart`/`LineChart` follow the same split as every other catalog
// component: the tree carries data, never glyphs or markup. The one
// exception is `plotBraille` below — braille dot rasterization is pure
// geometry (character selection from a bit pattern), not a presentation
// decision, so it lives here as a unit-testable pure function and both
// lowerings call it rather than each re-deriving the dot bit order.
//
// A series is `points: number[]` — plain y-values at implied, evenly spaced
// x positions — rather than `{ x, y }` pairs. Every chart here plots
// categorical or time-bucketed data (bar categories, sampled line series)
// where the x axis is a uniform index; `labels` (on `BarChart`) supplies the
// category names for that axis when the index itself isn't the label. Pairs
// would add a second coordinate every call site has to invent (usually just
// the index again) for no chart in this catalog that actually has irregular
// x spacing — if one shows up later, it can grow its own prop rather than
// bending every chart through `{x,y}` today.
// ---------------------------------------------------------------------------
