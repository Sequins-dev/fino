...
describe('fino:ui/slides', () => {
  ...
  it('provides a combined router with a relative presenter branch', async (t) => {
    ...
    const presenter = (await app.handle(
      new Request('http://local/conference/talk/_presenter'),
    )) as Response;
    t.ok(
      (await viewer.text()).includes('First slide'),
      'combined viewer is relative to the app mount',
    );
    t.ok(
      (await presenter.text()).includes('Presenter'),
      'combined presenter uses the documented relative branch',
    );
  });
  it('emits valid client scripts for mounted route endpoints', async (t) => {
    ...
    const presenterHtml = await presenter.text();
    t.ok(
      viewerHtml.includes("replace(/\\/$/,'')"),
      'viewer emits an escaped trailing-slash expression',
    );
    t.ok(
      presenterHtml.includes("replace(/\\/$/,'')"),
      'presenter emits an escaped trailing-slash expression',
    );
    t.notOk(viewerHtml.includes('replace(//$/'), 'viewer script is not parsed as a line comment');
    t.notOk(
      presenterHtml.includes('replace(//$/'),
      'presenter script is not parsed as a line comment',
    );
  });
  it('gives follow=false viewers an isolated server-driven SSE session', async (t) => {
    ...
    t.ok(
      pageHtml.includes('First slide'),
      'independent viewer starts at the beginning of the deck',
    );
    await presentation.next();
    const response = (await app.handle(
      new Request(`http://local/talk/_events?session=${session.id}`),
    )) as Response;
    const events = parseEventStream(response.body!)[Symbol.asyncIterator]();
    const initial = await events.next();
    t.ok(
      initial.value?.data.includes('First slide') === true,
      'independent stream starts with its own current slide',
    );
    const moved = (await app.handle(
      viewerCommand('http://local/talk/_command', 'next', session),
    )) as Response;
    t.equal(moved.status, 200, 'direction command updates the independent session');
    const update = await events.next();
    t.ok(
      update.value?.data.includes('Second slide') === true,
      'independent stream receives its navigation patch',
    );
    t.deepEqual(
      presentation.state,
      {
    ...
    t.ok(
      (await followed.text()).includes('Second slide'),
      'normal audience remains on presenter state',
    );
    const rejected = (await app.handle(
      viewerCommand('http://local/talk/_command', 'next', {
        ...session,
        nonce: 'wrong',
      }),
    )) as Response;
  ...
  it('maps direction keys to an independent viewer session without changing follow mode', async (t) => {
    ...
    app.route('/talk').mount(presentation.viewer());
    const followed = (await app.handle(new Request('http://local/talk'))) as Response;
    const independent = (await app.handle(
      new Request('http://local/talk?follow=false'),
    )) as Response;
    const followedHtml = await followed.text();
    const independentHtml = await independent.text();
    t.notOk(
      followedHtml.includes('data-fino-viewer-session'),
      'default viewer continues to follow presenter state',
    );
    t.ok(
      independentHtml.includes(
        "new EventSource(endpoint+'/_events?session='+encodeURIComponent(session))",
      ),
      'independent viewer subscribes to its own SSE session',
    );
    t.ok(
      independentHtml.includes("event.key==='ArrowRight'||event.key==='ArrowDown'"),
      'right and down arrows advance',
    );
    t.ok(
      independentHtml.includes("event.key==='ArrowLeft'||event.key==='ArrowUp'"),
      'left and up arrows go back',
    );
    t.ok(independentHtml.includes("send('next')"), 'forward keys send a server navigation command');
    t.ok(
      independentHtml.includes("send('previous')"),
      'back keys send a server navigation command',
    );
    await presentation.close();
  });
  it('bounds retained independent viewer sessions', async (t) => {
    ...
    const expired = (await app.handle(
      viewerCommand('http://local/talk/_command', 'next', oldest),
    )) as Response;
  ...
  it('fits one responsive slide into viewer and presenter viewports without layout JavaScript', async (t) => {
    ...
    const presenterHtml = await presenter.text();
    t.ok(
      viewerHtml.includes('class="fino-slide-viewport fino-audience"'),
      'viewer wraps the responsive slide in a fitting viewport',
    );
    t.ok(
      presenterHtml.includes('class="fino-presenter-preview fino-slide-viewport"'),
      'presenter current slide uses the same fitting viewport',
    );
    t.ok(
      presenterHtml.includes('class="fino-next-viewport fino-slide-viewport"'),
      'presenter next slide uses the same fitting viewport',
    );
    t.ok(
      viewerHtml.includes('<div class="fino-slide-surface">'),
      'viewer renders a responsive HTML slide surface',
    );
    t.ok(
      presenterHtml.match(/<div class="fino-slide-surface">/g)?.length === 2,
      'presenter renders responsive HTML surfaces for both previews',
    );
    ...
    t.ok(
      viewerHtml.includes(
        '.fino-slide-viewport{position:relative;display:grid;place-items:center;overflow:hidden;container-type:size}',
      ),
      'each host exposes both dimensions to its slide',
    );
    t.ok(
      viewerHtml.includes(
        '.fino-slide-surface{width:min(100cqw,177.7777778cqh);height:min(100cqh,56.25cqw);aspect-ratio:16/9;container-type:inline-size}',
      ),
      'slide surface takes the largest contained 16:9 area',
    );
    t.ok(
      viewerHtml.includes('.fino-slide-frame{position:relative;width:100%;height:100%;'),
      'slide content fills its responsive surface',
    );
    t.ok(
      viewerHtml.includes('font-size:1.25cqw'),
      'slide-local content remains legible as the responsive surface scales',
    );
    t.ok(
      viewerHtml.includes('margin:0;max-width:100%'),
      'slide headings can use the full content width',
    );
    t.ok(
      viewerHtml.includes('.fino-slide-frame h1{font-size:5.5cqw}'),
      'slide titles leave more room for supporting content',
    );
    t.ok(
      viewerHtml.includes('.fino-slide-frame h2{font-size:4.6cqw}'),
      'section titles remain prominent without dominating the slide',
    );
    t.ok(
      viewerHtml.includes('.fino-slide-frame p,.fino-slide-frame li{font-size:2.5cqw'),
      'body copy uses a readable presentation scale',
    );
    t.notOk(
      viewerHtml.includes('background-size:5px 5px'),
      'slide decoration does not retain fixed pixel dimensions',
    );
    t.ok(
      viewerHtml.includes('background-size:.3125em .3125em'),
      'slide decoration follows the slide-local scale',
    );
    t.ok(
      presenterHtml.includes('color:var(--slides-ink)'),
      'slide foreground does not inherit from the presenter shell',
    );
    t.ok(
      viewerHtml.includes('.fino-audience{width:100vw;height:100vh'),
      'viewer gives the slide scaler the full browser viewport',
    );
    t.ok(
      presenterHtml.includes(
        '.fino-presenter-preview{width:100%;height:100%;min-width:0;min-height:0}',
      ),
      'presenter current slide fills its allocated preview cell',
    );
    t.ok(
      presenterHtml.includes('.fino-next-viewport{width:100%;aspect-ratio:16/9}'),
      'presenter next slide fills its bounded preview area',
    );
    t.notOk(
      viewerHtml.includes(
        '.fino-slide-viewport{position:relative;overflow:hidden;aspect-ratio:16/9}',
      ),
      'shared viewport does not force a width-driven box around the slide',
    );
    t.ok(
      viewerHtml.includes('.fino-fullscreen{position:absolute;right:0;top:0;'),
      'fullscreen control overlays the slide without reserving space',
    );
    t.ok(
      viewerHtml.includes('opacity:0;transition:opacity'),
      'fullscreen control is hidden until its corner is targeted',
    );
    t.ok(
      viewerHtml.includes('.fino-fullscreen:hover,.fino-fullscreen:focus-visible{opacity:1}'),
      'fullscreen control fades in for pointer and keyboard users',
    );
    t.notOk(viewerHtml.includes('ResizeObserver'), 'viewer needs no JavaScript layout observer');
    t.notOk(
      presenterHtml.includes('ResizeObserver'),
      'presenter needs no JavaScript layout observer',
    );
  });
  it('navigates shared state through same-origin nonce-protected commands', async (t) => {
    ...
    const rejectedOrigin = (await app.handle(
      command('http://local/control/_command', 'next', token, 'https://evil.example'),
    )) as Response;
    const rejectedNonce = (await app.handle(
      command('http://local/control/_command', 'next', 'wrong'),
    )) as Response;
    ...
    const moved = (await app.handle(
      command('http://local/control/_command', 'next', token),
    )) as Response;
    const movedBody = await moved.text();
    t.equal(moved.status, 200, `valid presenter command succeeds (${movedBody})`);
    const viewer = (await app.handle(new Request('http://local/talk'))) as Response;
    t.ok(
      (await viewer.text()).includes('Second slide'),
      'viewer observes presenter-controlled state',
    );
  });
  ...
  it('hot-patches file decks and keeps the last good audience on errors', async (t) => {
    ...
    try {
      ...
      t.ok(
        (await retained.text()).includes('First version'),
        'invalid edit retains the last valid audience',
      );
