...
describe('JobsStore', () => {
  it('claims atomically via UPDATE...RETURNING (strategy gate)', async (t) => {
    ...
    const claimed = await store.claimReady('me', 3e4, 5);
    ...
    t.equal((await store.claimReady('me', 3e4, 5)).length, 0, 'no double-claim');
  });
  it('two connections claim disjoint sets', async (t) => {
    ...
    for (let round = 0; round < 40 && fromA.length + fromB.length < 10; round++) {
      const [ra, rb] = await Promise.allSettled([a.claimReady('worker-a', 3e4, 2), b.claimReady('worker-b', 3e4, 2)]);
  ...
  it('concurrent claimants on one connection get disjoint sets', async (t) => {
    ...
    const [fromA, fromB] = await Promise.all([store.claimReady('worker-a', 3e4, 6), store.claimReady('worker-b', 3e4, 6)]);
    const ids = new Set([...fromA.map((j) => j.id), ...fromB.map((j) => j.id)]);
  ...
  it('claiming a waiting job does not consume an attempt', async (t) => {
    ...
    const [first] = await store.claimReady('me', 3e4, 1);
    t.equal(first!.attempts, 1, 'pending claim consumed attempt');
    await store.markWaiting(job.id, 'wf-run-1', {
      type: 'timer',
      dueAt: Date.now() - 1
    }, Date.now() - 1);
    const [resumed] = await store.claimReady('me', 3e4, 1);
  ...
  it('orders claims by priority then run_at', async (t) => {
    ...
    const claimed = await store.claimReady('me', 3e4, 2);
  ...
  it('computes nextWakeAt across jobs, leases, and schedules', async (t) => {
    ...
    t.equal(await store.nextWakeAt(), soon - 1e3, 'earlier schedule wins');
  });
  ...
  it('backoff delay stays within half-jitter bounds', (t) => {
    ...
    for (let attempt = 1; attempt <= 4; attempt++) {
      const cap = Math.min(6e4, 1e3 * Math.pow(2, attempt - 1));
      for (let i = 0; i < 20; i++) {
    ...
    t.equal(backoffDelayMs({
      ...policy,
      jitter: false
    }, 3), 4e3, 'no-jitter delay is exact');
  });
