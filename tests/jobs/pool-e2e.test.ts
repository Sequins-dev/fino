...
describe('fino:jobs pool processors', () => {
  it('runs jobs in an exclusive pool from a Task-file entry', async (t) => {
    ...
    const done = await jobs.wait(job.id, { timeoutMs: 3e4 });
    ...
    const rootDone = await jobs.wait(root.id, { timeoutMs: 3e4 });
  ...
  it('retries across fresh worker realms', async (t) => {
    ...
    const job = await jobs.push('pool-marker-flaky', { marker }, { retry: {
      maxAttempts: 3,
      baseMs: 20,
      jitter: false
    } });
  ...
  it('a plain RealmPool accepts a Task-file entry directly', async (t) => {
    ...
    try {
      ...
      t.equal((result as {
        output: unknown;
      }).output, 10, 'result came back over the pool wire');
    } finally {
      await pool.close();
    }
  });
