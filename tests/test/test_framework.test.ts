...
describe('runner behavior', () => {
  ...
  it('filters matching top-level test leaves', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', '--filter', 'needle', 'filter.test.ts'], { cwd: dir });
  ...
  it('supports live console output for debugging', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', '--show-output=always', 'show-output.test.ts'], { cwd: dir });
  ...
  it('rejects invalid output reporting modes', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', '--show-output=bad', 'tap.test.ts'], { cwd: dir });
  ...
  it('adds duration metadata to every result line when requested', async (t) => {
    await withTempProject({
      ...
      const { stdout, result } = await runCli(['test', '--durations', 'durations.test.ts'], { cwd: dir });
