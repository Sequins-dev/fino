...
async function withTempProject<T>(
  ...
  const dir = '/tmp/fino-test-framework-' + Math.floor(Math.random() * 1e9);
  await fs.mkdir(dir);
  try {
...
describe('runner behavior', () => {
  it('emits per-test metadata on passing leaves', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(['test', 'meta-pass.test.ts'], {
          cwd: dir,
        });
        ...
        t.ok(
          stdout.includes(
            'ok 1 - metadata pass # case=empty, rows=0, nullable=null, missing=undefined, ready=true, count=2',
          ),
          'passing leaf includes metadata',
        );
      },
    );
  });
  it('emits per-test metadata on failing leaves', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'meta-fail.test.ts'], { cwd: dir });
        t.equal(result.code, 1, 'failing metadata fixture exits nonzero');
        t.ok(
          stdout.includes('not ok 1 - metadata fail # case=bad'),
          'failing leaf includes metadata',
        );
      },
    );
  });
  it('merges repeated metadata calls and overwrites older keys', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'meta-merge.test.ts'], { cwd: dir });
        t.equal(result.code, 0, 'merged metadata fixture exits successfully');
        t.ok(
          stdout.includes('ok 1 - metadata merge # case=second, rows=1, done=false'),
          'later metadata overwrites existing keys',
        );
      },
    );
  });
  it('quotes unsafe string metadata values', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'meta-quote.test.ts'], { cwd: dir });
        t.equal(result.code, 0, 'quoted metadata fixture exits successfully');
        t.ok(
          stdout.includes(
            'ok 1 - metadata quote # simple=alpha, spaced="two words", comma="a,b", quoted="say \\"hi\\"", hash="a#b"',
          ),
          'unsafe strings are JSON quoted',
        );
      },
    );
  });
  it('leaves TAP output unchanged when metadata is absent', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'no-meta.test.ts'], { cwd: dir });
        ...
        t.ok(
          !stdout.includes('ok 1 - plain pass #'),
          'plain result has no trailing metadata segment',
        );
      },
    );
  });
  it('keeps metadata off standalone Assert instances', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'assert-meta.test.ts'], { cwd: dir });
  ...
  it('rejects invalid metadata keys', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'meta-invalid.test.ts'], { cwd: dir });
        ...
        t.ok(
          stdout.includes('TypeError: Invalid test metadata key "bad key"'),
          'invalid metadata reports TypeError',
        );
      },
    );
  });
  it('emits TAP output for passing files', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(['test', 'tap.test.ts'], { cwd: dir });
        ...
        t.ok(
          new RegExp(String.raw`# time  ${DURATION_RE}`).test(stdout),
          'summary reports total runtime',
        );
      },
    );
  });
  it('suppresses console output from passing tests by default', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(['test', 'quiet-pass.test.ts'], {
          cwd: dir,
        });
        t.equal(result.code, 0, 'passing fixture exits successfully');
        ...
        t.ok(
          !stdout.includes('# Failure details'),
          'success output has no failure details section',
        );
      },
    );
  });
  it('prints captured stdout and stderr with failing test details', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(['test', 'quiet-fail.test.ts'], {
          cwd: dir,
        });
        ...
        t.ok(
          ...
          'command failure summary is still written to stderr',
        );
  ...
  it('runs hooks in lifecycle order and tears down after body failures', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'hooks.test.ts'], { cwd: dir });
        ...
        t.ok(
          before < beforeEach && beforeEach < body && body < afterEach && afterEach < after,
          'hooks run in lifecycle order',
        );
  ...
  it('propagates skip reasons through describe groups', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(['test', 'skip.test.ts'], { cwd: dir });
  ...
  it('reports nested failures through parent subtests', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'nested-failure.test.ts'], { cwd: dir });
    ...
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(
          ['test', '--filter', 'needle', 'filter.test.ts'],
          { cwd: dir },
        );
  ...
  it('makes teardown after hook failures explicit', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', 'hook-failure.test.ts'], { cwd: dir });
  ...
  it('reports after hook failures with captured output', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        ...
        t.ok(
          stderr.includes('[error] Error: 1 test(s) failed'),
          'command failure summary is written to stderr',
        );
        ...
        t.ok(
          stdout.includes('# 1) after hook: after failure'),
          'failure details include after hook title',
        );
        t.ok(
          stdout.includes('#   Error: after failed'),
          'failure details include after hook error',
        );
        t.ok(
          stdout.includes('# Captured stdout:'),
          'failure details include captured stdout section',
        );
  ...
  it('supports live console output for debugging', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(
          ['test', '--show-output=always', 'show-output.test.ts'],
          { cwd: dir },
        );
  ...
  it('rejects invalid output reporting modes', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, stderr, result } = await runCli(
          ['test', '--show-output=bad', 'tap.test.ts'],
          { cwd: dir },
        );
  ...
  it('adds duration metadata to every result line when requested', async (t) => {
    await withTempProject(
      ...
      async (dir) => {
        const { stdout, result } = await runCli(['test', '--durations', 'durations.test.ts'], {
          cwd: dir,
        });
        ...
        for (const line of resultLines) {
          t.ok(
            new RegExp(String.raw`# duration=${DURATION_RE}`).test(line),
            'result line includes duration metadata: ' + line,
          );
        }
        t.ok(
          stdout.includes('ok 1 - leaf pass # duration='),
          'runner duration overrides user duration',
        );
        t.ok(
          new RegExp(String.raw`ok \d+ - leaf skip # SKIP blocked # duration=${DURATION_RE}`).test(
            stdout,
          ),
          'skipped leaf keeps SKIP directive before duration',
        );
        t.ok(
          new RegExp(
            String.raw`ok \d+ - skipped group # SKIP blocked # duration=${DURATION_RE}`,
          ).test(stdout),
          'skipped group keeps SKIP directive before duration',
        );
        t.ok(
          new RegExp(String.raw`not ok \d+ - after fail group # duration=${DURATION_RE}`).test(
            stdout,
          ),
          'hook-generated group failure includes duration',
        );
      },
    );
  });
