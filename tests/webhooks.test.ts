...
describe('fino:webhooks inbound verification', () => {
  ...
  it('provides middleware with actionable errors and preserves the handler body', async (t) => {
    ...
    t.deepEqual(await accepted.json(), {
      id: 'evt_middleware',
      body: 'payload'
    }, 'middleware verifies a clone and leaves the handler body readable');
    const rejected = await app.handle(new Request('https://example.test/hooks', {
      method: 'POST',
      body: 'unsigned'
    }));
...
describe('fino:webhooks outbound delivery', () => {
  it('queues signed delivery through fino:jobs with retry and a stable idempotency id', async (t) => {
    ...
    const queued = await enqueueWebhook(jobs, {
      id: 'evt_delivery',
    ...
    t.ok(received.every((attempt) => attempt.signature?.startsWith('v1,') === true), 'every attempt is HMAC signed');
    t.ok(received.every((attempt) => attempt.timestamp === '1800000100'), 'timestamp header is included');
  });
});
