...
describe('fino:config', () => {
  ...
  it('loads explicit sources in list order with later sources taking precedence', async (t) => {
    ...
    await writeText(
      fs,
      TEST_DIR + '/app.toml',
      ['mode = "toml"', '[server]', 'host = "toml-host"', 'port = 5000', ''].join('\n'),
    );
    await writeText(
      fs,
      TEST_DIR + '/.env',
      ['APP_PORT=6000', 'APP_SECRET=from-dotenv', ''].join('\n'),
    );
    ...
    const loaded = await loadConfig({
      ...
      sources: [
        ...
        {
          type: 'argv',
          args: ['--server.port', '7000', '--mode', 'prod'],
          map: {
  ...
  it('wraps validation failures in ConfigError with redacted secret values', async (t) => {
    ...
    await t.rejects(
      ...
      (err) =>
        ...
        !JSON.stringify(err.issues).includes('short'),
      'config errors redact configured secret paths in messages and structured issues',
    );
  });
  it('tags secret env values and redacts accidental rendering', async (t) => {
    ...
    t.equal(
      ...
      'secret access requires an explicit reveal',
    );
    ...
    t.deepEqual(
      ...
      ['database.password'],
      'source reports contain paths, not secret values',
    );
    t.equal(
      JSON.stringify(loaded.sources).includes('correct horse'),
      false,
      'source reports do not disclose secret values',
    );
  });
  it('loads authenticated sealed secret files and rejects the wrong key', async (t) => {
    ...
    await t.rejects(
      ...
      (err) => err instanceof ConfigError && !err.message.includes('sealed-token'),
      'authentication failures do not expose plaintext',
    );
    await writeText(
      fs,
    ...
    await t.rejects(
      ...
      (err) =>
        ...
        !err.message.includes('must-not-appear'),
      'sealed plaintext is omitted from parse errors',
    );
  });
  it('grants named secrets to realms through a provider-backed facade', async (t) => {
    ...
    await t.rejects(
      () => grant.get('api.internal'),
      /not granted/i,
      'the parent-side grant rejects ungranted names',
    );
    ...
    t.equal(
      result.processEnv,
      'undefined',
      'the child does not receive ambient process-global secrets',
    );
  });
  it('maps quoted dotenv and prefixed env values with schema coercion', async (t) => {
    ...
    t.deepEqual(
      loaded.sources[0].keys.sort(),
      ['server.host', 'server.port'],
      'dotenv report includes mapped keys',
    );
  ...
  it('rejects malformed files and unknown source types', async (t) => {
    ...
    await t.rejects(
      ...
      (err) =>
        err instanceof ConfigError && err.message.includes("Unknown config source type 'mystery'"),
