...
describe('fino:security/oauth', () => {
  ...
  it('stores OAuth transaction state in a sealed cookie and clears it on callback', async (t) => {
    ...
    const cookie = loginResponse.headers
      .getSetCookie()
      .find((value) => value.startsWith('fino_oauth='));
    ...
    const callbackResponse = await callback(
      new Request('https://app.example/callback?code=code-1&state=state-1', {
    ...
    t.ok(
      callbackResponse.headers
        .getSetCookie()
        .some((value) => value.startsWith('fino_oauth=') && value.includes('Max-Age=0')),
