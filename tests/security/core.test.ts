...
import {
  issueToken as issueDirectToken,
  verifyToken as verifyDirectToken,
} from 'fino:security/token';
describe('fino:security core helpers', () => {
  ...
  it('rejects invalid random helper bounds', (t) => {
    t.throws(
      ...
      'negative random byte length is rejected',
    );
    t.throws(
      () => randomBytes(1.5),
      /length must be a non-negative integer/,
      'fractional random byte length is rejected',
    );
    t.throws(
      () => randomToken(Number.NaN),
      /length must be a non-negative integer/,
      'invalid token byte count is rejected',
    );
    t.throws(
      () => randomInt(10, 10),
      /max must be greater than min/,
      'empty integer range is rejected',
    );
    t.throws(
      () => randomInt(0, 2 ** 32 + 1),
      /range must be <= 2\^32 - 1/,
      'too-large integer range is rejected',
    );
  });
  ...
  it('handles CORS denied, wildcard credential, predicate, and invalid list cases', (t) => {
    ...
    t.equal(
      wildcardCredentials['access-control-allow-origin'],
      'https://app.example',
      'wildcard with credentials reflects origin',
    );
    t.equal(
      wildcardCredentials['access-control-allow-credentials'],
      'true',
    ...
    t.equal(
      predicate['access-control-allow-origin'],
      'https://api.example',
      'predicate origin is reflected',
    );
  ...
  it('serializes, parses, signs, and verifies cookies', (t) => {
    ...
    t.deepEqual(
      parseCookieHeader('sid=abc%20123; theme=dark'),
      {
        sid: 'abc 123',
        theme: 'dark',
      },
  ...
  it('rejects invalid cookie expiration and browser policy attributes', (t) => {
    ...
    t.throws(
      () =>
        serializeCookie('__Host-sid', 'abc', {
          secure: true,
          path: '/app',
        }),
  ...
  it('seals cookies and rejects tampering', (t) => {
    ...
    t.equal(
      unsealCookie(sealed, '0123456789abcdef0123456789abcdef'),
      'sensitive',
      'sealed cookie round trips',
    );
    t.equal(
      unsealCookie(sealed.slice(0, -1) + 'x', '0123456789abcdef0123456789abcdef'),
      null,
...
describe('fino:security password helpers', () => {
  ...
  it('supports non-default PBKDF2 parameters and rejects invalid options', (t) => {
    ...
    t.equal(
      record.startsWith('pbkdf2$sha-512$1000$'),
      true,
      'record stores non-default algorithm parameters',
    );
    ...
    t.throws(
      () => hashPassword('secret', { iterations: 0 }),
      /iterations must be a positive integer/,
      'zero iterations are rejected',
    );
    t.throws(
      () => hashPassword('secret', { saltLength: 0 }),
      /saltLength must be a positive integer/,
      'zero salt length is rejected',
    );
    t.throws(
      () => hashPassword('secret', { keyLength: 0 }),
      /keyLength must be a positive integer/,
      'zero key length is rejected',
    );
  });
  it('rejects malformed PBKDF2 password records', (t) => {
    const valid = hashPassword('secret', { iterations: 1e3 });
    const [kind, hash, iterations, salt, derived] = valid.split('$') as [
      string,
      string,
      string,
      string,
    ...
    t.equal(
      verifyPassword('secret', `argon2$${hash}$${iterations}$${salt}$${derived}`),
      false,
      'wrong record kind is rejected',
    );
    t.equal(
      verifyPassword('secret', `${kind}$sha-999$${iterations}$${salt}$${derived}`),
      false,
      'unsupported hash is rejected',
    );
    t.equal(
      verifyPassword('secret', `${kind}$${hash}$0$${salt}$${derived}`),
      false,
      'invalid iteration count is rejected',
    );
    t.equal(
      verifyPassword('secret', `${kind}$${hash}$${iterations}$not+base64$${derived}`),
      false,
      'malformed salt is rejected',
    );
    t.equal(
      verifyPassword('secret', `${kind}$${hash}$${iterations}$${salt}$not+base64`),
      false,
      'malformed derived key is rejected',
    );
  });
});
describe('fino:security token helpers', () => {
  ...
  it('rejects tampered, malformed, expired, and purpose-mismatched tokens', (t) => {
    ...
    t.equal(
      verifyDirectToken(`${tamperedBody}.${sig}`, 'token-secret', { purpose: 'session' }),
      null,
      'tampered payload fails',
    );
    t.equal(
      verifyDirectToken(token, 'wrong-secret', { purpose: 'session' }),
      null,
      'wrong secret fails',
    );
    t.equal(verifyDirectToken('not.a.token', 'token-secret'), null, 'malformed token fails');
    t.equal(
      verifyDirectToken(token, 'token-secret', { purpose: 'password-reset' }),
      null,
      'purpose mismatch fails',
    );
    t.equal(
      verifyDirectToken(token, 'token-secret', {
        purpose: 'session',
        now: exp + 1,
      }),
      null,
    ...
    t.ok(
      ...
      'clock tolerance is honored',
    );
  });
  it('rejects invalid token issue and verify options', (t) => {
    t.throws(
      () => issueDirectToken({ sub: 'user-123' }, 'token-secret', { expiresIn: Number.NaN }),
      /expiresIn must be a finite number/,
      'NaN expiresIn is rejected',
    );
    t.throws(
      () => issueDirectToken({ sub: 'user-123' }, 'token-secret', { expiresIn: Infinity }),
      /expiresIn must be a finite number/,
      'infinite expiresIn is rejected',
    );
    t.throws(
      () => issueDirectToken({ sub: 'user-123' }, 'token-secret', { purpose: 1 as any }),
      /purpose must be a string/,
      'non-string issue purpose is rejected',
    );
    const token = issueDirectToken({ sub: 'user-123' }, 'token-secret', {
      purpose: 'session',
      expiresIn: 60,
    ...
    t.equal(
      verifyDirectToken(token, 'token-secret', { now: Number.NaN }),
      null,
      'invalid verification time fails closed',
    );
    t.equal(
      verifyDirectToken(token, 'token-secret', { clockTolerance: Number.NaN }),
      null,
      'invalid clock tolerance fails closed',
    );
    t.equal(
      verifyDirectToken(token, 'token-secret', { purpose: 1 as any }),
      null,
      'non-string required purpose fails closed',
    );
    t.equal(
      verifyDirectToken('abc.def', 'token-secret'),
      null,
      'malformed signed payload fails closed',
    );
  });
});
describe('fino:security encoding edge cases', () => {
  it('handles ArrayBufferView secrets with offsets through signed cookies', (t) => {
    ...
    t.equal(
      verifyCookie(signed, equivalent),
      'view-secret',
      'view byteOffset and byteLength are honored',
    );
  });
  it('normalizes short secrets through sealed cookies', (t) => {
    const sealed = sealCookie('normalized', 'short secret');
    t.equal(
      unsealCookie(sealed, 'short secret'),
      'normalized',
      'short secret normalizes consistently',
    );
    t.equal(
      unsealCookie(sealed, 'other secret'),
      null,
      'different normalized secret fails authentication',
    );
  });
  it('rejects malformed base64url payloads and timing-safe mismatches through verifiers', (t) => {
    ...
    t.equal(
      verifyCookie(`${payload}=${sig}`, 'secret'),
      null,
      'non-canonical padded payload is rejected',
    );
    t.equal(
      verifyCookie(`${payload}.${sig!.slice(0, -1)}x`, 'secret'),
      null,
