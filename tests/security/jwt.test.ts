...
import { exportPublicJwk, generateJwk, importJwk, jwkFromSecret, jwkThumbprint, jwtDecrypt, jwtEncrypt, jwtSign, jwtVerify, selectJwk } from 'fino:security';
...
describe('fino:security JWK helpers', () => {
  it('selects JWKs permissively when optional metadata is omitted', (t) => {
    ...
    t.equal(selectJwk([generic], {
      alg: 'HS256',
      use: 'sig',
      key_ops: ['verify']
    })?.kid, 'generic', 'missing alg, use, and key_ops are permissive');
    t.equal(selectJwk([restricted, keyWithoutAlg], {
      alg: 'HS256',
      use: 'sig',
      key_ops: ['verify']
    })?.kid, 'no-alg', 'missing alg can match when other declared metadata matches');
    t.equal(selectJwk([restricted, keyWithoutUse], {
      alg: 'HS256',
      use: 'sig',
      key_ops: ['verify']
    })?.kid, 'no-use', 'missing use can match when other declared metadata matches');
    t.equal(selectJwk([restricted, keyWithoutOps], {
      alg: 'HS256',
      use: 'sig',
      key_ops: ['verify']
    })?.kid, 'no-ops', 'missing key_ops can match when other declared metadata matches');
    t.equal(selectJwk([restricted], {
      alg: 'HS256',
      use: 'sig',
      key_ops: ['verify']
    }), undefined, 'declared incompatible alg, use, and key_ops do not match');
  });
  it('generates, imports, exports, selects, and thumbprints keys', async (t) => {
    ...
    t.equal(selectJwk({ keys: [key] }, {
      kid: 'sig-1',
      alg: 'HS256'
    })?.kid, 'sig-1', 'JWKS lookup selects matching key');
  });
  it('computes RFC 7638 thumbprints from required public members only', async (t) => {
    ...
    t.equal(await jwkThumbprint(rsaPublic), 'NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs', 'RFC 7638 RSA example thumbprint matches');
    ...
    t.equal(await jwkThumbprint(withPrivateFields), await jwkThumbprint(rsaPublic), 'private fields and metadata do not affect thumbprint');
    await t.rejects(() => jwkThumbprint({
      kty: 'RSA',
      n: rsaPublic.n
    }), /missing/i);
  });
...
describe('fino:security JWT/JWE helpers', () => {
  it('signs and verifies HS256 JWTs with claim checks', async (t) => {
    ...
    await t.rejects(() => jwtVerify(token + 'tamper', { keys: [key] }), (err) => err instanceof Error && (err.message.includes('JWT signature verification failed') || err.message.includes('Invalid compact JWT') || err.message.includes('Invalid base64url string')), 'tampered JWT is rejected');
  });
  it('encrypts and decrypts compact JWE tokens', async (t) => {
    ...
    await t.rejects(() => jwtDecrypt(token.slice(0, -1) + 'x', { keys: [key] }), (err) => err instanceof Error && (err.message.includes('JWE decryption failed') || err.message.includes('Invalid compact JWE')), 'tampered JWE is rejected');
  });
  ...
  it('validates JWT audience arrays, nbf, iat, and clock tolerance', async (t) => {
    ...
    await t.rejects(() => jwtVerify(token, key, {
      audience: 'other',
      now: 1700000005
    }), /audience/);
  ...
  it('rejects unsupported JWT algorithms and signs deterministic HS256 tokens', async (t) => {
    ...
    t.equal((await jwtVerify(first, key, {
      issuer: 'issuer',
      subject: 'subject'
    })).payload.sub, 'subject');
