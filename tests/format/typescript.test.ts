...
`;
    ...
    t.equal(
      parsed.ast.body[0].type,
      'ExportNamedDeclaration',
      'AST includes module declaration nodes',
    );
    ...
    t.ok(
      parsed.tokens.some((token) => token.text === 'export' && token.kind === 'export'),
      'tokens include keyword text',
    );
    t.ok(
      parsed.tokens.some((token) => token.text === 'Promise' && token.kind === 'Identifier'),
      'tokens include identifier text',
    );
    t.ok(
      parsed.tokens.every((token) => typeof token.start === 'number' && token.end > token.start),
      'tokens expose source spans',
    );
  });
...
`;
    ...
    t.ok(
      result.code.includes('export function read(shape)'),
      'function parameter type is stripped',
    );
...
`;
    ...
    t.ok(
      result.code.includes('Route parameter documentation.'),
      'property documentation is preserved',
    );
    t.ok(
      result.code.includes('Recovery is intentionally best effort.'),
      'statement comments are preserved',
    );
  });
...
`;
    ...
    t.ok(/\n\s+\.map\(/.test(result.code), 'fluent chains may break onto indented lines');
  });
...
`;
  ...
  it('formats source using filename inference and explicit source mode', (t) => {
    ...
    t.ok(
      inferred.code.includes('<main className="app">{value}</main>'),
      'formatter preserves JSX attribute quotes',
    );
  ...
  it('lints parse errors and the OXC correctness baseline', (t) => {
    const parsed = lint('export function broken( {', { filename: 'broken.ts' });
    const debuggerResult = lint('function nested() { debugger; }\n', { filename: 'debugger.ts' });
    const reassignment = lint('const value = 1;\nvalue = 2;\n', { filename: 'reassignment.ts' });
    const duplicateKey = lint('export const value = { answer: 1, answer: 2 };\n', {
      filename: 'duplicate.ts',
    });
    const disabled = lint('// eslint-disable-next-line no-debugger\ndebugger;\n', {
      filename: 'disabled.ts',
    });
    t.equal(parsed.ok, false, 'parse errors fail lint');
    t.ok(
      parsed.diagnostics.some((diagnostic) => diagnostic.code === 'parse'),
      'parse diagnostics are included',
    );
    t.equal(debuggerResult.ok, false, 'nested debugger statements fail lint');
    t.ok(
      debuggerResult.diagnostics.some((diagnostic) => diagnostic.code === 'no-debugger'),
      'default rules include no-debugger throughout the syntax tree',
    );
    t.equal(reassignment.ok, false, 'const reassignment fails lint');
    t.ok(
      reassignment.diagnostics.some((diagnostic) => diagnostic.code === 'no-const-assign'),
      'default rules include OXC correctness checks',
    );
    t.equal(duplicateKey.ok, false, 'duplicate object keys fail lint');
    t.ok(
      duplicateKey.diagnostics.some((diagnostic) => diagnostic.code === 'no-dupe-keys'),
      'the correctness baseline includes duplicate-key checks',
    );
    t.equal(disabled.ok, true, 'OXC disable directives suppress matching rules');
  });
  it('applies safe OXC lint fixes when requested', (t) => {
    const result = lint('export function nested() {\n  debugger;\n  return 1;\n}\n', {
      filename: 'fix.ts',
      fix: true,
    });
    t.equal(result.ok, true, 'fixed source has no remaining diagnostics');
    t.ok(typeof result.fixedCode === 'string', 'fixed source is returned');
    t.equal(result.fixedCode?.includes('debugger'), false, 'the debugger statement is removed');
    t.ok(result.fixedCode?.includes('return 1;'), 'surrounding source is preserved');
  });
  it('lints TSX source and reports diagnostic locations', (t) => {
    ...
    t.ok(
      result.diagnostics.some(
        (diagnostic) =>
          diagnostic.code === 'no-debugger' && diagnostic.line === 1 && diagnostic.column === 1,
      ),
