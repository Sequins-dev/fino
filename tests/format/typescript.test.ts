...
`;
    const parsed = parse(source, { filename: 'sample.ts', tokens: true });
  ...
  it('parses source files with mixed unicode and hex escapes for docs extraction', async (t) => {
    ...
    const parsed = parse(source, { filename: 'tests/internal/globals/encoding.test.ts', tokens: true });
