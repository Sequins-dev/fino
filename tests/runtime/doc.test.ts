...
`);
  ...
  it('writes README-backed root html index and mirrors source paths', async (t) => {
    ...
    t.ok(index.includes('data-docs-client-navigation'), 'docs html enables client-side navigation for local links');
    t.ok(index.includes('<\/script>'), 'docs html closes the client-side navigation script tag');
    t.ok(index.includes('docs-loading'), 'docs html includes a loading state for client-side navigation');
    t.ok(index.includes('showLoading()'), 'docs html swaps the main content and page index while local pages load');
    t.ok(index.includes('document.addEventListener(\'click\''), 'docs html intercepts normal local link clicks');
    t.ok(index.includes('fetch(url.href'), 'docs html fetches local html pages without replacing the sidebar');
