...
describe('CLI commands', () => {
  ...
  it('keeps the root runtime alive until Atomics.waitAsync settles', async (t) => {
    const { stdout, stderr, result } = await runCli(['./tests/fixtures/atomics-waitasync-keepalive.ts']);
  ...
  it('reportError writes a diagnostic without failing the process', async (t) => {
    await withTempProject({ 'report-error.ts': [
  ...
  it('reports root script top-level throw failures', async (t) => {
    await withTempProject({ 'throws.ts': 'throw new Error(\'root top-level throw\');\n' }, async (dir) => {
  ...
  it('reports root script top-level await rejections', async (t) => {
    await withTempProject({ 'rejects.ts': 'await Promise.reject(new Error(\'root top-level await rejection\'));\n' }, async (dir) => {
  ...
  it('keeps the script failure primary when shutdown also fails', async (t) => {
    const { stdout, stderr, result } = await runCli(['./tests/fixtures/script-and-shutdown-fail.ts']);
  ...
  it('runs tasks loaded from a project tasks directory', async (t) => {
    await withTempProject({ 'tasks/build.ts': [
  ...
  it('prints help for loaded project tasks', async (t) => {
    await withTempProject({ 'tasks/build.ts': [
      ...
      t.ok(childHelp.stdout.includes('Usage: fino task build'), 'task child help shows child usage');
  ...
  it('passes JSON output mode to loaded project tasks', async (t) => {
    await withTempProject({ 'tasks/inspect.ts': [
      'import { task } from \'fino:task\';',
      'export default task({',
      '  name: \'inspect\',',
      '  outputMode: \'both\',',
  ...
  it('loads tasks from a custom directory', async (t) => {
    await withTempProject({ 'custom/deploy.ts': [
      'import { task } from \'fino:task\';',
  ...
  it('reports task directory loading errors clearly', async (t) => {
    await withTempProject({
      ...
      t.ok(missing.stderr.includes('no tasks directory found'), 'missing directory error names the issue');
      ...
      t.ok(invalid.stderr.includes('default-export a Task'), 'invalid export error explains the contract');
      ...
      t.ok(duplicate.stderr.includes('Duplicate task "same"'), 'duplicate task error names the task');
    });
  });
  ...
  it('does not expand run command directory or glob inputs', async (t) => {
    await withTempProject({ 'scripts/entry.ts': 'console.log(\'should-not-run\');\n' }, async (dir) => {
      for (const [label, args] of [['directory input', ['run', 'scripts']], ['glob input', ['run', 'scripts/*.ts']]] as [string, string[]][]) {
        const { stdout, stderr, result } = await runRootInProcess(args, { cwd: dir });
        t.equal(result.code, 1, `${label} exits nonzero`);
  ...
  it('fmt fails missing explicit directory and glob inputs without writing', async (t) => {
    await withTempProject({ 'src/app.ts': 'const value = "hello";\n' }, async (dir, fs) => {
      const before = await fs.readFile(dir + '/src/app.ts');
      for (const [label, args] of [['missing directory', ['fmt', 'missing-dir']], ['missing glob', ['fmt', 'src/missing-*.ts']]] as [string, string[]][]) {
  ...
  it('lint reports diagnostics and lint --fix does not format', async (t) => {
    await withTempProject({ 'src/app.ts': 'debugger;\nconst value = "hello";\n' }, async (dir, fs) => {
  ...
  it('lint fails missing explicit directory and glob inputs without writing', async (t) => {
    await withTempProject({ 'src/app.ts': 'const value = "hello";\n' }, async (dir, fs) => {
      const before = await fs.readFile(dir + '/src/app.ts');
      for (const [label, args] of [['missing directory', ['lint', 'missing-dir']], ['missing glob', ['lint', 'src/missing-*.ts']]] as [string, string[]][]) {
        const { stdout, stderr, result } = await runCli(args, { cwd: dir });
        const after = await fs.readFile(dir + '/src/app.ts');
  ...
  it('does not accept the legacy --test shortcut', async (t) => {
    const { stdout, stderr, result } = await runRootInProcess(['--test', './tests/util/topic.test.ts']);
  ...
  it('passes --show-output modes to the test command', async (t) => {
    await withTempProject({ 'output.test.ts': [
  ...
  it('passes --durations to the test command', async (t) => {
    await withTempProject({ 'durations.test.ts': [
      'import { test } from \'fino:test/test\';',
  ...
  it('reports after hook failures from the test command', async (t) => {
    await withTempProject({ 'after-failure.test.ts': [
      ...
      const { stdout, stderr, result } = await runCli(['test', 'after-failure.test.ts'], { cwd: dir });
  ...
  it('passes --filter to the bench command', async (t) => {
    const { stdout, stderr, result } = await runCli([
      ...
      '--filter',
      'needle',
      './tests/fixtures/filter-bench.ts'
  ...
  it('prints human benchmark output without JSON results', async (t) => {
    await withTempProject({ 'benchmarks/human.bench.ts': [
  ...
  it('fails when expanded benchmark inputs match no files', async (t) => {
    await withTempProject({}, async (dir) => {
      for (const [label, args] of [['empty benchmark directory', ['bench', 'benchmarks']], ['empty benchmark glob', ['bench', 'benchmarks/**/*.bench.ts']]] as [string, string[]][]) {
        const { stdout, stderr, result } = await runRootInProcess(args, { cwd: dir });
  ...
  it('runs async benchmarks and setup/teardown in order', async (t) => {
    await withTempProject({ 'benchmarks/async.bench.ts': [
      'import { bench } from \'fino:bench\';',
      'let logged = false;',
  ...
  it('runs benchmark teardown when the measured function throws', async (t) => {
    await withTempProject({ 'benchmarks/failing.bench.ts': [
      'import { bench } from \'fino:bench\';',
  ...
  it('applies OTEL per-signal endpoints, headers, compression, and resource env vars', async (t) => {
    const { stdout, stderr, result } = await runCli(['./tests/fixtures/cli-otel-entrypoint.ts'], { env: {
