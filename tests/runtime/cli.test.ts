...
describe('CLI commands', () => {
  ...
  it('exposes root script arguments through process argv after --', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['argv.ts', '--', '--user-flag', 'value'], { cwd: dir });
  ...
  it('exposes run command script arguments through process argv after --', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['run', 'argv.ts', '--', '--user-flag', 'value'], { cwd: dir });
  ...
  it('reruns run --watch when an imported module changes', async (t) => {
    await withTempProject({
      ...
      const proc = new Process(execPath, ['run', '--watch', 'entry.ts'], { cwd: dir, env: childEnv });
      proc.stdin.close();
  ...
  it('fmt discovery uses supported extensions, recursion, ignores, sorting, and dedupe', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['fmt', 'src/b.ts', 'src', 'src/*.ts'], { cwd: dir });
  ...
  it('passes --filter to the test command', async (t) => {
    const { stdout, stderr, result } = await runCli(['test', '--filter', 'needle', './tests/fixtures/filter-tests.ts']);
  ...
  it('passes --show-output modes to the test command', async (t) => {
    await withTempProject({
      ...
      const failures = await runCli(['test', '--show-output=failures', 'output.test.ts'], { cwd: dir });
      ...
      const always = await runCli(['test', '--show-output=always', 'output.test.ts'], { cwd: dir });
  ...
  it('passes --durations to the test command', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', '--durations', 'durations.test.ts'], { cwd: dir });
  ...
  it('expands test globs in the test command', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', '--filter', 'needle glob', 'tests/**/*.test.ts'], { cwd: dir });
  ...
  it('ignores non-test files passed to the test command', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['test', 'tests/alpha.test.ts', 'tests/helper.ts'], { cwd: dir });
  ...
  it('passes --filter to the bench command', async (t) => {
    const { stdout, stderr, result } = await runCli(['bench', '--filter', 'needle', './tests/fixtures/filter-bench.ts'], {
  ...
  it('expands benchmark globs in the bench command', async (t) => {
    await withTempProject({
      ...
      const { stdout, stderr, result } = await runCli(['bench', '--filter', 'needle glob', 'benchmarks/**/*.bench.ts'], {
        cwd: dir,
        env: { FINO_BENCH_MIN_NS: '1000' },
      });
