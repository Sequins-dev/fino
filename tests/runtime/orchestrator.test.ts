...
describe('internal orchestrator workload signal', () => {
  it('tracks workload registration and release', (t) => {
    ...
    t.ok(
      workloadsSignal()
        .get()
        .some((entry) => entry.id === workload.id),
      'signal includes registered workload',
    );
    ...
    t.equal(
      workloadsSignal()
        ...
        .some((entry) => entry.id === workload.id),
      false,
      'signal removes released workload',
    );
    t.ok(
      seen.some((count) => count > 0),
      'subscriber saw a non-empty workload list',
    );
    dispose();
  });
});
