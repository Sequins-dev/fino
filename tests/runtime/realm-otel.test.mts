...
import type echoFn from '../realm/fixtures/echo-fn.mts';
import type sumFn from '../realm/fixtures/sum-fn.mts';
import type errorFn from '../realm/fixtures/error-fn.mts';
...
describe('Realm OTel topics', () => {
  it('fino.realm.spawn.start fires when a thread realm is constructed', (t) => {
    ...
    const realm = new Realm({
      entry: new URL('../realm/fixtures/echo-fn.mts', import.meta.url).pathname,
      thread: true,
    });

    return pending.then((ev) => {
      iter.return!();
      const data = ev.value as { kind?: string; entry?: string };
  ...
  it('fino.realm.call.start fires when call() is invoked', async (t) => {
    ...
    const data = ev.value as { kind?: string; topic?: string; domain?: string; operation?: string; phase?: string };
  ...
  it('fino.realm.call.end fires after call() resolves', async (t) => {
    ...
    const data = ev.value as { kind?: string; durationMs?: number; topic?: string; phase?: string };
  ...
  it('fino.realm.call.end fires with error:true when call() rejects', async (t) => {
    ...
    const realm = new Realm<typeof errorFn>({
      entry: new URL('../realm/fixtures/error-fn.mts', import.meta.url).pathname,
      thread: true,
    });
    ...
    const data = ev.value as { error?: boolean; durationMs?: number };
...
describe('RealmPool OTel topics', () => {
  it('fino.realm_pool.call.start fires for each pool.call()', async (t) => {
    ...
    const pool = new RealmPool<typeof sumFn>({
      entry: new URL('../realm/fixtures/sum-fn.mts', import.meta.url).pathname,
      size: 1,
    });
    ...
    const data = ev.value as { correlationId?: number; poolSize?: number; topic?: string; domain?: string; operation?: string; phase?: string };
  ...
  it('fino.realm_pool.call.end fires after pool.call() resolves', async (t) => {
    ...
    const data = ev.value as { correlationId?: number; durationMs?: number };
