...
async function run(cmd: string, args: string[], sandbox: unknown): Promise<{ code: number | null; out: string }> {
  ...
  it('confines reads and writes to the granted paths', async (t) => {
    ...
    try {
      ...
      const writeOk = await run('/usr/bin/touch', [`${allowed}/w.txt`],
        { mode: 'strict', filesystem: { writable: [allowed] }, process: { allowedBinaries: ['/usr/bin/touch'] } });
      ...
      const writeDenied = await run('/usr/bin/touch', [`${allowed}/nope.txt`],
        { mode: 'strict', filesystem: { readonly: [allowed] }, process: { allowedBinaries: ['/usr/bin/touch'] } });
...
describe('macOS Seatbelt process-exec scoping', () => {
  it('permits allowlisted execs and denies the rest', async (t) => {
    ...
    const denied = await run('/bin/sh', ['-c', 'exec /bin/ls /'],
      { mode: 'strict', process: { allowExec: false, allowedBinaries: ['/bin/sh', '/bin/bash'] } });
    t.notEqual(denied.code, 0, 'exec of a non-allowlisted binary is denied');
    const allowed = await run('/bin/sh', ['-c', 'exec /bin/ls / >/dev/null'],
      { mode: 'strict', process: { allowedBinaries: ['/bin/sh', '/bin/bash', '/bin/ls'] } });
  ...
  it('reports filesystem and process enforcement via Seatbelt', async (t) => {
    ...
    const proc = new Process('/usr/bin/true', [], {
      sandbox: { mode: 'strict', filesystem: { readonly: ['/usr'] }, process: { allowedBinaries: ['/usr/bin/true'] } }
    });
...
describe('macOS strict rejects Linux-only categories pre-spawn', () => {
  it('rejects syscalls and fork-denial policy', (t) => {
    if (os !== 'darwin') return;
    t.throws(() => new Process('/bin/echo', ['x'], { sandbox: { mode: 'strict', syscalls: { mode: 'denylist', names: ['ptrace'] } } }),
      /seccomp/, 'syscall policy is rejected on macOS');
    t.throws(() => new Process('/bin/echo', ['x'], { sandbox: { mode: 'strict', process: { allowFork: false } } }),
