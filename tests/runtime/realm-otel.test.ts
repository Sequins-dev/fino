/**
* Tests for OTel topic emission from fino:realm.
*
...
describe('Realm OTel topics', () => {
  it('fino.realm.spawn.start fires when a realm is allocated', (t) => {
    ...
    const realm = new Realm({ entry: new URL('../realm/fixtures/echo-fn.ts', import.meta.url).pathname });
    return pending.then((ev) => {
      ...
      t.equal(data.kind, undefined, 'logical realm telemetry does not expose a placement kind');
  ...
  it('fino.realm.call.start fires when call() is invoked', async (t) => {
    ...
    const realm = new Realm<typeof echoFn>({ entry: new URL('../realm/fixtures/echo-fn.ts', import.meta.url).pathname });
  ...
  it('fino.realm.call.end fires with error:true when call() rejects', async (t) => {
    const endTopic = topic(T_REALM_END);
    const iter = endTopic[Symbol.asyncIterator]();
    const realm = new Realm<typeof errorFn>({ entry: new URL('../realm/fixtures/error-fn.ts', import.meta.url).pathname });
    const pendingEnd = iter.next();
    try {
