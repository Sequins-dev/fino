...
import { MigrationError, MigrationParseError, defineMigration, loadMigrations, migrate, rollback } from 'fino:database/migrate';
...
`));
  ...
  it('reports TypeScript signature errors as MigrationParseError', (t) => {
    t.throws(() => parseSqlModule('-- function bad(input: )\nSELECT 1', { source: 'bad.sql' }), (err) => err instanceof MigrationParseError && /bad\.sql:1/.test(String(err)));
  });
...
describe('fino:database/migrate — runner', () => {
  it('applies up migrations and rolls back with down migrations', async (t) => {
    ...
    const applied = await migrate(db, [defineMigration({
      id: '001_create_users',
    ...
    await migrate(db, [defineMigration({
      id: '001_create_users',
      up: 'CREATE TABLE users (id TEXT PRIMARY KEY, name TEXT NOT NULL)',
      down: 'DROP TABLE users'
    })]);
    ...
    const rolledBack = await rollback(db, [defineMigration({
      id: '001_create_users',
      up: 'CREATE TABLE users (id TEXT PRIMARY KEY, name TEXT NOT NULL)',
      down: 'DROP TABLE users'
    })]);
  ...
  it('rejects checksum drift before applying later migrations', async (t) => {
    ...
    try {
      await migrate(db, [defineMigration({
        id: '001_create_users',
        up: 'CREATE TABLE users (id TEXT, name TEXT)'
      }), defineMigration({
        id: '002_never_runs',
        up: 'CREATE TABLE never_runs (id TEXT)'
      })]);
    } catch (err) {
      checksumError = err;
    }
...
`);
  ...
  it('rolls back transaction state when a migration fails', async (t) => {
    await using db = await Database.open(':memory:');
    await t.rejects(() => migrate(db, [defineMigration({
      id: '001_broken',
  ...
  it('requires down SQL for rollback', async (t) => {
    await using db = await Database.open(':memory:');
    const migrations = [defineMigration({
      id: '001_create_users',
      up: 'CREATE TABLE users (id TEXT)'
    })];
    await migrate(db, migrations);
