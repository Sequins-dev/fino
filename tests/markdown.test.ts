...
\`\`\``,
    ...
    t.ok(
      html.includes(
        'Use <strong>bold</strong>, <em>emphasis</em>, <code>code</code>, and <a href="https://example.test">docs</a>.',
      ),
      'renders inline markdown',
    );
    t.ok(html.includes('<ul>\n<li>one</li>\n<li>two</li>\n</ul>'), 'renders unordered lists');
    t.ok(
      html.includes('<pre><code class="language-ts">const value = &quot;&lt;safe&gt;&quot;'),
      'renders escaped fenced code',
    );
  });
  ...
  it('renders image spans', (t) => {
    t.equal(
      renderMarkdownInline('![fino logo](./logo.svg)'),
      '<img src="./logo.svg" alt="fino logo">',
    );
  });
...
[box]: ./advanced.ts#ResourceBox`);
    const html = renderMarkdown(doc, {
      resolveLink(href) {
        return href === './advanced.ts#ResourceBox' ? 'advanced.html#advanced.ResourceBox' : href;
      },
    });
    t.ok(
      html.includes('<a href="advanced.html#advanced.ResourceBox">ResourceBox</a>'),
      'renders reference link',
    );
    t.ok(
      html.includes('<a href="https://example.test/docs">https://example.test/docs</a>.'),
      'linkifies bare URL without trailing punctuation',
    );
  });
  it('omits unsafe links and image URLs by default', (t) => {
    t.equal(
      ...
      'run',
      'unsafe link renders as label text',
    );
    t.equal(
      renderMarkdownInline('![logo](javascript:alert(1))'),
      'logo',
      'unsafe image renders as alt text',
    );
  });
  ...
  it('uses the last duplicate reference definition', (t) => {
    ...
    t.equal(
      html,
      '<p><a href="/second">Docs</a></p>',
      'normalized duplicate references are last-write-wins',
    );
  });
  ...
  it('sanitizes default URL handling without enabling unsafe protocols', (t) => {
    t.equal(
      renderMarkdownInline('[http](http://example.test)'),
      '<a href="http://example.test">http</a>',
      'http links are allowed',
    );
    t.equal(
      renderMarkdownInline('[https](https://example.test)'),
      '<a href="https://example.test">https</a>',
      'https links are allowed',
    );
    t.equal(
      renderMarkdownInline('[relative](../guide)'),
      '<a href="../guide">relative</a>',
      'relative links are allowed',
    );
    t.equal(
      renderMarkdownInline('[mail](mailto:team@example.test)'),
      'mail',
      'other protocols are omitted by default',
    );
  });
  ...
  it('follows CommonMark tight and loose list behavior', (t) => {
    t.equal(
      renderMarkdown('- tight\n- list'),
      '<ul>\n<li>tight</li>\n<li>list</li>\n</ul>\n',
      'tight lists omit paragraph wrappers',
    );
    t.equal(
      renderMarkdown('- loose\n\n- list'),
      '<ul>\n<li>\n<p>loose</p>\n</li>\n<li>\n<p>list</p>\n</li>\n</ul>\n',
  ...
  it('renders focused GFM inline and table extensions', (t) => {
    ...
    t.equal(
      renderMarkdownInline('https://example.test/docs'),
      '<a href="https://example.test/docs">https://example.test/docs</a>',
      'bare autolinks render as anchors',
    );
    t.equal(
  ...
  it('filters disallowed raw HTML tags even when raw HTML is enabled', (t) => {
    t.equal(
      renderMarkdown('<xmp>unsafe</xmp>', { allowRawHtml: true }),
      '&lt;xmp>unsafe&lt;/xmp>\n',
    );
  });
