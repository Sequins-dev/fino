...
[box]: ./advanced.ts#ResourceBox`);
    const html = renderMarkdown(doc, { resolveLink(href) {
  ...
  it('allows unsafe links only when explicitly requested', (t) => {
    t.equal(renderMarkdownInline('[run](javascript:alert(1))', { allowUnsafeLinks: true }), '<a href="javascript:alert(1)">run</a>', 'allowUnsafeLinks permits protocol URLs');
    t.equal(renderMarkdownInline('![logo](javascript:alert(1))', { allowUnsafeLinks: true }), '<img src="javascript:alert(1)" alt="logo">', 'allowUnsafeLinks permits image URLs');
  });
  it('checks resolved links against the safe-link policy', (t) => {
    const html = renderMarkdownInline('[docs](/docs)', { resolveLink() {
  ...
  it('renders escaped punctuation literally', (t) => {
    t.equal(renderMarkdownInline('Use \\*stars\\*, \\[brackets\\], and \\`ticks\\`.'), 'Use *stars*, [brackets], and `ticks`.', 'backslash escapes punctuation that would otherwise start inline spans');
  });
  it('renders malformed links as escaped plain text', (t) => {
    t.equal(renderMarkdownInline('[docs](https://example.test'), '[docs](https://example.test', 'missing closing paren does not emit an anchor');
    t.equal(renderMarkdownInline('[docs][missing'), '[docs][missing', 'missing reference close bracket stays plain text');
  });
