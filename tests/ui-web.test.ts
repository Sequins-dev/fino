...
function makeApp() {
  ...
  const todos = view({
    ...
    embed: ['draft'],
    actions: { add: { async handler({ state, checkpoint }, input) {
      state.items.set((items) => items.concat(String(input.text ?? '')));
      await checkpoint();
      state.draft.set('');
    } } },
    render({ state, actions }) {
      return h('section', { id: 'todos' }, h('ul', { id: 'items' }, state.items.get().map((item) => h('li', null, item))), h('form', { action: actions.add }, h('input', {
        name: 'text',
...
function makeSecureApp() {
  ...
  const secure = view({
    ...
    actions: { add: { async handler({ state }) {
      handlerRuns++;
      state.items.set((items) => items.concat(String(state.token.get())));
    } } },
    render({ state, actions }) {
      return h('section', { id: 'secure' }, h('ul', null, state.items.get().map((item) => h('li', null, item))), h('form', { action: actions.add }, h('button', null, 'Add')));
    }
  });
  const app = new App();
  const ui = app.value('cookies', cookies()).value('session', sessions({
