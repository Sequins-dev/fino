...
async function collectEvents(response: Response) {
  const events: Array<{ type: string; data: unknown; id: string | null }> = [];
  for await (const event of parseEventStream(response.body!)) {
    events.push({ type: event.type, data: JSON.parse(event.data), id: event.id });
  }
  return events;
}
...
function makeApp() {
  ...
  const todos = view({
    ...
    render({ state, actions }) {
      return h('section', { id: 'todos' },
        h('ul', { id: 'items' }, state.items.get().map((item) => h('li', null, item))),
        h('form', { action: actions.add },
          h('input', { name: 'text', value: state.draft.get() }),
          h('button', null, 'Add')
        )
      );
    }
...
function makeSecureApp() {
  ...
  const secure = view({
    ...
    render({ state, actions }) {
      return h('section', { id: 'secure' },
        h('ul', null, state.items.get().map((item) => h('li', null, item))),
...
describe('fino:ui/web', () => {
  it('renders a page and applies an enhanced action as SSE patches', async (t) => {
    ...
    const action = await app.handle(new Request('http://local/?_action=todos.add', {
      method: 'POST',
      headers: {
  ...
  it('uses PRG for no-JS actions and rejects invalid CSRF tokens', async (t) => {
    ...
    const redirect = await app.handle(new Request('http://local/?_action=todos.add', {
      method: 'POST',
      headers: {
        cookie,
        'content-type': 'application/x-www-form-urlencoded',
    ...
    const rejected = await app.handle(new Request('http://local/?_action=todos.add', {
      method: 'POST',
      headers: {
        cookie,
        accept: 'text/event-stream',
      ...
      body: new URLSearchParams({ ...base, _csrf: 'bad' }).toString()
    })) as Response;
  ...
  it('rejects tampered sealed embedded state and ignores replayed nonces', async (t) => {
    ...
    const tampered = await app.handle(new Request('http://local/secure?_action=secure-view.add', {
      method: 'POST',
      headers: {
        cookie,
        accept: 'text/event-stream',
      ...
      body: new URLSearchParams({ ...base, '$token': 'bad' }).toString()
    })) as Response;
    t.equal(tampered.status, 400, 'tampered sealed embed rejects');
    t.equal(runs(), 0, 'handler did not run for tampered embed');

    const ok = await app.handle(new Request('http://local/secure?_action=secure-view.add', {
      method: 'POST',
      headers: {
        cookie,
        accept: 'text/event-stream',
    ...
    const replay = await app.handle(new Request('http://local/secure?_action=secure-view.add', {
      method: 'POST',
      headers: {
        cookie,
        accept: 'text/event-stream',
