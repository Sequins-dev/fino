...
describe('EvalReporter', () => {
  it('EvalProgressReporter exposes retained progress', async (t) => {
    ...
    const seen: string[] = [];
    reporter.progress.subscribe((progress) =>
      seen.push(`${progress.completed}/${progress.total}:${progress.passed}`),
    );
