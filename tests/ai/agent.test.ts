...
describe('Agent', () => {
  it('delegates append and read history decisions to the strategy', async (t) => {
    ...
    const model: Model = {
      ...
      stream(req: GenerateRequest): ModelStream {
        ...
        async function* gen() {
          yield { type: 'text_delta' as const, index: 0, text: 'viewed' };
    ...
    const strategy: HistoryStrategy = {
      ...
      async onRead() {
        return {
          ...
          messages: [{ role: 'user', content: 'temporary view only' }],
        };
      },
    };

    const a = agent({ model, history: strategy });
  ...
  it('generate(string) runs the loop and returns text', async (t) => {
    const a = agent({
      model: scriptModel([
        toolCallTurn('c1', 'greet', '{"name":"Alice"}'),
        endTurn('Hi Alice!'),
      ]),
  ...
  it('structured output: valid respond call sets result.object', async (t) => {
    ...
    const respondTurn: StreamEvent[] = [
      { type: 'tool_call_start', index: 0, id: 'r1', name: 'respond' },
      { type: 'tool_call_delta', index: 0, json: '{"value":42}' },
      { type: 'tool_call_end', index: 0 },
      { type: 'usage', usage: { inputTokens: 6, outputTokens: 3 } },
      { type: 'stop', reason: 'tool_use' },
    ];
  ...
  it('structured output: invalid-then-valid uses one repair step', async (t) => {
    ...
    const badRespondTurn: StreamEvent[] = [
      { type: 'tool_call_start', index: 0, id: 'r1', name: 'respond' },
      { type: 'tool_call_delta', index: 0, json: '{"value":"not-a-number"}' },
      { type: 'tool_call_end', index: 0 },
      { type: 'usage', usage: { inputTokens: 6, outputTokens: 3 } },
      { type: 'stop', reason: 'tool_use' },
    ];
    ...
    const goodRespondTurn: StreamEvent[] = [
      { type: 'tool_call_start', index: 0, id: 'r2', name: 'respond' },
      { type: 'tool_call_delta', index: 0, json: '{"value":7}' },
      { type: 'tool_call_end', index: 0 },
  ...
  it('structured output: permanently invalid throws', async (t) => {
    ...
    const badTurn: StreamEvent[] = [
      { type: 'tool_call_start', index: 0, id: 'r1', name: 'respond' },
      { type: 'tool_call_delta', index: 0, json: '{"value":"bad"}' },
      { type: 'tool_call_end', index: 0 },
      { type: 'usage', usage: { inputTokens: 6, outputTokens: 3 } },
      { type: 'stop', reason: 'tool_use' },
    ];
    ...
    const a = agent({
      model: scriptModel([badTurn, badTurn, badTurn]),
      output: schema.schema,
    });

    await t.rejects(
      () => a.generate('give me a number'),
  ...
  it('asTool() wraps agent as a tool for composition', async (t) => {
    ...
    const outer = agent({
      model: scriptModel([
        toolCallTurn('t1', 'inner', '{"task":"do something"}'),
        endTurn('outer done'),
      ]),
      tools: [innerTool],
    });
