...
import type { EmbeddingModel, GenerateRequest, GenerateResult, Model, ModelStream, StreamEvent } from 'fino:ai/model';
...
describe('fino:ai/cache', () => {
  it('exact cache prevents repeated generate calls and reports avoided tokens', async (t) => {
    ...
    const second = await model.generate({ messages: [{ content: 'hello', role: 'user' as const }] });
  ...
  it('stream cache hits replay cached events', async (t) => {
    ...
    t.equal(first.text, second.text);
    t.ok(events.some((event) => event.type === 'usage' && event.usage.localCacheReadOutputTokens === 4), 'replayed usage reports saved output tokens');
  });
  ...
  it('semantic cache serves similar requests and bypasses tool requests', async (t) => {
    ...
    const first = await model.generate({ messages: [{ role: 'user', content: 'What is the refund policy?' }] });
    const second = await model.generate({ messages: [{ role: 'user', content: 'Can I return this?' }] });
