...
describe('MessageHistory', () => {
  it('append is immutable and render returns the active sequence', async (t) => {
    ...
    t.deepEqual(h2.render().map((m) => m.content), ['a']);
    t.deepEqual(h3.render().map((m) => m.content), ['a', 'b']);
  });
  ...
  it('edit removes, replaces, inserts, moves, and restores summaries', async (t) => {
    ...
    const [a, b, c] = h.refs();
    h = await h.edit([
      { op: 'insertAfter', id: a!.id, entries: [userMsg('after-a')] },
      { op: 'replace', id: b!.id, entries: [assistantMsg('b1'), assistantMsg('b2')] },
      { op: 'move', ids: [c!.id], before: a!.id },
      { op: 'remove', id: a!.id },
    ]);

    t.deepEqual(h.render().map((m) => m.content), ['c', 'after-a', 'b1', 'b2']);

    const sourceIds = h.refs().slice(0, 2).map((entry) => entry.id);
    h = await h.edit({
      op: 'summary',
      sourceIds,
    ...
    t.deepEqual(h.render().map((m) => m.content), ['[summary]', 'b1', 'b2']);
    t.deepEqual(h.restore().map((m) => m.content), ['c', 'after-a', 'b1', 'b2']);
  });
  ...
  it('fork and withView create independent immutable revisions', async (t) => {
    ...
    t.deepEqual(h.render().map((m) => m.content), ['base']);
    t.deepEqual(changed.render().map((m) => m.content), ['base', 'branch']);
...
describe('costOf', () => {
  ...
  it('includes cache read cost', async (t) => {
    ...
    const cost = costOf(usage, 'claude-opus-4-8');
    t.equal(cost, 0.5, '$0.5 per million cache read');
  });
...
describe('maxTokens', () => {
  it('returns false below the limit', async (t) => {
    ...
    const state = { usage: { inputTokens: 40, outputTokens: 40 }, stepIndex: 0, messages: [] };
    t.ok(!cond(state as never, { stopReason: 'end_turn' }));
  });

  it('returns true at or above the limit', async (t) => {
    const cond = maxTokens(100);
    const state = { usage: { inputTokens: 60, outputTokens: 40 }, stepIndex: 0, messages: [] };
    t.ok(cond(state as never, { stopReason: 'end_turn' }));
  });
});
...
describe('maxCost', () => {
  it('returns false below the budget', async (t) => {
    const cond = maxCost(1.0);
    const state = { usage: { inputTokens: 0, outputTokens: 0 }, cost: 0.5, stepIndex: 0, messages: [] };
    t.ok(!cond(state as never, { stopReason: 'end_turn' }));
  });

  it('returns true at or above the budget', async (t) => {
    const cond = maxCost(1.0);
    const state = { usage: { inputTokens: 0, outputTokens: 0 }, cost: 1.0, stepIndex: 0, messages: [] };
    t.ok(cond(state as never, { stopReason: 'end_turn' }));
  });

  it('treats missing cost as 0', async (t) => {
    const cond = maxCost(0.5);
    const state = { usage: { inputTokens: 0, outputTokens: 0 }, stepIndex: 0, messages: [] };
    t.ok(!cond(state as never, { stopReason: 'end_turn' }));
  });
});
...
describe('HistoryStrategy', () => {
  it('summarizingHistoryStrategy compacts lazily on read', async (t) => {
    ...
    await strategy.onAppend(userMsg('alpha alpha alpha'), { model: summaryModel, budgetTokens: 10 });
    await strategy.onAppend(assistantMsg('beta beta beta'), { model: summaryModel, budgetTokens: 10 });
    await strategy.onAppend(userMsg('recent'), { model: summaryModel, budgetTokens: 10 });

    const before = strategy.history.revisionId;
    const view = await strategy.onRead({ model: summaryModel, budgetTokens: 10 });

    t.ok(view.history.revisionId !== before, 'read hook compacted by assigning a new immutable revision');
    t.deepEqual(view.messages.map((m) => m.content), ['[Conversation summary]\nlazy summary', 'recent']);
  });

  it('read hooks can select a temporary view without persisting it', async (t) => {
    const base = appendOnlyHistoryStrategy();
    await base.onAppend(userMsg('hidden'), { model: scriptGenerateModel([]), budgetTokens: 100 });
    await base.onAppend(assistantMsg('visible'), { model: scriptGenerateModel([]), budgetTokens: 100 });

    const originalRevision = base.history.revisionId;
    const selected = base.history.render([base.history.refs()[1]!.id]);

    t.deepEqual(selected.map((m) => m.content), ['visible']);
    t.equal(base.history.revisionId, originalRevision, 'temporary render does not persist a new revision');
    t.deepEqual(base.history.render().map((m) => m.content), ['hidden', 'visible']);
  });
  ...
  it('selectiveSummaryHistoryStrategy summarizes an arbitrary selected subset', async (t) => {
    ...
    const strategy = selectiveSummaryHistoryStrategy({
      ...
      selector: (history) => history.refs().filter((_, i) => i !== 1).map((entry) => entry.id),
    });

    await strategy.onAppend(userMsg('first'), { model: summaryModel, budgetTokens: 100 });
    await strategy.onAppend(assistantMsg('middle'), { model: summaryModel, budgetTokens: 100 });
    await strategy.onAppend(userMsg('last'), { model: summaryModel, budgetTokens: 100 });

    const view = await strategy.onRead({ model: summaryModel, budgetTokens: 100 });
    t.deepEqual(view.messages.map((m) => m.content), ['[Selected history summary]\nselected subset summary', 'middle']);
    t.deepEqual(strategy.history.restore().map((m) => m.content), ['first', 'last', 'middle']);
  });
  ...
  it('strategies can extract partial content and replace active entries', async (t) => {
    ...
    await strategy.onAppend(userMsg('public | secret'), { model, budgetTokens: 100 });
    ...
    strategy.history = await strategy.history.edit({
      ...
      id: original.id,
      entries: [
        { message: userMsg('public'), meta: { labels: ['retained'] } },
        { message: userMsg('secret'), meta: { labels: ['extracted'] } },
      ],
    });
    const extracted = strategy.history.refs().find((entry) => entry.meta?.labels?.includes('extracted'))!;
    strategy.history = await strategy.history.edit({ op: 'remove', id: extracted.id });

    t.deepEqual(strategy.history.render().map((m) => m.content), ['public']);
    t.deepEqual(strategy.history.refs().map((entry) => entry.meta?.labels?.[0]), ['retained']);
  });

  it('memory emission is owned entirely by the strategy', async (t) => {
    const ingested: { text: string; metadata?: Record<string, unknown> }[] = [];
    ...
    await strategy.onAppend(userMsg('memory source'), { model: summaryModel, budgetTokens: 10 });
    await strategy.onAppend(assistantMsg('memory response'), { model: summaryModel, budgetTokens: 10 });
    await strategy.onRead({ model: summaryModel, budgetTokens: 10 });

    t.deepEqual(ingested.map((doc) => doc.text), ['memory summary']);
...
describe('Agent integration', () => {
  it('AgentResult.cost is populated when pricing is available', async (t) => {
    const h = agent({
      model: { ...scriptModel([endTurnEvents('hi')]), name: 'claude-opus-4-8' },
  ...
  it('AgentResult.cost is 0 for unknown model', async (t) => {
    const h = agent({
      model: scriptModel([endTurnEvents('hi')]),  // name is 'mock', not in PRICING
      stopWhen: (state) => state.stepIndex >= 1,
    });
  ...
  it('maxTokens stop condition halts the loop', async (t) => {
    ...
    const h = agent({
      ...
      stopWhen: mkMaxTokens(12),  // 10+5=15 tokens after first step exceeds 12
    });
