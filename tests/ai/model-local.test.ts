...
describe('local llama.cpp model adapter', () => {
  ...
  it(
    'generates text with real libllama and a tiny GGUF model',
    { skip: hasLlamaCpp ? false : 'libllama is not available' },
    async (t) => {
      ...
      try {
        const result = await model.generate({
          messages: [
            {
              role: 'user',
              content: 'Complete this sentence: Once upon a',
            },
          ],
        });
        ...
        t.ok(
          ...
          `expected non-empty generated text, got ${JSON.stringify(result.text)}`,
        );
      } finally {
        (
          model as {
  ...
  it('loads a local GGUF path through injected bindings', async (t) => {
    ...
    t.equal(
      model.capabilities?.structuredOutput?.native,
      false,
      'local model declares no native structured output',
    );
  ...
  it('rejects unsupported local model options clearly', async (t) => {
    ...
    await t.rejects(() => model.embed(['x']), LocalModelUnsupportedError);
  });
  it('throws LocalModelLibraryError when no llama.cpp binding is available', async (t) => {
    try {
      await local({
        model: '/tmp/model.gguf',
        libraryPath: '/definitely/not/libllama.dylib',
      });
