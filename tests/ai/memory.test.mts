...
function tmpPath(): string {
  return `/tmp/fino-memory-test-${Math.floor(Math.random() * 1_000_000_000)}.db`;
}
...
describe('SqliteMemory', () => {
  it('appends messages and retrieves them in chronological order', async (t) => {
    ...
    const mem = await SqliteMemory.open({ path, embedder: deterministicEmbedder(4) });
    try {
      const m1 = await mem.append({ role: 'user', content: 'hello' });
      const m2 = await mem.append({ role: 'assistant', content: 'hi there' });
      const m3 = await mem.append({ role: 'user', content: 'bye' });
  ...
  it('round-trips structured content as JSON', async (t) => {
    ...
    const mem = await SqliteMemory.open({ path, embedder: deterministicEmbedder(4) });
    try {
      const parts = [{ type: 'text', text: 'structured message' }];
      const appended = await mem.append({ role: 'assistant', content: parts as never });
  ...
  it('working memory: set, get, merge, replace', async (t) => {
    ...
    try {
      ...
      await mem.setWorkingMemory({ name: 'Alice', score: 10 });
      t.deepEqual(await mem.getWorkingMemory(), { name: 'Alice', score: 10 }, 'initial set');

      await mem.setWorkingMemory({ score: 20, rank: 1 }, 'merge');
      t.deepEqual(
  ...
  it('recall without query text returns history and workingMemory, no recalled items', async (t) => {
    ...
    const mem = await SqliteMemory.open({ path, embedder: deterministicEmbedder(4) });
    try {
      await mem.append({ role: 'user', content: 'msg1' });
      await mem.append({ role: 'assistant', content: 'msg2' });
  ...
  it('thread(id) re-scopes to a separate conversation on the same store', async (t) => {
    ...
    const fs = new DiskFileSystem();
    try { await fs.unlink(path); } catch {}
    const memA = await SqliteMemory.open({ path, embedder: deterministicEmbedder(4) });
    try {
      await memA.append({ role: 'user', content: 'in thread A' });

      const memB = memA.thread('thread-B');
      t.ok(memB.threadId !== memA.threadId, 'different threadId');
      await memB.append({ role: 'user', content: 'in thread B' });
  ...
  it('semantic recall: ingest and KNN when available; degrades gracefully otherwise', async (t) => {
    ...
    try {
      if (mem.semanticAvailable) {
        await mem.ingest([
          { text: 'the quick brown fox', metadata: { tag: 'fox' } },
          { text: 'lazy dog sleeping', metadata: { tag: 'dog' } },
          { text: 'a fast orange fox running', metadata: { tag: 'fox2' } },
        ]);

        const ctx = await mem.recall({ text: 'quick fox', topK: 2 });
