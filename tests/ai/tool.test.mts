...
describe('tool factory', () => {
  ...
  it('invoke() executes with validated args on success', async (t) => {
    ...
    const t1 = tool<{ x: number }>({
      name: 'fn',
      description: 'd',
      parameters: schema,
      execute: (args) => { receivedArgs = args; return 'done'; },
    });
  ...
  it('invoke() returns object result pass-through', async (t) => {
    const t1 = tool({
      ...
      description: 'd',
      parameters: schema,
      execute: async () => ({ content: 'object result', isError: false }),
    });
  ...
  it('invoke() returns isError result on object with isError', async (t) => {
    const t1 = tool({
      ...
      description: 'd',
      parameters: schema,
      execute: async () => ({ content: 'something broke', isError: true }),
    });
    const result = await t1.invoke({ x: 1 }, makeCtx());
  ...
  it('invoke() passes context to execute', async (t) => {
    ...
    const t1 = tool({
      ...
      description: 'd',
      parameters: schema,
      execute: (_args, ctx) => { receivedCtx = ctx; return 'ok'; },
    });
    const ctx = makeCtx({ toolCallId: 'call_test', step: 7 });
  ...
  it('invoke() returns isError summary when validation fails', async (t) => {
    const t1 = tool({ name: 'fn', description: 'd', parameters: schema, execute: async () => 'ok' });
  ...
  it('invoke() rethrows when throwOnError is true', async (t) => {
    const t1 = tool({
      ...
      description: 'd',
      parameters: schema,
      execute: () => { throw new Error('hard failure'); },
      throwOnError: true,
    });
  ...
  it('validator is memoized (compiled once)', async (t) => {
    ...
    const t1 = tool({ name: 'fn', description: 'd', parameters: schema, execute: async () => { invokeCount++; return 'ok'; } });
...
describe('toToolDefinition', () => {
  it('returns name, description, and parameters', (t) => {
    const t1 = tool({ name: 'search', description: 'Search the web', parameters: schema, execute: async () => 'result' });
    const def = toToolDefinition(t1);
  ...
  it('passes through describe() text in parameter schema', (t) => {
    ...
    const t1 = tool({ name: 'search', description: 'Search', parameters: schemaWithDescription, execute: async () => 'ok' });
    const def = toToolDefinition(t1);
