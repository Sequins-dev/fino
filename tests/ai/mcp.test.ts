...
function makeMockMcpService(): JsonRpcService {
  return new JsonRpcService()
    ...
    .handle(() => ({ resources: MOCK_RESOURCES }))
    .method('resources/templates/list')
    .handle(() => ({ resourceTemplates: MOCK_RESOURCE_TEMPLATES }))
...
describe('fino:ai/mcp — MCPClient', () => {
  ...
  it('client rejects malformed roots, sampling, and elicitation params', async (t) => {
    ...
    await client.connect();
    await t.rejects(
      () => server.call('roots/list', { unexpected: true }),
      /Invalid roots\/list params/i,
    );
    await t.rejects(
      () => server.call('sampling/createMessage', { maxTokens: 1 }),
      /Invalid sampling params/i,
    );
    await t.rejects(
      () => server.call('elicitation/create', { requestedSchema: {} }),
      /Invalid elicitation params/i,
    );
    await client.close();
    await server.close();
  });
  ...
  it('client list signals seed and refresh from list change notifications', async (t) => {
    ...
    const client = new MCPClient({ transport: clientTransport });
    await client.connect();
    const disposeTools = client.tools.subscribe((items) =>
      seen.push(`tools:${items.map((item) => item.name).join(',')}`),
    );
    const disposeResources = client.resources.subscribe((items) =>
      seen.push(`resources:${items.map((item) => item.uri).join(',')}`),
    );
    const disposePrompts = client.prompts.subscribe((items) =>
      seen.push(`prompts:${items.map((item) => item.name).join(',')}`),
    );
    await new Promise((resolve) => setTimeout(resolve, 0));
    t.equal(client.tools.get()[0]?.name, 'first', 'tools signal seeds from remote list');
    t.equal(
      client.resources.get()[0]?.uri,
      'memo://one',
      'resources signal seeds from remote list',
    );
    ...
    t.equal(
      client.resources.get()[0]?.uri,
      'memo://two',
      'resources signal refreshes on notification',
    );
...
describe('fino:ai/mcp — MCPServer', () => {
  ...
  it('enforces Streamable HTTP session, GET, DELETE, and origin behavior', async (t) => {
    ...
    const beforeInit = await handler(
      new Request('http://127.0.0.1/mcp', {
        method: 'POST',
        headers: {
          accept: 'application/json, text/event-stream',
  ...
  it('HTTP GET SSE receives notifications and replays after Last-Event-ID', async (t) => {
    ...
    const sessionId = init.headers.get('mcp-session-id')!;
    const firstStream = await handler(
      new Request('http://127.0.0.1/mcp', {
        method: 'GET',
        headers: {
          accept: 'text/event-stream',
    ...
    t.equal(
      deliveredToOriginal.value.id,
      replayed.value.id,
      'second event was also delivered on the original open stream',
    );
    const deleted = await handler(
      new Request('http://127.0.0.1/mcp', {
        method: 'DELETE',
  ...
  it('paginated list handlers return nextCursor from cursor-aware sources', async (t) => {
    ...
    t.equal(
      (
        (await call(3, 'tools/list', { cursor: '2' })).result.tools as Array<{
          name: string;
        }>
      )[0]?.name,
      'b',
