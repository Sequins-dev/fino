...
function fakeSkillsFetch(log: string[] = []): typeof fetch {
  return (async (input: RequestInfo | URL) => {
    ...
    if (url === 'https://api.github.com/repos/acme/readme-skill/contents/skill.md' || url === 'https://api.github.com/repos/acme/readme-skill/contents/SKILL.md') {
      return new Response('not found', { status: 404 });
    }
    ...
    if (url === 'https://api.github.com/repos/acme/missing-skill/contents/skill.md' || url === 'https://api.github.com/repos/acme/missing-skill/contents/SKILL.md') {
      return new Response('not found', { status: 404 });
    }
...
describe('SkillRegistry', () => {
  it('manifest() returns name+description only, not full instructions', async (t) => {
    const registry = skillRegistry([skill({
      name: 'coding',
  ...
  it('load_skill tool call injects instructions into messages', async (t) => {
    ...
    const result = await a.generate({ messages: [{
      role: 'user',
    ...
    const toolResults = messages.flatMap((m) => m.role === 'user' && Array.isArray(m.content) ? (m.content as Array<{
      type: string;
      toolCallId?: string;
      content?: unknown;
    }>).filter((p) => p.type === 'tool_result' && p.toolCallId === 'ls1') : []);
    t.ok(toolResults.length > 0, 'load_skill tool result exists in messages');
    const content = (toolResults[0] as {
      content: unknown;
    }).content;
  ...
  it('resume safety: fresh agent over same messages re-registers skill tools', async (t) => {
    ...
    const turns1: StreamEvent[][] = [toolCallTurn('ls1', 'load_skill', '{"name":"math"}'), endTurn('loaded math skill')];
    const agent1 = agent({
      model: scriptModel(turns1),
      skills: registry1
    });
    const result1 = await agent1.generate({ messages: [{
      role: 'user',
      content: 'load math'
    }] });
    ...
    const turns2: StreamEvent[][] = [toolCallTurn('add1', 'add', '{"a":3,"b":4}'), endTurn('result is 7')];
    const agent2 = agent({
      model: scriptModel(turns2),
      skills: registry2
    });
    const result2 = await agent2.generate({ messages: [...priorMessages, {
      role: 'user',
      content: 'what is 3+4?'
    }] });
  ...
  it('local and remote skills mix in one registry and load through the same tool', async (t) => {
    ...
    const local = await loader.invoke({ name: 'local' }, {
    ...
    const remote = await loader.invoke({ name: 'remote-skill' }, {
      signal: new AbortController().signal,
      toolCallId: 'r1',
      step: 0,
      runId: 'r1',
  ...
  it('remote skill fetches are coalesced and cached per registry', async (t) => {
    ...
    const [a, b] = await Promise.all([registry.load('remote-skill'), registry.load('remote-skill')]);
  ...
  it('invalid remote repo identifiers reject before fetching', async (t) => {
    ...
    t.throws(() => skill({
      ...
      skillsmd: { fetch: fakeSkillsFetch(calls) }
    }), /owner\/repo/, 'URLs are rejected');
    t.throws(() => skill({
      repo: 'acme/remote-skill/path',
      skillsmd: { fetch: fakeSkillsFetch(calls) }
    }), /owner\/repo/, 'extra path segments are rejected');
  ...
  it('remote skill reports clear errors for missing markdown', async (t) => {
    ...
    await t.rejects(() => registry.load('missing-skill'), /No skill markdown found/, 'missing skill markdown rejects clearly');
  });
});
