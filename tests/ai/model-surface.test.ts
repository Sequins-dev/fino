...
describe('model surface — responseFormat (structured output)', () => {
  it('provider adapters resolve from nested model specifiers', async (t) => {
    const o = directOpenAI({
      client: makeOpenAIClient('ok') as never,
      model: 'gpt-direct',
      apiKey: 'test'
    });
    const a = directAnthropic({
      client: makeAnthropicClient('ok') as never,
      model: 'claude-direct',
      apiKey: 'test'
    });
...
describe('model surface — guardrails', () => {
  it('blocking input guardrail throws GuardrailError before the model is called', async (t) => {
    ...
    const h = agent({
      model: trackingModel,
      guardrails: { input: (messages) => {
        const hasBlocked = messages.some((m) => typeof m.content === 'string' && m.content.includes('blocked'));
        return {
          action: hasBlocked ? 'block' : 'allow',
  ...
  it('redacting output guardrail rewrites response text', async (t) => {
    ...
    const result = await h.generate({ messages: [{
      role: 'user',
      content: 'tell me your ssn'
    }] });
    t.equal(result.text, 'My SSN is [SSN REDACTED]', 'SSN is redacted in result text');
    const lastMsg = result.messages[result.messages.length - 1];
    t.equal(typeof lastMsg?.content === 'string' ? lastMsg.content : '', 'My SSN is [SSN REDACTED]', 'redacted text stored in conversation history');
  });
  ...
  it('Agent with output schema falls back to forced-tool path for unknown provider', async (t) => {
    ...
    const toolModel: Model = {
      ...
      stream(req: GenerateRequest) {
        ...
        async function* gen() {
          if (hasRespondTool && req.toolChoice && typeof req.toolChoice === 'object' && (req.toolChoice as {
            name: string;
          }).name === 'respond') {
            toolCallSeen = true;
            yield {
