...
describe('assembleResult', () => {
  ...
  it('concatenates text across multiple indices in order', async (t) => {
    const result = await assembleResult(events(
      ...
      { type: 'text_delta', index: 0, text: '-cont' },
      { type: 'stop', reason: 'end_turn' },
    ));
  ...
  it('assembles a single tool call', async (t) => {
    const result = await assembleResult(events(
      { type: 'tool_call_start', index: 0, id: 'toolu_1', name: 'get_weather' },
      { type: 'tool_call_delta', index: 0, json: '{"loc' },
      { type: 'tool_call_delta', index: 0, json: 'ation":"NYC"}' },
  ...
  it('assembles multiple tool calls in index order', async (t) => {
    const result = await assembleResult(events(
      { type: 'tool_call_start', index: 1, id: 'toolu_b', name: 'tool_b' },
      { type: 'tool_call_start', index: 0, id: 'toolu_a', name: 'tool_a' },
      { type: 'tool_call_delta', index: 0, json: '{"x":1}' },
      { type: 'tool_call_delta', index: 1, json: '{"y":2}' },
      { type: 'tool_call_end', index: 0 },
  ...
  it('handles malformed tool args JSON gracefully', async (t) => {
    const result = await assembleResult(events(
      { type: 'tool_call_start', index: 0, id: 'id', name: 'fn' },
      { type: 'tool_call_delta', index: 0, json: '{bad json' },
      { type: 'tool_call_end', index: 0 },
      { type: 'stop', reason: 'end_turn' },
    ));
  ...
  it('accumulates cache usage fields when present', async (t) => {
    const result = await assembleResult(events(
      { type: 'usage', usage: { inputTokens: 100, outputTokens: 50, cacheReadInputTokens: 80, cacheCreationInputTokens: 20 } },
      { type: 'stop', reason: 'end_turn' },
    ));
  ...
  it('maps each StopReason variant through unmodified', async (t) => {
    ...
    for (const reason of reasons) {
      const result = await assembleResult(events({ type: 'stop', reason }));
  ...
  it('throws on error event', async (t) => {
    await t.rejects(
      () => assembleResult(events({ type: 'error', message: 'upstream failed' })),
      /upstream failed/,
    );
  });
