...
function scriptModel(turns: Array<StreamEvent[] | 'throw429' | 'throw503'>): Model {
  ...
  return {
    ...
    stream(_req: GenerateRequest): ModelStream {
      ...
      idx++;
      if (turn === 'throw429') {
        throw new ModelError('Provider API error 429: rate limited', {
          status: 429,
          retryAfterMs: 50
        });
      }
...
function countingModel(name: string, results: Array<StreamEvent[] | Error>): {
  model: Model;
  callCount: () => number;
} {
  let count = 0;
...
describe('agent resilience', () => {
  ...
  it('retries on 503 and succeeds', async (t) => {
    const { model, callCount } = countingModel('claude-test', [new ModelError('unavailable', { status: 503 }), endTurnEvents('done')]);
    const h = agent({
      model,
      retry: {
        maxRetries: 2,
  ...
  it('does not retry 4xx errors (except 429)', async (t) => {
    const { model, callCount } = countingModel('claude-test', [new ModelError('API error 401: unauthorized', { status: 401 })]);
    const h = agent({
      model,
      retry: {
        maxRetries: 3,
    ...
    await t.rejects(() => h.generate({ messages: [{
      role: 'user',
      content: 'hi'
    }] }), /401/);
  ...
  it('throws after exhausting maxRetries', async (t) => {
    const { model, callCount } = countingModel('claude-test', [new ModelError('API error 429: rate limited', {
      status: 429,
      retryAfterMs: 1
    })]);
    const h = agent({
  ...
  it('falls back to secondary model when primary fails', async (t) => {
    const primary = countingModel('claude-primary', [new ModelError('rate limited', {
      status: 429,
      retryAfterMs: 1
    })]);
  ...
  it('retries primary before falling back', async (t) => {
    const primary = countingModel('claude-primary', [new ModelError('429', {
      status: 429,
      retryAfterMs: 1
  ...
  it('throws after all models exhausted', async (t) => {
    const primary = countingModel('claude-a', [new ModelError('API error 429: rate limited', {
      status: 429,
      retryAfterMs: 1
    })]);
  ...
  it('custom retryOn predicate controls retry behaviour', async (t) => {
    const { model, callCount } = countingModel('gpt-test', [new ModelError('custom error', { status: 503 }), endTurnEvents('ok')]);
    const h = agent({
      model,
      retry: {
        maxRetries: 2,
