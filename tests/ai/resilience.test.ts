...
function countingModel(
  name: string,
  results: Array<StreamEvent[] | Error>,
): {
  model: Model;
  callCount: () => number;
} {
...
describe('agent resilience', () => {
  ...
  it('throws after exhausting maxRetries', async (t) => {
    const { model, callCount } = countingModel('claude-test', [
      new ModelError('API error 429: rate limited', {
        status: 429,
        retryAfterMs: 1,
      }),
    ]);
  ...
  it('throws after all models exhausted', async (t) => {
    const primary = countingModel('claude-a', [
      new ModelError('API error 429: rate limited', {
        status: 429,
        retryAfterMs: 1,
      }),
    ]);
    const fallback = countingModel('gpt-b', [
      new ModelError('API error 503: unavailable', { status: 503 }),
    ]);
    const h = agent({
      model: primary.model,
