...
function scriptModel(turns: StreamEvent[][]): Model {
  ...
  return {
    ...
    async embed(texts: string[]): Promise<Float32Array[]> {
      return texts.map((t) => {
        const v = t.length > 0 ? 1.0 / Math.sqrt(3) : 0;
  ...
  return {
    ...
    stream(_req: GenerateRequest): ModelStream {
      const events: StreamEvent[] = [
        { type: 'text_delta', index: 0, text: JSON.stringify(responseObj) },
...
evaluate({
  ...
  cases: [
    { name: 'uppercase a', input: 'a', expected: 'A' },
    { name: 'uppercase hello', input: 'hello', expected: 'HELLO' },
  ],
  scorers: [exactMatch()],
  threshold: 1.0,
});
...
describe('scorers', () => {
  it('exactMatch passes when output equals expected', async (t) => {
    ...
    const r = scorer('hello', { name: 'c', input: 'q', expected: 'hello' }) as ScoreResult;
  ...
  it('exactMatch fails when output does not equal expected', async (t) => {
    const scorer = exactMatch();
    const r = scorer('hello', { name: 'c', input: 'q', expected: 'world' }) as ScoreResult;
  ...
  it('contains passes when output includes substring', async (t) => {
    ...
    const r = scorer('say hello there', { name: 'c', input: 'q' }) as ScoreResult;
  ...
  it('schemaScorer passes valid output', async (t) => {
    ...
    const r = scorer({ name: 'alice' }, { name: 'c', input: 'q' }) as ScoreResult;
  ...
  it('schemaScorer fails invalid output with explanation', async (t) => {
    ...
    const r = scorer({ age: 42 }, { name: 'c', input: 'q' }) as ScoreResult;
  ...
  it('llmJudge uses model output for score', async (t) => {
    const m = jsonResponseModel({ score: 0.9, explanation: 'very good' });
    const scorer = llmJudge(m, 'Is the output high quality?');
    const r = await (scorer as (out: unknown, c: { name: string; input: unknown }) => Promise<ScoreResult>)(
      'great output',
      { name: 'c', input: 'q' },
    );
  ...
  it('semanticSimilarity returns near-1.0 for identical-embedding texts', async (t) => {
    const m = scriptModel([]);
    const scorer = semanticSimilarity(m, 0.5);
    const r = await (scorer as (out: unknown, c: { name: string; input: unknown; expected?: unknown }) => Promise<ScoreResult>)(
      'hello world',
      { name: 'c', input: 'q', expected: 'hello world' },
    );
    t.ok(r.value > 0.99, `cosine similarity close to 1: ${r.value}`);
  ...
  it('scorer functions expose scorerName property', (t) => {
    t.equal((exactMatch() as { scorerName?: string }).scorerName, 'exactMatch', 'exactMatch.scorerName');
    t.equal((contains('x') as { scorerName?: string }).scorerName, 'contains(x)', 'contains.scorerName');
    t.ok((schemaScorer({}) as { scorerName?: string }).scorerName, 'schemaScorer.scorerName set');
  });
});
...
describe('EvalReporter', () => {
  it('base class methods are no-ops that resolve', async (t) => {
    ...
    await r.onStart({ name: 'x', cases: 1 });
    await r.onCase({ name: 'c', input: 'q', output: 'a', scores: {}, score: 1, pass: true });
    await r.onFinish({ name: 'x', mean: 1, passed: 1, total: 1 });
  ...
  it('subclass can override any hook', async (t) => {
    ...
    class Spy extends EvalReporter {
      override async onStart(_s: { name: string; cases: number }): Promise<void> {
        seen.push('start');
      }
    ...
    const spy = new Spy();
    await spy.onStart({ name: 'x', cases: 2 });
    await spy.onCase({ name: 'c', input: 'q', output: 'a', scores: {}, score: 1, pass: true });
    await spy.onFinish({ name: 'x', mean: 1, passed: 1, total: 1 });

    t.deepEqual(seen, ['start', 'case', 'finish'], 'hooks called in order');
  });
  ...
  it('onCase receives scores keyed by scorer name', async (t) => {
    ...
    await reporter.onCase({
      ...
      expected: 'a',
      scores: {
        exactMatch: { value: 1, pass: true },
        contains: { value: 0, pass: false, explanation: 'missing' },
      },
      score: 0.5,
      pass: false,
    });
...
describe('OpenTelemetryReporter', () => {
  it('emits gen_ai.evaluation.result log event per scorer per case', async (t) => {
    ...
    await reporter.onStart({ name: 'my-eval', cases: 1 });
    await reporter.onCase({
      name: 'case-1',
      input: 'q',
      output: 'a',
      ...
      scores: {
        exactMatch: { value: 1, pass: true },
        contains: { value: 0, pass: false, explanation: 'not found' },
      },
      score: 0.5,
      pass: false,
    });
    await reporter.onFinish({ name: 'my-eval', mean: 0.5, passed: 0, total: 1 });
  ...
  it('records gen_ai.client.evaluation.score histogram per scorer', async (t) => {
    ...
    await reporter.onStart({ name: 'metric-eval', cases: 1 });
    await reporter.onCase({
      name: 'case-1',
      input: 'q',
      output: 'a',
    ...
    await reporter.onFinish({ name: 'metric-eval', mean: 1, passed: 1, total: 1 });
  ...
  it('reporter with custom exporter calls onFinish without error', async (t) => {
    ...
    const stubExporter = {
      async exportSpans() { received.push('spans'); return { code: 'success' as const }; },
      async exportLogs() { received.push('logs'); return { code: 'success' as const }; },
      async exportMetrics() { received.push('metrics'); return { code: 'success' as const }; },
      async shutdown() { received.push('shutdown'); },
    };

    const reporter = new OpenTelemetryReporter({ exporter: stubExporter });

    await reporter.onStart({ name: 'export-test', cases: 1 });
    await reporter.onCase({
      name: 'case-1',
      input: 'q',
      output: 'a',
      scores: { exactMatch: { value: 1, pass: true } },
      score: 1,
      pass: true,
    });
    await reporter.onFinish({ name: 'export-test', mean: 1, passed: 1, total: 1 });
  ...
  it('reporter without endpoint emits into ambient providers without starting SDK', async (t) => {
    ...
    await reporter.onStart({ name: 'ambient-eval', cases: 1 });
    await reporter.onCase({
      name: 'case-1',
      input: 'q',
      output: 'a',
    ...
    await reporter.onFinish({ name: 'ambient-eval', mean: 1, passed: 1, total: 1 });
