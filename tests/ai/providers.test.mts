...
function fakeClient(responses: FakeResponse[]) {
  ...
  return {
    ...
    request(url: string, init?: { body?: string | null; headers?: Record<string, string> }) {
...
function streamResponse(body: AsyncIterable<Uint8Array>): FakeResponse {
  return { status: 200, body, text: () => Promise.resolve(''), json: () => Promise.resolve({}) };
}
...
function errorResponse(status: number, msg: string): FakeResponse {
  return { status, body: null, text: () => Promise.resolve(msg), json: () => Promise.resolve({}) };
}
...
const anthropicTextSse = [
  sseFrame('message_start', { message: { usage: { input_tokens: 10, output_tokens: 0, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 } } }),
  sseFrame('content_block_start', { index: 0, content_block: { type: 'text', text: '' } }),
  sseFrame('content_block_delta', { index: 0, delta: { type: 'text_delta', text: 'Hi' } }),
  sseFrame('content_block_delta', { index: 0, delta: { type: 'text_delta', text: ' there' } }),
...
const anthropicToolSse = [
  sseFrame('message_start', { message: { usage: { input_tokens: 20, output_tokens: 0, cache_read_input_tokens: 80, cache_creation_input_tokens: 0 } } }),
  sseFrame('content_block_start', { index: 0, content_block: { type: 'tool_use', id: 'toolu_1', name: 'calculator' } }),
  sseFrame('content_block_delta', { index: 0, delta: { type: 'input_json_delta', partial_json: '{"op' } }),
  sseFrame('content_block_delta', { index: 0, delta: { type: 'input_json_delta', partial_json: '":"add","a":1,"b":2}' } }),
  sseFrame('content_block_stop', { index: 0 }),
  sseFrame('message_delta', { delta: { stop_reason: 'tool_use' }, usage: { output_tokens: 12 } }),
  sseFrame('message_stop', {}),
];
...
describe('anthropic provider', () => {
  it('streams text and assembles result', async (t) => {
    ...
    const model = anthropic({ apiKey: 'test', client });
    const result = await model.stream({ messages: [{ role: 'user', content: 'hi' }] }).result();
  ...
  it('yields StreamEvents in order when iterating', async (t) => {
    ...
    t.ok(collected.some((e) => e.type === 'text_delta'), 'has text_delta events');
    t.ok(collected.some((e) => e.type === 'usage'), 'has usage event');
    t.ok(collected.some((e) => e.type === 'stop'), 'has stop event');
  });

  it('streams tool calls and assembles args', async (t) => {
    const client = fakeClient([streamResponse(sseBytes(...anthropicToolSse))]);
    const model = anthropic({ apiKey: 'test', client });
    const result = await model.stream({ messages: [{ role: 'user', content: 'calc' }] }).result();
    ...
    t.deepEqual(result.toolCalls[0].args, { op: 'add', a: 1, b: 2 });
  });
  ...
  it('generate() returns assembled result from JSON response', async (t) => {
    ...
    const model = anthropic({ apiKey: 'test', client });
    const result = await model.generate({ messages: [{ role: 'user', content: 'test' }] });
  ...
  it('generate() returns tool calls from JSON response', async (t) => {
    const responseData = {
      content: [
        { type: 'text', text: '' },
        { type: 'tool_use', id: 'toolu_x', name: 'search', input: { query: 'fino' } },
      ],
    ...
    const client = fakeClient([jsonResponse(responseData)]);
    const model = anthropic({ apiKey: 'test', client });
    const result = await model.generate({ messages: [{ role: 'user', content: 'search' }] });
    t.equal(result.toolCalls.length, 1);
  ...
  it('throws on non-2xx streaming response', async (t) => {
    ...
    const model = anthropic({ apiKey: 'test', client });
    await t.rejects(
      () => model.stream({ messages: [{ role: 'user', content: 'hi' }] }).result(),
      /401/,
    );
  });
  ...
  it('embed() rejects with clear unsupported message', async (t) => {
    const model = anthropic({ apiKey: 'test', client: fakeClient([]) });
    await t.rejects(
      () => model.embed(['hello']),
      /does not provide a native embeddings endpoint/i,
    );
  });
  ...
  it('sends correct request shape for tools', async (t) => {
    ...
    await model.stream({
      ...
      tools: [{ name: 'fn', description: 'does thing', parameters: { type: 'object', properties: {} } }],
      toolChoice: 'auto',
    }).result();
  ...
  it('sends x-api-key and anthropic-version headers', async (t) => {
    const client = fakeClient([streamResponse(sseBytes(...anthropicTextSse))]);
    const model = anthropic({ apiKey: 'my-key', client });
  ...
  it('uses baseUrl override in request URL', async (t) => {
    const client = fakeClient([streamResponse(sseBytes(...anthropicTextSse))]);
    const model = anthropic({ apiKey: 'test', baseUrl: 'https://proxy.example.com', client });
    await model.stream({ messages: [{ role: 'user', content: 'hi' }] }).result();
  ...
  it('maps refusal stop reason to refusal', async (t) => {
    const frames = [
      sseFrame('message_start', { message: { usage: { input_tokens: 5, output_tokens: 0 } } }),
      sseFrame('message_delta', { delta: { stop_reason: 'refusal' }, usage: { output_tokens: 0 } }),
      sseFrame('message_stop', {}),
    ];
...
describe('openai provider', () => {
  it('streams text and assembles result', async (t) => {
    ...
    const model = openai({ apiKey: 'test', client });
    const result = await model.stream({ messages: [{ role: 'user', content: 'hi' }] }).result();
  ...
  it('streams tool calls and assembles args', async (t) => {
    ...
    const model = openai({ apiKey: 'test', client });
    const result = await model.stream({ messages: [{ role: 'user', content: 'search' }] }).result();
    t.equal(result.stopReason, 'tool_use');
    t.equal(result.toolCalls.length, 1);
  ...
  it('generate() returns assembled result from JSON response', async (t) => {
    const responseData = {
      choices: [{ message: { content: 'World', tool_calls: null }, finish_reason: 'stop' }],
  ...
  it('generate() returns tool calls from JSON response', async (t) => {
    const responseData = {
      choices: [{
        message: {
          content: null,
          tool_calls: [{ id: 'call_x', type: 'function', function: { name: 'fn', arguments: '{"k":"v"}' } }],
        },
    ...
    const client = fakeClient([jsonResponse(responseData)]);
    const model = openai({ apiKey: 'test', client });
    const result = await model.generate({ messages: [{ role: 'user', content: 'go' }] });
    t.equal(result.toolCalls.length, 1);
  ...
  it('embed() returns Float32Array per input text', async (t) => {
    ...
    t.equal(result[0][0], 0.10000000149011612); // float32 precision
    t.equal(result[1][2], 0.6000000238418579);
  });
  ...
  it('embed() returns embeddings in index order when response is unordered', async (t) => {
    ...
    t.equal(result[0][0], 0.10000000149011612);
    t.equal(result[1][0], 0.8999999761581421);
  });
  ...
  it('parses prompt_tokens_details.cached_tokens in streaming usage', async (t) => {
    ...
    const model = openai({ apiKey: 'test', client });
    const result = await model.stream({ messages: [{ role: 'user', content: 'hi' }] }).result();
    t.equal(result.usage.cacheReadInputTokens, 15, 'cacheReadInputTokens from prompt_tokens_details.cached_tokens');
  });

  it('parses prompt_tokens_details.cached_tokens in generate() response', async (t) => {
    const responseData = {
      choices: [{ message: { content: 'Done' }, finish_reason: 'stop' }],
      usage: { prompt_tokens: 20, completion_tokens: 5, prompt_tokens_details: { cached_tokens: 15 } },
    };
    const client = fakeClient([jsonResponse(responseData)]);
    const model = openai({ apiKey: 'test', client });
    const result = await model.generate({ messages: [{ role: 'user', content: 'hi' }] });
    t.equal(result.usage.cacheReadInputTokens, 15, 'cacheReadInputTokens from generate() prompt_tokens_details.cached_tokens');
  });
