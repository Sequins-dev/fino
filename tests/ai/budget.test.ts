...
describe('AI budgets', () => {
  ...
  it('reserves concurrent capacity atomically and releases provider failures', async (t) => {
    ...
    second.commit({ inputTokens: 2, outputTokens: 1 }, 0.25);
    t.equal(control.snapshot().usedTokens, 3);
    t.equal(control.snapshot().usedUsd, 0.25);
  });

  it('suspends for approval and resumes from a durable snapshot after a grant', async (t) => {
    const control = new Budget({ tokens: 4, onExhausted: 'suspend' });
    ...
    restored.grant({ tokens: 6 }, { approvedBy: 'operator@example.com', reason: 'finish run' });
    const lease = restored.reserve({ tokens: 5 });
    lease.commit({ inputTokens: 2, outputTokens: 3 }, 0);
  ...
  it('enforces wall-clock ceilings with an injected deterministic clock', (t) => {
    let now = 100;
    const control = new Budget({ wallClockMs: 50, clock: () => now });
    control.check();
    now = 151;
  ...
  it('persists suspension, accepts a grant, and resumes without replaying completed tools', async (t) => {
    ...
    const model: Model = {
      ...
      stream(): ModelStream {
        ...
        async function* events(): AsyncGenerator<StreamEvent> {
          if (modelCalls === 1) {
            yield { type: 'tool_call_start', index: 0, id: 'call-1', name: 'lookup' };
            yield { type: 'tool_call_delta', index: 0, json: '{}' };
    ...
    const control = new Budget({ tokens: 5, onExhausted: 'suspend' });
