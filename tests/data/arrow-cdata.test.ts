...
describe('arrow C Data Interface format codec', () => {
  ...
  it('encodes nested formats', (t) => {
    ...
    t.equal(
      typeToFormat(arrow.fixedSizeList(3, new Field('item', arrow.float32(), true))),
      '+w:3',
      'fixed size list',
    );
...
describe('arrow C Data Interface export/import round-trips', () => {
  ...
  it('round-trips int32 through real pointers', (t) => {
    t.deepEqual(roundTripVector([1, 2, 3, null, 5], arrow.int32()), [1, 2, 3, null, 5], 'int32');
  });
  it('round-trips int64', (t) => {
    t.deepEqual(
      roundTripVector([1n, 2n, 9007199254740993n], arrow.int64()),
      [1n, 2n, 9007199254740993n],
      'int64',
    );
  });
  it('round-trips float64 with nulls', (t) => {
    t.deepEqual(roundTripVector([1.5, null, 2.25], arrow.float64()), [1.5, null, 2.25], 'float64');
  });
  it('round-trips bool', (t) => {
    t.deepEqual(
      roundTripVector([true, false, null, true], arrow.bool()),
  ...
  it('round-trips utf8', (t) => {
    t.deepEqual(
      roundTripVector(['alpha', 'grüße', '', null], arrow.utf8()),
      ['alpha', 'grüße', '', null],
      'utf8',
    );
  });
  it('round-trips list<int32>', (t) => {
    ...
    t.deepEqual(roundTripVector([[1, 2], [], [3]], type), [[1, 2], [], [3]], 'list');
  });
  it('round-trips struct', (t) => {
    const type = arrow.struct([
      new Field('a', arrow.int32(), true),
      new Field('b', arrow.utf8(), true),
    ]);
    t.deepEqual(
      roundTripVector(
        [
