...
describe('fino:task', () => {
  ...
  it('parses nested CLI tasks and honors global --json', async (t) => {
    ...
    const seen: unknown[] = [];
    const result = await doc.parse(['--json', 'build', '--out-dir', 'api', 'index.mts'], {
      writer: { mode: 'json', writeJson: (value) => { seen.push(value); } },
    });

    t.deepEqual(result, { entry: 'index.mts', outDir: 'api' });
    t.deepEqual(seen, [{ input: { entry: 'index.mts', outDir: 'api' } }]);
  });
