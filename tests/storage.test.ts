...
describe('fino:storage', () => {
  ...
  it('maps S3 client methods to signed HTTP requests', async (t) => {
    ...
    const client = new S3Client({
      ...
      fetch: async (req) => {
        calls.push(req);
        if (req.method === 'HEAD') return new Response(null, { headers: { 'content-length': '5', etag: '"e"' } });
  ...
  it('provides an async object-backed FileSystem', async (t) => {
    ...
    const fs = new S3FileSystem(new S3Client({
      ...
      fetch: async (req) => {
        ...
        if (req.method === 'HEAD') return new Response(null, { status: objects.has(key) ? 200 : 404, headers: { 'content-length': String(objects.get(key)?.byteLength ?? 0) } });
