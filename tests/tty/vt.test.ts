...
describe('vt plain text', () => {
  it('requires positive integer dimensions', (t) => {
    t.throws(() => new Terminal({ cols: 0, rows: 2 }), /positive integers/);
    t.throws(() => new Terminal({ cols: 2, rows: 1.5 }), /positive integers/);
    const term = new Terminal({ cols: 2, rows: 2 });
    t.throws(() => term.resize(-1, 2), /positive integers/);
  });
...
describe('vt input chunking', () => {
  ...
  it('consumes unsupported CSI sequences and C0 controls without painting them', (t) => {
    const term = new Terminal({ cols: 10, rows: 2 });
    term.write('a\x1b[1$');
    term.write('zb\x00\x7fc');
    t.equal(term.text()[0], 'abc');
  });

  it('accepts colon-delimited SGR color parameters', (t) => {
    const term = new Terminal({ cols: 10, rows: 2 });
    term.write('\x1b[38:2:1:2:3mX');
    t.deepEqual([...term.cellAt(0, 0).sgr], [38, 2, 1, 2, 3]);
  });
