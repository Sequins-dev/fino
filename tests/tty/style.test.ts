...
import { charWidth, stringWidth, graphemes, clusterWidth } from 'fino:tty/frame';
...
describe('fino:tty/style', () => {
  ...
  it('detects truecolor support from COLORTERM values', (t) => {
    t.ok(supportsTruecolor('truecolor'), 'truecolor is supported');
    t.ok(supportsTruecolor('24bit'), '24bit is supported');
    t.ok(!supportsTruecolor('256color'), 'other values are not truecolor');
    t.ok(!supportsTruecolor(undefined), 'an absent value is not truecolor');
  });

  it('maps RGB colors to the nearest xterm palette entry', (t) => {
    t.equal(nearestAnsi256(255, 0, 0), 196, 'red cube corner');
    t.equal(nearestAnsi256(128, 128, 128), 244, 'neutral gray uses the grayscale ramp');
    t.equal(nearestAnsi256(0, 0, 0), 16, 'black prefers the exact cube corner');
    t.equal(nearestAnsi256(255, 255, 255), 231, 'white prefers the exact cube corner');
  });
});

describe('fino:tty/frame width helpers', () => {
