...
describe('fino:tty/frame', () => {
  ...
  it('diffs frames row-wise for screen repaints', (t) => {
    ...
    t.equal(paint, '\x1b[2;1H\x1b[KTWO', 'only the changed row is written, erased first');
    t.ok(
  ...
  it('erases a full-width row before writing its final cell', (t) => {
    const frame = frameOf([textRow('1234')], 4);
    t.equal(
      frameToScreen(frame, null),
      '\x1b[1;1H\x1b[K1234',
      'no erase follows a character written into the right margin',
    );
  });
