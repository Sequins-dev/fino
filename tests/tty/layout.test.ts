...
describe('internal:tty/layout flexbox', () => {
  ...
  it('lets growing children absorb overflow when no shrink weight is set', (t) => {
    const frame = renderFrame(
      h(
        Box,
        { direction: 'row' },
        h(Text, { grow: 1, basis: 8 }, 'abcdefgh'),
        h(Text, null, 'keep'),
      ),
      { width: 8, height: 1 },
    );
    t.equal(lines(frame)[0], 'abcdkeep', 'a grow-based row remains inside its assigned width');
  });
  ...
  it('clips overflow by default and allows explicit visible overflow', (t) => {
    const hidden = renderFrame(
      h(
        Box,
        { direction: 'row' },
        h(Box, { width: 3, height: 1 }, h(Text, { width: 6 }, 'abcdef')),
      ),
      { width: 6, height: 1 },
    );
    const visible = renderFrame(
      h(
        Box,
        { direction: 'row' },
        h(Box, { width: 3, height: 1, overflow: 'visible' }, h(Text, { width: 6 }, 'abcdef')),
      ),
      { width: 6, height: 1 },
    );
    t.equal(hidden, 'abc   ', 'box content is clipped to its assigned rect');
    t.equal(visible, 'abcdef', 'visible overflow may paint into the parent canvas');
  });
});
...
describe('internal:tty/layout layers', () => {
  ...
  it('paints sibling layers in deterministic source order', (t) => {
    const frame = renderFrame(
      h(
        Box,
        null,
        h(Text, null, 'base'),
        h(Layer, { anchor: { x: 0, y: 0 } }, h(Text, null, 'first')),
        h(Layer, { anchor: { x: 0, y: 0 } }, h(Text, null, 'later')),
      ),
      { width: 6, height: 2 },
    );
    t.equal(lines(frame)[1], 'later ', 'a later sibling paints above an earlier sibling');
  });
});
