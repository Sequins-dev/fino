...
describe('fino:tty/tui renderFrame', () => {
  it('renders text, borders, padding, gap, and flex rows deterministically', (t) => {
    const frame = renderFrame(
      h(Box, { direction: 'row', gap: 1, border: true, padding: 1 },
        h(Text, null, 'A'),
        h(Spacer, { flex: 1 }),
        h(Text, null, 'B'),
      ),
  ...
  it('renders terminal controls with focus order markers', (t) => {
    const frame = renderFrame(
      h(Box, { direction: 'column' },
        h(Input, { value: 'query', focused: true }),
        h(Button, { label: 'Go' }),
        h(List, { items: ['one', 'two'], selectedIndex: 1 }),
  ...
  it('renders input background styling without changing cell width', (t) => {
    const frame = renderFrame(
      h(Input, { value: 'name', focused: true, background: 'brightBlack' }),
      { width: 10, height: 1 },
    );
  ...
  it('renders background-colored vertical padding bands', (t) => {
    const frame = renderFrame(
      h(Box, { height: 3, paddingY: 1, background: 'brightBlack' },
        h(Text, null, 'entry'),
      ),
      { width: 8, height: 3 },
    );
...
describe('fino:tty/tui input decoding', () => {
  it('decodes printable text, control keys, and arrows', (t) => {
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('a')), [{ type: 'key', key: 'a', text: 'a' }], 'printable key');
    t.deepEqual(decodeTuiInput(Uint8Array.of(0x03)), [{ type: 'key', key: 'c', ctrl: true }], 'ctrl-c');
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1b[A')), [{ type: 'key', key: 'up' }], 'up arrow');
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1b[Z')), [{ type: 'key', key: 'tab', shift: true }], 'shift-tab');
  });

  it('decodes SGR mouse events', (t) => {
    t.deepEqual(
      decodeTuiInput(new TextEncoder().encode('\x1b[<0;12;5M')),
      [{ type: 'mouse', action: 'press', button: 'left', x: 11, y: 4, ctrl: false, alt: false, shift: false }],
      'left press uses zero-based coordinates',
    );
    t.deepEqual(
      decodeTuiInput(new TextEncoder().encode('\x1b[<35;12;5M')),
      [{ type: 'mouse', action: 'release', button: 'left', x: 11, y: 4, ctrl: false, alt: false, shift: false }],
      'release event',
    );
    t.deepEqual(
      decodeTuiInput(new TextEncoder().encode('\x1b[<64;2;3M')),
      [{ type: 'mouse', action: 'wheel', button: 'wheel-up', x: 1, y: 2, ctrl: false, alt: false, shift: false }],
      'wheel event',
    );
  });
});
