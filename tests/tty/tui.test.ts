...
import { Box, Button, Input, List, ScrollView, Spacer, Text, decodeTuiInput, renderFrame } from 'fino:tty/tui';
describe('fino:tty/tui renderFrame', () => {
  it('renders text, borders, padding, gap, and flex rows deterministically', (t) => {
    const frame = renderFrame(h(Box, {
      direction: 'row',
      gap: 1,
      border: true,
      padding: 1
    }, h(Text, null, 'A'), h(Spacer, { flex: 1 }), h(Text, null, 'B')), {
      width: 10,
      height: 5
    });
    t.equal(frame, [
  ...
  it('renders terminal controls with focus order markers', (t) => {
    const frame = renderFrame(h(Box, { direction: 'column' }, h(Input, {
      value: 'query',
      focused: true
    }), h(Button, { label: 'Go' }), h(List, {
      items: ['one', 'two'],
      selectedIndex: 1
  ...
  it('renders input background styling without changing cell width', (t) => {
    const frame = renderFrame(h(Input, {
      value: 'name',
      focused: true,
...
describe('fino:tty/tui input decoding', () => {
  it('decodes printable text, control keys, and arrows', (t) => {
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('a')), [{
      type: 'key',
      key: 'a',
      text: 'a'
    }], 'printable key');
    t.deepEqual(decodeTuiInput(Uint8Array.of(3)), [{
      type: 'key',
      key: 'c',
      ctrl: true
    }], 'ctrl-c');
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1B[A')), [{
      type: 'key',
      key: 'up'
    }], 'up arrow');
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1B[Z')), [{
      type: 'key',
      key: 'tab',
      shift: true
    }], 'shift-tab');
  ...
  it('decodes SGR mouse events', (t) => {
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1B[<0;12;5M')), [{
      type: 'mouse',
      action: 'press',
      button: 'left',
      x: 11,
    ...
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1B[<35;12;5M')), [{
      type: 'mouse',
      action: 'release',
      button: 'left',
      x: 11,
    ...
    t.deepEqual(decodeTuiInput(new TextEncoder().encode('\x1B[<64;2;3M')), [{
      type: 'mouse',
      action: 'wheel',
      button: 'wheel-up',
      x: 1,
