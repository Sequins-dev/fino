...
describe('fino:semver — OR range branch coverage', () => {
  it('version matching only the SECOND OR branch is satisfied', (t) => {
    t.equal(satisfies('2.5.0', '^1.0.0 || ^2.1.0'), true, '2.5.0 satisfies second branch ^2.1.0');
    t.equal(satisfies('0.9.0', '^1.0.0 || ^2.1.0'), false, '0.9.0 satisfies neither branch');
    t.equal(satisfies('1.0.0', '^1.0.0 || ^2.1.0'), true, '1.0.0 satisfies first branch ^1.0.0');
  });
});
describe('fino:semver — prerelease identifier ordering (numeric vs string)', () => {
  ...
  it('longer prerelease has higher precedence when shared prefix is equal', (t) => {
    ...
    t.ok(compare('1.0.0-alpha.1', '1.0.0-alpha') > 0, 'alpha.1 has higher precedence than alpha');
  });
});
