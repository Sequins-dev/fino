...
describe('fino:jsonrpc — JsonRpcService', () => {
  it('handle() dispatches a request and returns the JSON response', async (t) => {
    ...
    const response = await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'multiply', params: { a: 6, b: 7 }, id: 1 }));
  ...
  it('handle() returns null for notifications (no id)', async (t) => {
    const svc = new JsonRpcService();
    let called = false;
    svc.method('ping').handle(() => { called = true; });

    const response = await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'ping', params: {} }));
  ...
  it('handle() returns METHOD_NOT_FOUND for unknown methods', async (t) => {
    const svc = new JsonRpcService();
    const response = await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'unknown', id: 5 }));
    t.ok(response, 'response is not null');
    const parsed = JSON.parse(response!);
  ...
  it('handle() propagates JsonRpcError code from handler', async (t) => {
    const svc = new JsonRpcService();
    svc.method('strict').handle(() => { throw new JsonRpcError('bad param', INVALID_PARAMS); });

    const response = await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'strict', id: 9 }));
    const parsed = JSON.parse(response!);
  ...
  it('list() returns registered method descriptors', (t) => {
    ...
    svc.method('add')
      ...
      .params({ type: 'object', properties: { a: { type: 'number' }, b: { type: 'number' } } })
      .handle(() => null);
  ...
  it('params schema validates incoming params automatically', async (t) => {
    ...
    svc.method('add')
      ...
      .handle((p) => (p as { a: number; b: number }).a + (p as { a: number; b: number }).b);

    // valid params
    const ok = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'add', params: { a: 3, b: 4 }, id: 1 })))!);
    t.equal(ok.result, 7, 'valid params return result');

    // invalid params (string instead of number)
    const bad = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'add', params: { a: 'x', b: 4 }, id: 2 })))!);
  ...
  it('params schema accepts raw JSON Schema objects', async (t) => {
    ...
    svc.method('greet')
      ...
      .handle((p) => `hello ${(p as { name: string }).name}`);

    const ok = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'greet', params: { name: 'world' }, id: 1 })))!);
    t.equal(ok.result, 'hello world', 'valid raw schema passes');

    const bad = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'greet', params: { name: 42 }, id: 2 })))!);
  ...
  it('methods can be chained fluently', async (t) => {
    const svc = new JsonRpcService();
    svc
      .method('double').handle((p) => (p as { n: number }).n * 2)
      .method('triple').handle((p) => (p as { n: number }).n * 3);

    const r1 = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'double', params: { n: 5 }, id: 1 })))!);
    const r2 = JSON.parse((await svc.handle(JSON.stringify({ jsonrpc: '2.0', method: 'triple', params: { n: 5 }, id: 2 })))!);
...
describe('fino:jsonrpc — JsonRpcPeer', () => {
  ...
  it('call() rejects with JsonRpcError when the handler throws', async (t) => {
    ...
    const client = new JsonRpcPeer(ta);
    const svc = new JsonRpcService();
    svc.method('fail').handle(() => { throw new Error('intentional failure'); });
    new JsonRpcPeer(tb, svc);

    let caught: unknown;
    try { await client.call('fail'); } catch (err) { caught = err; }
  ...
  it('call() rejects with JsonRpcError when the method is unknown', async (t) => {
    ...
    let caught: unknown;
    try { await client.call('nonexistent'); } catch (err) { caught = err; }
    t.ok(caught instanceof JsonRpcError, 'throws JsonRpcError');
  ...
  it('peer can act as both client and server simultaneously', async (t) => {
    ...
    svcA.method('greet').handle((params) => `hello from A: ${(params as { name: string }).name}`);
    const peerA = new JsonRpcPeer(ta, svcA);

    const svcB = new JsonRpcService();
    svcB.method('greet').handle((params) => `hello from B: ${(params as { name: string }).name}`);
...
describe('fino:jsonrpc — JsonRpcServer', () => {
  it('serve() handles requests over a persistent transport', async (t) => {
    ...
    const svc = new JsonRpcService();
    svc.method('double').handle((params) => (params as { n: number }).n * 2);
  ...
  it('listen() serves JSON-RPC over HTTP and responds to requests', async (t) => {
    ...
    svc.method('greet')
      ...
      .handle((params) => `Hello, ${(params as { name: string }).name}!`);
    ...
    const res = await fetch(`http://127.0.0.1:${port}/`, {
      ...
      body: JSON.stringify({ jsonrpc: '2.0', method: 'greet', params: { name: 'World' }, id: 1 }),
    });
  ...
  it('listen() returns 204 for notifications sent over HTTP', async (t) => {
    const svc = new JsonRpcService();
    let notified = false;
    svc.method('event').handle(() => { notified = true; });
    ...
    const res = await fetch(`http://127.0.0.1:${port}/`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', method: 'event', params: {} }),
    });
  ...
  it('httpHandler() works with fino:net/http/app App.rpc()', async (t) => {
    ...
    const svc = new JsonRpcService();
    svc.method('greet').handle((p) => `hi ${(p as { name: string }).name}`);
    ...
    const res = await fetch(`http://127.0.0.1:${server.port}/rpc`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', method: 'greet', params: { name: 'fino' }, id: 1 }),
    });
  ...
  it('listen() returns 404 for requests to the wrong path', async (t) => {
    const svc = new JsonRpcService();
    const server = new JsonRpcServer(svc);
    const handle = server.listen({ port: 0, path: '/rpc' });
    await handle.ready;
