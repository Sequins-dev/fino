...
describe('one-shot compression', () => {
  ...
  it('gzip with custom level', (t) => {
    ...
    t.equal(
      decompress(c1, { format: 'gzip' }).byteLength,
      LONG.byteLength,
      'level 1 decompresses correctly',
    );
    t.equal(
      decompress(c9, { format: 'gzip' }).byteLength,
      LONG.byteLength,
      'level 9 decompresses correctly',
    );
  });
  ...
  it('gzip produces a valid member frame', (t) => {
    ...
    t.equal(
      readU32LE(compressed, compressed.byteLength - 4),
      input.byteLength,
      'ISIZE matches original input length',
    );
  });
  it('rejects corrupt compressed data', (t) => {
    t.throws(
      () => decompress(new Uint8Array([0, 1, 2, 3]), { format: 'gzip' }),
      /inflate error/,
      'throws on corrupt gzip',
    );
    t.throws(
      () => decompress(new Uint8Array([0, 1, 2, 3]), { format: 'deflate' }),
      /inflate error/,
      'throws on corrupt deflate',
    );
  });
  it('rejects trailing junk after zlib-wrapped compressed data', (t) => {
    for (const format of ['gzip', 'deflate'] as const) {
      ...
      t.throws(
        () => decompress(extended, { format }),
        /trailing/i,
        `${format} rejects trailing input`,
      );
    }
  });
  it('rejects trailing junk after brotli compressed data', (t) => {
    ...
    extended.set(compressed);
    t.throws(
      () => decompress(extended, { format: 'brotli' }),
      /trailing/i,
      'brotli rejects trailing input',
    );
  });
  it('validates format', (t) => {
    t.throws(
      () => compress(HELLO, { format: 'zip' as CompressionFormat }),
      /unsupported compression format/i,
      'compress rejects invalid format',
    );
    t.throws(
      () => decompress(HELLO, { format: 'zip' as CompressionFormat }),
      /unsupported compression format/i,
      'decompress rejects invalid format',
    );
  });
  it('reports brotli availability consistently', (t) => {
    if (brotliAvailable) {
      const compressed = compress(HELLO, { format: 'brotli' });
      t.equal(
        str(decompress(compressed, { format: 'brotli' })),
        str(HELLO),
        'brotli works when available',
      );
    } else {
      t.throws(
        () => compress(HELLO, { format: 'brotli' }),
        /brotli library not available/,
        'brotli compress throws when unavailable',
      );
      t.throws(
        () => decompress(HELLO, { format: 'brotli' }),
        /brotli library not available/,
        'brotli decompress throws when unavailable',
      );
    }
  });
});
  ...
  it('zlib streaming decompression rejects trailing junk', (t) => {
    for (const format of ['gzip', 'deflate'] as const) {
      ...
      t.throws(
        () => decompressor.finish(),
        /trailing/i,
        `${format} streaming rejects trailing input`,
      );
    }
  });
  ...
  for (const format of supportedFormats()) {
    it(`${format} transform roundtrip`, async (t) => {
      const compressed = await collect(
        createCompressor({ format }).transform(
          asAsyncIterable([bytes('Hello, '), bytes('streaming '), bytes('world!').buffer]),
        ),
      );
      const restored = await collect(
        createDecompressor({ format }).transform(asAsyncIterable([compressed])),
      );
...
describe('fino:compress release contract', () => {
  it('exposes a compact format/options surface without advanced zlib helpers', (t) => {
    for (const name of [
      ...
      t.equal(
        Object.prototype.hasOwnProperty.call(compression, name),
        false,
  ...
  it('requires explicit formats and binary input', (t) => {
    t.throws(
      () => compress(HELLO, {} as any),
      /unsupported compression format/i,
      'compress requires format',
    );
    t.throws(
      () => decompress(HELLO, {} as any),
      /unsupported compression format/i,
      'decompress requires format',
    );
    t.throws(
      () => compress('hello' as any, { format: 'gzip' }),
      /binary input/i,
      'compress rejects strings',
    );
    t.throws(
      () => decompress('hello' as any, { format: 'gzip' }),
      /binary input/i,
      'decompress rejects strings',
    );
    const compressor = createCompressor({ format: 'gzip' });
    t.throws(
      () => compressor.write('hello' as any),
      /binary input/i,
      'stream compressor rejects strings',
    );
    compressor.close();
  });
...
for (const format of ['zstd', 'lz4'] as const) {
  ...
  describe(`fino:compress — ${format}`, () => {
    ...
    it('round-trips binary data with all byte values', (t) => {
      ...
      const packed = compress(input, { format });
      t.deepEqual(
        Array.from(decompress(packed, { format })),
        Array.from(input),
        'binary round-trips',
      );
    });
    it('streams via write/finish across chunks', (t) => {
      ...
      t.equal(
        str(decompress(packed, { format })),
        'part one part two part three',
        'chunked compress round-trips',
      );
    });
    it('streams via async transform', async (t) => {
      ...
      const restored = await collect(
        d.transform(asAsyncIterable([packed]) as AsyncIterable<Uint8Array>),
      );
      d.close();
...
describe('fino:compress — snappy', () => {
  if (!snappyAvailable) {
    ...
    it('round-trips binary data with all byte values', (t) => {
      ...
      t.deepEqual(
        Array.from(decompress(packed, { format: 'snappy' })),
        Array.from(input),
        'binary round-trips',
      );
    });
    it('streams as a single block via write/finish', (t) => {
      ...
      c.close();
      t.equal(
        str(decompress(concat(parts), { format: 'snappy' })),
        'part one part two ',
        'buffered block round-trips',
      );
    });
