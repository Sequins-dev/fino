...
describe('sandbox backend naming', () => {
  it('lists linuxNative and never the retired linuxSandboxd label', (t) => {
    ...
    t.ok(
      ...
      'linuxNative backend is listed',
    );
    t.ok(
      capabilities.backends.every((backend) => !String(backend.name).includes('Sandboxd')),
      'no daemon-implying backend names remain',
    );
    t.ok(
      capabilities.backends.every((backend) => !backend.reason.includes('Sandboxd')),
      'no daemon-implying backend reasons remain',
    );
  });
  it('words backend availability reasons for the current platform', (t) => {
    ...
    if (os === 'linux') {
      ...
      t.ok(
        !linuxNative!.reason.includes('probed Linux enforcement features'),
        'macOS does not claim live Linux enforcement',
      );
    }
  });
});
describe('strict sandbox pre-spawn network rejection', () => {
  it('rejects outbound rules carrying destination detail before spawning', (t) => {
    t.throws(
      ...
      /filtered egress/,
      'hostname destinations are rejected pre-spawn',
    );
    t.throws(
      () =>
        new Process('/bin/echo', ['x'], {
      ...
      /filtered egress/,
      'CIDR destinations are rejected pre-spawn',
    );
  });
  it('rejects rules carrying port or protocol detail before spawning', (t) => {
    t.throws(
      ...
      /sandbox\.network\.outbound\[0\]\.port/,
      'port detail is rejected pre-spawn',
    );
    t.throws(
      () =>
        new Process('/bin/echo', ['x'], {
      ...
      /sandbox\.network\.inbound\[0\]\.protocol/,
      'protocol detail is rejected pre-spawn',
    );
  });
  it('still accepts coarse allow/deny-all rules in strict mode', async (t) => {
    ...
    if (!capabilities.strictAvailable) {
      t.throws(
        () => new Process('/bin/echo', ['coarse'], options),
        /strict sandbox/,
        'coarse rules still fail closed without a strict backend',
      );
      return;
    }
