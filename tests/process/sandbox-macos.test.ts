...
async function run(
  ...
  return { code: (await proc.wait()).code, out: joinChunks(chunks).trim() };
}
...
describe('macOS Seatbelt process-exec scoping', () => {
  ...
  it('reports filesystem and process enforcement via Seatbelt', async (t) => {
    if (os !== 'darwin') return;
    if (!processSandboxCapabilities().strictAvailable) return;
    const proc = new Process('/usr/bin/true', [], {
      sandbox: {
        mode: 'strict',
    ...
    t.equal(
      proc.sandboxReport.securityBoundary,
      true,
      'strict Seatbelt spawn is a security boundary',
    );
    t.ok(
      proc.sandboxReport.enforced.some((e) => e.category === 'filesystem'),
      'filesystem is enforced',
    );
    t.ok(
      proc.sandboxReport.enforced.some((e) => e.category === 'process'),
      'process policy is enforced',
    );
    t.ok(
      proc.sandboxReport.enforced.every((e) => !/Landlock|seccomp|cgroup/.test(e.reason)),
      'no Linux mechanism wording',
    );
    proc.stdin.close();
    ...
    t.throws(
      () =>
        new Process('/bin/echo', ['x'], {
          sandbox: { mode: 'strict', syscalls: { mode: 'denylist', names: ['ptrace'] } },
        }),
      /seccomp/,
      'syscall policy is rejected on macOS',
    );
    t.throws(
      () =>
        new Process('/bin/echo', ['x'], {
          sandbox: { mode: 'strict', process: { allowFork: false } },
        }),
      /seccomp/,
      'fork denial is rejected on macOS',
    );
  });
});
