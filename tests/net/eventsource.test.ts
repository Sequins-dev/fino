...
describe('EventSourceReader', () => {
  ...
  it('multi-line data joined with newline', async (t) => {
    const events = await collect(
      new EventSourceReader(source('data: line1\ndata: line2\ndata: line3\n\n')),
    );
    t.equal(events.length, 1);
...
describe('EventSource integration', () => {
  ...
  it('resolves an empty URL against global location', (t) => {
    ...
    try {
      t.equal(
        source.url,
        'https://web-platform.test/eventsource/eventsource-constructor-empty-url.any.js',
      );
    } finally {
      source.close();
  ...
  it('onopen and onmessage run as EventTarget listeners in dispatch order', async (t) => {
    ...
    await new Promise<void>((resolve, reject) => {
      const tid = setTimeout(
        () => reject(new Error('timeout waiting for EventSource message')),
        2e3,
      );
      const es = new EventSource(`http://127.0.0.1:${server.port}/events`);
    ...
    t.equal(
      openCurrentTarget instanceof EventSource,
      true,
      'open currentTarget is set during handler',
    );
    t.equal(openPhase, Event.AT_TARGET, 'open handler runs at AT_TARGET');
    t.equal(
      messageCurrentTarget instanceof EventSource,
      true,
      'message currentTarget is set during handler',
    );
  ...
  it('readyState is OPEN while receiving events', async (t) => {
    let stateWhenOpen = -1;
    const server = serveHttp({ port: 0 }, async (_req) => sseResponse(sseBody({ data: 'check' })));
    await new Promise<void>((resolve) => {
      const es = new EventSource(`http://127.0.0.1:${server.port}/events`);
      es.onmessage = () => {
  ...
  it('readyState is CLOSED after close()', async (t) => {
    const server = serveHttp({ port: 0 }, async (_req) => sseResponse(sseBody({ data: 'x' })));
    await new Promise<void>((resolve) => {
      const es = new EventSource(`http://127.0.0.1:${server.port}/events`);
      es.onmessage = () => {
        es.close();
        t.equal(
          es.readyState,
          EventSource.CLOSED,
          'readyState is CLOSED immediately after close()',
        );
        resolve();
      };
    });
  ...
  it('wrong content-type causes fatal error (no reconnect)', async (t) => {
    ...
    const server = serveHttp(
      { port: 0 },
      async (_req) => new Response('not sse', { headers: { 'content-type': 'text/html' } }),
    );
    await new Promise<void>((resolve) => {
      const es = new EventSource(`http://127.0.0.1:${server.port}/events`);
      es.onerror = () => {
  ...
  it('Last-Event-ID sent on reconnect', async (t) => {
    ...
    new EventSource(`http://127.0.0.1:${server.port}/events`);
    await reconnected;
    await loop.timeout(50);
  ...
  it('follows local redirects with relative Location before opening the stream', async (t) => {
    ...
    t.deepEqual(
      seenPaths,
      ['/events', '/events-final'],
      'redirect is followed with a relative Location',
    );
  ...
  it('does not retain Set-Cookie across reconnects unless Cookie is explicit', async (t) => {
    ...
    const explicitServer = serveHttp({ port: 0 }, async (req) => {
      explicitCookies.push(req.headers.get('cookie'));
      return sseResponse(sseBody({ data: 'explicit-cookie' }));
    });
    await new Promise<void>((resolve) => {
      const es = new EventSource(`http://127.0.0.1:${explicitServer.port}/events`, {
  ...
  it('connects to HTTPS SSE with a fixture CA', { skip: skipTls }, async (t) => {
    ...
    let received = '';
    await new Promise<void>((resolve) => {
      const es = new EventSource(`https://localhost:${server.port}/events`, {
        tls: { ca: CERT_PATH },
      });
      es.onmessage = (e) => {
        received = e.data;
