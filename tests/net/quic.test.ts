...
describe('QUIC bindings', () => {
  ...
  it('keeps conformance matrix statuses canonical and fixture rows mapped', async (t) => {
    ...
    for (const entry of fixtures.evidence ?? []) {
      ...
      t.ok(source.includes(`it('${entry.testName}'`) || source.includes(`it("${entry.testName}"`) || source.includes(entry.testName), `evidence test exists: ${entry.testFile} :: ${entry.testName}`);
    }
  });
});
