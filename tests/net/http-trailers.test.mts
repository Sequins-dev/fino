...
const _H2_SETTINGS_EMPTY = new Uint8Array([0x00,0x00,0x00, 0x04, 0x00, 0x00,0x00,0x00,0x00]);

// HEADERS: POST / (stream 1, END_HEADERS only — body+trailers follow)
const _H2_POST_HEADERS = new Uint8Array([
  0x00,0x00,0x0e, 0x01, 0x04, 0x00,0x00,0x00,0x01,
  0x83, 0x84, 0x86, 0x41, 0x09,
...
const _H2_TRAILER_HEADERS = new Uint8Array([
  0x00,0x00,0x16, 0x01, 0x05, 0x00,0x00,0x00,0x01,
  0x00,                           // literal no-index, new name
  0x09,                           // name length = 9
...
async function h2RawRoundTrip(port: number, ...frames: Uint8Array[]): Promise<Uint8Array> {
  ...
  const all = new Uint8Array(total);
  let pos = 0;
  for (const f of [_H2_PREFACE, _H2_SETTINGS_EMPTY, ...frames]) { all.set(f, pos); pos += f.byteLength; }
  ...
  let outPos = 0;
  for (const c of chunks) { out.set(c, outPos); outPos += c.byteLength; }
  return out;
}
...
describe('HTTP/2 trailers', () => {
  it('server sends response with outgoing trailers without crashing', async (t) => {
    ...
    for (let i = 0; i < raw.byteLength; i++) {
      if (raw[i] === 0x88) { found200 = true; break; }
    }
