...
async function runScenario(t: any, scenario: NodeInteropScenario): Promise<void> {
  ...
  try {
    ...
    const finoClient = spawnFino('tests/net/fixtures/quic-node-fino-client.ts', [String(address.port), `--scenario=${scenario}`]);
  ...
  const finoServer = spawnFino('tests/net/fixtures/quic-node-fino-server.ts', [`--scenario=${scenario}`]);
