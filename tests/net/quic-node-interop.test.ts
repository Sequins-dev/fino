...
import { cleanupProcess, collect, configuredNodeQuicAvailable, parseReadyAddress, spawnFino, spawnNodeQuic, withTimeout } from './fixtures/quic/interop-harness.ts';
...
async function runScenario(t: any, scenario: NodeInteropScenario): Promise<void> {
  ...
  try {
    ...
    const finoClientStatus = await withTimeout(finoClient.wait(), 1e4, 'Fino client to Node server');
    ...
    if (scenario !== 'alpn-mismatch') await withTimeout(nodeServerWait, 1e4, 'Node QUIC server exit');
  } finally {
  ...
  try {
    ...
    const nodeClientStatus = await withTimeout(nodeClient.wait(), 1e4, 'Node client to Fino server');
