...
import {
  ...
  spawnNodeQuic,
  withTimeout,
} from './fixtures/quic/interop-harness.mts';

type NodeInteropScenario = 'h3' | 'echo' | 'datagram' | 'resume' | 'keyupdate' | 'large' | 'retry' | 'alpn-mismatch';
...
async function runScenario(t: any, scenario: NodeInteropScenario): Promise<void> {
  ...
  if (nodeBin === null) {
    t.ok(true, 'NODE_QUIC_BIN is not configured or does not expose node:quic; Node QUIC interop skipped');
    return;
  }
  const expectedNodeReply = expectedReply('node', scenario);
  const expectedFinoReply = expectedReply('fino', scenario);

  const nodeServer = spawnNodeQuic(nodeBin, 'tests/net/fixtures/quic-node-server.mjs', [`--scenario=${scenario}`]);
  const nodeServerWait = nodeServer.wait();
  let nodeServerOutText = '';
  let nodeReadyResolve!: (address: { port: number }) => void;
  const nodeReady = new Promise<{ port: number }>((resolve) => { nodeReadyResolve = resolve; });
  ...
  try {
    const address = await withTimeout(nodeReady, 5_000, 'Node QUIC server readiness');
    const finoClient = spawnFino('tests/net/fixtures/quic-node-fino-client.mts', [String(address.port), `--scenario=${scenario}`]);
    const finoClientOut = collect(finoClient.stdout);
    const finoClientErr = collect(finoClient.stderr);
    const finoClientStatus = await withTimeout(finoClient.wait(), 10_000, 'Fino client to Node server');
    t.equal(finoClientStatus.code, 0, await finoClientErr);
    t.equal((await finoClientOut).trim(), expectedNodeReply, `Fino client completes ${scenario} scenario with Node server`);
    if (scenario !== 'alpn-mismatch') await withTimeout(nodeServerWait, 10_000, 'Node QUIC server exit');
  } finally {
  ...
  const finoServer = spawnFino('tests/net/fixtures/quic-node-fino-server.mts', [`--scenario=${scenario}`]);
  const finoServerWait = finoServer.wait();
  let finoServerOutText = '';
  let finoReadyResolve!: (address: { port: number }) => void;
  const finoReady = new Promise<{ port: number }>((resolve) => { finoReadyResolve = resolve; });
  ...
  try {
    const address = await withTimeout(finoReady, 5_000, 'Fino QUIC server readiness');
    const nodeClient = spawnNodeQuic(nodeBin, 'tests/net/fixtures/quic-node-client.mjs', [String(address.port), `--scenario=${scenario}`]);
    const nodeClientOut = collect(nodeClient.stdout);
    const nodeClientErr = collect(nodeClient.stderr);
    const nodeClientStatus = await withTimeout(nodeClient.wait(), 10_000, 'Node client to Fino server');
    t.equal(nodeClientStatus.code, 0, await nodeClientErr);
    t.equal((await nodeClientOut).trim(), expectedFinoReply, `Node client completes ${scenario} scenario with Fino server`);
    if (scenario !== 'alpn-mismatch') {
      const finoServerStatus = await withTimeout(finoServerWait, 10_000, 'Fino QUIC server exit');
