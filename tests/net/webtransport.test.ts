...
class FakeH3Connection extends EventTarget {
  ...
  emitDatagram(data: Uint8Array): void {
    const event = new Event('datagram') as Event & {
      data: Uint8Array;
    };
    event.data = data;
  ...
  emitStream(stream: FakeQuicStream): void {
    const event = new Event('stream') as Event & {
...
describe('WebTransport over HTTP/3 public API', () => {
  ...
  it('exposes standards-shaped datagram streams for connected H3 sessions', async (t) => {
    ...
    await writer.write(new Uint8Array([170, 187]));
    writer.releaseLock();
    t.deepEqual([...connection.sent[0]!], [...encodeHttpDatagram(8n, new Uint8Array([170, 187]))]);
    const received = readOne(wt.datagrams.readable);
    connection.emitDatagram(encodeHttpDatagram(12n, new Uint8Array([0])));
