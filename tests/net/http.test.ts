...
describe('Request parsing basics', () => {
  it('GET request with no body', async (t) => {
    const req = await parseRequest(source('GET /index.html HTTP/1.1\r\nHost: localhost\r\n\r\n'));
...
describe('Response parsing', () => {
  ...
  it('204 No Content has empty body', async (t) => {
    const res = await parseResponse(source('HTTP/1.1 204 No Content\r\n\r\n'));
    t.equal(res.status, 204);
    const data = await collectBody(res.body);
    t.equal(data.byteLength, 0, 'no body for 204');
  });
  it('304 Not Modified has empty body', async (t) => {
    const res = await parseResponse(source('HTTP/1.1 304 Not Modified\r\nETag: "abc123"\r\n\r\n'));
  ...
  it('1xx response has no body', async (t) => {
    const res = await parseResponse(source('HTTP/1.1 100 Continue\r\n\r\n'));
...
describe('Response parsing — edge cases', () => {
  ...
  it('OPTIONS * request is parsed with pathname /*, not malformed URL', async (t) => {
    ...
    const req = await parseRequest(source('OPTIONS * HTTP/1.1\r\nHost: example.com\r\n\r\n'));
