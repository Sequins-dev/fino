...
import {
  ...
  type LookupResult,
  type RRType,
} from '../../js/internal/net/dns-provider.mts';
...
class MemoryDnsProvider extends DnsProvider {
  ...
  async lookup(hostname: string, opts: { family?: 4 | 6 } = {}): Promise<LookupResult> {
    ...
    for (const type of types) {
      ...
      if (address) return { address, family: type === 'AAAA' ? 6 : 4 };
    }
...
describe('internal:net/dns-provider contract', () => {
  const records: RecordMap = {
    'service.test': {
      A: ['192.0.2.10'],
      AAAA: ['2001:db8::10'],
      MX: [{ priority: 10, exchange: 'mail.service.test' }],
      TXT: [['v=spf1 -all']],
      SRV: [{ priority: 1, weight: 5, port: 443, name: 'api.service.test' }],
      SOA: [{ nsname: 'ns.service.test', hostmaster: 'hostmaster.service.test', serial: 1, refresh: 3600, retry: 600, expire: 86400, minttl: 60 }],
  ...
  it('lookup honors requested address family', async (t) => {
    ...
    t.deepEqual(await provider.lookup('service.test', { family: 4 }), { address: '192.0.2.10', family: 4 });
    t.deepEqual(await provider.lookup('service.test', { family: 6 }), { address: '2001:db8::10', family: 6 });
  ...
  it('resolve returns record shapes and snapshots', async (t) => {
    ...
    t.deepEqual(await provider.resolve('service.test', 'MX'), [{ priority: 10, exchange: 'mail.service.test' }]);
    t.deepEqual(await provider.resolve('service.test', 'TXT'), [['v=spf1 -all']]);
    t.deepEqual(await provider.resolve('service.test', 'SRV'), [{ priority: 1, weight: 5, port: 443, name: 'api.service.test' }]);
    t.deepEqual(await provider.resolve('service.test', 'SOA'), [{ nsname: 'ns.service.test', hostmaster: 'hostmaster.service.test', serial: 1, refresh: 3600, retry: 600, expire: 86400, minttl: 60 }]);
