...
function bitmapWindow(types: number[]): Uint8Array {
  let max = 0;
  for (const type of types) max = Math.max(max, type & 255);
...
function buildDnsResponse(query: Uint8Array, records: LocalDnsRecord[], opts: {
  ...
  push16(33152 | (opts.truncated ? 512 : 0) | (opts.rcode ?? 0));
  push16(1);
...
function parseQuestion(query: Uint8Array): {
  name: string;
  type: number;
} {
...
describe('Wire protocol', () => {
  ...
  it('_buildQuery — header fields', (t) => {
    ...
    t.equal(view.getUint16(0, false), 4660, 'ID');
    t.equal(view.getUint16(2, false), 256, 'flags (RD=1)');
  ...
  it('_buildQuery — DNSSEC adds EDNS OPT with DO bit', (t) => {
    const pkt = _buildQuery(4660, 'example.com', RECORD_TYPES.A, { dnssec: true });
    ...
    t.equal((view.getUint16(optOffset + 7, false) & 32768) !== 0, true, 'DO bit is set');
  ...
  it('_randomQueryId returns non-zero 16-bit transaction IDs', (t) => {
    for (let i = 0; i < 128; i++) {
      ...
      t.ok(id >= 1 && id <= 65535, 'query ID stays in DNS transaction range');
    }
  });
  ...
  it('_decodeName — compression pointer', (t) => {
    ...
    msg.set(encoded, 12);
    msg[12 + encoded.length] = 192;
    msg[12 + encoded.length + 1] = 12;
  ...
  it('_decodeName — rejects truncated compression pointers', (t) => {
    t.throws(() => _decodeName(new Uint8Array([192]), 0), /truncated compression pointer/i);
  });
  it('_decodeName — rejects compression pointer loops', (t) => {
    const msg = new Uint8Array([192, 0]);
  ...
  it('_parseResponse — synthetic A record response', (t) => {
    ...
    const totalLen = 12 + qname.length + 4 + qname.length + 4 + 2 + 4 + 2 + 4;
    const msg = new Uint8Array(totalLen);
    const view = new DataView(msg.buffer);
    view.setUint16(0, 43981, false);
    view.setUint16(2, 33152, false);
    view.setUint16(4, 1, false);
    view.setUint16(6, 1, false);
    let off = 12;
    msg.set(qname, off);
    off += qname.length;
    view.setUint16(off, RECORD_TYPES.A, false);
    off += 2;
    ...
    view.setUint16(off, 1, false);
    off += 2;
    view.setUint32(off, 300, false);
    off += 4;
    view.setUint16(off, 4, false);
    off += 2;
    msg[off] = 93;
    msg[off + 1] = 184;
    msg[off + 2] = 216;
    ...
    t.equal(parsed.id, 43981, 'ID matches');
  ...
  it('_parseResponse — DNSSEC records and OPT metadata', (t) => {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      msg[off++] = 192;
      msg[off++] = 12;
      view.setUint16(off, record.type, false);
      off += 2;
      view.setUint16(off, 1, false);
      off += 2;
      view.setUint32(off, record.ttl ?? 300, false);
      off += 4;
      view.setUint16(off, rdata.byteLength, false);
      off += 2;
      msg.set(rdata, off);
      off += rdata.byteLength;
    }
    msg[off++] = 0;
    view.setUint16(off, 41, false);
    off += 2;
    view.setUint16(off, 1232, false);
    off += 2;
    msg[off++] = 0;
    msg[off++] = 0;
    view.setUint16(off, 32768, false);
    off += 2;
...
describe('Integration', () => {
  ...
  it('resolver — NXDOMAIN throws with err.code === ENOTFOUND', async (t) => {
    ...
    const e = caughtErr as {
      code?: string;
      message?: string;
    };
    t.equal(e.code, 'ENOTFOUND', 'err.code is exactly "ENOTFOUND" (not just in the message)');
    t.ok((e.message ?? '').length > 0, 'error message is non-empty: ' + e.message);
  });
