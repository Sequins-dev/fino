...
import {
  ...
  decodeUtf8,
  encodeUtf8,
} from './fixtures/quic/sim-harness.mts';
import { QUIC_V2, makeVersionNegotiationPacket } from './fixtures/quic/packet-craft.mts';
import { parseQuicHeader } from './fixtures/quic/packet-parse.mts';
...
function makeInitialProbe(version = QUIC_V1, seed = 0, token: Uint8Array = new Uint8Array()): Uint8Array {
...
function drainDatagrams(socket: any): Array<{ data: Uint8Array; addr: unknown }> {
  const packets: Array<{ data: Uint8Array; addr: unknown }> = [];
  for (;;) {
...
function trackPromise<T>(promise: Promise<T>): { settled(): boolean; value(): T | undefined; failure(): unknown } {
  let settled = false;
...
function bytesQueued(trace: ReturnType<QuicPipe['trace']>, fromPort: number, toPort: number): number {
  return trace
    .filter((event) => (
...
function fixedTokenSecret(seed = 0x51): Uint8Array {
  const secret = new Uint8Array(32);
  for (let i = 0; i < secret.byteLength; i++) secret[i] = (seed + i) & 0xff;
  return secret;
}
...
function newSimClient(
  ...
  return new QuicEndpoint({ alpnProtocols: ['fino-hq'], ...options }, {
...
async function startSimConnect(pipe: QuicPipe, clientEndpoint: QuicEndpoint): Promise<ReturnType<typeof trackPromise<QuicConnection>>> {
  const listener = pipe.listener ?? await pipe.listen();
  const connected = trackPromise(clientEndpoint.connect({
    address: listener.address,
    serverName: 'localhost',
  }));
  return connected;
...
async function sendInitialProbe(pipe: QuicPipe, localAddress: QuicAddress, token: Uint8Array, seed: number): Promise<Uint8Array | null> {
  const listener = pipe.listener ?? await pipe.listen();
...
async function readStreamBytes(stream: Awaited<ReturnType<QuicConnection['acceptStream']>>, pipe: QuicPipe): Promise<Uint8Array> {
...
describe('QUIC simulator conformance', () => {
  it('RFC 9000 sections 6, 7, and 14.1: v2 handshake completes and Initial datagrams are at least 1200 bytes', async (t) => {
    ...
    try {
      ...
      t.equal(client.handshakeComplete, true, 'client handshake completed through simulated datagrams');
      t.equal(server.handshakeComplete, true, 'server handshake completed through simulated datagrams');
  ...
  it('RFC 9368 section 2: default client first flight uses v1 and upgrades after compatible VN', async (t) => {
    ...
    const clientAddress = { family: 'ipv4' as const, ip: '10.0.0.1', port: 55_010 };
    const pipe = new QuicPipe();
    ...
    try {
      ...
      const [client, server] = await pipe.pumpUntil(Promise.all([connected, accepted]));
      t.equal(client.version, 'v2', 'client upgrades to preferred QUIC v2 after compatible Version Negotiation');
      t.equal(server.version, 'v2', 'server negotiates QUIC v2 after the client retry');
      t.equal(client.handshakeComplete, true, 'client handshake completes after compatible Version Negotiation retry');
      t.equal(server.handshakeComplete, true, 'server handshake completes after compatible Version Negotiation retry');
    } finally {
  ...
  it('RFC 9000 section 6: unsupported-version Initial receives Version Negotiation', async (t) => {
    ...
    const pipe = new QuicPipe();
    const probe = await pipe.net.datagram({ family: 'ipv4', ip: '10.0.0.1', port: 0 });
    try {
      const listener = await pipe.listen();
      await probe.send(makeInitialProbe(0x57414954), listener.address);
      ...
      t.equal(response.data[0] & 0x80, 0x80, 'Version Negotiation uses long header form');
      ...
      t.equal(pipe.server.stats.packetsReceived, 1, 'Version Negotiation probes count as processed packets');
      t.equal(pipe.server.stats.bytesReceived, 0, 'Version Negotiation probes do not count endpoint received bytes');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 sections 5.2.2 and 6: unsupported-version Initial does not create accept state', async (t) => {
    ...
    try {
      ...
      await probe.send(makeInitialProbe(0x57414954, 7), listener.address);
      await pipe.runUntilIdle();

      t.equal(accepted.settled(), false, 'unsupported-version probe does not resolve endpoint accept');
      t.equal(pipe.server.cidTable.get('4041424344454647'), undefined, 'unsupported-version probe does not register its DCID');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 sections 7.2 and 14.1: malformed Initial does not create accept state', async (t) => {
    ...
    try {
      ...
      t.equal(accepted.settled(), false, 'malformed Initial probe does not resolve endpoint accept');
      t.equal(drainDatagrams(probe).length, 0, 'malformed Initial probe receives no Retry amplification');
      t.equal(pipe.server.stats.packetsReceived, 0, 'malformed Initial probes do not count as processed packets');
      t.equal(pipe.server.stats.bytesReceived, 0, 'malformed Initial probes do not count endpoint received bytes');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 malformed packet fuzz corpus leaves endpoint usable', async (t) => {
    ...
    const pipe = new QuicPipe();
    const listener = await pipe.listen();
    const probe = await pipe.net.datagram({ family: 'ipv4', ip: '10.0.0.9', port: 55_090 });
    try {
      const seedPacket = makeInitialProbe(QUIC_V1, 0x33);
      const corpus: Uint8Array[] = [
        new Uint8Array(),
        new Uint8Array([0xff]),
        makeInitialProbe(0x0a0a0a0a, 0x34),
      ];
      for (let n = 1; n <= 64; n++) corpus.push(seedPacket.slice(0, n));
      for (let n = 1; n <= 20; n++) {
        const packet = new Uint8Array(n);
        packet[0] = 0x40;
        for (let i = 1; i < packet.byteLength; i++) packet[i] = (0x90 + n + i) & 0xff;
        corpus.push(packet);
      }
      ...
      t.equal(pipe.server.stats.activeServerConnections, 0, 'malformed corpus creates no accepted sessions');
      t.ok(pipe.server.stats.packetsReceived >= 0, 'endpoint stats remain readable after fuzz corpus');
  ...
  it('RFC 9002 sections 6.2 and 6.2.1: dropped first flight recovers through PTO', async (t) => {
    ...
    try {
      const { client, server } = await pipe.handshake();
      const droppedInitials = pipe.trace().filter((event) => (
        event.type === 'datagram:dropped'
        && event.reason === 'loss'
        && event.bytes >= 1200
      ));
      const queuedInitials = pipe.trace().filter((event) => event.type === 'datagram:queued' && event.bytes >= 1200);

      t.equal(client.handshakeComplete, true, 'client handshake recovers after the first Initial flight is lost');
      t.equal(server.handshakeComplete, true, 'server handshake recovers after client PTO retransmission');
  ...
  it('RFC 9000 section 8.1.2: unvalidated Initial receives Retry by default', async (t) => {
    ...
    try {
      ...
      await pipe.runUntilIdle();
      const response = await pipe.pumpUntil(probe.recv());
      const version = new DataView(response.data.buffer, response.data.byteOffset, response.data.byteLength).getUint32(1, false);
      const stats = pipe.server.stats;

      t.equal(response.data[0] & 0xf0, 0xf0, 'Retry uses QUIC long-header Retry form');
  ...
  it('Node transport parity: source-address deny filtering blocks packets before QUIC parsing', async (t) => {
    ...
    try {
      ...
      t.equal(stats.packetsBlocked, 1, 'source-filtered packets contribute to blocked packet stats');
      t.equal(stats.packetsReceived, 0, 'source-filtered packets do not count as Node received packets');
      t.equal(stats.bytesReceived, 0, 'source-filtered packets do not count as Node received bytes');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('Node transport parity: source-address allow filtering blocks non-matching packets', async (t) => {
    ...
    const blocked = await pipe.net.datagram({ family: 'ipv4', ip: '10.0.0.3', port: 0 });
    try {
      const listener = await pipe.listen();
      const accepted = trackPromise(pipe.server.accept());
      ...
      t.equal(stats.packetsReceived, 0, 'allow-list blocked packets do not count as Node received packets');
      t.equal(stats.bytesReceived, 0, 'allow-list blocked packets do not count as Node received bytes');
    } finally {
      blocked.close();
      await pipe.close();
    }
  ...
  it('Node transport parity: busy mode refuses new server connection Initials', async (t) => {
    ...
    try {
      ...
      t.ok(drainDatagrams(probe).length > 0, 'busy endpoint sends an immediate CONNECTION_REFUSED close');
      t.equal(stats.serverBusyCount, 1, 'endpoint counts busy-gated packets using Node serverBusyCount semantics');
      t.equal(stats.packetsBlocked, 0, 'busy packets do not contribute to Node packetsBlocked stats');
      t.equal(stats.immediateCloseSent, 1, 'endpoint counts immediate CONNECTION_REFUSED close packets');
      t.equal(stats.packetsReceived, 1, 'busy refusal counts the Initial as processed');
      t.equal(stats.bytesReceived, 0, 'busy refusal does not count payload bytes at endpoint level');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('Node transport parity: busy mode can be toggled at runtime', async (t) => {
    ...
    try {
      ...
      t.equal(pipe.server.stats.serverBusyCount, 1, 'setBusy(true) increments Node serverBusyCount');
      ...
      t.equal(pipe.server.stats.serverBusyCount, 2, 'runtime busy packets increment Node serverBusyCount');
      t.equal(pipe.server.stats.packetsBlocked, 0, 'runtime busy packets do not increment packetsBlocked');
      ...
      t.equal(pipe.server.stats.serverBusyCount, 2, 'setBusy(false) does not increment serverBusyCount');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('Node transport parity: maxConnections gates excess accepted server sessions', async (t) => {
    ...
    try {
      await pipe.listen();
      firstClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.1', port: 55_110 });
      ...
      secondClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.3', port: 55_111 }, { versions: ['v1'] });
      await startSimConnect(pipe, secondClient);
      await pipe.pumpUntilCondition(() => (
        pipe.server.stats.connectionLimitPackets > 0 && pipe.server.stats.immediateCloseSent > 0 ? pipe.server.stats : null
      ));
      const stats = pipe.server.stats;
      ...
      t.equal(stats.packetsBlocked, 0, 'connection-limit packets do not contribute to Node packetsBlocked stats');
      t.equal(stats.immediateCloseSent, 1, 'connection-limit refusal sends immediate close');
      t.equal(stats.packetsReceived - packetsBeforeLimit, 1, 'connection-limit refusal counts the refused Initial as processed');
    } finally {
  ...
  it('Node transport parity: maxConnectionsPerRemoteAddress gates only the matching remote address', async (t) => {
    ...
    try {
      await pipe.listen();
      firstClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.1', port: 55_130 });
      const first = await connectSimClient(pipe, firstClient);
      t.equal(first.handshakeComplete, true, 'first connection from the remote address is accepted');

      blockedClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.1', port: 55_131 }, { versions: ['v1'] });
      await startSimConnect(pipe, blockedClient);
      await pipe.pumpUntilCondition(() => (
        pipe.server.stats.connectionLimitPackets > 0 && pipe.server.stats.immediateCloseSent > 0 ? pipe.server.stats : null
      ));
      const limitedStats = pipe.server.stats;
      t.equal(limitedStats.connectionLimitPackets, 1, 'same remote address is gated by the per-remote limit');
      t.equal(limitedStats.packetsBlocked, 0, 'per-remote limit does not increment Node packetsBlocked');

      differentHostClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.2', port: 55_132 });
      ...
      t.equal(pipe.server.stats.activeServerConnections, 2, 'two accepted server connections remain active');
    } finally {
      await firstClient?.close();
    ...
    const pipe = new QuicPipe({
      server: {
        ...
        transport: {
          ...
          sessionCreationRateLimit: { rate: 0, burst: 1 },
        },
      },
    });
    const probe = await pipe.net.datagram({ family: 'ipv4', ip: '10.0.0.1', port: 55_140 });
    try {
      const listener = await pipe.listen();
      ...
      t.equal(pipe.server.stats.serverConnections, 1, 'first Initial consumes the session-creation burst');
      ...
      const stats = pipe.server.stats;
      t.equal(stats.sessionCreationRateLimited, 1, 'session creation rate-limit counter increments');
      t.equal(stats.packetsBlocked, 0, 'session creation rate-limit does not increment Node packetsBlocked');
      t.equal(stats.connectionLimitPackets, 0, 'session creation rate-limit is distinct from connection limits');
      t.equal(stats.packetsReceived, packetsBeforeRateLimit, 'session creation rate-limit drop does not count the second Initial as processed');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('Node transport parity: maxConnections zero means unlimited', async (t) => {
    ...
    try {
      await pipe.listen();
      for (let i = 0; i < 2; i++) {
        const client = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.1', port: 55_120 + i });
        clients.push(client);
        const connection = await connectSimClient(pipe, client);
        t.equal(connection.handshakeComplete, true, `connection ${i + 1} is accepted when limits are zero`);
      }
      t.equal(pipe.server.stats.connectionLimitPackets, 0, 'zero limits do not gate connections');
      t.equal(pipe.server.stats.activeServerConnections, 2, 'endpoint accepts multiple server connections with zero limits');
    } finally {
  ...
  it('RFC 9000 section 8.1: Retry-disabled Initial handling respects the 3x anti-amplification limit', async (t) => {
    ...
    try {
      ...
      t.ok(serverBytes <= clientBytes * 3, 'server response before validation stays within the 3x amplification limit');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 section 8.1.3: server-issued NEW_TOKEN is received and persisted by the client', async (t) => {
    ...
    try {
      ...
      t.ok(state?.addressToken instanceof Uint8Array, 'client persists a real server-issued NEW_TOKEN');
  ...
  it('RFC 9000 section 8.1.3: same-address NEW_TOKEN reconnect validates without Retry', async (t) => {
    ...
    const sessions = new Map<string, any>();
    const sessionStore = memorySessionStore(sessions);
    const localAddress = { family: 'ipv4' as const, ip: '10.0.0.1', port: 55_100 };
    const pipe = new QuicPipe({
      server: {
        retry: { tokenSecret: fixedTokenSecret() },
        transport: { addressTokenTimeoutMs: 300_000 },
      },
    });
    let firstClient: QuicEndpoint | null = null;
    let secondClient: QuicEndpoint | null = null;
    try {
      ...
      const first = await connectSimClient(pipe, firstClient);
      await pipe.runUntilSettled();
      t.ok(sessions.get('localhost|fino-hq')?.addressToken instanceof Uint8Array, 'first connection persists a server-issued NEW_TOKEN');
      await first.close();
      await firstClient.close();
      await pipe.runUntilSettled();

      pipe.advance(120_000);
      await pipe.runUntilSettled();
  ...
  it('Node transport parity: resumed 0-RTT connect defers packets until first early write', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { sessionStore, earlyData: { replaySafe: true, maxBytes: 1024 } },
      server: { sessionStore: serverSessionStore, earlyData: { replaySafe: true, maxBytes: 1024 } },
    });
    let resumedClient: QuicEndpoint | null = null;
    try {
      await pipe.listen();
      const warmup = await connectSimClient(pipe, pipe.client);
      await pipe.pumpUntilCondition(() => sessions.get('localhost|fino-hq')?.ticket instanceof Uint8Array ? true : null);
      t.ok(sessions.get('localhost|fino-hq')?.ticket instanceof Uint8Array, 'warmup stores a TLS session ticket');
      await warmup.close();
      await pipe.runUntilSettled();

      const traceBefore = pipe.trace().length;
      resumedClient = newSimClient(pipe, { family: 'ipv4', ip: '10.0.0.1', port: 55_120 }, {
        sessionStore,
        earlyData: { replaySafe: true, maxBytes: 1024 },
      });
      const earlyConnection = await resumedClient.connect({
        address: pipe.listener!.address,
        serverName: 'localhost',
      });
      const queuedBeforeWrite = pipe.trace().slice(traceBefore).filter((event) => event.type === 'datagram:queued');

      t.equal(earlyConnection.handshakeComplete, false, 'resumed 0-RTT connect returns before handshake completion');
      t.equal(queuedBeforeWrite.length, 0, 'deferred 0-RTT connect does not queue a ClientHello before application data');
      ...
      t.equal(decodeUtf8(data!), 'early-write-starts-handshake', 'first early stream write starts the deferred handshake');
      t.ok(
        pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued'),
  ...
  it('RFC 9000 section 8.1.3: tampered, expired, and wrong-address NEW_TOKENs are rejected', async (t) => {
    ...
    const sessions = new Map<string, any>();
    const sessionStore = memorySessionStore(sessions);
    const originalAddress = { family: 'ipv4' as const, ip: '10.0.0.1', port: 55_101 };
    const pipe = new QuicPipe({ server: { retry: { tokenSecret: fixedTokenSecret(0x61) } } });
    ...
    try {
      ...
      tampered[Math.min(1, tampered.byteLength - 1)] ^= 0x01;
      ...
      t.ok(tamperedResponse === null || (tamperedResponse[0] & 0xf0) === 0xf0, 'tampered regular token is dropped or challenged with Retry');

      pipe.advance(120_000);
      await pipe.runUntilSettled();
      const beforeWrongAddress = pipe.server._inspectAddressValidationStats();
      const wrongAddressResponse = await sendInitialProbe(pipe, { family: 'ipv4', ip: '10.0.0.9', port: 55_102 }, validToken, 32);
      ...
      t.ok(wrongAddressResponse === null || (wrongAddressResponse[0] & 0xf0) === 0xf0, 'wrong-address regular token is dropped or challenged with Retry');

      pipe.advance(24 * 60 * 60 * 1000 + 120_000);
      await pipe.runUntilSettled();
      ...
      t.ok(expiredResponse === null || (expiredResponse[0] & 0xf0) === 0xf0, 'expired regular token is dropped or challenged with Retry');
    } finally {
      await clientEndpoint?.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 sections 2 and 3: stream data survives simulator delivery and reassembly', async (t) => {
    ...
    const pipe = new QuicPipe({ link: { latencyMs: 1, reorderRate: 1, reorderDelayMs: 3 } });
    try {
      await pipe.handshake();
      ...
      t.equal(`${decodeUtf8(first!)}${second === null ? '' : decodeUtf8(second)}`, 'simulated stream', 'peer reads reordered stream bytes in order');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 sections 12.3 and 13.4: duplicate packets do not duplicate stream bytes', async (t) => {
    ...
    try {
      ...
      t.equal(chunks.join(''), 'deduplicated-payload', 'stream bytes are exposed exactly once despite duplicated packets');
      t.ok(
        pipe.trace().slice(traceBeforeStream).some((event) => event.type === 'datagram:duplicated'),
  ...
  it('RFC 9002 sustained-loss recovery completes a 4MiB transfer', async (t) => {
    ...
    const pipe = new QuicPipe({
      link: { latencyMs: 8, lossRate: 0.06 },
      client: {
        connection: {
    ...
    try {
      ...
      const [, received] = await pipe.pumpUntil(Promise.all([write, read]), 20_000);

      t.equal(received.byteLength, totalBytes, 'loss recovery delivers the full 4MiB stream');
      t.ok(client.stats.packetsLost > 0 || server.stats.packetsLost > 0, 'loss recovery records lost packets');
      t.ok(client.stats.bytesLost > 0 || server.stats.bytesLost > 0, 'loss recovery records lost bytes');
      t.ok(client.stats.congestionWindow > 0, 'client exposes congestion state after sustained loss');
  ...
  it('RFC 9002 congestion-control selections complete stream traffic', async (t) => {
    ...
    for (const congestionControl of ['cubic', 'reno', 'bbr'] as const) {
      ...
      try {
        ...
        t.equal(pipe.client.connection.congestionControl, congestionControl, `${congestionControl} client option resolves`);
        t.equal(pipe.server.connection.congestionControl, congestionControl, `${congestionControl} server option resolves`);
        t.equal(echoed, `cc-${congestionControl}`, `${congestionControl} stream traffic completes`);
        t.ok(client.stats.congestionWindow > 0, `${congestionControl} exposes native congestion window stats`);
      } finally {
        await pipe.close();
      }
    }
  ...
  it('Node transport parity: write drain flushes encoded packets through sendBatch', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { datagrams: { enabled: true, maxFrameSize: 1200 } },
      server: { datagrams: { enabled: true, maxFrameSize: 1200 } },
    });
    try {
      const { client, server } = await pipe.handshake();
      ...
      await Promise.all(Array.from({ length: 8 }, (_, index) => client.sendDatagram(encodeUtf8(`batch-${index}`))));
      await pipe.runUntilSettled();

      t.equal(received.length, 8, 'batched send path preserves queued DATAGRAM payloads');
      t.ok(pipe.transportFactory.sendBatchSizes.some((size) => size > 1), 'transport observed a multi-packet sendBatch call');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('Node transport parity: ECN metadata marks simulator packets as ECT(0)', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { datagrams: { enabled: true, maxFrameSize: 1200 }, transport: { ecn: true } },
      server: { datagrams: { enabled: true, maxFrameSize: 1200 }, transport: { ecn: true } },
    });
    try {
      const { client, server } = await pipe.handshake();
  ...
  it('RFC 9000 sections 4.1 and 4.2: connection flow-control credit unblocks aggregate writes across streams', async (t) => {
    ...
    const topicHandle = topic<any>('quic.stream.blocked').subscribe((event) => blockedTopics.push(event));
    try {
      const { client, server } = await pipe.handshake();
      const streamCount = 4;
      ...
      const serverStreams = [];
      const blockedEvents: Array<{ stream: unknown; connection: unknown }> = [];
      for (let i = 0; i < streamCount; i++) {
        const clientStream = await client.openBidirectionalStream();
        clientStream.addEventListener('blocked', (event: any) => {
          blockedEvents.push({ stream: event.stream, connection: event.connection });
        });
        const serverStreamPromise = server.acceptStream();
      ...
      for (let i = 0; i < serverStreams.length; i++) {
        ...
        t.equal(received.byteLength, payloads[i].byteLength, `stream ${i} receives its full payload after connection credit returns`);
  ...
  it('RFC 9000 section 9: NAT rebinding validates a new peer path and preserves streams', async (t) => {
    ...
    try {
      ...
      t.equal(pathEvent.path.remoteAddress.port, reboundPort, 'path-validation event reports the rebound peer port');
      t.ok(
        pipe.trace().some((event) => (
  ...
  it('RFC 9000 section 9: active migration uses a new local path and preserves streams', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { migration: { enabled: true, usePreferredAddress: true } },
      server: { migration: { enabled: true } },
    });
    try {
      const { client, server } = await pipe.handshake();
      const migratedPort = 55_002;
      const migration = once(server, 'migration');

      await client.migrate({ family: 'ipv4', ip: '10.0.0.1', port: migratedPort });
      ...
      t.equal(migrationEvent.path.remoteAddress.port, migratedPort, 'migration event reports the client migrated port');
      ...
      t.ok(
        pipe.trace().some((event) => event.type === 'datagram:delivered' && event.localAddress.port === migratedPort),
  ...
  it('RFC 9000 section 9: failed active migration falls back to the previous path', async (t) => {
    ...
    try {
      ...
      const migratedAddress = { family: 'ipv4' as const, ip: '10.0.0.1', port: 55_003 };
      ...
      t.equal(pathEvent.result, 'failure', 'client reports failed validation for the unusable migrated path');
      t.equal(client.localAddress.port, oldClientAddress.port, 'client falls back to the previous local path');
      ...
      await clientStream.writer.close();
      const serverStream = await pipe.pumpUntil(serverStreamPromise);
      t.equal(decodeUtf8((await serverStream.reader.read())!), 'after-failed-migration', 'stream data continues over the fallback path');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 9.6: preferred-address migration validates the advertised server path', async (t) => {
    ...
    const preferredAddress = { family: 'ipv4' as const, ip: '10.0.0.3', port: 4434 };
    const pipe = new QuicPipe({
      client: { migration: { enabled: true, usePreferredAddress: true } },
      server: { migration: { enabled: true, preferredAddress } },
    });
    try {
      const { client, server } = await pipe.handshake();
      await pipe.runUntilSettled();
      ...
      await clientStream.writer.close();
      const serverStream = await pipe.pumpUntil(serverStreamPromise);
      t.equal(decodeUtf8((await serverStream.reader.read())!), 'after-preferred-address', 'stream data survives preferred-address migration');
      t.ok(
        pipe.trace().some((traceEvent) => (
  ...
  it('RFC 9000 section 9.6: ignores preferred address unless client opts in', async (t) => {
    ...
    try {
      ...
      await clientStream.writer.close();
      const serverStream = await pipe.pumpUntil(serverStreamPromise);
      t.equal(decodeUtf8((await serverStream.reader.read())!), 'without-preferred-address', 'stream data is delivered without preferred-address migration');
      t.notOk(
        pipe.trace().some((traceEvent) => (
          traceEvent.type === 'datagram:delivered'
          && traceEvent.to.ip === preferredAddress.ip
  ...
  it('RFC 9000 section 9.6: failed preferred-address validation falls back to the original server path', async (t) => {
    ...
    try {
      ...
      t.equal(pathEvent.preferredAddress, true, 'path-validation event identifies preferred-address validation');
  ...
  it('RFC 9002 sections 5 and 6: corrupted stream packets are dropped and retransmitted', async (t) => {
    ...
    try {
      const { client, server } = await pipe.handshake();
      pipe.net.corruptNextDatagrams(1, { from: client.localAddress, to: server.localAddress });
      ...
      t.equal(chunks.join(''), 'recover-after-corruption', 'stream payload is retransmitted after one corrupt packet is dropped');
      t.ok(
        pipe.trace().filter((event) => event.type === 'datagram:delivered' && event.localAddress.port === client.localAddress.port).length >= 2,
  ...
  it('RFC 9221 sections 3 and 5.2: DATAGRAM delivery is negotiated and size-checked', async (t) => {
    ...
    try {
      ...
      t.equal(decodeUtf8(data), 'datagram payload', 'peer receives negotiated unreliable DATAGRAM payload');
      t.equal(ack.status, 'ack', 'sender receives DATAGRAM ACK status separately from payload delivery');
      t.ok(pipe.trace().some((event) => event.type === 'datagram:delivered'), 'simulator trace records datagram packet delivery');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9221 section 5: 0-RTT DATAGRAM rejects when remembered peer parameters omit DATAGRAM support', async (t) => {
    ...
    const sessionStore = memorySessionStore(sessions);
    const pipe = new QuicPipe({
      client: { sessionStore, earlyData: { replaySafe: true, maxBytes: 4096 }, datagrams: { enabled: true } },
      server: { sessionStore: memorySessionStore(new Map()), earlyData: { replaySafe: true, maxBytes: 4096 }, datagrams: { enabled: false } },
    });
    try {
      ...
      const earlyClient = new QuicEndpoint({
        ...
        sessionStore,
        earlyData: { replaySafe: true, maxBytes: 4096 },
  ...
  it('RFC 9221 section 5.3: lost DATAGRAMs are not retransmitted and surface loss status', async (t) => {
    ...
    try {
      ...
      pipe.net.dropNextDatagrams(1, { from: client.localAddress, to: server.localAddress });
      ...
      t.ok(
        pipe.trace().some((traceEvent) => traceEvent.type === 'datagram:dropped' && traceEvent.reason === 'loss'),
  ...
  it('RFC 9221 section 5.3: abandoned DATAGRAM status remains distinct from loss', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { datagrams: { enabled: true, maxFrameSize: 1200, maxPending: 1, dropPolicy: 'drop-newest' } },
      server: { datagrams: { enabled: true, maxFrameSize: 1200 } },
    });
    try {
      const { client } = await pipe.handshake();
      ...
      t.ok(events.includes('abandoned:2:abandoned'), 'abandoned DATAGRAM emits abandoned-specific status');
      t.equal(events.some((event) => event.startsWith('lost:2:')), false, 'abandoned DATAGRAM is not also reported as lost');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 sections 3.5, 4.4, and 4.5: RESET_STREAM surfaces public stream errors', async (t) => {
    ...
    try {
      ...
      t.ok(/77/.test(resetEvent.error.message), 'reset event exposes the RESET_STREAM application code');
      t.equal(resetEvent.errorCode, 77, 'reset event exposes the RESET_STREAM application code as a field');
  ...
  it('RFC 9000 sections 3.5 and 4.4: STOP_SENDING surfaces public stream errors', async (t) => {
    ...
    try {
      ...
      t.equal(decodeUtf8(preserved!), 'cancel-me', 'STOP_SENDING preserves bytes already received before local readable shutdown');
      t.equal(await pipe.pumpUntil(serverStream.reader.read()), null, 'local readable side ends after queued bytes are consumed');
      ...
      t.ok(/55/.test(stopSendingEvent.error.message), 'STOP_SENDING error includes the application code');
      const resetEvent = await pipe.pumpUntil(reset);
      t.ok(
        pipe.trace().slice(traceBeforeStop).some((event) => event.type === 'datagram:queued' && event.localAddress.port === server.localAddress.port),
        'STOP_SENDING queues control traffic toward the peer',
      );

      t.ok(/55/.test(resetEvent.error.message), 'peer RESET_STREAM response keeps the STOP_SENDING application code distinct');
  ...
  it('RFC 9000 sections 3.5 and 4.4: STOP_SENDING is legal on receive-only unidirectional streams', async (t) => {
    ...
    try {
      ...
      t.equal(serverStream.direction, 'unidirectional', 'local send-only stream exposes unidirectional direction');
      ...
      t.equal(decodeUtf8((await pipe.pumpUntil(clientStream.reader.read()))!), 'server-push', 'queued receive-only bytes remain readable');
      t.equal(await pipe.pumpUntil(clientStream.reader.read()), null, 'receive-only stream ends after STOP_SENDING');
      const localStop = await pipe.pumpUntil(localStopSending);
      await pipe.runUntilSettled();

      t.equal(localStop.errorCode, 66, 'local receive-only STOP_SENDING event exposes the application code');
  ...
  it('RFC 9001 key update behavior: controlled key update preserves stream traffic', async (t) => {
    ...
    try {
      ...
      t.equal(decodeUtf8((await readyServerStream.reader.read())!), 'ready', 'server reads 1-RTT data before key update');
      ...
      t.equal(decodeUtf8((await postServerStream.reader.read())!), 'after-key-update', 'stream traffic works after the controlled key update');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9001 key update behavior: peer-initiated key update preserves stream traffic', async (t) => {
    ...
    try {
      ...
      t.equal(decodeUtf8((await postServerStream.reader.read())!), 'after-peer-key-update', 'stream traffic works after a peer-initiated key update');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10.2: application close is peer-visible under simulator delivery', async (t) => {
    ...
    try {
      const { client, server } = await pipe.handshake();
      const closed = new Promise((resolve) => server.addEventListener('close', () => resolve('closed'), { once: true }));

      await client.close({ errorCode: 42, reason: 'simulated shutdown' });
  ...
  it('RFC 9000 section 10.2: peer transport close rejects operations without stateless reset', async (t) => {
    ...
    try {
      ...
      await t.rejects(() => server.sendDatagram(encodeUtf8('after-transport-close')), /closed|connected/i, 'DATAGRAM send rejects after transport close');
      await t.rejects(() => server.acceptStream(), /closed/i, 'pending stream accept rejects after transport close');
      t.equal(
        pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued' && event.localAddress.port === server.localAddress.port),
        false,
  ...
  it('RFC 9000 section 10.2: protocol-error transport close is peer-visible', async (t) => {
    ...
    try {
      ...
      await t.rejects(() => client.sendDatagram(encodeUtf8('after-protocol-close')), /closed|connected/i, 'client DATAGRAM send rejects after protocol-error close');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10.1: idle timeout closes connections and rejects stream operations', async (t) => {
    ...
    try {
      ...
      await t.rejects(() => client.openBidirectionalStream(), 'opening a stream rejects after idle close');
      await t.rejects(() => client.sendDatagram(encodeUtf8('after-idle')), 'DATAGRAM send rejects after idle close');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('Node transport parity: peer-initiated stream idle timeout closes only the idle stream', async (t) => {
    ...
    try {
      ...
      t.equal(client.stats.streamsIdleTimedOut, 1, 'client counts the peer-initiated idle stream timeout');
      ...
      t.equal(decodeUtf8((await activeServerStream.reader.read())!), 'still-active', 'unrelated streams remain usable after stream idle timeout');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10.1: handshake timeout rejects pending connect and accept', async (t) => {
    ...
    try {
      const listener = await pipe.listen();
      const accepted = trackPromise(pipe.server.accept());
      const connected = trackPromise(pipe.client.connect({
        address: listener.address,
        serverName: 'localhost',
      }));

      await pipe.pumpUntilCondition(() => (
        pipe.trace().some((event) => event.type === 'datagram:dropped' && event.reason === 'loss') ? true : null
      ), 20);
      pipe.advance(10_001);
      await pipe.runUntilSettled();
      await Promise.resolve();

      t.equal(accepted.settled(), false, 'server accept remains pending when no Initial reaches the listener');
      ...
      t.equal(pipe.server.cidTable.get(''), undefined, 'handshake timeout does not leak empty CID routes');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10: draining retains known CIDs before removing routes', async (t) => {
    ...
    try {
      ...
      await client.close({ errorCode: 7, reason: 'draining' });
      await pipe.runUntilIdle();

      t.equal(pipe.client.cidTable.get(routeCid), client, 'closed connection route is retained during draining');
      pipe.advance(3_001);
      await pipe.runUntilIdle();
      t.equal(pipe.client.cidTable.get(routeCid), undefined, 'closed connection route is removed after the draining period');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('Node transport parity: custom drainingPeriodMultiplier extends draining route retention', async (t) => {
    ...
    try {
      ...
      await client.close({ errorCode: 7, reason: 'custom-draining' });
      await pipe.runUntilIdle();

      t.equal(pipe.client.cidTable.get(routeCid), client, 'closed connection route is retained during draining');
      pipe.advance(3_001);
      await pipe.runUntilIdle();
      t.equal(pipe.client.cidTable.get(routeCid), client, 'custom draining multiplier keeps the route beyond the old fixed 3000ms timeout');
      pipe.advance(60_000);
      await pipe.runUntilIdle();
      t.equal(pipe.client.cidTable.get(routeCid), undefined, 'closed connection route is eventually removed after the custom draining period');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 sections 10 and 10.3: draining ignores late packets without stateless reset', async (t) => {
    ...
    const pipe = new QuicPipe();
    const probe = await pipe.net.datagram({ family: 'ipv4', ip: '10.0.0.9', port: 0 });
    try {
      const { client } = await pipe.handshake();
      const routeCid = client.routeCids[0];
      if (routeCid === undefined) throw new Error('client did not register a route CID');
      ...
      latePacket.fill(0xa5, 1 + cid.byteLength);

      await client.close({ errorCode: 9, reason: 'draining-late-packet' });
      await pipe.runUntilIdle();
      const traceBefore = pipe.trace().length;
      ...
      t.equal(pipe.client.cidTable.get(routeCid), client, 'known CID still routes to the draining connection');
      t.equal(drainDatagrams(probe).length, 0, 'late packet for a draining CID does not elicit a stateless reset');
      t.equal(
        pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued' && event.localAddress.port === client.localAddress.port),
        false,
  ...
  it('RFC 9000 section 10.2.1: closing endpoint retransmits CONNECTION_CLOSE with backoff', async (t) => {
    ...
    try {
      ...
      await server.close({ errorCode: 15, reason: 'closing-period-retransmit' });
      await pipe.runUntilIdle();
      const traceBefore = pipe.trace().length;
      ...
      const retransmits = pipe.trace().slice(traceBefore).filter((event) => (
        event.type === 'datagram:queued'
        && event.localAddress.port === server.localAddress.port
        && event.to.port === probe.address.port
      ));
      t.equal(retransmits.length, 2, 'closed local endpoint retransmits close on packet counts 1 and 2, then backs off before 4');
      t.ok(retransmits.every((event) => event.bytes > 0), 'retransmitted close packets carry bytes');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 section 5.1.1: active CID limit migration rotation retires old CIDs and replenishes the pool', async (t) => {
    ...
    const pipe = new QuicPipe({
      client: { migration: { enabled: true }, connection: { activeConnectionIdLimit: 2 } },
      server: { migration: { enabled: true }, connection: { activeConnectionIdLimit: 2 } },
    });
    try {
      const { client, server } = await pipe.handshake();
      ...
      const migration = once(server, 'migration');
      await client.migrate({ family: 'ipv4', ip: '10.0.0.1', port: 55_004 });
      const migrationEvent = await pipe.pumpUntil(migration);
      t.equal(migrationEvent.result, 'success', 'migration succeeds with activeConnectionIdLimit: 2');
      t.equal(migrationEvent.path.remoteAddress.port, 55_004, 'migration event records the new client port');
      ...
      const serverStream = await pipe.pumpUntil(serverStreamPromise);
      const data = await pipe.pumpUntil(serverStream.reader.read());
      t.equal(decodeUtf8(data!), 'post-migration-routing', 'stream data routes correctly after CID rotation');
      t.ok(
        pipe.trace().some((e) => e.type === 'datagram:delivered' && e.localAddress.port === 55_004),
