...
function bytesQueued(trace: ReturnType<QuicPipe['trace']>, fromPort: number, toPort: number): number {
  return trace.filter((event) => event.type === 'datagram:queued' && event.from.port === fromPort && event.to.port === toPort).reduce((total, event) => total + event.bytes, 0);
}
...
function newSimClient(pipe: QuicPipe, localAddress: QuicAddress, options: ConstructorParameters<typeof QuicEndpoint>[0] = {}): QuicEndpoint {
...
async function connectSimClient(pipe: QuicPipe, clientEndpoint: QuicEndpoint): Promise<QuicConnection> {
...
describe('QUIC simulator conformance', () => {
  it('RFC 9000 sections 6, 7, and 14.1: v2 handshake completes and Initial datagrams are at least 1200 bytes', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((event) => event.type === 'datagram:queued' && event.bytes >= 1200), 'client Initial datagram is padded to at least 1200 bytes');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9002 sections 6.2 and 6.2.1: dropped first flight recovers through PTO', async (t) => {
    ...
    try {
      const { client, server } = await pipe.handshake();
      const droppedInitials = pipe.trace().filter((event) => event.type === 'datagram:dropped' && event.reason === 'loss' && event.bytes >= 1200);
  ...
  it('Node transport parity: source-address allow filtering blocks non-matching packets', async (t) => {
    if (!quicAvailable) return;
    const pipe = new QuicPipe({ server: { transport: { sourceAddress: { allow: ['10.0.0.2'] } } } });
  ...
  it('Node transport parity: maxConnections gates excess accepted server sessions', async (t) => {
    if (!quicAvailable) return;
    const pipe = new QuicPipe({ server: {
      retry: false,
    ...
    try {
      ...
      secondClient = newSimClient(pipe, {
        family: 'ipv4',
        ip: '10.0.0.3',
        port: 55111
      }, { versions: ['v1'] });
      await startSimConnect(pipe, secondClient);
      await pipe.pumpUntilCondition(() => pipe.server.stats.connectionLimitPackets > 0 && pipe.server.stats.immediateCloseSent > 0 ? pipe.server.stats : null);
      const stats = pipe.server.stats;
  ...
  it('Node transport parity: maxConnectionsPerRemoteAddress gates only the matching remote address', async (t) => {
    ...
    try {
      ...
      blockedClient = newSimClient(pipe, {
        family: 'ipv4',
        ip: '10.0.0.1',
        port: 55131
      }, { versions: ['v1'] });
  ...
  it('Node transport parity: session creation rate-limit is counted separately from packetsBlocked', async (t) => {
    ...
    try {
      ...
      await pipe.pumpUntilCondition(() => pipe.server.stats.serverConnections > 0 ? pipe.server.stats : null);
      ...
      await pipe.pumpUntilCondition(() => pipe.server.stats.sessionCreationRateLimited > 0 ? pipe.server.stats : null);
      const stats = pipe.server.stats;
  ...
  it('Node transport parity: configurable Retry rate limit reports sent and limited packets separately', async (t) => {
    if (!quicAvailable) return;
    const pipe = new QuicPipe({ server: { transport: { retryRateLimit: {
      rate: 0,
      burst: 1
    } } } });
    const probe = await pipe.net.datagram({
  ...
  it('Node transport parity: resumed 0-RTT connect defers packets until first early write', async (t) => {
    ...
    try {
      ...
      resumedClient = newSimClient(pipe, {
        family: 'ipv4',
        ip: '10.0.0.1',
        port: 55120
      }, {
      ...
      t.ok(pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued'), 'application data queues the first 0-RTT flight');
    } finally {
  ...
  it('RFC 9000 sections 12.3 and 13.4: duplicate packets do not duplicate stream bytes', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().slice(traceBeforeStream).some((event) => event.type === 'datagram:duplicated'), 'simulator duplicated at least one application packet');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9002 congestion-control selections complete stream traffic', async (t) => {
    if (!quicAvailable) return;
    for (const congestionControl of [
      'cubic',
      'reno',
      'bbr'
    ] as const) {
  ...
  it('Node transport parity: ECN metadata marks simulator packets as ECT(0)', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((event) => event.type === 'datagram:queued' && event.ecn === 2), 'simulator trace records ngtcp2 ECT(0) packet marking');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 9: NAT rebinding validates a new peer path and preserves streams', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((event) => event.type === 'datagram:delivered' && event.localAddress.port === client.localAddress.port && event.from.port === reboundPort && event.to.port === server.localAddress.port), 'simulator trace shows client packets arriving from the rebound source port');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9000 section 9: active migration uses a new local path and preserves streams', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((event) => event.type === 'datagram:delivered' && event.localAddress.port === migratedPort), 'simulator trace shows packets sent from the migrated local port');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 9.6: preferred-address migration validates the advertised server path', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((traceEvent) => traceEvent.type === 'datagram:delivered' && traceEvent.to.ip === preferredAddress.ip && traceEvent.to.port === preferredAddress.port), 'simulator delivered traffic to the preferred server address');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9000 section 9.6: ignores preferred address unless client opts in', async (t) => {
    ...
    try {
      ...
      t.notOk(pipe.trace().some((traceEvent) => traceEvent.type === 'datagram:delivered' && traceEvent.to.ip === preferredAddress.ip && traceEvent.to.port === preferredAddress.port), 'client does not send traffic to the advertised preferred address by default');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9000 section 9.6: failed preferred-address validation falls back to the original server path', async (t) => {
    ...
    try {
      ...
      await clientStream.writer.close();
      const serverStream = await pipe.pumpUntil(serverStreamPromise);
      t.equal(decodeUtf8((await serverStream.reader.read())!), 'after-failed-preferred-address', 'stream data continues on the original server path after preferred-address failure');
      t.ok(pipe.trace().some((traceEvent) => traceEvent.type === 'datagram:delivered' && traceEvent.to.ip === pipe.listener!.address.ip && traceEvent.to.port === pipe.listener!.address.port), 'traffic remains deliverable to the original listener address');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9002 sections 5 and 6: corrupted stream packets are dropped and retransmitted', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().filter((event) => event.type === 'datagram:delivered' && event.localAddress.port === client.localAddress.port).length >= 2, 'client sends more than one datagram after the corrupted packet is ignored');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9221 sections 3 and 5.2: DATAGRAM delivery is negotiated and size-checked', async (t) => {
    ...
    try {
      ...
      await t.rejects(() => client.sendDatagram(new Uint8Array(65536)), /exceeds/, 'oversized DATAGRAM payload rejects before send');
  ...
  it('RFC 9221 section 5: 0-RTT DATAGRAM rejects when remembered peer parameters omit DATAGRAM support', async (t) => {
    ...
    try {
      ...
      try {
        ...
        await t.rejects(() => early.sendDatagram(encodeUtf8('early-datagram')), /peer did not negotiate QUIC DATAGRAM support/, '0-RTT DATAGRAM rejects from remembered peer parameters without DATAGRAM support');
  ...
  it('RFC 9221 section 5.3: lost DATAGRAMs are not retransmitted and surface loss status', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().some((traceEvent) => traceEvent.type === 'datagram:dropped' && traceEvent.reason === 'loss'), 'simulator trace records the DATAGRAM packet loss');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9221 section 5.3: abandoned DATAGRAM status remains distinct from loss', async (t) => {
    ...
    try {
      ...
      await Promise.all([client.sendDatagram(encodeUtf8('queued-one')), client.sendDatagram(encodeUtf8('queued-two'))]);
      await pipe.runUntilSettled();
  ...
  it('RFC 9000 sections 3.5, 4.4, and 4.5: RESET_STREAM surfaces public stream errors', async (t) => {
    ...
    try {
      ...
      await t.rejects(() => serverStream.reader.read(), /reset: 77/, 'server reader rejects with the RESET_STREAM application code');
  ...
  it('RFC 9000 sections 3.5 and 4.4: STOP_SENDING surfaces public stream errors', async (t) => {
    ...
    try {
      ...
      t.ok(pipe.trace().slice(traceBeforeStop).some((event) => event.type === 'datagram:queued' && event.localAddress.port === server.localAddress.port), 'STOP_SENDING queues control traffic toward the peer');
      ...
      await pipe.runUntilSettled();
      await t.rejects(() => clientStream.writer.write(encodeUtf8('after-stop-again')), /closed|stop sending/i, 'peer writer rejects future writes after native STOP_SENDING shutdown is drained');
    } finally {
      await pipe.close();
    }
  });
  it('RFC 9000 sections 3.5 and 4.4: STOP_SENDING is legal on receive-only unidirectional streams', async (t) => {
    ...
    try {
      ...
      await pipe.runUntilSettled();
      await t.rejects(() => serverStream.writer.write(encodeUtf8('after-unidirectional-stop-again')), /closed|stop sending/i, 'peer send-only writer rejects after STOP_SENDING is drained');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10.2: peer transport close rejects operations without stateless reset', async (t) => {
    ...
    try {
      ...
      t.equal(pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued' && event.localAddress.port === server.localAddress.port), false, 'known peer transport close does not cause stateless reset traffic from the closed peer');
    } finally {
      await pipe.close();
    }
  });
  ...
  it('RFC 9000 section 10.1: handshake timeout rejects pending connect and accept', async (t) => {
    ...
    try {
      ...
      await pipe.pumpUntilCondition(() => pipe.trace().some((event) => event.type === 'datagram:dropped' && event.reason === 'loss') ? true : null, 20);
      pipe.advance(10001);
      await pipe.runUntilSettled();
  ...
  it('RFC 9000 sections 10 and 10.3: draining ignores late packets without stateless reset', async (t) => {
    ...
    try {
      ...
      t.equal(pipe.trace().slice(traceBefore).some((event) => event.type === 'datagram:queued' && event.localAddress.port === client.localAddress.port), false, 'draining connection does not queue response packets for late traffic');
    } finally {
      probe.close();
      await pipe.close();
    }
  ...
  it('RFC 9000 section 10.2.1: closing endpoint retransmits CONNECTION_CLOSE with backoff', async (t) => {
    ...
    try {
      ...
      const retransmits = pipe.trace().slice(traceBefore).filter((event) => event.type === 'datagram:queued' && event.localAddress.port === server.localAddress.port && event.to.port === probe.address.port);
  ...
  it('RFC 9000 section 5.1.1: active CID limit migration rotation retires old CIDs and replenishes the pool', async (t) => {
    ...
    try {
      const { client, server } = await pipe.handshake();
      t.equal(client.remoteTransportParameters.activeConnectionIdLimit, 2, 'server advertises activeConnectionIdLimit of 2');
      const migration = once(server, 'migration');
      await client.migrate({
        family: 'ipv4',
        ip: '10.0.0.1',
      ...
      t.equal(migrationEvent.path.remoteAddress.port, 55004, 'migration event records the new client port');
      ...
      t.ok(pipe.trace().some((e) => e.type === 'datagram:delivered' && e.localAddress.port === 55004), 'packets delivered through the migrated local address (old CID route replaced)');
    } finally {
      await pipe.close();
    }
  });
