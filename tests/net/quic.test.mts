...
import {
  ...
  Pointer,
  sym as ngtcp2Sym,
} from '../../js/internal/net/quic/ngtcp2/bindings.mts';
import { sym as cryptoSym } from '../../js/internal/net/quic/ngtcp2/crypto.mts';
...
function testListenOptions<T extends Record<string, unknown>>(options: T): T & { certificateFile: string; privateKeyFile: string } {
  return {
    ...options,
...
function makeInitialProbe(
  ...
  options: { dcid?: Uint8Array; scid?: Uint8Array; version?: number } = {},
): Uint8Array {
  ...
  if (options.dcid === undefined || options.scid === undefined) {
    for (let i = 0; i < 8; i++) {
      if (options.dcid === undefined) dcid[i] = (0x40 + seed + i) & 0xff;
      if (options.scid === undefined) scid[i] = (0x80 + seed + i) & 0xff;
    }
  }
...
function parseRetryPacket(packet: Uint8Array): { dcid: Uint8Array; scid: Uint8Array; token: Uint8Array; version: number } {
...
describe('QUIC bindings', () => {
  it('exports availability and version metadata', (t) => {
    ...
    t.ok(
      quicAvailable ? cryptoBackend === 'ossl' || cryptoBackend === 'gnutls' : cryptoBackend === null,
  ...
  it('keeps conformance matrix statuses canonical and fixture rows mapped', async (t) => {
    const matrix = await fs.readFile('research-docs/research/quic-conformance-matrix.md');
    const fixtures = JSON.parse(await fs.readFile('tests/net/fixtures/quic/spec-fixtures.json')) as {
      packetCases: Array<{ matrixRow: string }>;
    ...
    const requiredPlanRows = [
      ...
      'Unidirectional MAX_STREAMS exhaustion',
      'Expired Retry token',
      'Client Version Negotiation bad-CID rejection',
      'Client incompatible Version Negotiation failure',
      'RFC 9368 compatible Version Negotiation retry',
      'Key-updated traffic under loss',
      'Parsed TLS keylog validity',
      'ACK-frequency threshold effect',
      'Blackhole recovery and RTT stats',
      'Path-validation abort on close',
      'Active CID limit migration rotation',
      'Node interop DATAGRAM scenario',
      'Node interop resumption scenario',
      'Node interop key-update scenario',
      'Node interop Retry scenario',
      'Node interop ALPN mismatch',
      'Node interop migration and Version Negotiation',
      'Before/after QUIC throughput benchmark',
    ];
    ...
    for (const testCase of [...fixtures.packetCases, ...fixtures.simulatorCases]) {
      t.equal(rows.has(testCase.matrixRow), true, `fixture matrix row exists: ${testCase.matrixRow}`);
    }
    for (const requiredRow of requiredPlanRows) {
      t.equal(rows.has(requiredRow), true, `original alignment plan row is represented: ${requiredRow}`);
    }
    const evidenceRows = new Set((fixtures.evidence ?? []).map((entry) => entry.matrixRow));
    for (const row of directRows) {
      t.equal(evidenceRows.has(row), true, `Direct matrix row has fixture evidence: ${row}`);
    }
    for (const entry of fixtures.evidence ?? []) {
      t.equal(rows.has(entry.matrixRow), true, `evidence matrix row exists: ${entry.matrixRow}`);
      t.ok(entry.testFile.length > 0, `evidence test file is non-empty for ${entry.matrixRow}`);
      t.ok(entry.testName.length > 0, `evidence test name is non-empty for ${entry.matrixRow}`);
      const source = await fs.readFile(entry.testFile);
      t.ok(
        source.includes(`it('${entry.testName}'`) || source.includes(`it("${entry.testName}"`) || source.includes(entry.testName),
        `evidence test exists: ${entry.testFile} :: ${entry.testName}`,
      );
    }
  });
});
...
describe('QUIC event classes', () => {
  it('carry typed connection, stream, and error payloads', (t) => {
    ...
    const datagramEvent = new QuicDatagramEvent('datagram', { data: datagram, earlyData: true });
    const datagramStatusEvent = new QuicDatagramStatusEvent('datagramstatus', { id: 7, status: 'ack' });
    const newTokenEvent = new QuicNewTokenEvent('newtoken', {
      token: new Uint8Array([5, 6]),
      address: { family: 'ipv4', ip: '127.0.0.1', port: 4433 },
    });
    const earlyDataEvent = new QuicEarlyDataEvent('earlydata', { accepted: true, rejected: false, reason: 'accepted' });
    ...
    const pathValidationEvent = new QuicPathValidationEvent('pathvalidation', {
      result: 'success',
      path: {
        localAddress: { family: 'ipv4', ip: '127.0.0.1', port: 4433 },
        remoteAddress: { family: 'ipv4', ip: '127.0.0.1', port: 4434 },
      },
      previousPath: null,
...
describe('QUIC hardening options', () => {
  it('uses Node-aligned transport defaults while keeping 0-RTT opt-in', (t) => {
    ...
    t.deepEqual(endpoint.migration, { enabled: false, usePreferredAddress: false }, 'active migration and preferred-address use are disabled by default');
    ...
    t.equal(endpoint.transport.maxConnections, 10_000, 'default total connection limit matches Node');
    t.equal(endpoint.transport.maxConnectionsPerRemoteAddress, 100, 'default per-remote connection limit matches Node');
    t.equal(endpoint.transport.retryTokenTimeoutMs, 10_000, 'Retry token timeout defaults to 10 seconds');
    t.equal(endpoint.transport.addressTokenTimeoutMs, 10_000, 'regular address token timeout defaults to 10 seconds');
  ...
  it('sends keep-alive PINGs only when keep-alive is enabled', async (t) => {
    ...
    const activeListener = await activeServer.listen(testListenOptions({
    ...
    const idleListener = await idleServer.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
  ...
  it('clamps transport token expiries to Node-compatible ranges', (t) => {
    const endpoint = new QuicEndpoint({
      transport: {
        ...
        addressTokenTimeoutMs: 10 * 60 * 1000,
      },
    });

    t.equal(endpoint.transport.retryTokenTimeoutMs, 1_000, 'Retry token timeout clamps to at least one second');
    t.equal(endpoint.transport.addressTokenTimeoutMs, 5 * 60 * 1000, 'regular address token timeout clamps to five minutes');
  });
  ...
  it('requires replay-safe opt-in and session storage before enabling early data', (t) => {
    ...
    t.throws(
      () => new QuicEndpoint({
        sessionStore: { load: async () => null, save: async () => {}, delete: async () => {} },
  ...
  it('validates endpoint UDP socket options', async (t) => {
    ...
    t.throws(
      () => new QuicEndpoint({ socket: { ttl: 300 } }),
      /ttl/,
      'TTL is bounded to one byte',
    );
    if (!quicAvailable) return;
...
describe('QUIC endpoint lifecycle', () => {
  ...
  it('listener answers unsupported-version Initial probes with Version Negotiation', async (t) => {
    ...
    setNonblocking(fd);
    socketBind(fd, { family: 'ipv4', ip: '127.0.0.1', port: 0 });

    try {
      const clientDcid = new Uint8Array([0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47]);
      const clientScid = new Uint8Array([0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87]);
      ...
      t.equal(response[0] & 0x80, 0x80, 'response uses long header form');
  ...
  it('listener filters Version Negotiation advertisements to configured supported versions', async (t) => {
    ...
    const endpoint = new QuicEndpoint({ alpnProtocols: ['fino-hq'], versions: ['v1'] });
    const listener = await endpoint.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      versions: ['v1'],
    }));
    ...
    try {
      const probe = makeInitialProbe(22, new Uint8Array(), { version: 0x57414954 });
  ...
  it('listener ignores long-header Version Negotiation packets without stateless reset', async (t) => {
    ...
    try {
      ...
      packet[5] = 8;
      for (let i = 0; i < 8; i++) packet[6 + i] = 0x40 + i;
      packet[14] = 8;
      for (let i = 0; i < 8; i++) packet[15 + i] = 0x80 + i;
  ...
  it('rate-limits Version Negotiation responses for unsupported-version floods', async (t) => {
    ...
    socketBind(responseFd, { family: 'ipv4', ip: '127.0.0.1', port: 0 });
    socketBind(sendFd, { family: 'ipv4', ip: '127.0.0.1', port: 0 });
    ...
    try {
      ...
      for (let attempt = 0; attempt < attempts; attempt++) {
        ...
        probe[0] = 0xc0;
        writeU32BE(probe, 1, 0x57414954);
        probe[5] = 8;
        for (let i = 0; i < 8; i++) probe[6 + i] = (attempt + i) & 0xff;
        probe[14] = 8;
        for (let i = 0; i < 8; i++) probe[15 + i] = (0x80 + attempt + i) & 0xff;
  ...
  it('listener sends Retry by default for an unvalidated Initial', async (t) => {
    ...
    try {
      ...
      t.equal(response[0] & 0xf0, 0xf0, 'response is a QUIC v1 Retry packet');
  ...
  it('listener rejects tampered and address-mismatched Retry tokens', async (t) => {
    ...
    setNonblocking(fdA);
    setNonblocking(fdB);
    socketBind(fdA, { family: 'ipv4', ip: '127.0.0.1', port: 0 });
    socketBind(fdB, { family: 'ipv4', ip: '127.0.0.1', port: 0 });
    ...
    try {
      ...
      tamperedToken[tamperedToken.byteLength - 1] ^= 0x01;
      const retryDcid = retry.scid;
      const retryScid = retry.dcid;
      t.equal(
        sendto(fdA, makeInitialProbe(11, tamperedToken, { dcid: retryDcid, scid: retryScid }), listener.address),
        1200,
      ...
      t.notEqual(tamperedResponse[0] & 0xf0, 0xf0, 'tampered Retry token receives immediate close instead of fresh Retry');

      const wrongRetryDcid = retryDcid.slice();
      wrongRetryDcid[wrongRetryDcid.byteLength - 1] ^= 0x02;
      t.equal(
        sendto(fdA, makeInitialProbe(13, retry.token, { dcid: wrongRetryDcid, scid: retryScid }), listener.address),
        1200,
      ...
      t.notEqual(wrongDcidResponse[0] & 0xf0, 0xf0, 'Retry-token DCID mismatch receives immediate close instead of fresh Retry');

      t.equal(
        sendto(fdB, makeInitialProbe(12, retry.token, { dcid: retryDcid, scid: retryScid }), listener.address),
        1200,
      ...
      t.notEqual(addressMismatchResponse[0] & 0xf0, 0xf0, 'address-mismatched Retry token receives immediate close instead of fresh Retry');
  ...
  it('listener rejects expired Retry tokens with an immediate close', async (t) => {
    ...
    const endpoint = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      transport: { retryTokenTimeoutMs: 1_000 },
    });
    const listener = await endpoint.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
    ...
    try {
      ...
      if (firstResponse === null) throw new Error('listener did not send initial Retry response');
      const retry = parseRetryPacket(firstResponse);
      await loop.timeout(1_050);

      t.equal(
        sendto(fd, makeInitialProbe(15, retry.token, { dcid: retry.scid, scid: retry.dcid }), listener.address),
        1200,
      ...
      t.notEqual(expiredResponse[0] & 0xf0, 0xf0, 'expired Retry token receives immediate close instead of fresh Retry');
  ...
  it('listener close does not close the endpoint', async (t) => {
    ...
    const second = await endpoint.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
...
describe('QUIC loopback object model', () => {
  ...
  it('keeps a large stream open until a late missing range arrives before FIN', async (t) => {
    ...
    const gapStart = 6_685_332;
    const gapEnd = gapStart + 2_599;
    const stream = new QuicStream(0, 'bidirectional', null as any);
    ...
    t.equal(checksum, 484_562_562, 'late range and tail bytes preserve order');
  });

  it('reaches FIN after overlapping retransmits fill a late stream gap', async (t) => {
    const body = new Uint8Array(512 * 1024);
    for (let i = 0; i < body.byteLength; i++) body[i] = (i * 17) & 0xff;

    const gapStart = 180_000;
    const gapEnd = 183_000;
    const stream = new QuicStream(0, 'bidirectional', null as any);
    const chunks: Uint8Array[] = [];
    let total = 0;
    ...
    let checksum = 0;
    for (;;) {
      const chunk = await Promise.race([stream.reader.read(), timeoutValue(1000, undefined)]);
    ...
    t.equal(checksum, 66_846_720, 'reassembled bytes preserve order and duplicates are not exposed');
  });
  ...
  it('pre-closes unavailable unidirectional stream sides', async (t) => {
    ...
    t.deepEqual(await readableReader.read(), { value: undefined, done: true }, 'local send-only Web readable is closed');
  ...
  it('coalesces stream FIN with pending write data', async (t) => {
    const queued: { stream: QuicStream; data: Uint8Array; fin: boolean }[] = [];
    const stream = new QuicStream(0, 'bidirectional', {
      _queueStreamData(stream: QuicStream, data: Uint8Array, fin: boolean) {
        queued.push({ stream, data, fin });
      },
    } as any);
  ...
  it('supports async disposal for endpoints, listeners, and connections', async (t) => {
    ...
    const disposableListener = await listenerEndpoint.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
  ...
  it('negotiates QUIC TLS with configured supported groups', async (t) => {
    ...
    const server = new QuicEndpoint({ alpnProtocols: ['fino-hq'], tlsGroups: ['X25519'] });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      tlsGroups: ['X25519'],
    }));
    const client = new QuicEndpoint({ alpnProtocols: ['fino-hq'], tlsGroups: ['X25519'] });

    try {
      const accepted = server.accept();
      const clientConnection = await client.connect({ address: listener.address, tlsGroups: ['X25519'] });
      const serverConnection = await accepted;
  ...
  it('negotiates h3 ALPN over raw QUIC streams without HTTP integration', async (t) => {
    ...
    try {
      const accepted = server.accept();
      const clientConnection = await client.connect({ address: listener.address, alpnProtocols: ['h3'] });
      const serverConnection = await accepted;
  ...
  it('routes simultaneous connections for multiple listeners on one endpoint', async (t) => {
    ...
    const server = new QuicEndpoint({ alpnProtocols: ['fino-hq'] });
    const listenerA = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
    const listenerB = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
  ...
  it('returns resumed early-data connections before handshake completion', async (t) => {
    ...
    const server = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      sessionStore,
      earlyData: { replaySafe: true, maxBytes: 1024 * 1024 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      sessionStore,
    ...
    try {
      ...
      const earlyDataEvent = await Promise.race([
        new Promise<any>((resolve) => connection.addEventListener('earlydata', resolve, { once: true })),
  ...
  it('enforces the configured 0-RTT byte limit before handshake completion', async (t) => {
    ...
    const server = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      sessionStore,
      earlyData: { replaySafe: true, maxBytes: 16 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      sessionStore,
  ...
  it('sends DATAGRAM frames as 0-RTT data before handshake completion', async (t) => {
    ...
    const server = new QuicEndpoint({
      ...
      sessionStore,
      earlyData: { replaySafe: true, maxBytes: 16 },
      datagrams: { enabled: true, maxFrameSize: 1200 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      sessionStore,
  ...
  it('reports rejected early data for incompatible stored transport parameters', async (t) => {
    ...
    sessions.set('localhost|fino-hq', {
      ...state,
      transportParameters: new Uint8Array([0xff, 0x00, 0xff]),
    });
    ...
    const cases = [
      {
        ...
        versions: undefined,
        state: {
          ticket: new Uint8Array([0x01, 0x02, 0x03]),
          transportParameters: new Uint8Array([0x00]),
          earlyDataMax: 1024,
          version: 'v2',
      ...
      },
      {
        ...
        state: {
          ticket: new Uint8Array([0x04, 0x05, 0x06]),
          transportParameters: new Uint8Array([0x00]),
          earlyDataMax: 1024,
          version: 'v2',
        },
  ...
  it('persists received NEW_TOKEN address-validation tokens with session state', async (t) => {
    ...
    const client = new QuicEndpoint({ alpnProtocols: ['fino-hq'], sessionStore });
    ...
    try {
      ...
      clientConnection.addEventListener('sessionticket', () => { sessionTicketEvents++; });
  ...
  it('publishes Node-aligned QUIC transport lifecycle topics through Fino topics', async (t) => {
    ...
    try {
      ...
      clientConnection._onNewToken(new Uint8Array([3, 4, 5]));
      ...
      t.ok(
        saw('quic.session.handshake', (event) => (
      ...
      clientConnection._onVersionNegotiationForTest(0x00000001, [0x00000001, 0x709a50c4], [0x00000001, 0x709a50c4]);
      t.ok(
        saw('quic.session.version.negotiation', (event) => (
          event.connection === clientConnection
          && event.wireVersion === 0x00000001
          && event.requestedWireVersions?.includes(0x709a50c4)
        )),
  ...
  it('advertises peer active migration while keeping local migration API gated', async (t) => {
    ...
    try {
      ...
      await t.rejects(
        () => clientConnection.migrate({ family: 'ipv4', ip: '0.0.0.0', port: 0 }),
  ...
  it('advertises configured transport parameters to the peer', async (t) => {
    ...
    try {
      ...
      t.equal(Number(Pointer.readU64(serverParams, TP_MAX_IDLE_TIMEOUT) / 1_000_000n), 1750, 'native server params carry configured idle timeout');
      t.equal(Number(Pointer.readU64(serverParams, TP_ACTIVE_CONNECTION_ID_LIMIT)), 8, 'native server params carry clamped CID limit');
      t.equal(Number(Pointer.readU64(serverParams, TP_MAX_ACK_DELAY) / 1_000_000n), 17, 'native server params carry configured max ACK delay');
  ...
  it('derives preferred-address stateless reset tokens from the listener reset secret', async (t) => {
    ...
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      retry: { enabled: true, tokenSecret },
      migration: {
        enabled: true,
        preferredAddress: { family: 'ipv4', ip: '127.0.0.1', port: 4433 },
      },
    }));
    const client = new QuicEndpoint({ alpnProtocols: ['fino-hq'] });
  ...
  it('surfaces NEW_TOKEN requests on path-validation events', async (t) => {
    ...
    try {
      ...
      clientConnection._onPathValidationFinished(
        null,
        null,
        NGTCP2_PATH_VALIDATION_RESULT_SUCCESS,
        NGTCP2_PATH_VALIDATION_FLAG_NEW_TOKEN,
  ...
  it('encodes RFC-required transport parameter invariants', async (t) => {
    ...
    try {
      ...
      t.equal(Pointer.readU64(clientParams, TP_MAX_ACK_DELAY), 25_000_000n, 'client max_ack_delay is encoded in nanoseconds');
      t.equal(Pointer.readU64(serverParams, TP_MAX_ACK_DELAY), 25_000_000n, 'server max_ack_delay is encoded in nanoseconds');
  ...
  it('writes qlog output when qlog path is configured', async (t) => {
    ...
    const fs = new DiskFileSystem();
    const qlogPath = `/tmp/fino-quic-qlog-${Date.now()}-${Math.floor(Math.random() * 1_000_000)}.sqlog`;
    const server = new QuicEndpoint({ alpnProtocols: ['fino-hq'], qlog: { path: qlogPath } });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      qlog: { path: qlogPath },
    }));
    ...
    try {
      ...
      await client.close();
      await server.close();
      try { await fs.unlink(qlogPath); } catch {}
    }
  });
  ...
  it('writes TLS keylog output when keylog path is configured', async (t) => {
    ...
    const fs = new DiskFileSystem();
    const keylogPath = `/tmp/fino-quic-keylog-${Date.now()}-${Math.floor(Math.random() * 1_000_000)}.log`;
    const server = new QuicEndpoint({ alpnProtocols: ['fino-hq'], keylog: { path: keylogPath } });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
      keylog: { path: keylogPath },
    }));
    const client = new QuicEndpoint({ alpnProtocols: ['fino-hq'], keylog: { path: keylogPath } });

    try {
      const clientConnection = await client.connect({ address: listener.address, keylog: { path: keylogPath } });
      const serverConnection = await server.accept();
      await clientConnection.connected;
      await serverConnection.connected;
      ...
      await client.close();
      await server.close();
      try { await fs.unlink(keylogPath); } catch {}
    }
  });
  ...
  it('client-initiated unidirectional streams expose only the writable side locally', async (t) => {
    ...
    try {
      ...
      t.deepEqual(await localReadable.read(), { value: undefined, done: true }, 'client local uni Web readable is closed');
  ...
  it('server-initiated unidirectional streams expose only the writable side locally', async (t) => {
    ...
    try {
      ...
      t.deepEqual(await localReadable.read(), { value: undefined, done: true }, 'server local uni Web readable is closed');
  ...
  it('close sends a peer-visible application connection close', async (t) => {
    ...
    try {
      ...
      await clientConnection.close({ errorCode: 42, reason: 'application shutdown' });
  ...
  it('exposes Node-aligned connection close state and destroy API', async (t) => {
    ...
    try {
      ...
      const closePromise = clientConnection.close({ errorCode: 9, reason: 'done' });
      t.equal(
      ...
      await server.accept();
      secondConnection.destroy(new Error('forced'), { errorCode: 11, type: 'transport', reason: 'forced' });
  ...
  it('accepts Node-covered QUIC stream source view types', async (t) => {
    ...
    try {
      const backing = new Uint8Array([0, 0x70, 0x61, 0x72, 0x74, 0]);
      const buffer = new Uint8Array([0xde, 0xad, 0xca, 0xfe]).buffer;
  ...
  it('migrates a client connection to a new local UDP port', async (t) => {
    ...
    try {
      ...
      await clientConnection.migrate({ family: 'ipv4', ip: '0.0.0.0', port: 0 });
  ...
  it('exchanges DATAGRAM frames over real UDP with ECN enabled', async (t) => {
    ...
    try {
      const clientConnection = await client.connect({ address: listener.address, transport: { ecn: true } });
      const serverConnection = await server.accept();
      const reader = serverConnection.datagramReadable.getReader();
  ...
  it('returns DATAGRAM ids that correlate with status events and topics', async (t) => {
    ...
    try {
      ...
      t.ok(
        statusTopicEvents.some((event) => event.connection === clientConnection && event.id === id && event.status === 'ack'),
        'DATAGRAM status topic uses the returned id',
      );
      t.ok(
        sendTopicEvents.some((event) => event.connection === clientConnection && event.id === id && event.length === 'id-correlates'.length),
  ...
  it('accepts Node-covered DATAGRAM source types and copies source bytes', async (t) => {
    ...
    try {
      ...
      sharedView.set([0x53, 0x41, 0x42]);
      const buffer = new Uint8Array([0xde, 0xad, 0xca, 0xfe]).buffer;
      const dataView = new DataView(buffer, 2, 2);
      const partial = new Uint8Array([0, 0x70, 0x61, 0x72, 0x74, 0]);
      ...
      const cases: Array<[() => Promise<number>, number[]]> = [
        ...
        [() => clientConnection.sendDatagram(Promise.resolve(new Uint8Array([0x50])) as any), [0x50]],
        [() => clientConnection.sendDatagram(sharedView as any), Array.from(encodeUtf8('SAB'))],
        [() => clientConnection.sendDatagram(dataView as any), [0xca, 0xfe]],
      ...
      for (let i = 0; i < cases.length; i++) {
        const [send] = cases[i];
        const id = await send();
        if (i === cases.length - 1) source.set([0x58, 0x59, 0x5a]);
        ids.push(id);
  ...
  it('abandons pending datagrams according to drop-newest overflow policy', async (t) => {
    ...
    const server = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      datagrams: { enabled: true, maxFrameSize: 1200, maxPending: 1 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
    const client = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      datagrams: { enabled: true, maxFrameSize: 1200, maxPending: 1, dropPolicy: 'drop-newest' },
    });
  ...
  it('abandons pending datagrams according to drop-oldest overflow policy', async (t) => {
    ...
    const client = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      datagrams: { enabled: true, maxFrameSize: 1200, maxPending: 1, dropPolicy: 'drop-oldest' },
    });
  ...
  it('rejects oversized datagrams before writing packets', async (t) => {
    ...
    const server = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      datagrams: { enabled: true, maxFrameSize: 32 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
  ...
  it('rejects datagrams larger than the peer negotiated DATAGRAM payload', async (t) => {
    ...
    const server = new QuicEndpoint({
      alpnProtocols: ['fino-hq'],
      datagrams: { enabled: true, maxFrameSize: 16 },
    });
    const listener = await server.listen(testListenOptions({
      address: { family: 'ipv4', ip: '127.0.0.1', port: 0 },
    }));
  ...
  it('closes a connection when an unknown short packet matches a stateless reset token', async (t) => {
    ...
    const cid = makeNativeCid(new Uint8Array([0x10, 0x20, 0x30, 0x40]));
    const token = new Uint8Array(16);
    for (let i = 0; i < token.byteLength; i++) token[i] = 0xa0 + i;
  ...
  it('registers and unregisters issued CIDs in the endpoint route table', async (t) => {
    ...
    try {
      const clientConnection = await client.connect({ address: listener.address });
      await server.accept();
      const issuedCidBytes = new Uint8Array([0x31, 0x32, 0x33, 0x34, 0x35]);
  ...
  it('deactivates destination-CID stateless reset tokens', async (t) => {
    ...
    try {
      const clientConnection = await client.connect({ address: listener.address });
      await server.accept();
      const cid = makeNativeCid(new Uint8Array([0x50, 0x60, 0x70, 0x80]));
      const token = new Uint8Array(16);
      for (let i = 0; i < token.byteLength; i++) token[i] = 0xc0 + i;
  ...
  it('sends stateless reset for unknown short-header packets', async (t) => {
    ...
    try {
      ...
      t.equal(response[0] & 0x80, 0, 'stateless reset is a short-header-looking packet');
    } finally {
      socketClose(responseFd);
      socketClose(sendFd);
      await endpoint.close();
  ...
  it('rate-limits stateless resets for unknown short-header floods', async (t) => {
    ...
    try {
      ...
      const started = performance.now();
      for (let attempt = 0; attempt < attempts; attempt++) {
        for (let i = 1; i < packet.byteLength; i++) packet[i] = (attempt + i) & 0xff;
        endpoint._handleDatagram(listener, sendFd, listener.address, packet, bound);
      }
      const elapsedSeconds = (performance.now() - started) / 1000;
...
describe('QUIC GnuTLS backend parity', () => {
  it('completes TLS handshake and exposes ALPN on GnuTLS', async (t) => {
    ...
    try {
      const accepted = server.accept();
      const clientConnection = await client.connect({ address: listener.address, serverName: 'localhost', verifyPeer: false });
      const serverConnection = await accepted;
  ...
  it('requires and exposes client cert for mTLS on GnuTLS', async (t) => {
    ...
    try {
      ...
      t.deepEqual(serverConnection.peerVerification, { verified: true, errorCode: 0, reason: null }, 'GnuTLS server reports successful client cert verification');
      await clientConnection.close();
      await serverConnection.close();
    } finally {
      await client.close();
