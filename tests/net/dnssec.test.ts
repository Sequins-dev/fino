...
function dnskeyFromRsaJwk(jwk: { n: string; e: string }): Uint8Array {
  ...
  const exponentLength =
    exponent.byteLength < 256
      ? new Uint8Array([exponent.byteLength])
      : concatBytes([new Uint8Array([0]), writeU16(exponent.byteLength)]);
  return concatBytes([writeU16(257), new Uint8Array([3, 8]), exponentLength, exponent, modulus]);
}
function dnskeyFromEcJwk(
  jwk: {
    x: string;
    y: string;
  },
...
function dnskeyFromOkpJwk(
  jwk: {
    x: string;
  },
  algorithm = 15,
): Uint8Array {
  return concatBytes([writeU16(257), new Uint8Array([3, algorithm]), base64urlDecode(jwk.x)]);
}
...
async function nsec3Hash(name: string, salt: Uint8Array, iterations: number): Promise<Uint8Array> {
  let digest = new Uint8Array(
    await crypto.subtle.digest('SHA-1', concatBytes([_encodeName(name.toLowerCase()), salt])),
  );
...
async function signedRrsig(
  ...
  rrsig.signature = new Uint8Array(
...
describe('DNSSEC helpers', () => {
  ...
  it('digestDnskey hashes canonical owner name plus DNSKEY RDATA', async (t) => {
    ...
    const expected = new Uint8Array(
      await crypto.subtle.digest('SHA-256', concatBytes([_encodeName('example.com'), dnskey])),
    );
    ...
    const rrset = [
      {
        ...
        type: RECORD_TYPES.A,
        ttl: 300,
        rawData: new Uint8Array([192, 0, 2, 20]),
      },
      {
        name: 'example.com.',
        type: RECORD_TYPES.A,
        ttl: 60,
        rawData: new Uint8Array([192, 0, 2, 10]),
      },
    ];
    const data = canonicalRrsetData(rrset, { originalTtl: 120 });
    const firstRdataOffset = _encodeName('example.com').byteLength + 10;
    t.deepEqual(
      Array.from(data.slice(firstRdataOffset, firstRdataOffset + 4)),
      [192, 0, 2, 10],
      'lower RDATA sorts first',
    );
    t.equal(
      new DataView(data.buffer, data.byteOffset, data.byteLength).getUint32(
        _encodeName('example.com').byteLength + 4,
        false,
      ),
      120,
      'original TTL is used',
    );
  });
  it('nsecCoversType checks decoded type bitmaps', (t) => {
    t.equal(
      nsecCoversType([RECORD_TYPES.A, RECORD_TYPES.RRSIG], RECORD_TYPES.RRSIG),
      true,
      'present type is covered',
    );
    t.equal(
      nsecCoversType([RECORD_TYPES.A], RECORD_TYPES.AAAA),
      false,
      'missing type is not covered',
    );
  });
});
describe('DNSSEC signature validation', { skip: skipCrypto }, () => {
  it('verifyRrsig verifies a signed RSA/SHA-256 RRset', async (t) => {
    ...
    rrsig.signature = new Uint8Array(
      await crypto.subtle.sign({ name: 'RSASSA-PKCS1-V1_5' }, keyPair.privateKey, signedData),
    );
    t.equal(
      await verifyRrsig(
        rrsig,
    ...
    const tampered = [
      {
        ...rrset[0]!,
        rawData: new Uint8Array([192, 0, 2, 56]),
      },
    ];
    t.equal(
      await verifyRrsig(
  ...
  it('verifyRrsig verifies a DNSSEC raw ECDSAP256SHA256 signature', async (t) => {
    ...
    rrsig.signature =
      derSignature.byteLength === 64 ? derSignature : derEcdsaToRaw(derSignature, 32);
    t.equal(
      await verifyRrsig(
        rrsig,
        rrset,
      ...
      true,
      'raw DNSSEC ECDSA signature verifies',
    );
  });
  it('verifyRrsig verifies a DNSSEC Ed25519 signature', async (t) => {
    ...
    rrsig.signature = new Uint8Array(
      await crypto.subtle.sign('Ed25519', keyPair.privateKey, rrsigSignedData(rrsig, rrset)),
    );
    t.equal(
      await verifyRrsig(
        rrsig,
      ...
      true,
      'Ed25519 DNSSEC signature verifies',
    );
    const tampered = [
      {
        ...rrset[0]!,
        rawData: new Uint8Array([192, 0, 2, 99]),
      },
    ];
    t.equal(
      await verifyRrsig(
  ...
  it('validateSignedResponse accepts an answer covered only by Ed25519', async (t) => {
    ...
    rrsig.signature = new Uint8Array(
      await crypto.subtle.sign('Ed25519', keyPair.privateKey, rrsigSignedData(rrsig, [answer])),
    );
  ...
  it('validateSignedResponse accepts a trusted signed answer and rejects tampering', async (t) => {
    ...
    await t.rejects(
      () =>
        validateSignedResponse(response, 'www.test', RECORD_TYPES.A, {
          trustAnchors,
          now: 2e3,
        }),
      (err) =>
  ...
  it('validateDnssecResponse builds a DS/DNSKEY chain from a root trust anchor', async (t) => {
    ...
    const rootDnskey = dnskeyFromRsaJwk(
      (await crypto.subtle.exportKey('jwk', rootPair.publicKey)) as {
        n: string;
        e: string;
      },
    );
    const childDnskey = dnskeyFromRsaJwk(
      (await crypto.subtle.exportKey('jwk', childPair.publicKey)) as {
        n: string;
        e: string;
      },
    );
    ...
    const dsSig = await signedRrsig(RECORD_TYPES.DS, 'test', '.', rootKeyTag, rootPair.privateKey, [
      dsRecord,
    ]);
    const dnskeyRecord = {
      name: 'test',
    ...
    const dnskeySig = await signedRrsig(
      RECORD_TYPES.DNSKEY,
      'test',
      'test',
      childKeyTag,
    ...
    const answerSig = await signedRrsig(
      RECORD_TYPES.A,
      'www.test',
      'test',
      childKeyTag,
  ...
  it('validateDnssecResponse bootstraps root DNSKEYs from a trust anchor before validating a child chain', async (t) => {
    ...
    const rootDnskeySig = await signedRrsig(
      RECORD_TYPES.DNSKEY,
      '.',
      '.',
      rootKeyTag,
    ...
    t.equal(
      fetched[0],
      `.:${RECORD_TYPES.DNSKEY}`,
      'root DNSKEY RRset is fetched before child validation',
    );
  });
  it('validateDnssecResponse accepts a positive CNAME answer covered by DNSSEC', async () => {
    ...
    const cnameSig = await signedRrsig(
      RECORD_TYPES.CNAME,
      'www.test',
      'test',
      keyTag,
  ...
  it('validateSignedResponse rejects expired and not-yet-valid RRSIGs', async (t) => {
    ...
    await t.rejects(
      async () =>
        validateSignedResponse(await responseWithWindow(1, 1999), 'www.test', RECORD_TYPES.A, {
          trustAnchors: [
            {
              name: 'test',
              rawData: dnskey,
    ...
    await t.rejects(
      async () =>
        validateSignedResponse(
          await responseWithWindow(2001, 4102444800),
          'www.test',
          RECORD_TYPES.A,
          {
            trustAnchors: [
  ...
  it('validateSignedResponse rejects unsupported-only signatures and falls back to a supported signature', async (t) => {
    ...
    const supported = await signedRrsig(
      RECORD_TYPES.A,
      'www.test',
      'test',
      keyTag,
  ...
  it('validateDnssecResponse accepts an unsigned child only with a signed NSEC DS-negative proof', async (t) => {
    ...
    const nsecSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      'test',
      '.',
      rootKeyTag,
  ...
  it('validateDnssecResponse validates NSEC NODATA only when the signed NSEC proves the type is absent', async (t) => {
    ...
    const validSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      'www.test',
      'test',
      keyTag,
    ...
    const invalidSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      'www.test',
      'test',
      keyTag,
  ...
  it('validateDnssecResponse requires NSEC closest-encloser and wildcard proofs for NXDOMAIN', async (t) => {
    ...
    const closestSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      'test',
      'test',
      keyTag,
    ...
    const nextCloserSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      'a.test',
      'test',
      keyTag,
    ...
    const wildcardSig = await signedRrsig(
      RECORD_TYPES.NSEC,
      '*.test',
      'test',
      keyTag,
  ...
  it('validateDnssecResponse validates NSEC3 exact-match NODATA and rejects excessive iterations', async (t) => {
    ...
    const proofSig = await signedRrsig(
      RECORD_TYPES.NSEC3,
      proof.name,
      'test',
      keyTag,
    ...
    const excessive = nsec3Record(
      ownerHash,
      'test',
      nextHash,
      [RECORD_TYPES.A, RECORD_TYPES.RRSIG],
      {
        salt,
        iterations: 251,
      },
    );
    const excessiveSig = await signedRrsig(
      RECORD_TYPES.NSEC3,
      excessive.name,
      'test',
      keyTag,
  ...
  it('validateDnssecResponse accepts NSEC3 opt-out DS-negative proof for insecure delegation', async (t) => {
    ...
    const optOutSig = await signedRrsig(
      RECORD_TYPES.NSEC3,
      optOut.name,
      '.',
      rootKeyTag,
