...
import {
  ...
  verifyRrsig,
} from '../../js/internal/net/dnssec.mts';
...
function dnskeyFromRsaJwk(jwk: { n: string; e: string }): Uint8Array {
  const exponent = base64urlDecode(jwk.e);
  const modulus = base64urlDecode(jwk.n);
  const exponentLength = exponent.byteLength < 256
    ? new Uint8Array([exponent.byteLength])
    : concatBytes([new Uint8Array([0]), writeU16(exponent.byteLength)]);
  return concatBytes([
    writeU16(257),
    new Uint8Array([3, 8]),
    exponentLength,
...
function dnskeyFromEcJwk(jwk: { x: string; y: string }, algorithm = 13): Uint8Array {
  return concatBytes([
    writeU16(257),
...
function dnskeyFromOkpJwk(jwk: { x: string }, algorithm = 15): Uint8Array {
  return concatBytes([
    writeU16(257),
    new Uint8Array([3, algorithm]),
    base64urlDecode(jwk.x),
...
function bitmapWindow(types: number[]): Uint8Array {
  ...
  for (const [window, offsets] of [...windows.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    for (const offset of offsets) bitmap[Math.floor(offset / 8)] |= 0x80 >> (offset % 8);
...
function nsec3Record(ownerHash: Uint8Array, zone: string, nextHash: Uint8Array, types: number[], options: {
  ...
  return {
    ...
    data: { hashAlgorithm: 1, flags, iterations, salt, nextHashedOwnerName: nextHash, types },
  };
}
...
function nsecRecord(name: string, nextName: string, types: number[], ttl = 300) {
  return {
    ...
    data: { nextDomainName: nextName, types },
  };
}

function aRecord(name: string, bytes = new Uint8Array([192, 0, 2, 55]), ttl = 300) {
  return {
    name,
    type: RECORD_TYPES.A,
    ttl,
...
async function signedRrsig(typeCovered: number, name: string, signerName: string, signerKeyTag: number, privateKey: CryptoKey, rrset: Array<{ name: string; type: number; ttl: number; rawData: Uint8Array }>) {
  const rrsig = {
    typeCovered,
    algorithm: 8,
...
describe('DNSSEC helpers', () => {
  it('canonicalName lowercases and preserves DNS wire label encoding', (t) => {
    t.deepEqual(
      Array.from(canonicalName('WwW.Example.COM.')),
      Array.from(_encodeName('www.example.com')),
      'canonical name bytes are lower-case wire format',
    );
  });
  ...
  it('dnskeyKeyTag matches RFC 4034 appendix B algorithm', (t) => {
    const dnskey = concatBytes([
      writeU16(257),
      new Uint8Array([3, 8]),
      new Uint8Array([1, 0, 1, 3, 1, 0, 1, 9, 8, 7, 6, 5]),
    ]);
    let ac = 0;
  ...
  it('canonicalRrsetData sorts records by canonical RDATA', (t) => {
    const rrset = [
      ...
      { name: 'example.com.', type: RECORD_TYPES.A, ttl: 60, rawData: new Uint8Array([192, 0, 2, 10]) },
    ];
    const data = canonicalRrsetData(rrset, { originalTtl: 120 });
    const firstRdataOffset = _encodeName('example.com').byteLength + 10;
    t.deepEqual(Array.from(data.slice(firstRdataOffset, firstRdataOffset + 4)), [192, 0, 2, 10], 'lower RDATA sorts first');
...
describe('DNSSEC signature validation', { skip: skipCrypto }, () => {
  it('verifyRrsig verifies a signed RSA/SHA-256 RRset', async (t) => {
    ...
    const jwk = await crypto.subtle.exportKey('jwk', keyPair.publicKey) as { n: string; e: string };
    const dnskey = dnskeyFromRsaJwk(jwk);
    const keyTag = dnskeyKeyTag(dnskey);
    const rrset = [
      { name: 'www.test', type: RECORD_TYPES.A, ttl: 300, rawData: new Uint8Array([192, 0, 2, 55]) },
    ];
    const rrsig = {
    ...
    t.equal(await verifyRrsig(rrsig, rrset, { name: 'test', rawData: dnskey }, 2_000), true, 'signature verifies');
    const tampered = [{ ...rrset[0]!, rawData: new Uint8Array([192, 0, 2, 56]) }];
    t.equal(await verifyRrsig(rrsig, tampered, { name: 'test', rawData: dnskey }, 2_000), false, 'tampered RRset fails');
  });
  ...
  it('verifyRrsig verifies a DNSSEC raw ECDSAP256SHA256 signature', async (t) => {
    ...
    const jwk = await crypto.subtle.exportKey('jwk', keyPair.publicKey) as { x: string; y: string };
    ...
    t.equal(await verifyRrsig(rrsig, rrset, { name: 'test', rawData: dnskey }, 2_000), true, 'raw DNSSEC ECDSA signature verifies');
  });
  ...
  it('verifyRrsig verifies a DNSSEC Ed25519 signature', async (t) => {
    ...
    t.equal(await verifyRrsig(rrsig, rrset, { name: 'test', rawData: dnskey }, 2_000), true, 'Ed25519 DNSSEC signature verifies');
    const tampered = [{ ...rrset[0]!, rawData: new Uint8Array([192, 0, 2, 99]) }];
    t.equal(await verifyRrsig(rrsig, tampered, { name: 'test', rawData: dnskey }, 2_000), false, 'tampered Ed25519 RRset fails');
  });
  ...
  it('validateSignedResponse accepts an answer covered only by Ed25519', async (t) => {
    ...
    const dnskey = dnskeyFromOkpJwk(await crypto.subtle.exportKey('jwk', keyPair.publicKey) as { x: string });
    const keyTag = dnskeyKeyTag(dnskey);
    ...
    await validateSignedResponse(
      ...
      'www.test',
      RECORD_TYPES.A,
      { trustAnchors: [{ name: 'test', rawData: dnskey }], now: 2_000 },
    );
  });
  ...
  it('validateSignedResponse accepts a trusted signed answer and rejects tampering', async (t) => {
    ...
    const dnskey = dnskeyFromRsaJwk(jwk);
    const keyTag = dnskeyKeyTag(dnskey);
    const answer = { name: 'www.test', type: RECORD_TYPES.A, ttl: 300, rawData: new Uint8Array([192, 0, 2, 55]), data: '192.0.2.55' };
    const rrsig = {
      typeCovered: RECORD_TYPES.A,
      algorithm: 8,
      labels: 3,
    ...
    const trustAnchors = [{ name: 'test', rawData: dnskey }];
    await validateSignedResponse(response, 'www.test', RECORD_TYPES.A, { trustAnchors, now: 2_000 });

    answer.rawData = new Uint8Array([192, 0, 2, 56]);
    await t.rejects(
      () => validateSignedResponse(response, 'www.test', RECORD_TYPES.A, { trustAnchors, now: 2_000 }),
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'tampered response rejects with EDNSSEC',
    );
  });
  ...
  it('validateDnssecResponse reuses bounded DNSSEC cache entries until TTL expiry', async (t) => {
    ...
    const cache = { entries: new Map(), maxEntries: 4 };
  ...
  it('validateDnssecResponse rejects an indeterminate DS lookup without a signed denial proof', async (t) => {
    ...
    await t.rejects(
      () => validateDnssecResponse(
        { rcode: 0, answers: [unsignedAnswer], authorities: [] },
        'www.insecure.test',
        RECORD_TYPES.A,
        {
          trustAnchors: [{ name: '.', rawData: rootDnskey }],
          now: 2_000,
          fetch: async (name, qtype) => {
            if (name === 'test' && qtype === RECORD_TYPES.DS) return { rcode: 0, answers: [], authorities: [] };
            throw new Error(`unexpected fetch ${name}:${qtype}`);
          },
        },
      ),
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'missing DS denial proof rejects as indeterminate',
    );
  });
  ...
  it('validateDnssecResponse accepts an unsigned child only with a signed NSEC DS-negative proof', async (t) => {
    ...
    const rootDnskey = dnskeyFromRsaJwk(await crypto.subtle.exportKey('jwk', rootPair.publicKey) as { n: string; e: string });
    const rootKeyTag = dnskeyKeyTag(rootDnskey);
    const nsec = nsecRecord('test', 'zzz.test', [RECORD_TYPES.NS, RECORD_TYPES.RRSIG, RECORD_TYPES.NSEC]);
    ...
    await validateDnssecResponse(
      ...
      RECORD_TYPES.A,
      {
        ...
        fetch: async (name, qtype) => {
          if (name === 'test' && qtype === RECORD_TYPES.DS) {
            return { rcode: 0, answers: [], authorities: [nsec, rrsigRecord('test', nsecSig)] };
          }
          throw new Error(`unexpected fetch ${name}:${qtype}`);
        },
      },
  ...
  it('validateDnssecResponse validates NSEC NODATA only when the signed NSEC proves the type is absent', async (t) => {
    ...
    const dnskey = dnskeyFromRsaJwk(await crypto.subtle.exportKey('jwk', keyPair.publicKey) as { n: string; e: string });
    const keyTag = dnskeyKeyTag(dnskey);
    const validNsec = nsecRecord('www.test', 'zzz.test', [RECORD_TYPES.A, RECORD_TYPES.RRSIG, RECORD_TYPES.NSEC]);
    const validSig = await signedRrsig(RECORD_TYPES.NSEC, 'www.test', 'test', keyTag, keyPair.privateKey, [validNsec]);

    await validateDnssecResponse(
      { rcode: 0, answers: [], authorities: [validNsec, rrsigRecord('www.test', validSig)] },
      'www.test',
      RECORD_TYPES.AAAA,
      {
        trustAnchors: [{ name: 'test', rawData: dnskey }],
    ...
    const invalidNsec = nsecRecord('www.test', 'zzz.test', [RECORD_TYPES.AAAA, RECORD_TYPES.RRSIG, RECORD_TYPES.NSEC]);
    const invalidSig = await signedRrsig(RECORD_TYPES.NSEC, 'www.test', 'test', keyTag, keyPair.privateKey, [invalidNsec]);
    await t.rejects(
      () => validateDnssecResponse(
        { rcode: 0, answers: [], authorities: [invalidNsec, rrsigRecord('www.test', invalidSig)] },
        'www.test',
        RECORD_TYPES.AAAA,
        {
          trustAnchors: [{ name: 'test', rawData: dnskey }],
      ...
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'NSEC listing requested type cannot prove NODATA',
    );
  });
  ...
  it('validateDnssecResponse requires NSEC closest-encloser and wildcard proofs for NXDOMAIN', async (t) => {
    ...
    const dnskey = dnskeyFromRsaJwk(await crypto.subtle.exportKey('jwk', keyPair.publicKey) as { n: string; e: string });
    const keyTag = dnskeyKeyTag(dnskey);
    const closest = nsecRecord('test', '*.test', [RECORD_TYPES.SOA, RECORD_TYPES.NS, RECORD_TYPES.RRSIG, RECORD_TYPES.NSEC]);
    ...
    await t.rejects(
      () => validateDnssecResponse(
        {
          rcode: 3,
          answers: [],
          authorities: [closest, rrsigRecord('test', closestSig), nextCloser, rrsigRecord('a.test', nextCloserSig)],
        },
        'missing.test',
        RECORD_TYPES.A,
        {
      ...
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'missing wildcard denial rejects NXDOMAIN proof',
    );
  });
  ...
  it('validateDnssecResponse validates NSEC3 exact-match NODATA and rejects excessive iterations', async (t) => {
    ...
    const proof = nsec3Record(ownerHash, 'test', nextHash, [RECORD_TYPES.A, RECORD_TYPES.RRSIG], { salt, iterations: 2 });
    const proofSig = await signedRrsig(RECORD_TYPES.NSEC3, proof.name, 'test', keyTag, keyPair.privateKey, [proof]);

    await validateDnssecResponse(
      { rcode: 0, answers: [], authorities: [proof, rrsigRecord(proof.name, proofSig)] },
      'www.test',
      RECORD_TYPES.AAAA,
      {
        trustAnchors: [{ name: 'test', rawData: dnskey }],
    ...
    const excessive = nsec3Record(ownerHash, 'test', nextHash, [RECORD_TYPES.A, RECORD_TYPES.RRSIG], { salt, iterations: 251 });
    const excessiveSig = await signedRrsig(RECORD_TYPES.NSEC3, excessive.name, 'test', keyTag, keyPair.privateKey, [excessive]);
    await t.rejects(
      () => validateDnssecResponse(
        { rcode: 0, answers: [], authorities: [excessive, rrsigRecord(excessive.name, excessiveSig)] },
        'www.test',
        RECORD_TYPES.AAAA,
        {
          trustAnchors: [{ name: 'test', rawData: dnskey }],
      ...
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'excessive NSEC3 iterations reject',
    );
  });
  ...
  it('validateDnssecResponse validates NSEC3 NXDOMAIN closest-encloser and wildcard proofs', async (t) => {
    ...
    const closest = nsec3Record(
      await nsec3Hash('test', salt, 1),
      'test',
      new Uint8Array(20).fill(0xff),
      [RECORD_TYPES.SOA, RECORD_TYPES.NS, RECORD_TYPES.RRSIG],
      { salt, iterations: 1 },
    );
    const nextCloserCover = nsec3Record(
      new Uint8Array(20),
      'test',
      new Uint8Array(20).fill(0xff),
      [RECORD_TYPES.RRSIG],
    ...
    await t.rejects(
      () => validateDnssecResponse(
        {
          rcode: 3,
          answers: [],
          authorities: [closest, rrsigRecord(closest.name, closestSig), nextCloserCover, rrsigRecord(nextCloserCover.name, nextCloserSig)],
        },
        'missing.test',
        RECORD_TYPES.A,
        {
      ...
      (err) => (err as { code?: string }).code === 'EDNSSEC',
      'missing NSEC3 wildcard proof rejects NXDOMAIN',
    );
  });
  ...
  it('validateDnssecResponse accepts NSEC3 opt-out DS-negative proof for insecure delegation', async (t) => {
    ...
    const rootDnskey = dnskeyFromRsaJwk(await crypto.subtle.exportKey('jwk', rootPair.publicKey) as { n: string; e: string });
    const rootKeyTag = dnskeyKeyTag(rootDnskey);
    const salt = new Uint8Array([5, 4, 3, 2]);
    const optOut = nsec3Record(
      new Uint8Array(20),
      'test',
      new Uint8Array(20).fill(0xff),
      [RECORD_TYPES.RRSIG],
      { flags: 1, salt, iterations: 1 },
    );
    const optOutSig = await signedRrsig(RECORD_TYPES.NSEC3, optOut.name, '.', rootKeyTag, rootPair.privateKey, [optOut]);

    await validateDnssecResponse(
      { rcode: 0, answers: [aRecord('www.child.test')], authorities: [] },
      'www.child.test',
      RECORD_TYPES.A,
      {
        trustAnchors: [{ name: '.', rawData: rootDnskey }],
        ...
        fetch: async (name, qtype) => {
          if (name === 'test' && qtype === RECORD_TYPES.DS) {
            return { rcode: 0, answers: [], authorities: [optOut, rrsigRecord(optOut.name, optOutSig)] };
          }
          throw new Error(`unexpected fetch ${name}:${qtype}`);
        },
      },
