...
const scenario = process.argv.find((arg) => arg.startsWith('--scenario='))?.slice('--scenario='.length) ?? 'h3';
...
try {
  ...
  const stream = await session.createBidirectionalStream({ body: scenario === 'large' ? new Uint8Array(1024 * 1024) : enc.encode(scenario === 'h3' ? 'from-node' : scenario) });
