...
export async function supportsNodeQuic(nodeBin: string): Promise<boolean> {
  ...
  const status = await withTimeout(proc.wait(), 5_000, 'Node QUIC capability probe');
  await err;
  await out;
...
export async function cleanupProcess(proc: Process, waitPromise: Promise<unknown>): Promise<void> {
  proc.kill();
  try {
    await withTimeout(waitPromise, 1_000, 'process cleanup');
  } catch {
    // Cleanup should not hide the primary interop assertion.
  }
}

export function parseReadyAddress(output: string): { port: number } | null {
  const match = /ready (\{[^\n]+\})/.exec(output);
