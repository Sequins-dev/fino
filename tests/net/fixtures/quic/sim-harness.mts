...
import { parseQuicHeader } from './packet-parse.mts';
...
export class SimulatedQuicRuntime {
  ...
  nowNs(): bigint {
    return BigInt(Math.floor(this.#net.now * 1_000_000));
  }
  ...
  setTimer(delayMs: number, callback: () => void): { cancel(): void } {
...
export class SimulatedQuicDatagramTransportFactory {
  ...
  async bind(address: QuicAddress) {
    ...
    return {
      ...
      sendBatch: (packets: Array<{ data: Uint8Array; dest: QuicAddress; ecn?: number }>) => {
        let sent = 0;
...
export class QuicPipe {
  ...
  constructor(options: {
    ...
    this.server = new QuicEndpoint({ alpnProtocols: ['fino-hq'], ...options.server }, internals);
    this.client = new QuicEndpoint({ alpnProtocols: ['fino-hq'], ...options.client }, internals);
  }

  async listen(address: QuicAddress = { family: 'ipv4', ip: '10.0.0.2', port: 4433 }): Promise<QuicListener> {
  ...
  async handshake(): Promise<{ client: QuicConnection; server: QuicConnection }> {
    ...
    return { client: this.clientConnection, server: this.serverConnection };
  }

  async openBidi(initialData: Uint8Array | string = new Uint8Array()): Promise<{ clientStream: QuicStream; serverStream: QuicStream }> {
  ...
  async pumpUntil<T>(promise: Promise<T>, maxTurns = 1000): Promise<T> {
    let settled = false;
  ...
  async pumpUntilCondition<T>(condition: () => T | null | undefined, maxTurns = 1000): Promise<T> {
    for (let i = 0; i < maxTurns; i++) {
  ...
  rawDatagramSocket(from: QuicAddress, to: QuicAddress): { send(data: Uint8Array): void } {
    return {
      send: (data) => this.rawDatagram(data, from, to),
    };
  }

  tracePacketCount(predicate?: (event: ReturnType<QuicPipe['trace']>[number]) => boolean): number {
    return this.trace().filter((event) => (
