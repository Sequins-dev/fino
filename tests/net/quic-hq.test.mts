...
describe('QUIC HQ interop', () => {
  it('exchanges HQ requests with ngtcp2 tools when installed', async (t) => {
    ...
    try {
      await delay(250);
      const finoClient = spawnFino('tests/net/fixtures/quic-hq-client.mts');
      const clientOut = collect(finoClient.stdout);
      const clientErr = collect(finoClient.stderr);
      const clientStatus = await withTimeout(finoClient.wait(), 5_000, 'Fino HQ client');
    ...
    const finoServer = spawnFino('tests/net/fixtures/quic-hq-server.mts');
    let serverOutText = '';
    ...
    await withTimeout(ready, 5_000, 'Fino HQ server readiness');
    ...
    const hqClientStatus = await withTimeout(hqClient.wait(), 5_000, 'ngtcp2 HQ client');
    t.equal(hqClientStatus.code, 0, (await hqClientErr) + (await hqClientOut));

    const finoServerStatus = await withTimeout(finoServer.wait(), 5_000, 'Fino HQ server');
