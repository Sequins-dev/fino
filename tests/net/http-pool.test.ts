...
import { _fetchAltSvcHas, _fetchH2PoolHas, _fetchH3PoolHas, _closeFetchH2PoolEntryForTest, _resetFetchAltSvc, _resetFetchH2Pool, _resetFetchH3Pool, _setFetchH3HandshakeTimeoutForTest } from 'internal:globals/fetch';
...
const skipGlobalH3 = (!quicAvailable || !h3Available || !tlsAvailable) && 'requires QUIC + libnghttp3 + OpenSSL';
...
// ---------------------------------------------------------------------------
async function connectPoolEntry(port: number, options?: {
  idleMs?: number;
}) {
...
describe('H2PoolEntry — close and GOAWAY', () => {
  ...
  it('peer GOAWAY rejects new streams and lets streams at or below lastStreamId finish', { skip }, async (t) => {
    ...
    try {
      ...
      await t.rejects(() => entry.send(makeReq(`http://127.0.0.1:${port}/after-goaway`)), /going away/i, 'new streams are rejected after GOAWAY');
      release();
...
describe('global fetch() — HTTPS H2 pool', () => {
  ...
  it('carries response and request trailers over pooled H2', { skip: skipHttps }, async (t) => {
    ...
    const server = serveHttp({
      ...
      return new Response(`echo:${body}`, { trailers: new Headers({ 'x-response-trailer': 'pooled' }) });
    });
    const origin = httpsOrigin(server.port);
    try {
...
describe('global fetch() — HTTPS H3 Alt-Svc pool', () => {
  it('discovers Alt-Svc and reuses one H3 session for later requests', { skip: skipGlobalH3 }, async (t) => {
    ...
    const server = serve({
      ...
      await accepted.respond(new Response(`tls:${++tlsRequests}`, { headers: { 'alt-svc': `h3=":${server.port}"; ma=60` } }));
    });
    await server.ready;
    const origin = httpsOrigin(server.port);
    try {
      t.equal(await (await fetch(`${origin}/one`, { tls: { rejectUnauthorized: false } } as any)).text(), 'tls:1', 'first request uses the advertised HTTPS transport');
  ...
  it('multiplexes concurrent requests over one discovered H3 session', { skip: skipGlobalH3 }, async (t) => {
    ...
    const server = serve({
      ...
      await accepted.respond(new Response('warm', { headers: { 'alt-svc': `h3=":${server.port}"; ma=60` } }));
    });
    await server.ready;
    const origin = httpsOrigin(server.port);
    try {
      await (await fetch(`${origin}/warm`, { tls: { rejectUnauthorized: false } } as any)).text();
      const [a, b] = await Promise.all([fetch(`${origin}/slow`, { tls: { rejectUnauthorized: false } } as any), fetch(`${origin}/fast`, { tls: { rejectUnauthorized: false } } as any)]);
  ...
  it('falls back and evicts broken automatic H3 alternatives', { skip: skipGlobalH3 }, async (t) => {
    ...
    const tls = serveHttp({
      ...
      idleTimeoutMs: 25,
      headersTimeoutMs: 25
    }, async () => new Response(`tls:${++tlsRequests}`, { headers: { 'alt-svc': 'h3=":9"; ma=60' } }));
    ...
    try {
      ...
      t.equal(await (await fetch(`${origin}/fallback`, { tls: { rejectUnauthorized: false } } as any)).text(), 'tls:2');
  ...
  it('protocol override controls automatic H3 and explicit H3 fallback', { skip: skipGlobalH3 }, async (t) => {
    ...
    try {
      t.equal(await (await fetch(`${origin}/warm`, { tls: { rejectUnauthorized: false } } as any)).text(), 'tls:1');
      t.equal(await (await fetch(`${origin}/bypass`, {
        protocol: 'http/1.1',
        tls: { rejectUnauthorized: false }
      } as any)).text(), 'tls:2', 'http/1.1 override bypasses cached H3');
      t.equal(await (await fetch(`${origin}/forced`, {
        protocol: 'h3',
        tls: { rejectUnauthorized: false }
  ...
  it('keeps non-replayable request bodies on H1/H2 despite cached Alt-Svc', { skip: skipGlobalH3 }, async (t) => {
    ...
    const tls = serveHttp({
      ...
      idleTimeoutMs: 25,
      headersTimeoutMs: 25
    }, async (request) => new Response(`tls:${request.method}:${await request.text()}`, { headers: { 'alt-svc': `h3=":${h3.port}"; ma=60` } }));
    const origin = httpsOrigin(tls.port);
    try {
      await (await fetch(`${origin}/warm`, { tls: { rejectUnauthorized: false } } as any)).text();
      t.ok(_fetchAltSvcHas(origin), 'Alt-Svc is cached');
...
describe('global fetch() — protocol override', () => {
  it('forces H2 when available and fails when ALPN does not negotiate h2', { skip: skipHttps }, async (t) => {
    ...
    try {
      ...
      t.equal(await h2Response.text(), 'h2-ok', 'forced H2 request succeeds against H2-capable server');
      t.ok(_fetchH2PoolHas(h2Origin), 'forced H2 request creates an H2 pool entry');
      await t.rejects(() => fetch(`http://127.0.0.1:${h1Server.port}/`, {
        protocol: 'h2',
        tls: { rejectUnauthorized: false }
