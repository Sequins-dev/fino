...
class MemoryDnsProvider extends DnsProvider {
  ...
  async lookup(
    ...
    const types: RRType[] =
      opts.family === 6 ? ['AAAA'] : opts.family === 4 ? ['A'] : ['A', 'AAAA'];
...
describe('internal:net/dns-provider contract', () => {
  ...
  it('lookup honors requested address family', async (t) => {
    ...
    await t.rejects(
      () => provider.lookup('empty.test', { family: 4 }),
      /lookup failed/,
      'empty address sets reject lookup',
    );
  });
  it('resolve returns record shapes and snapshots', async (t) => {
    ...
    t.deepEqual(
      await provider.resolve('empty.test', 'AAAA'),
      [],
      'existing names may return no matching records',
    );
    const resolved = await provider.resolve('service.test', 'A');
    resolved.push('192.0.2.99');
    t.deepEqual(
      await provider.resolve('service.test', 'A'),
      ['192.0.2.10'],
      'mutating returned records does not mutate provider state',
    );
  });
  it('reverse returns PTR name snapshots', async (t) => {
    ...
    t.deepEqual(
      await provider.reverse('192.0.2.99'),
      [],
      'missing reverse mappings return an empty list',
    );
  });
