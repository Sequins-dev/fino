...
describe('fino:validate', () => {
  ...
  it('accepts raw JSON Schema objects in all validation APIs', (t) => {
    ...
    t.equal(validator.safeParse({
      id: 'abc',
      count: 2
    }).success, true, 'compiled validator accepts valid input');
    t.equal(safeParse(schema, {
      id: 'ABC',
      count: 0
    }).success, false, 'safeParse reports invalid input');
  });
  it('applies defaults and reports nested validation errors', (t) => {
    ...
    t.throws(() => parse(schema, {
      service: { name: '' },
      tags: []
    }), (err) => err instanceof ValidationError && err.issues.length === 2, 'throws ValidationError with nested issues');
  });
  ...
  it('clones object and array defaults before returning parsed values', (t) => {
    const schema = v.object({ settings: v.any().default({
      tags: ['api'],
      nested: { enabled: true }
    }) });
  ...
  it('documents ignored keywords outside the supported JSON Schema subset', (t) => {
    t.equal(safeParse({
      ...
      minLength: 2,
      unknownKeyword: true
    }, 'a').success, false, 'supported keywords still apply');
    t.equal(safeParse({
      type: 'string',
      unknownKeyword: true
    }, 'a').success, true, 'unknown keywords are ignored');
    t.equal(safeParse({
      $ref: '#/$defs/name',
      $defs: { name: { type: 'string' } }
    }, 42).success, true, '$ref and $defs are not resolved');
    ...
    t.equal(safeParse({
      type: 'string',
      format: 'uuid'
    }, 'not-a-uuid').success, true, 'unsupported formats are ignored');
  });
});
