...
function captureFetchCall(
  ...
  responseFactory: (call: MockFetchCall, callCount: number) => Response | Promise<Response> = () => new Response('ok'),
...
describe('fino:opentelemetry', () => {
  ...
  it('builds scope-based topic names with stable lifecycle suffixes', (t) => {
    ...
    t.equal(
      otelTopic('metric', { name: 'http.server', version: '1.2.3' }, 'request', 'observe'),
  ...
  it('publishes canonical topic taxonomy helpers and schema envelopes', (t) => {
    t.deepEqual(OTEL_TOPIC_SUFFIXES.trace, ['start', 'end', 'error', 'event', 'attribute', 'link', 'status', 'rename'], 'trace suffixes are canonical');
  ...
  it('applies default resource identity to providers', (t) => {
    ...
    const custom = new LoggerProvider({ resource: new Resource({ 'service.name': 'custom-service', region: 'test' }) }).resource;
  ...
  it('applies SDK resource identity independently from provider resources', async (t) => {
    const exporter = new InMemoryExporter();
    const sdk = new OtelSDK({
      resource: new Resource({ 'service.name': 'sdk-service', 'service.version': '2.0.0' }),
      spanProcessors: [new BatchSpanProcessor(exporter, { scheduledDelayMillis: 0 })],
      instrumentations: [new TraceTopicInstrumentation()],
    }).start();
  ...
  it('captures direct spans through scoped trace topics without generic trace channels', async (t) => {
    ...
    let genericStarts = 0;
    const genericHandle = topic('otel:trace:start').subscribe(() => { genericStarts++; });
  ...
  it('applies sampling in the SDK without changing provider publication', async (t) => {
    ...
    const publishedHandle = events.subscribe(() => { published++; });
    const exporter = new InMemoryExporter();
    const sdk = new OtelSDK({
      sampler: new (class extends Sampler { shouldSample() { return false; } })(),
      spanProcessors: [new BatchSpanProcessor(exporter, { scheduledDelayMillis: 0 })],
      instrumentations: [
        {
          enable(targetSdk) {
            const handle = events.subscribe((evt: SpanRecord) => {
              targetSdk.recordSpan({
                ...
                traceId: evt.traceId,
                spanId: evt.spanId,
                startTimeUnixNano: (evt.timeUnixNano ?? 0) - 1000,
  ...
  it('supports span processor start hooks, span limits, and queue drop policy', async (t) => {
    ...
    const sdk = new OtelSDK({
      ...
      spanProcessors: [
        {
          onStart(span) { started.push(span); },
          onEnd() {},
          async forceFlush() {},
          async shutdown() {},
        },
    ...
    const first = tracer.startSpan('query', {
      attributes: { alpha: 'abcdef', beta: 'discard' },
      links: [
  ...
  it('supports span mutation APIs and active parent context', async (t) => {
    ...
    const tracer = new TracerProvider({ resource: new Resource({ 'service.name': 'trace-api-test' }) }).getTracer(
      'trace.api',
      '1.0.0',
      { attributes: { library: 'test' }, droppedAttributesCount: 1 },
    );
    ...
    await runWithActiveSpan(parent, async () => {
      ...
      child.setStatus({ code: 'ERROR', message: 'failed' });
      child.end();
    });
    parent.end();
  ...
  it('supports instrument-based metrics with SDK-side aggregation', async (t) => {
    ...
    const meter = new MeterProvider({ resource: new Resource({ 'service.name': 'metrics-test' }) }).getMeter(
      'metrics.api',
      '1.0.0',
    ...
    t.equal(gaugeMetric.value, 0.7, 'observable gauge collected');
  ...
  it('supports manual metric readers, views, and cardinality limits', async (t) => {
    ...
    const sdk = new OtelSDK({
      ...
      views: [{
        ...
        aggregation: { type: 'histogram', boundaries: [5, 10] },
      }],
  ...
  it('keeps cumulative and delta metric windows separate and records exemplars', async (t) => {
    ...
    await runWithActiveSpan(firstSpan, async () => {
      counter.add(2, { 'db.system': 'mysql', 'db.statement': 'select 1' });
    });
    firstSpan.end();
    ...
    await runWithActiveSpan(secondSpan, async () => {
      counter.add(3, { 'db.system': 'mysql', 'db.statement': 'select 2' });
    });
    secondSpan.end();
  ...
  it('supports richer logger APIs and propagation facade helpers', async (t) => {
    ...
    const sdk = new OtelSDK({
      logRecordProcessors: [{
        onEmit(log) { exporter.exportLogs([log]); },
        async forceFlush() {},
        async shutdown() {},
      }],
    }).start();
    const logger = new LoggerProvider({ resource: new Resource({ 'service.name': 'logger-test' }) })
  ...
  it('supports active-span log correlation, richer log records, and bounded log batching', async (t) => {
    ...
    const logger = new LoggerProvider({ resource: new Resource({ 'service.name': 'logs-test' }) })
    ...
    await runWithActiveSpan(span, async () => {
      logger.emitRecord(
        new LogRecordBuilder()
          ...
          .setCategory('auth')
          .setAttributes({ tenant: 'alpha', ignored: 'discarded' })
  ...
  it('supports propagation adapters, tracestate, and automatic header propagation in built-in instrumentations', async (t) => {
    ...
    const parent = new TracerProvider().getTracer('propagation.parent').startSpan('parent', {
  ...
  it('supports baggage-aware active context helpers and remote parent defaults', async (t) => {
    const exporter = new InMemoryExporter();
    const logProcessor: LogRecordProcessor & { logs: LogRecord[] } = {
      logs: [],
      onEmit(log) { this.logs.push(log); },
      async forceFlush() {},
      async shutdown() {},
    };
    const sdk = new OtelSDK({
      ...
      instrumentations: [
        ...
        {
          enable(targetSdk) {
            const startTopic = topic<SpanRecord>(otelTopic('trace', { name: 'context.remote', version: '1.0.0' }, 'start'));
            const endTopic = topic<SpanRecord>(otelTopic('trace', { name: 'context.remote', version: '1.0.0' }, 'end'));
            const starts = new Map<string, SpanRecord>();
            const a = startTopic.subscribe((evt: SpanRecord) => starts.set(evt.spanId, evt));
            const b = endTopic.subscribe((evt: SpanRecord) => {
              const start = starts.get(evt.spanId);
    ...
    await runWithActiveContext(remoteContext, async () => {
      ...
      await runWithBaggage(new Baggage({ tenant: 'nested', region: 'us' }), async () => {
  ...
  describe('OTLP exporter', () => {
    it('exports spans as OTLP JSON over HTTP via the SDK', async (t) => {
      ...
      await mockFetch(async (mock) => {
        ...
        t.ok(span.attributes.some((a: { key: string }) => a.key === 'http.request.method'), 'span attribute encoded');
    ...
    it('exports logs and metrics as OTLP JSON over HTTP', async (t) => {
      ...
      await mockFetch(async (mock) => {
        ...
        await exporter.exportMetrics([{
          ...
          max: 7.5,
          attributes: { db: 'mysql' },
          scope: { name: 'app.metrics', version: '1.0.0' },
          resource: new Resource({ 'service.name': 'otlp-test' }),
        }]);
        ...
        const sumMetric = metricsPayload.resourceMetrics[0].scopeMetrics[0].metrics.find((m: { name: string }) => m.name === 'db.query.count');
        t.ok(sumMetric?.sum, 'counter encoded as sum');
        const histMetric = metricsPayload.resourceMetrics[0].scopeMetrics[0].metrics.find((m: { name: string }) => m.name === 'db.query.duration');
    ...
    it('exports traces, logs, and metrics as OTLP JSON over HTTP', async (t) => {
      ...
      await mockFetch(async (mock) => {
        ...
        await exporter.exportSpans([{
          ...
          scope: { name: 'json.scope', version: '1.0.0' },
    ...
    it('supports exporter retries, timeout options, per-signal endpoints, compression, and hooks', async (t) => {
      ...
      await mockFetch(async (mock) => {
        mock.post('http://127.0.0.1:4318/custom-traces').replyWith(captureFetchCall(received, fetchCalls, () =>
          new Response(JSON.stringify({ partialSuccess: { rejectedSpans: 1, errorMessage: 'slow down' } }), { status: 200 })
        ));
        mock.post('http://127.0.0.1:4318/custom-logs').replyWith(captureFetchCall(received, fetchCalls, () =>
          new Response('retry me', { status: 503 })
        ));
        mock.post('http://127.0.0.1:4318/custom-logs').replyWith(captureFetchCall(received, fetchCalls, () =>
          new Response('ok', { status: 200 })
        ));
        mock.post('http://127.0.0.1:4318/custom-metrics').replyWith(captureFetchCall(received, fetchCalls, () =>
          new Response('ok', { status: 200 })
        ));
        ...
        const exporter = new OTLPHttpJsonExporter({
          ...
          onError(error) {
            events.push({ type: 'error', message: String(error.message || error) });
          },
          onPartialSuccess(result) {
            events.push({ type: 'partial', result });
          },
        });
