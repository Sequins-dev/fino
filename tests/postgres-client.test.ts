...
function rowDescription(name: string, oid: number): Uint8Array {
  return backend(
    'T',
    concat([i16(1), cstring(name), i32(0), i16(0), i32(oid), i16(4), i32(-1), i16(0)]),
  );
}
...
async function withFakePostgres(
  t: any,
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('postgres fake test timed out')), 3000),
      ),
    ]);
  } catch (err) {
    handlerError = err;
    ...
    if (handlerError) {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('postgres fake server timed out')), 3000),
        ),
      ]);
    }
  }
...
describe('fino:database/postgres client', () => {
  ...
  it('quotes LISTEN and NOTIFY channel names and payloads', async (t) => {
    ...
    t.deepEqual(simpleQueries, [
      'LISTEN "with ""quotes"""',
      'NOTIFY "with ""quotes""", \'payload \'\'quoted\'\'\'',
    ]);
  });
