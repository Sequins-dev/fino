...
async function readClientMessage(reader: any): Promise<{
  tag: string;
  body: Uint8Array;
}> {
...
async function withFakePostgres(t: any, handler: (url: string) => Promise<void>, response: {
  host?: string;
  value?: string;
  command?: string;
  ...
  const done = (async () => {
    ...
    await writeBackend(writer, backend('R', i32(0)), backend('K', concat([i32(99), new Uint8Array([
      1,
      2,
      3,
      4
    ])])), backend('S', concat([cstring('server_version'), cstring('18.0')])), backend('Z', enc.encode('I')));
    while (true) {
      const tags: string[] = [];
      let simpleQuery = '';
      ...
      if (tags.includes('Q')) {
        ...
        if (/COPY\b/i.test(simpleQuery) && /FROM\s+STDIN/i.test(simpleQuery)) {
          await writeBackend(writer, backend('G', new Uint8Array([
            0,
            0,
            0
          ])));
          ...
          await writeBackend(writer, backend('H', new Uint8Array([
            0,
            0,
            0
          ])), ...(response.copyOut ?? []).map((chunk) => backend('d', enc.encode(chunk))), backend('c'));
        }
        await writeBackend(writer, backend('C', cstring(response.command ?? 'SELECT 1')), backend('Z', enc.encode('I')));
        continue;
      }
      await writeBackend(writer, backend('1'), backend('2'), rowDescription('value', 23), ...response.value !== undefined ? [dataRow(response.value)] : [], backend('C', cstring(response.command ?? 'SELECT 1')), backend('Z', enc.encode('I')));
    }
    sock.close();
  })();
  ...
  try {
    ...
    await Promise.race([handler(`postgres://ada@${response.host ?? '127.0.0.1'}:${address.port}/app?sslmode=disable`), new Promise((_, reject) => setTimeout(() => reject(new Error('postgres fake test timed out')), 3e3))]);
  } catch (err) {
    handlerError = err;
  } finally {
    server.close();
    if (handlerError) {
      ...
      await Promise.race([done, new Promise((_, reject) => setTimeout(() => reject(new Error('postgres fake server timed out')), 3e3))]);
    }
  }
