...
describe('WPT generated manifest', () => {
  ...
  it('separates URLPattern local fixture coverage from tentative API debt', (t) => {
    for (const [path, reason] of [
      ['urlpattern/urlpattern.any.js', 'requires URLPattern tokenizer, canonicalization, and full data-driven conformance'],
      ['urlpattern/urlpattern.https.any.js', 'requires URLPattern tokenizer, canonicalization, and full data-driven conformance'],
    ] as const) {
      const entry = entryByPath.get(path);
      t.ok(entry !== undefined, `${path} exists in manifest`);
    ...
    for (const path of [
      ...
      'urlpattern/urlpattern-compare.tentative.https.any.js',
    ]) {
      const entry = entryByPath.get(path);
      t.ok(entry !== undefined, `${path} exists in manifest`);
      t.equal(entry?.runnable, true, `${path} is runnable`);
  ...
  it('keeps WebCrypto runnable files within the current release subset', (t) => {
    ...
    for (const path of [
      ...
      t.equal(
        ...
        `${path} has conformance-debt reason`,
      );
    }
  });
  ...
  it('separates Web Streams behavioral coverage from WebIDL descriptor debt', (t) => {
    ...
    t.equal(
      idlEntry?.reason,
      'requires Web Streams WebIDL descriptor and brand-check conformance',
      'streams idlharness has conformance-debt reason',
    );
