...
const DOCKER_CANDIDATES = ['/opt/homebrew/bin/docker', '/usr/local/bin/docker', '/usr/bin/docker'];
...
const AUTOBahn_SUITES = [
  {
    name: 'group 1 baseline cases',
    patterns: ['1.*'],
    requiredGroups: ['1'],
  },
  {
    name: 'group 2 baseline cases',
    patterns: ['2.*'],
    requiredGroups: ['2'],
  },
  {
    name: 'group 3 baseline cases',
    patterns: ['3.*'],
    requiredGroups: ['3'],
  },
  {
    name: 'group 4 baseline cases',
    patterns: ['4.*'],
    requiredGroups: ['4'],
  },
  {
    name: 'group 5 baseline cases',
    patterns: ['5.*'],
    requiredGroups: ['5'],
  },
  {
    name: 'group 6 baseline cases',
    patterns: ['6.*'],
    requiredGroups: ['6'],
  },
  {
    name: 'group 7 baseline cases',
    patterns: ['7.*'],
    requiredGroups: ['7'],
  },
  {
    name: 'group 9 baseline cases',
    patterns: ['9.*'],
    requiredGroups: ['9'],
  },
  ...
  {
    name: 'group 10 optional cases',
    patterns: ['10.*'],
    requiredGroups: ['10'],
  },
  {
    name: 'group 12.1 permessage-deflate cases',
    patterns: ['12.1.*'],
    requiredGroups: ['12'],
  },
  {
    name: 'group 12.2 permessage-deflate cases',
    patterns: ['12.2.*'],
    requiredGroups: ['12'],
  },
  {
    name: 'group 12.3 permessage-deflate cases',
    patterns: ['12.3.*'],
    requiredGroups: ['12'],
  },
  {
    name: 'group 12.4 permessage-deflate cases',
    patterns: ['12.4.*'],
    requiredGroups: ['12'],
  },
  {
    name: 'group 12.5 permessage-deflate cases',
    patterns: ['12.5.*'],
    requiredGroups: ['12'],
  },
  {
    name: 'group 13.1 permessage-deflate cases',
    patterns: ['13.1.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.2 permessage-deflate cases',
    patterns: ['13.2.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.3 permessage-deflate cases',
    patterns: ['13.3.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.4 permessage-deflate cases',
    patterns: ['13.4.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.5 permessage-deflate cases',
    patterns: ['13.5.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.6 permessage-deflate cases',
    patterns: ['13.6.*'],
    requiredGroups: ['13'],
  },
  {
    name: 'group 13.7 permessage-deflate cases',
    patterns: ['13.7.*'],
    requiredGroups: ['13'],
  },
];
...
async function runProcess(
  ...
  const drainOut = (async () => {
    for await (const c of proc.stdout) stdoutChunks.push(c);
  })();
...
function passingCase(c: AutobahnCase): boolean {
  const behaviorOk =
    c.behavior === 'OK' || c.behavior === 'INFORMATIONAL' || c.behavior === 'NON-STRICT';
  const closeOk =
    c.behaviorClose === '' ||
    c.behaviorClose === 'OK' ||
    c.behaviorClose === 'INFORMATIONAL' ||
    c.behaviorClose === 'NON-STRICT';
...
async function collectAutobahnCases(casePatterns: string[]): Promise<Map<string, AutobahnCase>> {
  ...
  if (autobahnTool === null) {
    throw new Error(
      'Autobahn harness unavailable: install wstest or pre-load the crossbario/autobahn-testsuite Docker image',
    );
  }
  ...
  try {
    ...
    return new Map(
      parseAutobahnIndex(await readText(`${reportDir}/index.json`)).map((c) => [c.id, c]),
    );
  } finally {
    await server.close();
    server = undefined;
...
describe('Autobahn — RFC 6455 WebSocket conformance', { skip: skipAutobahn }, () => {
  ...
  it('parses Autobahn report cases independently', (t) => {
    const parsed = parseAutobahnIndex(
      JSON.stringify({
        agent: 'fino',
        '1.2.1': {
          behavior: 'OK',
          ...
          duration: 1,
          remoteCloseCode: 1e3,
          reportfile: 'case1.html',
        },
        '1.1.1': {
          behavior: 'FAILED',
          behaviorClose: 'OK',
          reportfile: 'case2.html',
        },
      }),
    );
    t.deepEqual(
      parsed.map((c) => c.id),
      ['1.1.1', '1.2.1'],
      'cases are sorted by numeric case id',
    );
    ...
    describe(suite.name, () => {
      ...
      it('records expected Autobahn groups', (t) => {
        for (const group of suite.requiredGroups) {
          t.ok(
            emittedCaseIds.some((id) => id.startsWith(`${group}.`)),
            `Autobahn group ${group} was emitted`,
          );
        }
      });
      it('passes every emitted Autobahn case', (t) => {
        ...
        t.deepEqual(
          failed.map(
            (c) =>
              `${c.id}: behavior=${c.behavior || '<empty>'}, behaviorClose=${c.behaviorClose || '<empty>'}, report=${c.reportFile || '<none>'}`,
          ),
          [],
