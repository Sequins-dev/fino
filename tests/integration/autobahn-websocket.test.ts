...
const AUTOBahn_ACTIVE_SUITES = typeof AUTOBahn_SUITE_FILTER === 'string' && AUTOBahn_SUITE_FILTER.trim() !== '' ? AUTOBahn_SUITES.filter((suite) => suite.name.includes(AUTOBahn_SUITE_FILTER.trim()) || suite.patterns.some((pattern) => pattern.includes(AUTOBahn_SUITE_FILTER.trim()))) : AUTOBahn_SUITES;
...
async function runProcess(path: string, args: string[]): Promise<{
  code: number;
  stdout: string;
  stderr: string;
}> {
...
async function runAutobahn(tool: AutobahnTool, port: number, reportDir: string, casePatterns: string[]): Promise<void> {
  ...
  const result = tool.kind === 'wstest' ? await runProcess(tool.path, [
    '-m',
    'fuzzingclient',
    '-s',
    configPath
  ...
  for (const suite of AUTOBahn_ACTIVE_SUITES) {
    describe(suite.name, () => {
      ...
      it('passes every emitted Autobahn case', (t) => {
        ...
        t.deepEqual(failed.map((c) => `${c.id}: behavior=${c.behavior || '<empty>'}, behaviorClose=${c.behaviorClose || '<empty>'}, report=${c.reportFile || '<none>'}`), [], 'all emitted Autobahn cases pass');
      });
    });
  }
});
