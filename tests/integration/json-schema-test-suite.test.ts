...
function assertExample(
  t: TestContext,
  keywordFile: JsonSchemaKeywordFile,
  testCase: JsonSchemaCase,
  ...
  it('documents every unsupported fixture with an explicit reason', (t) => {
    for (const keywordFile of JSON_SCHEMA_TEST_SUITE_SUBSET) {
      if (!keywordFile.supported) {
        t.ok(
          keywordFile.reason && keywordFile.reason.length > 0,
          `${keywordFile.keyword} has an out-of-scope reason`,
        );
      }
    }
  });
    ...
    describe(`${keywordFile.draft}/${keywordFile.file}`, () => {
      for (const testCase of keywordFile.cases) {
        for (const example of testCase.tests) {
          it(
            exampleName(testCase, example),
            { skip: keywordFile.supported ? false : keywordFile.reason },
            (t) => {
