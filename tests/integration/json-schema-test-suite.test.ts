...
import { JSON_SCHEMA_TEST_SUITE_SUBSET, type JsonSchemaCase, type JsonSchemaExample, type JsonSchemaKeywordFile } from './fixtures/json-schema-test-suite-subset.ts';
...
function assertExample(t: TestContext, keywordFile: JsonSchemaKeywordFile, testCase: JsonSchemaCase, example: JsonSchemaExample): void {
  ...
  t.equal(result.success, example.valid, `${keywordFile.draft}/${keywordFile.file}: expected ${example.valid ? 'valid' : 'invalid'} for ${JSON.stringify(example.data)}`);
}
