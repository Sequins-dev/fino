...
describe('fino:format/mdx', () => {
  it('compiles MDX into an executable Fino UI module', async (t) => {
    ...
    t.ok(html.includes('<aside data-tone="warm">Custom component</aside>'), 'imported JSX components render');
    t.equal((html.match(/data-fino-slide/g) ?? []).length, 2, 'top-level thematic breaks split slides');
    t.ok(html.includes('<li>one</li>'), 'GFM-compatible block content renders');
    t.ok(html.includes('<table>') && html.includes('<del>draft</del>'), 'GFM tables and strikethrough render');
    t.ok(html.includes('type="checkbox" checked disabled'), 'GFM task items render through component mappings');

    const mapped = renderToHtml(module.default({
      name: 'Fino',
      components: { h1: (props: any) => ({ type: 'header', props: { 'data-mapped': true }, children: props.children, key: null }) }
    }));
    t.ok(mapped.includes('<header data-mapped>Hello, Fino</header>'), 'Markdown elements can be overridden with Fino components');
  });
