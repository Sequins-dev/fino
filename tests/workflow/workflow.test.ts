...
describe('fino:workflow', () => {
  it('runs checkpointed steps and skips completed steps on resume', async (t) => {
    ...
    const resumed = await wf.resume({ store, runId: first.runId });
  ...
  it('uses ordinary JavaScript branches, loops, foreach, and Promise.all', async (t) => {
    ...
    const wf = workflow({
      id: 'js-control-flow',
      async run(ctx, input: { items: number[]; positive: boolean }) {
    ...
    const result = await wf.start({ items: [1, 2, 3], positive: false }, { store });
    t.deepEqual(result.result, { total: -6, doubled: [2, 4, 6] });
  });
  ...
  it('keeps Promise.all checkpoints when steps finish out of order', async (t) => {
    ...
    t.deepEqual(loaded?.steps.map((step) => step.id), ['slow', 'fast']);

    calls.length = 0;
    await store.save({ ...loaded!, status: 'running', result: undefined });
    const resumed = await wf.resume({ store, runId: result.runId });
  ...
  it('waits for a signal and resumes exactly once', async (t) => {
    ...
    await wf.signal({ store, runId: first.runId, name: 'approval', payload: true });
    const resumed = await wf.resume({ store, runId: first.runId });
    t.equal(resumed.status, 'done');
    t.equal(resumed.result, 'deploy:true');

    await t.rejects(
      () => wf.signal({ store, runId: first.runId, name: 'approval', payload: false }),
  ...
  it('waits for a timer until it is due', async (t) => {
    ...
    const early = await wf.resume({ store, runId: first.runId });
    t.equal(early.status, 'waiting');

    const state = await store.load(first.runId);
    await store.save({
      ...state!,
      waitingOn: { ...state!.waitingOn!, dueAt: Date.now() - 1 },
    });
    const done = await wf.resume({ store, runId: first.runId });
  ...
  it('cancels a run and prevents further resume', async (t) => {
    ...
    const first = await wf.start(null, { store });
    const run = new WorkflowRun(wf, { store, runId: first.runId });
    await run.cancel();

    const loaded = await store.load(first.runId);
    t.equal(loaded?.status, 'cancelled');
    await t.rejects(() => wf.resume({ store, runId: first.runId }), /cancelled/i);
  });
