...
const libc = dlopen(libcPath, {
  ...
  snprintf: { parameters: ['buffer', 'usize', 'buffer', 'i32'], result: 'i32', variadic: 3 },
});
describe('ffiFunction binding', () => {
  ...
  it('keeps scalar calls correct after warmup', (t) => {
    ...
    t.equal(total, 499500, 'repeated calls stay correct');
  });
});
...
describe('ffiFunction with FfiCallback pointers', () => {
  it('calls back into JS through a bound callback pointer', (t) => {
    ...
    const invoke = ffiFunction(cb.pointer, { parameters: ['i32'], result: 'i32', fast: false });
...
describe('ffiFunction signature parity', () => {
  it('honors a pointer view offset', (t) => {
    const bytes = new Uint8Array(24);
    bytes.set(new Uint8Array(libc.pointers.abs), 8);
    const abs = ffiFunction(bytes.subarray(8, 16), {
      parameters: ['i32'],
      result: 'i32',
      fast: false,
    });
  ...
  it('uses the variadic ABI for a bound pointer', (t) => {
    const print = ffiFunction(libc.pointers.snprintf, {
      parameters: ['buffer', 'usize', 'buffer', 'i32'],
      result: 'i32',
      variadic: 3,
    });
    const out = new Uint8Array(32);
    t.equal(print(out, out.length, new TextEncoder().encode('%d\0'), 42), 2);
    t.equal(new TextDecoder().decode(out.subarray(0, 2)), '42');
  });
  it('rejects short buffers and zero addresses', (t) => {
    const def = { parameters: [], result: 'void' };
    t.throws(() => ffiFunction(new ArrayBuffer(7), def), /at least 8 bytes/);
    t.throws(() => ffiFunction(new Uint8Array(16).subarray(9), def), /at least 8 bytes/);
    t.throws(() => ffiFunction(0n, def), /must not be null/);
    t.throws(() => ffiFunction(new ArrayBuffer(8), def), /must not be null/);
  });
});
