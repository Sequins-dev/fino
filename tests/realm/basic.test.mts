...
describe('Realm.fromSource', () => {
  it('runs source with static imports and top-level await', async (t) => {
    const realm = Realm.fromSource(`
      ...
      const name = basename('/tmp/source-entry.mts');
      await Promise.resolve();
      if (name !== 'source-entry.mts') throw new Error('bad basename: ' + name);
    `);
  ...
  it('supports mixed value and type imports', async (t) => {
    const realm = Realm.fromSource(`
      ...
      const opts: Pick<RealmOptions, 'entry'> = { entry: 'virtual.mts' };
      if (typeof Realm !== 'function') throw new Error('missing Realm value');
      if (opts.entry !== 'virtual.mts') throw new Error('type import was not erased');
    `);
  ...
  it('works with an explicit specifier', async (t) => {
    const realm = Realm.fromSource(
      `import sum from './sum-fn.mts';
       if (sum(20, 22) !== 42) throw new Error('bad sum');`,
      {
        specifier: new URL('./fixtures/source-entry.mts', import.meta.url).pathname,
      },
    );
    ...
    const realm = Realm.fromSource(
       ...
       if (marker !== 'ok') throw new Error('bad marker');`,
      {
        overrides: ImportMap.deny([
          {
            pattern: 'virtual:dep',
            directive: { type: 'source', code: `export const marker = 'ok';`, source_map: '' },
          },
        ]),
      },
    );
