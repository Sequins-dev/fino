  ...
  it('delivered events are trusted and use location.origin', async (t) => {
    ...
    try {
      const event = await new Promise<MessageEvent>((resolve, reject) => {
        const tid = setTimeout(() => reject(new Error('timeout waiting for message')), 2e3);
  ...
  it('stopImmediatePropagation before onmessage prevents the handler property', async (t) => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const tid = setTimeout(() => reject(new Error('timeout waiting for listener')), 2e3);
  ...
  it('dispatches messageerror for invalid serialized bytes', async (t) => {
    ...
    try {
      const error = new Promise<MessageEvent>((resolve, reject) => {
        const tid = setTimeout(() => reject(new Error('timeout waiting for messageerror')), 2e3);
