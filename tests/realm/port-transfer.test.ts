 ...
 * (reactor-pooled realm) boundaries.
 *
 ...
 *
 * Cross-Isolate: a port transferred through a reactor-pooled Realm's
 * transport is used to post a message back.
 */
...
describe('MessagePort transfer', () => {
  ...
  it('cross-Isolate: port transferred to pooled realm receives message', async (t) => {
    ...
    // Create a channel; transfer port1 to the pooled realm via realm.port
    const { port1, port2 } = new MessageChannel();
    realm.port.start();
    realm.port.postMessage('use this port', [port1]);
    // port2 should receive the message the pooled realm sends through port1
    ...
    t.equal(reply, 'echo from thread', 'pooled realm sent message via transferred port');
    realm.terminate();
    port2.close();
  });
});
