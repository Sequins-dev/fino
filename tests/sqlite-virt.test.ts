...
describe('fino:database/sqlite — in-memory FileSystem (virtualization)', () => {
  ...
  it('reports VFS truncate failures as sqlite errors', async (t) => {
    ...
    try {
      await db.exec('CREATE TABLE t (value TEXT)');
      const rc = s.sqlite3_file_control(
        db.ptr,
        main,
        SQLITE_FCNTL_SIZE_HINT,
        Pointer.of(size),
  ...
  it('supports deterministic VFS file controls', async (t) => {
    ...
    try {
      ...
      arg = i32Arg(0);
      t.equal(
        control(SQLITE_FCNTL_POWERSAFE_OVERWRITE, arg),
        SQLITE_OK,
        'POWERSAFE_OVERWRITE set succeeds',
      );
      arg = i32Arg(-1);
      t.equal(
        control(SQLITE_FCNTL_POWERSAFE_OVERWRITE, arg),
        SQLITE_OK,
        'POWERSAFE_OVERWRITE query succeeds',
      );
      t.equal(
        new DataView(arg).getInt32(0, true),
        0,
        'POWERSAFE_OVERWRITE query returns tracked value',
      );
      ...
      arg = i32Arg(0);
      t.equal(
        control(SQLITE_FCNTL_DATA_VERSION, arg),
        SQLITE_OK,
        'DATA_VERSION initial query succeeds',
      );
      ...
      t.equal(
        control(SQLITE_FCNTL_DATA_VERSION, arg),
        SQLITE_OK,
        'DATA_VERSION second query succeeds',
      );
      t.ok(new DataView(arg).getInt32(0, true) > before, 'DATA_VERSION advances after writes');
      arg = i32Arg(0);
      t.equal(
        control(SQLITE_FCNTL_BEGIN_ATOMIC_WRITE, arg),
        SQLITE_NOTFOUND,
        'unsupported atomic writes return NOTFOUND',
      );
      t.equal(
        control(SQLITE_FCNTL_PRAGMA, arg),
        SQLITE_NOTFOUND,
        'unsupported PRAGMA control returns NOTFOUND',
      );
