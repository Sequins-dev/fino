...
describe('TypeScript performance deck', () => {
  ...
  it('uses a rich amber callout surface with ink text', (t) => {
    ...
    t.ok(
      html.includes('background:#d88a00;color:#0d1b1e'),
      'callout uses a deeper golden amber with dark text',
    );
  });
  it('sets callout copy in a readable non-condensed face', (t) => {
    ...
    t.ok(
      html.includes(
        'font-family:&quot;Avenir Next&quot;,&quot;Helvetica Neue&quot;,sans-serif;font-size:1.15em;font-weight:600;line-height:1.3;letter-spacing:.005em',
      ),
  ...
  it('organizes the architecture preview into five substantial chapters', async (t) => {
    ...
    t.equal(
      source.split('\n---\n').length,
      37,
      'the expanded deck has enough room to develop its argument',
    );
    for (const title of [
