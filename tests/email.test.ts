...
describe('fino:email', () => {
  it('renders MIME messages and rejects header injection', (t) => {
    ...
    t.ok(rendered.includes('multipart/alternative; boundary="b-test-alt"'), 'alternative multipart is rendered');
    t.ok(rendered.includes('<p>line one</p>'), 'html body is rendered');
    t.throws(() => renderMessage({
      from: 'a@example.test',
      to: 'b@example.test',
      subject: 'bad\r\nBcc: x@example.test',
      text: 'x'
  ...
  it('creates DKIM signatures', async (t) => {
    ...
    t.ok(header.startsWith('DKIM-Signature: v=1; a=hmac-sha256; d=example.test; s=s1;'), 'DKIM header is rendered');
