...
describe('ECDSA P-256 — exportKey / importKey (spki)', { skip }, () => {
  ...
  it('importKey("spki") round-trips through exportKey("spki")', async (t) => {
    ...
    const importedPubKey = await crypto.subtle.importKey('spki', spki, {
      name: 'ECDSA',
      hash: 'SHA-256'
    }, true, ['verify']);
    ...
    let same = true;
    for (let i = 0; i < b1.length; i++) if (b1[i] !== b2[i]) {
      same = false;
      break;
    }
  ...
  it('importKey("spki") rejects wrong byte length', async (t) => {
    const short = new Uint8Array(90);
    try {
      await crypto.subtle.importKey('spki', short, {
        name: 'ECDSA',
        hash: 'SHA-256'
      }, true, ['verify']);
  ...
  it('importKey("spki") rejects mismatched curve prefix', async (t) => {
    ...
    bad[0] = 49;
    try {
      await crypto.subtle.importKey('spki', bad, {
        name: 'ECDSA',
        hash: 'SHA-256'
      }, true, ['verify']);
      t.fail('should have thrown');
      ...
      t.ok((err as Error).message.toLowerCase().includes('p-256') || (err as Error).message.toLowerCase().includes('header') || (err as Error).message.toLowerCase().includes('match'), 'error mentions SPKI mismatch: ' + (err as Error).message);
    }
  });
  it('importKey("spki") rejects compressed EC point (0x02 marker)', async (t) => {
    ...
    spki[26] = 2;
    try {
      await crypto.subtle.importKey('spki', spki, {
        name: 'ECDSA',
        hash: 'SHA-256'
      }, true, ['verify']);
      t.fail('should have thrown');
      ...
      t.ok((err as Error).message.toLowerCase().includes('uncompressed') || (err as Error).message.toLowerCase().includes('0x04'), 'error mentions uncompressed point requirement: ' + (err as Error).message);
    }
  });
});
describe('ECDSA — PKCS8 private key export / import', { skip }, () => {
  it('P-256 private key round-trips through PKCS8 (sign with original, verify with imported)', async (t) => {
    ...
    const importedPriv = await crypto.subtle.importKey('pkcs8', pkcs8, {
      name: 'ECDSA',
      namedCurve: 'P-256'
    }, false, ['sign']);
    t.equal(importedPriv.type, 'private', 'imported pkcs8 key type is private');
    t.equal((importedPriv.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-256', 'namedCurve preserved');
  ...
  it('exportKey("pkcs8") throws for a public key', async (t) => {
    ...
    try {
      ...
      t.ok((err as Error).message.toLowerCase().includes('private'), 'error mentions private: ' + (err as Error).message);
    }
  });
});
describe('ECDSA P-256 — sign / verify', { skip }, () => {
  ...
  it('verify fails for tampered signature', async (t) => {
    ...
    const sig = await crypto.subtle.sign({
      name: 'ECDSA',
      hash: 'SHA-256'
    }, kp.privateKey, MSG) as ArrayBuffer;
    const tampered = new Uint8Array(sig);
    tampered[0] ^= 255;
    const valid = await crypto.subtle.verify({
      name: 'ECDSA',
      hash: 'SHA-256'
    }, kp.publicKey, tampered, MSG);
  ...
  it('verify works with imported public key', async (t) => {
    ...
    const spki = await crypto.subtle.exportKey('spki', kp.publicKey) as ArrayBuffer;
    const importedKey = await crypto.subtle.importKey('spki', spki, {
      name: 'ECDSA',
      hash: 'SHA-256'
    }, true, ['verify']);
    const valid = await crypto.subtle.verify({
  ...
  it('sign() rejects with an Error when called with a public key (wrong usage)', async (t) => {
    ...
    try {
      ...
      t.ok((err as Error).message.toLowerCase().includes('sign') || (err as Error).message.toLowerCase().includes('usage'), 'error mentions sign/usage: ' + (err as Error).message);
    }
  });
  it('verify() rejects when called with a private key (wrong usage)', async (t) => {
    ...
    try {
      await crypto.subtle.verify({
        name: 'ECDSA',
        hash: 'SHA-256'
      }, kp.privateKey, sig, MSG);
      t.fail('should have thrown — private key does not have verify usage');
    } catch (err) {
      t.ok(err instanceof Error, 'throws an Error');
      t.ok((err as Error).message.toLowerCase().includes('verify') || (err as Error).message.toLowerCase().includes('usage'), 'error mentions verify/usage: ' + (err as Error).message);
    }
  });
});
// ---------------------------------------------------------------------------
...
describe('ECDSA P-384 — generateKey', { skip }, () => {
  it('returns a key pair with P-384 namedCurve', async (t) => {
    ...
    t.equal((kp.privateKey.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-384', 'private namedCurve');
    t.equal((kp.publicKey.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-384', 'public namedCurve');
...
describe('ECDSA P-384 — exportKey / importKey (spki)', { skip }, () => {
  ...
  it('importKey("spki") round-trips (P-384)', async (t) => {
    ...
    const spki = await crypto.subtle.exportKey('spki', kp.publicKey) as ArrayBuffer;
    const imported = await crypto.subtle.importKey('spki', spki, {
      name: 'ECDSA',
      namedCurve: 'P-384',
      hash: 'SHA-384'
    }, true, ['verify']);
    t.equal(imported.type, 'public', 'imported type is public');
    t.equal((imported.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-384', 'imported namedCurve is P-384');
...
describe('ECDSA P-521 — generateKey', { skip }, () => {
  it('returns a key pair with P-521 namedCurve', async (t) => {
    ...
    t.equal((kp.privateKey.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-521', 'private namedCurve');
    t.equal((kp.publicKey.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-521', 'public namedCurve');
  });
});
describe('ECDSA P-521 — exportKey / importKey (spki)', { skip }, () => {
  ...
  it('importKey("spki") round-trips (P-521)', async (t) => {
    ...
    t.equal((imported.algorithm as {
      namedCurve?: string;
    }).namedCurve, 'P-521', 'imported namedCurve is P-521');
    const spki2 = new Uint8Array(await crypto.subtle.exportKey('spki', imported) as ArrayBuffer);
    const spki1 = new Uint8Array(spki);
    let same = spki1.length === spki2.length;
    for (let i = 0; same && i < spki1.length; i++) if (spki1[i] !== spki2[i]) same = false;
...
describe('ECDSA — EC JWK export / import', { skip }, () => {
  ...
  it('P-256 public key JWK round-trip: import and verify', async (t) => {
    ...
    const imported = await crypto.subtle.importKey('jwk', jwk as BufferSource, {
      name: 'ECDSA',
      namedCurve: 'P-256'
    }, true, ['verify']);
    t.equal(imported.type, 'public', 'imported type is public');
  ...
  it('P-256 private key JWK round-trip: import and sign', async (t) => {
    ...
    const imported = await crypto.subtle.importKey('jwk', jwkPriv as BufferSource, {
      name: 'ECDSA',
      namedCurve: 'P-256'
    }, false, ['sign']);
  ...
  it('P-384 JWK round-trip (public + private)', async (t) => {
    ...
    const jwkPub = await crypto.subtle.exportKey('jwk', kp.publicKey) as {
      crv: string;
    };
    const jwkPriv = await crypto.subtle.exportKey('jwk', kp.privateKey) as {
      crv: string;
      d?: string;
    };
    ...
    const importedPriv = await crypto.subtle.importKey('jwk', jwkPriv as BufferSource, {
      name: 'ECDSA',
      namedCurve: 'P-384'
    }, false, ['sign']);
    ...
    const importedPub = await crypto.subtle.importKey('jwk', jwkPub as BufferSource, {
      name: 'ECDSA',
      namedCurve: 'P-384'
    }, true, ['verify']);
    const ok = await crypto.subtle.verify({
