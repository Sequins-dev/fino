...
describe('Key management', { skip }, () => {
  ...
  it('generateKey HMAC records explicit and default key lengths', async (t) => {
    ...
    t.equal(
      (explicit.algorithm as HmacKeyAlgorithm).length,
      160,
      'explicit HMAC length is preserved',
    );
    t.equal(
      (defaultSha256.algorithm as HmacKeyAlgorithm).length,
      512,
      'SHA-256 HMAC defaults to block size',
    );
    t.equal(
      (defaultSha512.algorithm as HmacKeyAlgorithm).length,
      1024,
      'SHA-512 HMAC defaults to block size',
    );
  });
...
describe('HKDF', { skip }, () => {
  ...
  it('throws when requested key length exceeds 255 * hashLen (n > 255 guard)', async (t) => {
    ...
    try {
      ...
      t.ok(
        (err as Error).message.toLowerCase().includes('large') ||
          (err as Error).message.includes('255'),
...
describe('importKey / exportKey — JWK format (symmetric keys)', () => {
  it('AES-GCM 256 key round-trips via JWK', async (t) => {
    ...
    const reimported = await crypto.subtle.importKey('jwk', jwk as any, { name: 'AES-GCM' }, true, [
      'encrypt',
      'decrypt',
    ]);
...
describe('wrapKey / unwrapKey', () => {
  ...
  it('wraps and unwraps via JWK format', async (t) => {
    ...
    t.ok(
      unwrapped != null && typeof (unwrapped as CryptoKey).algorithm === 'object',
      'unwrapped is CryptoKey-like',
    );
...
describe('crypto and subtle [Symbol.toStringTag]', { skip }, () => {
  it('crypto[Symbol.toStringTag] is "Crypto"', (t) => {
    t.equal(
      (crypto as unknown as SymbolRecord)[Symbol.toStringTag],
      'Crypto',
      'crypto toStringTag',
    );
  });
  it('crypto.subtle[Symbol.toStringTag] is "SubtleCrypto"', (t) => {
    t.equal(
      (crypto.subtle as unknown as SymbolRecord)[Symbol.toStringTag],
      'SubtleCrypto',
      'subtle toStringTag',
    );
  });
  ...
  it('throws TypeMismatchError DOMException for non-integer views', (t) => {
    t.throws(
      ...
      (err) =>
        ...
        err.code === DOMException.TYPE_MISMATCH_ERR,
    );
    t.throws(
...
describe('Key usage validation', { skip }, () => {
  it('sign with a verify-only HMAC key throws', async (t) => {
    ...
    t.ok(
      key.usages.includes('verify') && !key.usages.includes('sign'),
      'key has only verify usage',
    );
    await t.rejects(
...
describe('WebCrypto release error names', { skip }, () => {
  it('uses NotSupportedError for unsupported algorithms and formats', async (t) => {
    ...
    t.equal(
      aesCtrKey.algorithm.name,
      'AES-CTR',
      'AES-CTR key generation is supported for metadata compatibility',
    );
    await t.rejects(
      () =>
        crypto.subtle.encrypt(
  ...
  it('uses InvalidAccessError for disallowed key operations', async (t) => {
    ...
    const publicSignKey = await crypto.subtle.importKey('raw', rawEd25519Public, 'Ed25519', true, [
      'sign',
    ]);
    const iv = new Uint8Array(12);
    await t.rejects(
    ...
    await t.rejects(
      () =>
        crypto.subtle.wrapKey('raw', nonExtractable, wrapKey, {
          name: 'AES-GCM',
          iv,
        }),
      rejectsWithName('InvalidAccessError'),
    ...
    await t.rejects(
      () =>
        crypto.subtle.wrapKey('raw', extractable, aesEncryptOnly, {
          name: 'AES-GCM',
          iv,
        }),
      rejectsWithName('InvalidAccessError'),
  ...
  it('uses NotSupportedError for unsupported derivation algorithms', async (t) => {
    const baseKey = await crypto.subtle.importKey('raw', new Uint8Array(16), 'HKDF', false, [
      'deriveBits',
      'deriveKey',
    ]);
    await t.rejects(
  ...
  it('uses OperationError for failed decrypt operations', async (t) => {
    const key = await crypto.subtle.importKey('raw', new Uint8Array(16).fill(1), 'AES-GCM', false, [
      'encrypt',
      'decrypt',
    ]);
    const iv = new Uint8Array(12);
...
describe('WebCrypto release algorithm matrix', { skip }, () => {
  it('covers required digest, MAC, cipher, KDF, and wrap algorithms', async (t) => {
    ...
    t.equal(
      await crypto.subtle.verify('HMAC', hmac, sig, data),
      true,
      'HMAC sign/verify is available',
    );
    ...
    t.equal(
      ...
      'release matrix',
      'AES-GCM encrypt/decrypt is available',
    );
    ...
    t.equal(
      ...
      'release matrix',
      'AES-CBC encrypt/decrypt is available',
    );
    ...
    t.equal(
      ...
      32,
      'PBKDF2 deriveBits is available',
    );
...
describe('WebCrypto key usage and JWK rejection edges', { skip }, () => {
  it('wrapKey and unwrapKey require explicit wrapping usages', async (t) => {
    ...
    const encryptOnly = await crypto.subtle.importKey('raw', wrappingBytes, 'AES-GCM', true, [
      'encrypt',
      'decrypt',
    ]);
    const wrapOnly = await crypto.subtle.importKey('raw', wrappingBytes, 'AES-GCM', true, [
      'wrapKey',
    ]);
    const unwrapOnly = await crypto.subtle.importKey('raw', wrappingBytes, 'AES-GCM', false, [
      'unwrapKey',
    ]);
    const iv = new Uint8Array(12);
    await t.rejects(
      () =>
        crypto.subtle.wrapKey('raw', target, encryptOnly, {
          name: 'AES-GCM',
          iv,
        }),
      /wrapKey/i,
  ...
  it('rejects incompatible symmetric JWK metadata', async (t) => {
    const valid = (await crypto.subtle.exportKey(
      'jwk',
      await crypto.subtle.importKey('raw', new Uint8Array(16).fill(7), 'AES-GCM', true, [
        'encrypt',
      ]),
    )) as JsonWebKey;
    await t.rejects(
