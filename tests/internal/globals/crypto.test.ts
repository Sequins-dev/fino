...
type AesKeyAlgorithm = KeyAlgorithm & {
  length: number;
};
type HmacKeyAlgorithm = KeyAlgorithm & {
  hash: {
    name: string;
  };
  length: number;
...
describe('subtle.digest', { skip }, () => {
  ...
  it('SHA-512 empty', async (t) => {
    ...
    const hex = toHex(result);
    t.equal(hex, 'cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e', 'SHA-512("")');
  });
...
describe('HMAC', { skip }, () => {
  it('sign/verify round-trip', async (t) => {
    const keyBytes = new Uint8Array(32).fill(66);
  ...
  it('HMAC-SHA256 known vector (RFC 4231 case 1)', async (t) => {
    const keyBytes = new Uint8Array(20).fill(11);
    const key = await crypto.subtle.importKey('raw', keyBytes, {
      name: 'HMAC',
      hash: 'SHA-256'
    }, false, ['sign']);
  ...
  it('HMAC-SHA384 known vector (RFC 4231 case 1)', async (t) => {
    ...
    const data = new TextEncoder().encode('Hi There');
    const sig = await crypto.subtle.sign({ name: 'HMAC' }, key, data);
    t.equal(toHex(sig), 'afd03944d84895626b0825f4ab46907f15f9dadbe4101ec682aa034c7cebc59cfaea9ea9076ede7f4af152e8b2fa9cb6', 'HMAC-SHA384 RFC4231 case 1');
  });
});
describe('AES-GCM', { skip }, () => {
  ...
  it('128-bit encrypt/decrypt round-trip', async (t) => {
    ...
    const dt = await crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv
    }, key, ct);
  ...
  it('wrong tag causes decryption failure', async (t) => {
    ...
    if (ctArr[0] !== undefined) ctArr[0] ^= 255;
    let threw = false;
    try {
...
describe('AES-CBC', { skip }, () => {
  ...
  it('AES-128-CBC known-answer vector (NIST CAVP)', async (t) => {
    ...
    const key = await crypto.subtle.importKey('raw', new Uint8Array(16).fill(0), {
      name: 'AES-CBC',
      length: 128
    }, false, ['encrypt']);
...
describe('Key management', { skip }, () => {
  it('importKey/exportKey raw round-trip', async (t) => {
    const keyBytes = new Uint8Array(32).fill(119);
    const key = await crypto.subtle.importKey('raw', keyBytes, {
      name: 'AES-GCM',
      length: 256
  ...
  it('generateKey AES rejects invalid usages with SyntaxError', async (t) => {
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'AES-CTR',
      length: 128
    }, true, ['sign'] as KeyUsage[]), rejectsWithDomException('SyntaxError'), 'AES-CTR rejects non-AES key usages');
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'AES-GCM',
      length: 128
    }, true, []), rejectsWithDomException('SyntaxError'), 'AES-GCM rejects empty usages for secret keys');
  });
  it('generateKey AES rejects invalid lengths with OperationError', async (t) => {
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'AES-CTR',
      length: 255
    }, true, []), rejectsWithDomException('OperationError'), 'AES-CTR invalid generated-key length is checked before empty usages');
  });
  ...
  it('generateKey HMAC records explicit and default key lengths', async (t) => {
    const explicit = await crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256',
      length: 160
    }, false, ['sign']) as CryptoKey;
    const defaultSha256 = await crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256'
    }, false, ['sign']) as CryptoKey;
    const defaultSha512 = await crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-512'
    }, false, ['sign']) as CryptoKey;
  ...
  it('generateKey HMAC rejects invalid usages with SyntaxError', async (t) => {
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256'
    }, true, ['encrypt'] as KeyUsage[]), rejectsWithDomException('SyntaxError'), 'HMAC rejects non-HMAC key usages');
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256'
    }, true, []), rejectsWithDomException('SyntaxError'), 'HMAC rejects empty usages for secret keys');
  });
  it('generateKey Ed25519 validates usages and keeps public keys extractable', async (t) => {
    await t.rejects(() => crypto.subtle.generateKey('Ed25519', true, ['encrypt'] as KeyUsage[]), rejectsWithDomException('SyntaxError'), 'Ed25519 rejects non-signature key usages');
    await t.rejects(() => crypto.subtle.generateKey('Ed25519', true, []), rejectsWithDomException('SyntaxError'), 'Ed25519 rejects empty usages for key pairs');
  ...
  it('structuredClone copies symmetric CryptoKey material and metadata', async (t) => {
    const keyBytes = new Uint8Array(32).fill(51);
    const key = await crypto.subtle.importKey('raw', keyBytes, {
      name: 'HMAC',
      hash: 'SHA-256'
    }, false, ['sign', 'verify']);
...
describe('PBKDF2', { skip }, () => {
  ...
  it('deriveKey — produces a usable AES-GCM key', async (t) => {
    ...
    const ct = await crypto.subtle.encrypt({
      name: 'AES-GCM',
      iv
    }, aesKey, enc.encode('hello pbkdf2'));
    const dt = await crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv
    }, aesKey, ct);
...
describe('HKDF', { skip }, () => {
  it('deriveBits — RFC 5869 test case 1 (SHA-256)', async (t) => {
    ...
    const hex = Array.from(new Uint8Array(bits)).map((b) => b.toString(16).padStart(2, '0')).join('');
    t.equal(hex, '3cb25f25faacd57a90434f64d0362f2a2d2d0a90cf1a5a4c5db02d56ecc4c5bf34007208d5b887185865', 'RFC 5869 case 1 OKM');
  });
  it('deriveBits — RFC 5869 test case 2 (SHA-256)', async (t) => {
    ...
    for (let i = 0; i < 80; i++) salt[i] = 96 + i;
    const info = new Uint8Array(80);
    for (let i = 0; i < 80; i++) info[i] = 176 + i;
    const key = await crypto.subtle.importKey('raw', ikm, { name: 'HKDF' }, false, ['deriveBits']);
    const bits = await crypto.subtle.deriveBits({
      name: 'HKDF',
      hash: 'SHA-256',
    ...
    const hex = Array.from(new Uint8Array(bits)).map((b) => b.toString(16).padStart(2, '0')).join('');
    t.equal(hex, 'b11e398dc80327a1c8e7f78c596a49344f012eda2d4efad8a050cc4c19afa97c59045a99cac7827271cb41c65e590e09da3275600c2f09b8367793a9aca3db71cc30c58179ec3e87c14c01d5c1f3434f1d87', 'RFC 5869 case 2 OKM');
  });
  it('deriveKey — produces a usable AES-GCM key', async (t) => {
    const enc = new TextEncoder();
  ...
  it('deriveBits rejects non-byte-aligned and missing HKDF lengths', async (t) => {
    ...
    for (const length of [
      ...
      await t.rejects(() => crypto.subtle.deriveBits(algorithm, key, length as number), rejectsWithDomException('OperationError'), `HKDF length ${length} rejects with OperationError`);
    }
  });
});
// ---------------------------------------------------------------------------
...
describe('HMAC — additional', { skip }, () => {
  it('HMAC with SHA-512', async (t) => {
    const keyBytes = new Uint8Array(64).fill(66);
    const key = await crypto.subtle.importKey('raw', keyBytes, {
      name: 'HMAC',
      hash: 'SHA-512'
    }, false, ['sign', 'verify']);
...
describe('importKey / exportKey — JWK format (symmetric keys)', () => {
  it('AES-GCM 256 key round-trips via JWK', async (t) => {
    ...
    const jwk = await crypto.subtle.exportKey('jwk', key) as {
      kty: string;
      k: string;
      alg: string;
      key_ops: string[];
  ...
  it('HMAC SHA-256 key round-trips via JWK', async (t) => {
    const keyBytes = new Uint8Array(32).fill(171);
    const key = await crypto.subtle.importKey('raw', keyBytes, {
      name: 'HMAC',
      hash: 'SHA-256'
    }, true, ['sign', 'verify']);
  ...
  it('importKey with "spki" format still throws (not yet implemented)', async (t) => {
    ...
    let threw = false;
    try {
      await crypto.subtle.importKey('spki' as any, keyBytes, {
        name: 'AES-GCM',
        length: 256
      }, true, ['encrypt']);
    } catch (_) {
...
describe('wrapKey / unwrapKey', () => {
  it('wraps and unwraps a raw AES key with AES-GCM', async (t) => {
    ...
    const wrappingKey = await crypto.subtle.importKey('raw', wrappingKeyBytes, {
      name: 'AES-GCM',
      length: 256
    }, false, ['wrapKey', 'unwrapKey']);
    const targetKeyBytes = new Uint8Array(32).fill(85);
    const targetKey = await crypto.subtle.importKey('raw', targetKeyBytes, {
      name: 'AES-GCM',
      length: 256
    }, true, ['encrypt']);
    const iv = new Uint8Array(12);
    ...
    let same = true;
    for (let i = 0; i < 32; i++) if (unwrappedBytes[i] !== 85) {
      same = false;
      break;
    }
  ...
  it('wrapKey throws if key is not extractable', async (t) => {
    ...
    let threw = false;
    try {
      await crypto.subtle.wrapKey('raw', nonExtractable, wrappingKey, {
        name: 'AES-GCM',
        iv
      });
    } catch (_) {
  ...
  it('wraps and unwraps via JWK format', async (t) => {
    ...
    const iv = new Uint8Array(12);
    crypto.getRandomValues(iv);
    const wrapped = await crypto.subtle.wrapKey('jwk', targetKey, wrappingKey, {
      name: 'AES-GCM',
      iv
    });
    t.ok(wrapped.byteLength > 0, 'jwk-wrapped key has bytes');
    const unwrapped = await crypto.subtle.unwrapKey('jwk', wrapped, wrappingKey, {
      name: 'AES-GCM',
      iv
    ...
    let same = true;
    for (let i = 0; i < ct1.length; i++) if (ct1[i] !== ct2[i]) {
      same = false;
      break;
    }
...
describe('Key management — additional', { skip }, () => {
  it('exportKey round-trip for HMAC key', async (t) => {
    ...
    const exported = await crypto.subtle.exportKey('raw', key);
    t.ok(exported instanceof ArrayBuffer, 'exported is ArrayBuffer');
    t.deepEqual(Array.from(new Uint8Array(exported)), Array.from(keyBytes), 'exported HMAC key bytes match original');
  });
...
describe('getRandomValues — additional coverage', { skip }, () => {
  ...
  it('works with non-zero byteOffset view', (t) => {
    ...
    t.deepEqual(Array.from(prefix), [
      0,
      0,
      0,
      0
    ], 'bytes before view unchanged');
  });
  it('throws TypeMismatchError DOMException for non-integer views', (t) => {
    t.throws(() => crypto.getRandomValues(new Float32Array(4)), (err) => err instanceof DOMException && err.name === 'TypeMismatchError' && err.code === DOMException.TYPE_MISMATCH_ERR);
    t.throws(() => crypto.getRandomValues((new DataView(new ArrayBuffer(4)) as unknown) as Uint8Array), (err) => err instanceof DOMException && err.name === 'TypeMismatchError' && err.code === DOMException.TYPE_MISMATCH_ERR);
  });
  it('throws QuotaExceededError DOMException above 65536 bytes', (t) => {
    t.throws(() => crypto.getRandomValues(new Uint8Array(65537)), (err) => err instanceof DOMException && err instanceof QuotaExceededError && err.name === 'QuotaExceededError' && err.code === DOMException.QUOTA_EXCEEDED_ERR && (err as {
      requested?: unknown;
    }).requested === null && (err as {
      quota?: unknown;
    }).quota === null);
  });
});
describe('Key usage validation', { skip }, () => {
  it('sign with a verify-only HMAC key throws', async (t) => {
    ...
    await t.rejects(() => crypto.subtle.sign('HMAC', key, new Uint8Array([
      1,
      2,
      3
    ])), undefined, 'sign with verify-only key rejects');
  });
});
...
describe('HKDF — edge cases', { skip }, () => {
  it('HKDF with empty salt and info', async (t) => {
    const enc = new TextEncoder();
    const rawKey = await crypto.subtle.importKey('raw', enc.encode('secret'), { name: 'HKDF' }, false, ['deriveBits']);
    const bits = await crypto.subtle.deriveBits({
      name: 'HKDF',
      hash: 'SHA-256',
      salt: new Uint8Array(0),
...
describe('Key usage enforcement', { skip }, () => {
  it('encrypt with verify-only key throws', async (t) => {
    ...
    const iv = new Uint8Array(12);
    await t.rejects(async () => crypto.subtle.encrypt({
      name: 'AES-GCM',
      iv
    }, key, new Uint8Array(8)), undefined, 'encrypt with decrypt-only key throws');
  });
  it('sign with verify-only HMAC key throws', async (t) => {
    ...
    await t.rejects(async () => crypto.subtle.sign('HMAC', key, new Uint8Array(8)), undefined, 'sign with verify-only key throws');
  });
});
...
describe('crypto.subtle — 192-bit AES key rejection', () => {
  it('importKey rejects 192-bit AES key', async (t) => {
    ...
    await t.rejects(async () => crypto.subtle.importKey('raw', key192, 'AES-GCM', true, ['encrypt', 'decrypt']), /128|256|bits|length/i, '192-bit AES key importKey throws with key size error');
  });
});
describe('WebCrypto release error names', { skip }, () => {
  it('uses NotSupportedError for unsupported algorithms and formats', async (t) => {
    await t.rejects(() => crypto.subtle.digest('MD5' as any, new Uint8Array(1)), rejectsWithName('NotSupportedError'), 'unsupported digest algorithm uses NotSupportedError');
    await t.rejects(() => crypto.subtle.importKey('raw', new Uint8Array(16), 'RSA-OAEP', false, ['encrypt']), rejectsWithName('NotSupportedError'), 'unsupported raw import algorithm uses NotSupportedError');
    await t.rejects(() => crypto.subtle.importKey('der' as any, new Uint8Array(16), 'AES-GCM', false, ['encrypt']), rejectsWithName('NotSupportedError'), 'unsupported key format uses NotSupportedError');
    const aesCtrKey = await crypto.subtle.generateKey({
      name: 'AES-CTR',
      length: 128
    } as any, false, ['encrypt']);
    t.equal(aesCtrKey.algorithm.name, 'AES-CTR', 'AES-CTR key generation is supported for metadata compatibility');
    await t.rejects(() => crypto.subtle.encrypt({
      name: 'AES-CTR',
      counter: new Uint8Array(16),
      length: 64
    } as any, aesCtrKey, new Uint8Array(1)), rejectsWithName('NotSupportedError'), 'AES-CTR encryption remains outside the release baseline');
    await t.rejects(() => crypto.subtle.generateKey({
      name: 'AES-KW',
      length: 128
    } as any, false, ['wrapKey']), rejectsWithName('NotSupportedError'), 'AES-KW remains outside the release baseline');
  });
  it('uses DataError for malformed key data', async (t) => {
    await t.rejects(() => crypto.subtle.importKey('raw', new Uint8Array(24), 'AES-GCM', true, ['encrypt']), rejectsWithName('DataError'), 'invalid AES key length uses DataError');
    await t.rejects(() => crypto.subtle.importKey('jwk', { kty: 'oct' } as JsonWebKey, 'AES-GCM', true, ['encrypt']), rejectsWithName('DataError'), 'malformed symmetric JWK uses DataError');
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      kty: 'EC',
      crv: 'P-256',
      x: 'AQ'
    } as JsonWebKey, {
      name: 'ECDSA',
      namedCurve: 'P-256'
    }, true, ['verify']), rejectsWithName('DataError'), 'malformed EC JWK uses DataError');
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      kty: 'RSA',
      n: 'AQ'
    } as JsonWebKey, {
      name: 'RSA-OAEP',
      hash: 'SHA-256'
    }, true, ['encrypt']), rejectsWithName('DataError'), 'malformed RSA JWK uses DataError');
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      kty: 'OKP',
      crv: 'Ed25519'
    } as JsonWebKey, 'Ed25519', true, ['verify']), rejectsWithName('DataError'), 'malformed OKP JWK uses DataError');
  });
  it('uses InvalidAccessError for disallowed key operations', async (t) => {
    ...
    const hmacSignOnly = await crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256'
    }, false, ['sign']);
    const hmacVerifyOnly = await crypto.subtle.generateKey({
      name: 'HMAC',
      hash: 'SHA-256'
    }, false, ['verify']);
    const aesEncryptOnly = await crypto.subtle.generateKey({
      name: 'AES-GCM',
      length: 128
    }, false, ['encrypt']);
    const aesDecryptOnly = await crypto.subtle.generateKey({
      name: 'AES-GCM',
      length: 128
    }, false, ['decrypt']);
    ...
    const iv = new Uint8Array(12);
    await t.rejects(() => crypto.subtle.sign('HMAC', hmacVerifyOnly, data), rejectsWithName('InvalidAccessError'), 'sign rejects keys without sign usage as InvalidAccessError');
    await t.rejects(() => crypto.subtle.verify('HMAC', hmacSignOnly, new Uint8Array(32), data), rejectsWithName('InvalidAccessError'), 'verify rejects keys without verify usage as InvalidAccessError');
    await t.rejects(() => crypto.subtle.sign('Ed25519', publicSignKey, data), rejectsWithName('InvalidAccessError'), 'sign rejects public signing keys as InvalidAccessError');
    await t.rejects(() => crypto.subtle.verify('Ed25519', ed25519.privateKey, new Uint8Array(64), data), rejectsWithName('InvalidAccessError'), 'verify rejects private verification keys as InvalidAccessError');
    await t.rejects(() => crypto.subtle.encrypt({
      name: 'AES-GCM',
      iv
    }, aesDecryptOnly, data), rejectsWithName('InvalidAccessError'), 'encrypt rejects keys without encrypt usage as InvalidAccessError');
    await t.rejects(() => crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv
    }, aesEncryptOnly, new Uint8Array(16)), rejectsWithName('InvalidAccessError'), 'decrypt rejects keys without decrypt usage as InvalidAccessError');
    await t.rejects(() => crypto.subtle.exportKey('raw', nonExtractable), rejectsWithName('InvalidAccessError'), 'exportKey rejects non-extractable keys as InvalidAccessError');
    await t.rejects(() => crypto.subtle.wrapKey('raw', nonExtractable, wrapKey, {
      name: 'AES-GCM',
      iv
    }), rejectsWithName('InvalidAccessError'), 'wrapKey rejects non-extractable keys as InvalidAccessError');
    await t.rejects(() => crypto.subtle.wrapKey('raw', extractable, aesEncryptOnly, {
      name: 'AES-GCM',
      iv
    }), rejectsWithName('InvalidAccessError'), 'wrapKey rejects wrapping keys without wrapKey usage as InvalidAccessError');
  });
  it('uses NotSupportedError for unsupported derivation algorithms', async (t) => {
    ...
    await t.rejects(() => crypto.subtle.deriveBits({ name: 'AES-GCM' } as any, baseKey, 128), rejectsWithName('NotSupportedError'), 'unsupported deriveBits algorithm uses NotSupportedError');
    await t.rejects(() => crypto.subtle.deriveKey({
      name: 'HKDF',
      hash: 'SHA-256',
      salt: new Uint8Array(0),
      info: new Uint8Array(0)
      ...
      name: 'AES-CTR',
      length: 128
    } as any, true, ['encrypt']), rejectsWithName('NotSupportedError'), 'unsupported deriveKey output algorithm uses NotSupportedError');
  });
  it('uses OperationError for failed decrypt operations', async (t) => {
    ...
    ciphertext[ciphertext.length - 1] ^= 255;
    await t.rejects(() => crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv
    }, key, ciphertext), rejectsWithDomException('OperationError'), 'AES-GCM authentication failure uses a DOMException OperationError');
  });
});
describe('WebCrypto release algorithm matrix', { skip }, () => {
  it('covers required digest, MAC, cipher, KDF, and wrap algorithms', async (t) => {
    ...
    const gcmCt = await crypto.subtle.encrypt({
      name: 'AES-GCM',
      iv: gcmIv
    }, gcm, data);
    t.equal(new TextDecoder().decode(await crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv: gcmIv
    }, gcm, gcmCt)), 'release matrix', 'AES-GCM encrypt/decrypt is available');
    const cbc = await crypto.subtle.generateKey({
      name: 'AES-CBC',
      length: 256
    }, false, ['encrypt', 'decrypt']);
    const cbcIv = crypto.getRandomValues(new Uint8Array(16));
    const cbcCt = await crypto.subtle.encrypt({
      name: 'AES-CBC',
      iv: cbcIv
    }, cbc, data);
    t.equal(new TextDecoder().decode(await crypto.subtle.decrypt({
      name: 'AES-CBC',
      iv: cbcIv
    }, cbc, cbcCt)), 'release matrix', 'AES-CBC encrypt/decrypt is available');
    const pbkdf2 = await crypto.subtle.importKey('raw', data, 'PBKDF2', false, ['deriveBits']);
    t.equal((await crypto.subtle.deriveBits({
      name: 'PBKDF2',
      salt: data,
      iterations: 2,
      hash: 'SHA-256'
    }, pbkdf2, 256)).byteLength, 32, 'PBKDF2 deriveBits is available');
    ...
    t.equal((await crypto.subtle.deriveBits({
      ...
      info: data,
      hash: 'SHA-256'
    }, hkdf, 256)).byteLength, 32, 'HKDF deriveBits is available');
    const wrapped = await crypto.subtle.wrapKey('raw', gcm, gcm, {
      name: 'AES-GCM',
      iv: gcmIv
    });
    const unwrapped = await crypto.subtle.unwrapKey('raw', wrapped, gcm, {
      name: 'AES-GCM',
      iv: gcmIv
...
describe('WebCrypto key usage and JWK rejection edges', { skip }, () => {
  it('wrapKey and unwrapKey require explicit wrapping usages', async (t) => {
    ...
    const wrappingBytes = new Uint8Array(16).fill(90);
    ...
    const iv = new Uint8Array(12);
    await t.rejects(() => crypto.subtle.wrapKey('raw', target, encryptOnly, {
      name: 'AES-GCM',
      iv
    }), /wrapKey/i, 'wrapKey rejects wrapping keys without wrapKey usage');
    const wrapped = await crypto.subtle.wrapKey('raw', target, wrapOnly, {
      name: 'AES-GCM',
      iv
    });
    await t.rejects(() => crypto.subtle.unwrapKey('raw', wrapped, encryptOnly, {
      name: 'AES-GCM',
      iv
    }, { name: 'AES-GCM' }, true, ['encrypt']), /unwrapKey/i, 'unwrapKey rejects unwrapping keys without unwrapKey usage');
    const nonExtractable = await crypto.subtle.unwrapKey('raw', wrapped, unwrapOnly, {
      name: 'AES-GCM',
      iv
    }, { name: 'AES-GCM' }, false, ['encrypt']);
    await t.rejects(() => crypto.subtle.exportKey('raw', nonExtractable), /extractable/i, 'unwrapKey honors requested non-extractability');
  });
  it('rejects incompatible symmetric JWK metadata', async (t) => {
    const valid = await crypto.subtle.exportKey('jwk', await crypto.subtle.importKey('raw', new Uint8Array(16).fill(7), 'AES-GCM', true, ['encrypt'])) as JsonWebKey;
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      ...valid,
      kty: 'RSA'
    } as JsonWebKey, 'AES-GCM', true, ['encrypt']), /kty|oct/i, 'wrong kty rejects');
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      ...valid,
      alg: 'HS256'
    } as JsonWebKey, 'AES-GCM', true, ['encrypt']), /alg|AES-GCM/i, 'wrong alg rejects');
    await t.rejects(() => crypto.subtle.importKey('jwk', {
      ...valid,
      key_ops: ['decrypt']
    } as JsonWebKey, 'AES-GCM', true, ['encrypt']), /key_ops|usage/i, 'incompatible key_ops rejects');
  });
});
