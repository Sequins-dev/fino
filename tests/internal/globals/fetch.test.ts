...
describe('Fetch API WebIDL descriptors', () => {
  ...
  it('exposes a Request signal and brands constant getters', (t) => {
    ...
    for (const name of [
      ...
      t.throws(() => Reflect.get(Request.prototype, name, Request.prototype), TypeError, `${name} getter requires a Request receiver`);
    }
  });
});
...
describe('Basic GET / POST', () => {
  it('basic GET returns 200', async (t) => {
    await withServer(() => new Response('hello world', { status: 200 }), async (url) => {
  ...
  it('response.url is set to the request URL', async (t) => {
    await withServer(() => new Response('ok'), async (url) => {
      const res = await fetch(url);
  ...
  it('resolves relative string URLs against globalThis.location', async (t) => {
    await withServer((req) => new Response(new URL(req.url).pathname), async (url) => {
  ...
  it('response headers are accessible', async (t) => {
    await withServer(() => new Response('body', { headers: { 'x-custom': 'fino' } }), async (url) => {
      const res = await fetch(url);
  ...
  it('response.json() parses JSON body', async (t) => {
    await withServer(() => Response.json({ hello: 'world' }), async (url) => {
      const data = await (await fetch(url)).json();
      t.equal(data.hello, 'world', 'json parsed');
    });
  });
  it('204 No Content has null body', async (t) => {
    await withServer(() => new Response(null, { status: 204 }), async (url) => {
      const res = await fetch(url);
...
describe('Body formData', () => {
  it('preserves a literal UTF-8 BOM in urlencoded field names', async (t) => {
    ...
    const response = new Response(body, { headers: { 'content-type': 'application/x-www-form-urlencoded' } });
  ...
  it('rejects malformed multipart closing boundaries', async (t) => {
    const body = '--Boundary_with_capital_letters\r\n' + 'Content-Type: application/json\r\n' + 'Content-Disposition: form-data; name="does_this_work"\r\n' + '\r\n' + 'YES\r\n' + '--Boundary_with_capital_letters-Random junk';
    const response = new Response(new Blob([body]), { headers: { 'content-type': 'multipart/form-data; boundary=Boundary_with_capital_letters' } });
  ...
  it('defaults multipart file parts without Content-Type to text/plain', async (t) => {
    ...
    const response = new Response(new Blob([body]), { headers: { 'content-type': 'multipart/form-data; boundary="boundary"' } });
    ...
    t.deepEqual(Array.from(new Uint8Array(await file.arrayBuffer())), Array.from(content), 'multipart file bytes are preserved');
  });
});
describe('Request BodyInit', () => {
  ...
  it('accepts ArrayBufferView bodies using their view byte range', async (t) => {
    ...
    t.equal(await new Request('https://example.com/', {
      method: 'POST',
      body: int8 as any
    }).text(), '"hi"');
    t.equal(await new Request('https://example.com/', {
...
describe('Redirects', () => {
  ...
  it('redirect: \'error\' throws on redirect response', async (t) => {
    await withServer((req) => new Response(null, {
      status: 302,
      headers: { location: new URL('/other', req.url).href }
    }), async (url) => {
      await t.rejects(() => fetch(url, { redirect: 'error' }), /redirect/i, 'TypeError on redirect');
    });
  });
  it('redirect: \'manual\' returns opaque redirect response', async (t) => {
    await withServer(() => new Response(null, {
      status: 302,
...
describe('Redirects — Authorization header security', () => {
  it('Authorization header is stripped on cross-origin redirect', async (t) => {
    ...
    const origin1 = serveHttp({ port: 0 }, (_req) => new Response(null, {
      status: 302,
      headers: { location: `http://127.0.0.1:${origin2.port}/` }
    }));
    try {
      await fetch(`http://127.0.0.1:${origin1.port}/`, { headers: { authorization: 'Bearer secret-token' } });
      t.equal(secondRequestHeaders!.get('authorization'), null, 'Authorization header stripped on cross-origin redirect');
    } finally {
      await origin1.close();
      await origin2.close();
    }
  ...
  it('Authorization header is preserved on same-origin redirect', async (t) => {
    ...
    try {
      await fetch(`http://127.0.0.1:${server.port}/first`, { headers: { authorization: 'Bearer secret-token' } });
      t.equal(secondRequestAuth, 'Bearer secret-token', 'Authorization header preserved on same-origin redirect');
    } finally {
      await server.close();
    }
  });
  it('Cookie and Cookie2 headers are stripped on cross-origin redirect', async (t) => {
    ...
    try {
      await fetch(`http://127.0.0.1:${origin1.port}/`, { headers: {
        cookie: 'sid=secret',
        cookie2: '$Version="1"'
      } });
  ...
  it('Cookie and Cookie2 headers are filtered on same-origin redirect', async (t) => {
    ...
    try {
      await fetch(`http://127.0.0.1:${server.port}/first`, { headers: {
        cookie: 'sid=secret',
        cookie2: '$Version="1"'
      } });
...
describe('AbortSignal', () => {
  it('pre-aborted signal rejects immediately', async (t) => {
    ...
    await withServer(() => new Response('never'), async (url) => {
...
describe('Misc', () => {
  ...
  it('accepts a Request object as input', async (t) => {
    await withServer(async (req) => new Response(req.method + ':' + new URL(req.url).pathname), async (url) => {
  ...
  it('rejects malformed Blob object URL ranges', async (t) => {
    for (const range of [
      ...
      try {
        await t.rejects(() => fetch(url, { headers: { range } }), TypeError, `invalid range ${range} rejects`);
      } finally {
        URL.revokeObjectURL(url);
      }
    }
  ...
  it('response.bytes() returns Uint8Array', async (t) => {
    await withServer(() => new Response('binary'), async (url) => {
  ...
  it('does not retain Set-Cookie or cache responses between fetch calls', async (t) => {
    ...
    await withServer((req) => {
      ...
      return new Response(`response-${requestCount}`, { headers: {
  ...
  it('307 redirect rejects a streaming request body instead of replaying it', async (t) => {
    ...
    await withServer((req) => {
      ...
      await t.rejects(() => fetch(url, {
        ...
        body: body() as any,
        duplex: 'half'
      } as any), /streaming|replay/i, 'streaming 307 redirect rejects');
...
describe('fetch — invalid URL', () => {
  ...
  it('fetch with a blocked port throws TypeError before connecting', async (t) => {
    await t.rejects(() => fetch('http://127.0.0.1:25/'), TypeError, 'blocked port rejects with TypeError');
  });
});
describe('Compression', () => {
  ...
  it('304 response is bodyless', async (t) => {
    await withServer(() => new Response(null, { status: 304 }), async (url) => {
      const res = await fetch(url);
...
describe('fetch() — body on GET/HEAD', () => {
  it('Request constructor with GET body throws TypeError', (t) => {
    t.throws(() => new Request('http://127.0.0.1/', {
      method: 'GET',
      body: 'hello'
    }), /body/i, 'GET Request with body throws TypeError');
  });
  it('Request constructor with HEAD body throws TypeError', (t) => {
    t.throws(() => new Request('http://127.0.0.1/', {
      method: 'HEAD',
      body: 'hello'
    }), /body/i, 'HEAD Request with body throws TypeError');
  });
  it('GET request with body throws TypeError', async (t) => {
    await t.rejects(async () => fetch('http://127.0.0.1:19824/', {
      method: 'GET',
      body: 'hello'
    }), /body/i, 'GET with body throws TypeError');
  });
  it('HEAD request with body throws TypeError', async (t) => {
    await t.rejects(async () => fetch('http://127.0.0.1:19824/', {
      method: 'HEAD',
      body: 'hello'
    }), /body/i, 'HEAD with body throws TypeError');
  });
});
describe('Response.redirect() status validation', () => {
  ...
  it('throws RangeError for invalid status codes', (t) => {
    for (const status of [
      ...
      t.throws(() => Response.redirect('https://example.com/', status), /RangeError|invalid/i, `status ${status} throws`);
    }
  });
...
describe('Response constructor validation', () => {
  it('validates status range and statusText bytes', (t) => {
    for (const status of [
      ...
      t.throws(() => new Response('', { status }), RangeError, `status ${status} is outside the Response constructor range`);
    }
    for (const statusText of ['\n', 'Ā']) {
      t.throws(() => new Response('', { statusText }), TypeError, `statusText ${JSON.stringify(statusText)} is invalid`);
    }
  });
  it('rejects bodies for null-body statuses', (t) => {
    for (const status of [
      ...
      t.throws(() => new Response('body', { status }), TypeError, `status ${status} cannot have a body`);
...
describe('Body textStream', () => {
  ...
  it('rejects consumed or locked bodies and always decodes as UTF-8', async (t) => {
    ...
    const response = new Response(bytes, { headers: { 'content-type': 'text/plain; charset=utf-16le' } });
...
describe('fetch() method normalization', () => {
  ...
  it('forbidden methods throw', (t) => {
    for (const method of [
      ...
      t.throws(() => new Request('https://example.com/', { method }), TypeError, `${method} is forbidden`);
    }
  });
  it('invalid method tokens throw', (t) => {
    for (const method of [
      ...
      t.throws(() => new Request('https://example.com/', { method }), TypeError, `${JSON.stringify(method)} is not an HTTP token`);
    }
  });
});
describe('Request init validation', () => {
  it('rejects invalid and credentialed input URLs', (t) => {
    ...
    t.throws(() => new Request('http://user:pass@example.com/'), TypeError, 'credentials in input URL throw');
  });
  it('rejects invalid RequestInit enum values', (t) => {
    for (const option of [
      ...
      t.throws(() => new Request('https://example.com/', { [option]: 'BAD' } as any), TypeError, `${option} rejects invalid values`);
    }
  });
  it('accepts valid priority metadata and rejects invalid fetch priority before network', async (t) => {
    ...
    await t.rejects(() => fetch('http://127.0.0.1:19824/', { priority: 'invalid' as any }), TypeError, 'invalid fetch priority rejects before connecting');
  });
  it('rejects invalid RequestInit combinations', (t) => {
    t.throws(() => new Request('https://example.com/', { window: 'https://example.com/' } as any), TypeError, 'window must be null when provided');
    t.throws(() => new Request('https://example.com/', { mode: 'navigate' } as any), TypeError, 'navigate mode is not allowed for constructed Requests');
    t.throws(() => new Request('https://example.com/', {
      mode: 'no-cors',
      method: 'PUT'
    } as any), TypeError, 'no-cors accepts only simple methods');
    t.throws(() => new Request('https://example.com/', {
      mode: 'cors',
      cache: 'only-if-cached'
    } as any), TypeError, 'only-if-cached requires same-origin mode');
    new Request('test', {
      cache: 'only-if-cached',
      mode: 'same-origin'
    } as any);
  ...
  it('rejects invalid referrer URLs', (t) => {
    t.throws(() => new Request('https://example.com/', { referrer: 'http://:not a valid URL' } as any), TypeError, 'invalid referrer throws');
  });
});
...
describe('Request disturbed state', () => {
  ...
  it('does not disturb input request when construction fails', (t) => {
    ...
    t.throws(() => new Request(input, { method: 'CONNECT' }), TypeError, 'forbidden method fails before body transfer');
  ...
  it('validates streaming request body init options', (t) => {
    ...
    }), TypeError, 'stream body requires duplex half');
    t.throws(() => new Request('https://example.com/', {
      method: 'POST',
      body: new ReadableStream() as any,
      duplex: 'half',
      keepalive: true
    } as any), TypeError, 'keepalive cannot use stream body');
    const locked = new ReadableStream();
    locked.getReader();
    t.throws(() => new Request('https://example.com/', {
      method: 'POST',
      body: locked as any,
      duplex: 'half'
    } as any), TypeError, 'locked stream body is rejected');
  });
...
describe('Integrity + referrerPolicy', () => {
  ...
  it('integrity check fails for wrong hash', async (t) => {
    ...
    const url = `http://127.0.0.1:${srv.port}`;
    try {
      await t.rejects(() => fetch(url, { integrity: 'sha256-wronghash=' }), /integrity/i, 'fetch rejects with TypeError when integrity does not match');
    } finally {
      await srv.close();
    }
  });
...
describe('fetch() — redirect safety', () => {
  it('redirect to javascript: URL throws TypeError', async (t) => {
    ...
    const srv = serveHttp({ port: 0 }, () => new Response(null, {
      status: 302,
      headers: { location: 'javascript:alert(1)' }
    }));
    try {
      await t.rejects(() => fetch(`http://127.0.0.1:${srv.port}/`), /non-HTTP|javascript|redirect/i, 'fetch throws when redirected to javascript: URL');
    } finally {
      await srv.close();
    }
  });
  it('redirect to file: URL throws TypeError', async (t) => {
    ...
    try {
      await t.rejects(() => fetch(`http://127.0.0.1:${srv.port}/`), /non-HTTP|file:|redirect/i, 'fetch throws when redirected to file: URL');
    } finally {
      await srv.close();
    }
  });
