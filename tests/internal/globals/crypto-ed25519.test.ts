...
describe('Ed25519 — sign / verify', { skip }, () => {
  it('generates a key pair, signs, verifies, and rejects tampering', async (t) => {
    ...
    t.equal(
      await crypto.subtle.verify('Ed25519', kp.publicKey, sig, MSG),
      true,
      'signature verifies',
    );
    sig[0] ^= 255;
  ...
  it('enforces key type and usages', async (t) => {
    ...
    const verifyOnly = (await crypto.subtle.generateKey('Ed25519', true, [
      'verify',
    ])) as CryptoKeyPair;
    await t.rejects(
      () => crypto.subtle.sign('Ed25519', signOnly.publicKey, MSG),
      (err) => err instanceof Error,
      'public key cannot sign',
    );
    await t.rejects(
      () => crypto.subtle.verify('Ed25519', signOnly.privateKey, new Uint8Array(64), MSG),
      (err) => err instanceof Error,
      'private key cannot verify',
    );
    await t.rejects(
      () => crypto.subtle.sign('Ed25519', verifyOnly.privateKey, MSG),
      (err) => err instanceof Error,
      'private key without sign usage cannot sign',
    );
  });
});
describe('Ed25519 — import / export', { skip }, () => {
  it('raw public key export/import round-trips', async (t) => {
    ...
    t.equal(
      await crypto.subtle.verify('Ed25519', imported, sig, MSG),
      true,
      'imported raw public key verifies',
    );
  });
  it('rejects raw private export/import and malformed raw public keys', async (t) => {
    ...
    await t.rejects(
      ...
      (err) => err instanceof Error,
      'raw private export is not supported',
    );
    await t.rejects(
      () => crypto.subtle.importKey('raw', new Uint8Array(31), 'Ed25519', true, ['verify']),
      (err) => err instanceof Error,
      'short raw public key rejects',
    );
  });
  it('SPKI and PKCS8 round-trip through sign/verify', async (t) => {
    ...
    t.equal(
      await crypto.subtle.verify('Ed25519', publicKey, sig, MSG),
      true,
      'DER imported keys verify',
    );
  });
