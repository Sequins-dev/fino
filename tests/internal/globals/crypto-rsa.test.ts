...
describe('RSA-OAEP — generateKey', { skip }, () => {
  it('generates 2048-bit key pair with correct metadata', async (t) => {
    ...
    }).modulusLength, 2048, 'modulusLength');
...
describe('RSA-OAEP — encrypt / decrypt', { skip }, () => {
  ...
  it('decrypt with the wrong OAEP label rejects', async (t) => {
    ...
    await t.rejects(() => crypto.subtle.decrypt({
      name: 'RSA-OAEP',
      label: wrongLabel
    }, kp.privateKey, ct), undefined, 'OAEP label mismatch rejects');
...
describe('RSA-OAEP — SPKI / PKCS8 round-trip', { skip }, () => {
  it('SPKI export/import of public key: encrypt still works', async (t) => {
    ...
    const importedPub = await crypto.subtle.importKey('spki', spki, {
      name: 'RSA-OAEP',
      hash: 'SHA-256'
    }, true, ['encrypt']);
...
describe('RSA-PSS — sign / verify', { skip }, () => {
  ...
  it('enforces RSA-PSS sign and verify key usages', async (t) => {
    ...
    const verifyOnly = await crypto.subtle.generateKey({
      ...
      hash: 'SHA-256'
    }, true, ['verify']) as CryptoKeyPair;
    const sig = await crypto.subtle.sign({
      name: 'RSA-PSS',
      saltLength: 32
    }, signOnly.privateKey, MSG);
    await t.rejects(() => crypto.subtle.sign({
      name: 'RSA-PSS',
      saltLength: 32
    }, verifyOnly.privateKey, MSG), /sign/i, 'sign rejects private keys without sign usage');
    await t.rejects(() => crypto.subtle.verify({
      name: 'RSA-PSS',
      saltLength: 32
    }, signOnly.publicKey, sig, MSG), /verify/i, 'verify rejects public keys without verify usage');
  });
});
// ---------------------------------------------------------------------------
