...
describe('Ed25519 — import / export', { skip }, () => {
  ...
  it('OKP JWK public/private export/import round-trips', async (t) => {
    const kp = await crypto.subtle.generateKey('Ed25519', true, ['sign', 'verify']) as CryptoKeyPair;
    const publicJwk = await crypto.subtle.exportKey('jwk', kp.publicKey) as { kty: string; crv: string; x: string; d?: string };
    const privateJwk = await crypto.subtle.exportKey('jwk', kp.privateKey) as { kty: string; crv: string; x: string; d: string };
