  ...
  for (const namedCurve of [
    ...
    it(`generateKey ${namedCurve} returns correct key pair`, async (t) => {
      const kp = await crypto.subtle.generateKey({
        name: 'ECDH',
        namedCurve
      }, true, ['deriveKey', 'deriveBits']) as CryptoKeyPair;
      ...
      t.equal((kp.privateKey.algorithm as {
        namedCurve?: string;
      }).namedCurve, namedCurve, 'namedCurve on private');
      t.equal((kp.publicKey.algorithm as {
        namedCurve?: string;
      }).namedCurve, namedCurve, 'namedCurve on public');
...
describe('ECDH — deriveBits', { skip }, () => {
  ...
  it('deriveBits masks non-byte-aligned output and defaults to full secret', async (t) => {
    ...
    const full = new Uint8Array(await crypto.subtle.deriveBits({
      name: 'ECDH',
      public: kpB.publicKey
    }, kpA.privateKey, (undefined as unknown) as number));
    const partial = new Uint8Array(await crypto.subtle.deriveBits({
      name: 'ECDH',
      public: kpB.publicKey
    }, kpA.privateKey, 230));
    ...
    t.equal(partial[28] & 3, 0, 'unused low bits in final byte are zeroed');
    await t.rejects(() => crypto.subtle.deriveBits({
      name: 'ECDH',
      public: kpB.publicKey
    }, kpA.privateKey, 384), (err: unknown) => err instanceof DOMException && err.name === 'OperationError', 'oversized ECDH length rejects with OperationError');
  });
  it('cross-curve derivation rejects', async (t) => {
    ...
    try {
      ...
      t.ok((err as Error).message.toLowerCase().includes('curve') || (err as Error).message.toLowerCase().includes('match'), 'error mentions curve mismatch: ' + (err as Error).message);
    }
  });
});
// ---------------------------------------------------------------------------
...
describe('ECDH — SPKI public key round-trip', { skip }, () => {
  it('P-256 ECDH public key exports 91-byte SPKI and re-imports', async (t) => {
    ...
    const s1 = await crypto.subtle.deriveBits({
      name: 'ECDH',
      public: kp.publicKey
    }, kpB.privateKey, 256);
    const s2 = await crypto.subtle.deriveBits({
      name: 'ECDH',
      public: imported
    }, kpB.privateKey, 256);
...
describe('ECDH — PKCS8 private key round-trip', { skip }, () => {
  it('P-256 ECDH private key exports as PKCS8 and re-imports', async (t) => {
    ...
    const imported = await crypto.subtle.importKey('pkcs8', pkcs8, {
      name: 'ECDH',
      namedCurve: 'P-256'
    }, false, ['deriveBits']);
...
describe('ECDH — EC JWK export / import', { skip }, () => {
  ...
  it('P-256 ECDH JWK round-trip: derive same secret', async (t) => {
    ...
    const imported = await crypto.subtle.importKey('jwk', jwkPub as BufferSource, {
      name: 'ECDH',
      namedCurve: 'P-256'
    }, true, []);
  ...
  it('P-256 ECDH private key JWK round-trip', async (t) => {
    ...
    const jwkPriv = await crypto.subtle.exportKey('jwk', kpA.privateKey) as {
      d?: string;
    };
