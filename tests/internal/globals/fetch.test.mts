...
describe('fetchLater WebIDL surface', () => {
  ...
  it('validates fetchLater URL and init inputs', (t) => {
    withFetchLaterLocation(() => {
      ...
      for (const url of ['/', 'https://example.com/', 'http://localhost/', 'http://127.0.0.1/', 'http://[::1]/']) {
        ...
        t.throws(() => { result.activated = true; }, TypeError, 'activated is readonly');
      }
      for (const url of ['http://example.com/', 'file://tmp', 'ftp://example.com/', 'wss://example.com/', 'about:blank', 'javascript:alert(1)', 'data:text/plain,Hello', 'blob:https://example.com/id']) {
