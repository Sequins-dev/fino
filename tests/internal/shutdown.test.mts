...
describe('runShutdownHooks — error isolation', () => {
  it('a throwing hook does not prevent subsequent hooks from running', async (t) => {
    ...
    registerShutdownHook(() => { ran.push('first'); });
    registerShutdownHook(() => { throw new Error('hook failure'); });
    registerShutdownHook(() => { ran.push('third'); });
  ...
  it('multiple throwing hooks are collected into an aggregate error', async (t) => {
    registerShutdownHook(() => { throw new Error('err-A'); });
    registerShutdownHook(() => { throw new Error('err-B'); });
    ...
    const agg = caught as Error & { errors?: unknown[] };
...
describe('registerShutdownHook — dispose / deregistration', () => {
  it('dispose() prevents the hook from running', async (t) => {
    let called = false;
    const handle = registerShutdownHook(() => { called = true; });
  ...
  it('dispose() only removes the specific registration, not others', async (t) => {
    ...
    const handleA = registerShutdownHook(() => { ran.push('A'); });
    registerShutdownHook(() => { ran.push('B'); });
