...
describe('properties', () => {
  ...
  it('import.meta.url parses back to the decoded filename', (t) => {
    t.equal(
      decodeURIComponent(new URL(thisUrl).pathname),
      thisFile,
      'url pathname decodes to filename',
    );
  });
});
describe('resolve()', () => {
  ...
  it('import.meta.resolve decodes local file URL paths', async (t) => {
    ...
    t.ok(
      resolved.endsWith('/space%20dir/mod.ts'),
      'percent-encoded file path resolves to encoded file URL',
    );
    t.ok(
      decodeURIComponent(new URL(resolved).pathname).endsWith('/space dir/mod.ts'),
    ...
    t.equal(
      imported.default,
      'decoded-url',
      'dynamic import accepts percent-encoded local file URL',
    );
  });
  it('import.meta file URLs encode reserved path characters', async (t) => {
    ...
    t.equal(
      info.dirname,
      info.filename.slice(0, -('/' + filename).length),
      'import.meta.dirname remains decoded',
    );
    t.ok(
      info.url.includes('meta%20space%20%23query%3Fpercent%25.ts'),
      'import.meta.url encodes reserved path bytes',
    );
    t.equal(
      decodeURIComponent(info.pathname),
      info.filename,
      'URL pathname decodes back to filename',
    );
  ...
  it('import.meta.resolve resolves package-map bare imports and exported subpaths', async (t) => {
    ...
    await writeText(
      fs,
      root + '/.fino/packages/pkg/dist/feature.js',
      'export default "feature";\n',
    );
    await writeText(
      fs,
      root + '/.fino/package-map.json',
      JSON.stringify(
        {
          version: 1,
          root,
    ...
    t.ok(
      lines[0]!.endsWith('/.fino/packages/pkg/index.js'),
      'bare package import resolves to mapped root entrypoint',
    );
    t.ok(lines[1]!.startsWith('file://'), 'exported package subpath resolves to a file URL');
    t.ok(
      lines[1]!.endsWith('/.fino/packages/pkg/dist/feature.js'),
      'exported package subpath resolves to mapped entrypoint',
    );
    t.ok(lines[2]!.startsWith('file://'), 'file URL input resolves to a file URL');
    t.ok(
      lines[2]!.endsWith('/resolve-ok.ts'),
      'file URL input resolves through import.meta.resolve',
    );
    const missing = await runCli(['resolve-missing-record.ts'], root);
    t.equal(missing.code, 1, 'missing package-map record exits nonzero');
    t.ok(
      missing.stderr.includes("missing package-map record for 'missing@1.0.0'"),
      'missing package-map record is reported',
    );
  });
  it('documents explicit Node ESM non-goals for package imports and directory indexes', async (t) => {
    ...
    t.ok(
      packageImports.stderr.includes("Cannot resolve package '#alias'") ||
        packageImports.stderr.includes("Cannot resolve module '#alias'"),
      'package imports failure is explicit',
    );
    ...
    t.ok(
      directoryIndex.stderr.includes("Cannot resolve module './dir'") ||
        directoryIndex.stderr.includes('Is a directory'),
      'directory import failure is explicit',
    );
    ...
    t.ok(
      barePackage.stderr.includes("Cannot resolve package 'pkg'") ||
        barePackage.stderr.includes("Cannot resolve module 'pkg'"),
      'bare package requires package-map support',
    );
  });
});
