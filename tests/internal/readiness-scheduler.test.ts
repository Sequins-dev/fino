...
import {
  ...
  runPooledResidentReadinessWorkloadsAsync,
...
function withTimeout<T>(promise: Promise<T>, message: string): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error(message)), 1e3);
    void promise.then(
      (value) => {
...
describe('readiness-only isolate scheduler', () => {
  it('shares one host loop while reads stay inside each workload', async (t) => {
    ...
    try {
      ...
      t.equal(
        ...
        baselineReads + 2,
        'both isolates register on the host loop',
      );
      ...
      t.equal(
        loop._activeHandleCounts().reads,
        baselineReads,
        'host readiness registrations drained',
      );
    } finally {
  ...
  it('keeps write buffers and retry loops inside the workload', async (t) => {
    ...
    try {
      ...
      const written = await withTimeout(result, 'scheduled write did not resume');
  ...
  it('resumes on the first of several readiness signals', async (t) => {
    ...
    try {
      ...
      t.equal(
        loop._activeHandleCounts().reads,
        baselineReads + 2,
        'both raced reads registered on the host loop',
      );
      socket.send(first.client, new TextEncoder().encode('winner'));
      const winner = await withTimeout(result, 'readiness race waited for every fd');
      t.equal(winner, 'winner', 'first ready fd resumed the workload');
      t.equal(
        loop._activeHandleCounts().reads,
        baselineReads,
        'losing readiness watch was cancelled',
      );
    } finally {
      closeAll(first.peer, first.client, first.server);
  ...
  it('keeps the selected pooled workload entered while it remains highest priority', (t) => {
    ...
    try {
      ...
      const result = runPooledResidentReadinessWorkloads<number>(
        ENTRY,
        [
          {
            fd: pair.peer,
  ...
  it('coordinates pooled readiness from TypeScript on the main realm', async (t) => {
    const pair = connectedPair();
    try {
      const iterations = 8;
      t.equal(socket.send(pair.client, new Uint8Array(iterations).fill(1)), iterations);
      const result = await runPooledResidentReadinessWorkloadsAsync<number>(
        ENTRY,
        [
          {
            fd: pair.peer,
      ...
      t.equal(result.values[0], iterations);
      t.ok(
        result.schedulerReadinessTurns > 0,
        'the TypeScript orchestrator processed readiness turns',
      );
      t.equal(result.isolateEntries, 1, 'the worker retained its current isolate');
      t.equal(result.isolateExits, 1, 'settlement performed the only exit');
    } finally {
      closeAll(pair.peer, pair.client, pair.server);
    }
  });
  it('routes pooled timers through the main TypeScript realm', async (t) => {
    const result = await runPooledResidentReadinessWorkloadsAsync<number>(
      ENTRY,
      [
        {
    ...
    t.equal(result.values[0], 5);
    t.equal(result.isolateEntries, 1);
    t.equal(result.isolateExits, 1);
  });
  it('moves parked workloads between available pool threads', (t) => {
    ...
    try {
      ...
      t.ok(
        result.values.every((value) => value === iterations),
        'every workload completed',
      );
      ...
      t.equal(
        result.isolateEntries,
        result.isolateExits,
        'every cross-thread entry was paired with an exit',
      );
    } finally {
  ...
  it('does not steal later thread-local readiness registrations', (t) => {
    ...
    try {
      ...
      t.equal(
        runPooledResidentReadinessWorkloads<number>(
          ENTRY,
          [
            {
              fd: pooled.peer,
      ...
      t.equal(
        runResidentReadinessWorkload<number>(ENTRY, {
          fd: resident.peer,
          readIterations: 1,
        }).value,
        1,
        'the thread-local reactor retained its own registration',
      );
    } finally {
