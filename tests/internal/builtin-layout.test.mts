...
describe('builtin module layout', () => {
  ...
  it('keeps the public HTTP facade at js/net/http.mts', async (t) => {
    const fs = new DiskFileSystem('/');
    const rootFacade = new URL('../../js/net/http.mts', import.meta.url).pathname;
    const nestedFacade = new URL('../../js/net/http/public.mts', import.meta.url).pathname;
