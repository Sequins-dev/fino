...
describe('fino:ui/web/flow', () => {
  it('starts, renders, signals, resumes, and redirects a workflow-backed page', async (t) => {
    ...
    app.get('/flow').handle(flowPage(approval, {
      ...
      start: () => ({ label: 'deploy' }),
      render(_ctx, state) {
        if (state.status === 'done') return h('p', { id: 'done' }, String((state.result as {
          approved: boolean;
        }).approved));
        return h('form', { method: 'post' }, h('input', {
          type: 'hidden',
          name: 'approval',
          value: 'true'
    ...
    const signaled = await app.handle(new Request(`http://local${location}`, {
      method: 'POST',
