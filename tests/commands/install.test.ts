...
const TEST_DIR = '/tmp/fino-install-test-' + Math.floor(Math.random() * 1e6);
const BASE_PORT = 3e4 + Math.floor(Math.random() * 2e4);
...
async function createPackageTarball(
  fs: DiskFileSystem,
  tarballPath: string,
  files: Record<string, ArchiveInput>,
): Promise<void> {
...
describe('fino install', () => {
  ...
  before(async () => {
    ...
    await createPackageTarball(fs, registryDir + '/pkg-1.0.0.tgz', {
      ...
      'package/index.js':
        'import dep from "dep"; export const value = dep + "+pkg"; export default value;\n',
  ...
  it('installs explicit packages, writes package.json, and resolves bare imports via package map', async (t) => {
    ...
    const install = await runCli(['install', 'pkg@^1.0.0'], appDir, {
    ...
    const packageMap = JSON.parse(
      await fs.readFile(appDir + '/.fino/package-map.json'),
    ) as PackageMapShape;
    t.equal(packageMap.rootDependencies.pkg, 'pkg@1.0.0', 'root dependency locked in package map');
    t.equal(
      packageMap.packages['pkg@1.0.0']!.dependencies.dep,
      'dep@1.0.0',
      'transitive dependency locked',
    );
    const run = await runCli(['index.ts'], appDir, {
      FINO_NPM_REGISTRY: `http://127.0.0.1:${port}`,
    });
  ...
  it('supports latest, exact, caret, and scoped package specs from the command line', async (t) => {
    ...
    const install = await runCli(
      ['install', 'dep@latest', 'fallback-pkg@1.0.0', 'pkg@^1.0.0', '@scope/pkg@^1.0.0'],
      specDir,
      { FINO_NPM_REGISTRY: `http://127.0.0.1:${port}` },
    );
    t.equal(install.result.code, 0, 'install exits successfully');
    ...
    t.equal(
      packageJson.dependencies['fallback-pkg'],
      '1.0.0',
      'exact version is preserved in package.json',
    );
    t.equal(packageJson.dependencies.pkg, '^1.0.0', 'caret range is preserved in package.json');
    t.equal(
      packageJson.dependencies['@scope/pkg'],
      '^1.0.0',
      'scoped package range is preserved in package.json',
    );
    const packageMap = JSON.parse(
      await fs.readFile(specDir + '/.fino/package-map.json'),
    ) as PackageMapShape;
    t.equal(packageMap.rootDependencies.dep, 'dep@1.5.0', 'latest tag resolves through dist-tags');
    t.equal(
      packageMap.rootDependencies['fallback-pkg'],
      'fallback-pkg@1.0.0',
      'exact version resolves',
    );
    t.equal(packageMap.rootDependencies.pkg, 'pkg@1.0.0', 'caret range resolves');
    t.equal(
      packageMap.rootDependencies['@scope/pkg'],
      '@scope/pkg@1.0.0',
      'scoped package range resolves',
    );
  });
  it('supports comparator ranges when resolving transitive dependencies', async (t) => {
    ...
    t.equal(
      packageMap.packages['pkg@1.0.0']!.dependencies.dep,
      'dep@1.5.0',
      'highest comparator match selected',
    );
  });
  it('resolves package-map entrypoints and reports missing records/subpaths', async (t) => {
    ...
    await fs.writeFile(
      manualDir + '/.fino/packages/pkg/package-json.js',
      'export default "pkg-package-json";\n',
    );
    await fs.writeFile(manualDir + '/.fino/packages/pkg/deep.js', 'export default "pkg-deep";\n');
    await fs.writeFile(
      manualDir + '/.fino/package-map.json',
      JSON.stringify(
        {
          version: 1,
          root: manualDir,
    ...
    await fs.writeFile(manualDir + '/missing-record.ts', 'import "missing";\n');
    await fs.writeFile(manualDir + '/missing-subpath.ts', 'import "pkg/not-exported";\n');
    const ok = await runCli(['ok.ts'], manualDir);
    ...
    t.ok(
      ok.stdout.includes('pkg-index|pkg-package-json|pkg-deep'),
      'bare, package JSON, and deep entrypoints resolved',
    );
    const missingRecord = await runCli(['missing-record.ts'], manualDir);
    t.equal(missingRecord.result.code, 1, 'missing package record exits nonzero');
    t.ok(
      missingRecord.stderr.includes("missing package-map record for 'missing@1.0.0'"),
      'missing record is reported',
    );
    const missingSubpath = await runCli(['missing-subpath.ts'], manualDir);
    t.equal(missingSubpath.result.code, 1, 'missing package subpath exits nonzero');
    t.ok(
      missingSubpath.stderr.includes("Cannot resolve package subpath 'pkg/not-exported'"),
      'missing subpath is reported',
    );
  });
  it('warns for optional and peer dependencies without installing peers', async (t) => {
    ...
    t.ok(
      install.stderr.includes('optional dependency failed: missing-optional@^1.0.0'),
      'optional failure warning is printed',
    );
    t.ok(
      install.stderr.includes(
        'peer dependency not installed automatically: optional-parent -> peer-missing',
      ),
      'peer warning is printed',
    );
    const packageMap = JSON.parse(
      await fs.readFile(appDir + '/.fino/package-map.json'),
    ) as PackageMapShape;
    t.equal(
      packageMap.rootDependencies['optional-parent'],
      'optional-parent@1.0.0',
      'parent package is installed',
    );
    t.equal(
      packageMap.packages['optional-parent@1.0.0']!.dependencies['missing-optional'],
      undefined,
      'failed optional dependency is omitted',
    );
    t.equal(
      packageMap.packages['optional-parent@1.0.0']!.dependencies['peer-missing'],
      undefined,
  ...
  it('generates package-map entries for export patterns, conditional exports, fallback entries, and deep imports', async (t) => {
    ...
    t.equal(
      packageMap.packages['exports-pkg@1.0.0']!.entrypoints!['./features/alpha'],
      'dist/alpha.js',
      'exports pattern is expanded',
    );
    t.equal(
      packageMap.packages['exports-pkg@1.0.0']!.entrypoints!['./conditional'],
      'import.js',
      'import condition wins over default',
    );
    t.equal(
      packageMap.packages['fallback-pkg@1.0.0']!.entrypoints!['.'],
      'module.js',
      'module fallback wins over main',
    );
    t.equal(
      packageMap.packages['deep-pkg@1.0.0']!.entrypoints!['./lib/tool'],
      'lib/tool.js',
      'deep import without exports is mapped',
    );
    const run = await runCli(['entrypoints.ts'], appDir);
    ...
    t.ok(
      run.stdout.includes('exports-root|pattern-alpha|import-condition|module-entry|deep-tool'),
      'all generated entrypoints resolve at runtime',
    );
  });
  it('does not implement package imports or # specifiers from installed package metadata', async (t) => {
      ...
      JSON.stringify(
        {
          name: 'fixture-app',
          type: 'module',
          dependencies: { 'imports-pkg': '1.0.0' },
        },
        null,
        2,
      ),
    ...
    await fs.writeFile(
      appDir + '/imports-entry.ts',
      ['import value from "imports-pkg";', 'console.log(value);', ''].join('\n'),
    );
    const install = await runCli(['install'], appDir, {
      FINO_NPM_REGISTRY: `http://127.0.0.1:${port}`,
    });
    ...
    t.equal(
      packageMap.packages['imports-pkg@1.0.0']!.entrypoints!['.'],
      'index.js',
      'root export is mapped',
    );
    t.equal(
      packageMap.packages['imports-pkg@1.0.0']!.entrypoints!['#internal'],
      undefined,
      'package imports are not added to package map',
    );
    const run = await runCli(['imports-entry.ts'], appDir);
    t.equal(run.result.code, 1, 'package # import fails at runtime');
    t.ok(
      run.stderr.includes("Cannot resolve package '#internal'") ||
        run.stderr.includes("Cannot resolve module '#internal'"),
      'unsupported # specifier is reported',
    );
  });
  it('cleans up temporary archive and package directories after corrupt tarball extraction failure', async (t) => {
    ...
    t.equal(
      await exists(fs, appDir + '/.fino/tmp/corrupt-pkg@1.0.0.tgz'),
      false,
      'temporary tarball is removed',
    );
    t.equal(
      await exists(fs, appDir + '/.fino/tmp/corrupt-pkg@1.0.0-extract'),
      false,
      'temporary extract directory is removed',
    );
    t.equal(
      await exists(fs, appDir + '/.fino/packages/corrupt-pkg@1.0.0'),
      false,
      'final package directory is not left behind',
    );
  });
});
