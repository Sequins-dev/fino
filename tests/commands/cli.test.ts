...
describe('CLI commands', () => {
  ...
  it('collects original-source coverage and exposes focused reports', async (t) => {
    await withTempProject(
      ...
      async (dir, fs) => {
        const collected = await runCli(['test', '--coverage', 'src/classify.test.ts'], {
          cwd: dir,
        });
        ...
        t.ok(artifact.realms.length >= 1, 'coverage artifact records participating Realms');
        ...
        const functions = await runCli(['coverage', 'functions', 'src/classify.ts'], { cwd: dir });
        ...
        const branches = await runCli(['coverage', 'branches', 'src/classify.ts'], { cwd: dir });
