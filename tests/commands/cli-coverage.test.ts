    ...
    await withTempProject(
      {
        ...
        'realms.test.ts': [
          "import { describe, it } from 'fino:test/test';",
          "import { Realm, type RealmObservation } from 'fino:realm';",
          ...
          '    const scheduledFrames: RealmObservation[] = [];',
          '    const scheduled = new Realm<typeof double>({',
          '      entry,',
          '      observe: { next: (frame) => scheduledFrames.push(frame) },',
          '    });',
          "    t.equal(await scheduled.call(2), 4, 'scheduled result');",
          '    await scheduled.run();',
          '    t.ok(',
          '      scheduledFrames.some((frame) => frame.kind === EnvelopeKind.Coverage),',
          "      'scheduled coverage crosses the observed channel',",
          '    );',
          '    const processFrames: RealmObservation[] = [];',
          '    const process = new Realm<typeof double>({',
          '      entry,',
          '      process: true,',
          '      observe: { next: (frame) => processFrames.push(frame) },',
          '    });',
          "    t.equal(await process.call(3), 6, 'process result');",
          '    await process.run();',
          '    t.ok(',
          '      processFrames.some((frame) => frame.kind === EnvelopeKind.Coverage),',
          "      'process coverage crosses the observed channel',",
          '    );',
          '  });',
          '});',
          '',
