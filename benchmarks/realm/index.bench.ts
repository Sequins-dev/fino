...
*/
import { DiskFsConfig, Facade, FacadeHandle, ImportMap, SystemDnsConfig, SystemNetConfig } from 'fino:realm';
import { bench } from 'fino:bench';
bench('realm import rules', (b) => {
  b.measure('ImportMap.inherit', () => ImportMap.inherit([{
  ...
  b.measure('ImportMap.deny', () => ImportMap.deny([{
    pattern: 'fino:context',
    directive: 'inherit'
  }]));
});
bench('realm facades and providers', (b) => {
  b.measure('Facade registration', () => {
    new Facade('bench:facade', ['sum']).handle('sum', async (a, b) => Number(a) + Number(b)).stream('chunks', async function* chunks() {
      yield 'chunk';
