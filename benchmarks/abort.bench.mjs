...
bench('AbortController / AbortSignal creation', (b) => {
  ...
  b.measure('controller.signal',      { setup: () => new AbortController(), fn: (ac) => ac.signal });
  b.measure('AbortSignal.abort()',    () => AbortSignal.abort());
  b.measure('AbortSignal.abort(reason)', () => AbortSignal.abort(new Error('cancelled')));
});

bench('abort() cycle', (b) => {
  b.measure('create + abort',     () => { const ac = new AbortController(); ac.abort(); });
  b.measure('create + abort twice', () => { const ac = new AbortController(); ac.abort(); ac.abort(); });
  b.measure('abort with reason',  () => { const ac = new AbortController(); ac.abort(new Error('done')); });
