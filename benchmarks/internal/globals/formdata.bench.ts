...
bench('FormData iteration', (b) => {
  ...
  b.group('by size', (g) => {
    g.measure('for-of 5 entries',  () => { for (const [k, v] of fd5) { /* noop */ void k; void v; } });
    g.measure('for-of 20 entries', () => { for (const [k, v] of fd20) { /* noop */ void k; void v; } });
  });

  b.group('iteration methods', (g) => {
    g.measure('keys()',    () => { for (const k of fd5.keys()) { /* noop */ void k; } });
    g.measure('values()',  () => { for (const v of fd5.values()) { /* noop */ void v; } });
    g.measure('entries()', () => { for (const e of fd5.entries()) { /* noop */ void e; } });
