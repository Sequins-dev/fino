...
bench('EventTarget.dispatchEvent', (b) => {
  ...
  b.group('by listener count', (g) => {
    ...
    t1.addEventListener('test', () => {});
    const t5 = new EventTarget();
    for (let i = 0; i < 5; i++) t5.addEventListener('test', () => {});
    const t10 = new EventTarget();
    for (let i = 0; i < 10; i++) t10.addEventListener('test', () => {});
  ...
  b.measure('wrong event type (miss)', {
    setup: () => {
      const t = new EventTarget();
