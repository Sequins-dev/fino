...
const N = 1e5;
const table = new arrow.Table(arrow.Schema.from({
  id: arrow.int32(),
  value: arrow.float64(),
  label: arrow.utf8()
}), [new arrow.RecordBatch(arrow.Schema.from({
  id: arrow.int32(),
  value: arrow.float64(),
  label: arrow.utf8()
}), [
  arrow.vectorFromArray(Array.from({ length: N }, (_, i) => i), arrow.int32()),
  arrow.vectorFromArray(Array.from({ length: N }, (_, i) => i * 1.5), arrow.float64()),
  arrow.vectorFromArray(Array.from({ length: N }, (_, i) => `item-${i % 100}`), arrow.utf8())
])]);
...
bench('write', (b) => {
  ...
  b.measure('dictionary + zstd (100K x 3 cols)', () => writeParquet(table, {
    compression: 'zstd',
    dictionary: true
  }));
  b.measure('uncompressed (100K x 3 cols)', () => writeParquet(table, { compression: 'uncompressed' }));
});
bench('read', (b) => {
