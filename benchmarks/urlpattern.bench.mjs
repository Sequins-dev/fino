...
bench('URLPattern construction', (b) => {
  b.group('object form', (g) => {
    ...
    g.measure('pathname + hostname',    () => new URLPattern({ pathname: '/api/:version', hostname: '*.example.com' }));
    g.measure('full spec',             () => new URLPattern({ protocol: 'https', hostname: 'example.com', pathname: '/users/:id', search: '?tab=:tab' }));
  });
...
bench('URLPattern.test()', (b) => {
  ...
  const full     = new URLPattern({ hostname: '*.example.com', pathname: '/v:ver/users/:id' });
