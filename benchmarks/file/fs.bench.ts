...
bench('stat', (b) => {
  b.measure('stat file', async () => {
    const fs = new DiskFileSystem();
    await fs.stat('/etc/hosts');
  });
  b.measure('stat directory', async () => {
    const fs = new DiskFileSystem();
    await fs.stat('/tmp');
  });
  b.measure('lstat file', async () => {
    const fs = new DiskFileSystem();
    await fs.lstat('/etc/hosts');
  });
});
bench('open + close', (b) => {
...
bench('readFile', (b) => {
  ...
  b.measure('readFile /etc/hosts', async () => {
    const fs = new DiskFileSystem();
