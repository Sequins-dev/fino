...
const sinkLogger = createLogger({ name: 'bench.sink', level: 'debug', context: { component: 'bench' } });
...
bench('log sinks', (b) => {
  b.measure('json sink write batch', () => {
    const lines: string[] = [];
    const sink = createJsonSink({ level: 'info', write(line) { lines.push(line); } });
  ...
  b.measure('console sink format batch', () => {
    const lines: string[] = [];
    const sink = createConsoleSink({ level: 'debug', write(line) { lines.push(line); } });
  ...
  b.measure('sink level filtering', () => {
    let writes = 0;
    const sink = createJsonSink({ level: 'fatal', write() { writes++; } });
