...
const MEDIUM_CSV = stringify(makeRows(1000, 10));
const LARGE_CSV  = stringify(makeRows(10000, 10));
const QUOTED_CSV = stringify(makeRows(1000, 5).map(r => r.map(v => `${v}, extra`)));
const UNICODE_CSV = (() => {
  const rows = [['lang', 'greeting']];
  const pairs = [['日本語', 'こんにちは'], ['Ελληνικά', 'Γεια σας'], ['中文', '你好'], ['한국어', '안녕하세요']];
...
bench('parseStream', (b) => {
  b.measure('1K rows × 4KB chunks', async () => {
    ...
    for await (const _ of parseStream(src)) { /* drain */ }
  });
