...
function buildAResponse(name: string, ip: string): Uint8Array {
  ...
  // Question section
  buf.set(nameWire, 12);
  view.setUint16(12 + nameWire.length, RECORD_TYPES.A, false);
  view.setUint16(12 + nameWire.length + 2, 1, false);
  // Answer section
  const ansOff = 12 + questionLen;
  view.setUint16(ansOff, 49164, false);
  view.setUint16(ansOff + 2, RECORD_TYPES.A, false);
  view.setUint16(ansOff + 4, 1, false);
  view.setUint32(ansOff + 6, 300, false);
  view.setUint16(ansOff + 10, 4, false);
...
function buildAAAAResponse(name: string): Uint8Array {
  const nameWire = _encodeName(name);
  const questionLen = nameWire.length + 4;
  const answerLen = 2 + 2 + 2 + 4 + 2 + 16;
  const total = 12 + questionLen + answerLen;
  const buf = new Uint8Array(total);
  const view = new DataView(buf.buffer);
  view.setUint16(0, 22136, false);
  view.setUint16(2, 33152, false);
  view.setUint16(4, 1, false);
  view.setUint16(6, 1, false);
  buf.set(nameWire, 12);
...
function buildDnssecResponse(name: string): Uint8Array {
  ...
  for (const record of rdatas) {
    ...
    off += 4;
    view.setUint16(off, record.data.byteLength, false);
    off += 2;
    buf.set(record.data, off);
    off += record.data.byteLength;
  }
  buf[off++] = 0;
...
bench('_buildQuery', (b) => {
  b.group('by record type', (g) => {
    g.measure('A', () => _buildQuery(4660, 'example.com', RECORD_TYPES.A));
    g.measure('AAAA', () => _buildQuery(4660, 'example.com', RECORD_TYPES.AAAA));
