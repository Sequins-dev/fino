...
async function readAll(
  stream: Awaited<ReturnType<import('fino:net/quic').QuicConnection['acceptStream']>>,
): Promise<number> {
  let received = 0;
  for (;;) {
...
async function runChild(label: string, binary: string): Promise<Record<string, unknown>> {
  ...
  const proc = new Process(binary, ['benchmarks/net/quic-loopback-transfer.bench.ts'], {
    env: childEnv,
  });
  proc.stdin.close();
  ...
  if (status.code !== 0)
    throw new Error(
      `${label} benchmark failed with exit ${status.code}${err.length > 0 ? `: ${err}` : ''}`,
    );
  const line = out
    .split(/\r?\n/)
...
if (
  baselineBin !== undefined &&
  baselineBin.length > 0 &&
  candidateBin !== undefined &&
  candidateBin.length > 0
) {
  ...
  try {
    ...
    if (received !== totalBytes)
      throw new Error(`QUIC benchmark received ${received} of ${totalBytes} bytes`);
