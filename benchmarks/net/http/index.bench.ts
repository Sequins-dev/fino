...
*/
import { _buildResponseHead, _headerTokenList, _parseHeaders, _parseResponseLine } from '../../../js/net/http/index.ts';
...
bench('Headers', (b) => {
  b.group('construction', (g) => {
    ...
    g.measure('from object 2', () => new Headers({
      'content-type': 'text/html',
      'x-custom': 'value'
    }));
    g.measure('from object 5', () => new Headers({
  ...
  b.group('iteration', (g) => {
    ...
    g.measure('for-of 10', () => {
      for (const [k, v] of h10) {
        /* noop */ void k;
        void v;
      }
    });
    g.measure('keys()', () => {
      for (const k of h5.keys()) {
        /* noop */ void k;
      }
    });
    g.measure('values()', () => {
      for (const v of h5.values()) {
        /* noop */ void v;
      }
    });
  });
...
bench('Request construction', (b) => {
  ...
  b.measure('GET with headers', () => new Request('https://example.com', { headers: {
    'Authorization': 'Bearer tok',
    'Accept': 'application/json'
  } }));
  b.measure('POST with body', () => new Request('https://api.example.com/data', {
    method: 'POST',
    body: '{"hello":"world"}',
    headers: { 'Content-Type': 'application/json' }
  }));
  b.measure('copy Request', {
    setup: () => new Request('https://example.com/path'),
    fn: (r) => new Request(r)
  });
});
bench('Response construction', (b) => {
  ...
  b.measure('JSON body', () => new Response('{"ok":true}', {
    status: 200,
    headers: { 'Content-Type': 'application/json' }
  }));
  b.measure('Response.json()', () => Response.json({ hello: 'world' }));
  b.measure('Response.error()', () => Response.error());
  b.measure('404 with headers', () => new Response('Not Found', {
    status: 404,
    headers: {
...
bench('HTTP response helpers', (b) => {
  ...
  b.measure('build response head', () => _buildResponseHead(new Response('ok', { headers: { 'content-type': 'text/plain' } })));
});
