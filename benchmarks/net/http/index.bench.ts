...
const GET_HEADERS = encodeUtf8(
  'GET /path HTTP/1.1\r\nHost: example.com\r\nAccept: application/json\r\nAccept-Encoding: gzip, deflate\r\nUser-Agent: surge-bench/1.0\r\nConnection: keep-alive\r\nAuthorization: Bearer tok123\r\n\r\n',
);
const RESP_HEADERS = encodeUtf8(
...
bench('Headers', (b) => {
  ...
  b.group('get', (g) => {
    ...
    const h10 = new Headers(
      Object.fromEntries(Array.from({ length: 10 }, (_, i) => [`x-header-${i}`, `value${i}`])),
    );
  ...
  b.group('iteration', (g) => {
    const h5 = new Headers(
      Object.fromEntries(Array.from({ length: 5 }, (_, i) => [`x-h${i}`, `v${i}`])),
    );
    const h10 = new Headers(
      Object.fromEntries(Array.from({ length: 10 }, (_, i) => [`x-h${i}`, `v${i}`])),
    );
    g.measure('for-of 5', () => {
      for (const [k, v] of h5) {
        /* noop */ void k;
...
bench('HTTP response helpers', (b) => {
  ...
  b.measure('build response head', () =>
    _buildResponseHead(new Response('ok', { headers: { 'content-type': 'text/plain' } })),
  );
});
