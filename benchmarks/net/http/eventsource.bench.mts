 ...
 *
 * Run with: cargo run -- bench benchmarks/net/http/eventsource.bench.mts
 */
...
const source = { async *[Symbol.asyncIterator]() { yield sseBytes; } };

bench('net/http eventsource', (b) => {
  b.measure('EventSourceReader construct', () => new EventSourceReader(source));
  b.measure('EventSourceWriter construct', () => new EventSourceWriter({
    write(chunk: Uint8Array) { return Promise.resolve(chunk.byteLength); },
  }));
});
