...
*/
import { socket, close, encodeAddr, decodeAddr, AF_INET, AF_INET6, AF_UNIX, SOCK_STREAM, SOCK_DGRAM } from 'fino:net/socket';
...
bench('socket() + close()', (b) => {
  b.group('by family', (g) => {
    ...
    g.measure('TCP IPv6', () => {
      const fd = socket(AF_INET6, SOCK_STREAM);
      close(fd);
    });
    g.measure('UDP IPv4', () => {
      const fd = socket(AF_INET, SOCK_DGRAM);
      close(fd);
    });
  });
});
bench('encodeAddr', (b) => {
  b.measure('IPv4 loopback', () => encodeAddr({
    family: 'ipv4',
    ip: '127.0.0.1',
    port: 8080
  }));
  b.measure('IPv4 external', () => encodeAddr({
    family: 'ipv4',
    ip: '93.184.216.34',
    port: 443
  }));
  b.measure('IPv6 loopback', () => encodeAddr({
    family: 'ipv6',
    ip: '::1',
    port: 8080
  }));
  b.measure('IPv6 full', () => encodeAddr({
    family: 'ipv6',
    ip: '2001:db8::1',
    port: 443
  }));
