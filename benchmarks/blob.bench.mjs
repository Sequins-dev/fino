...
bench('Blob construction', (b) => {
  ...
  b.measure('nested Blob',        { setup: () => new Blob(['inner']), fn: (inner) => new Blob([inner, ' outer']) });
...
bench('Blob body reading', (b) => {
  b.measure('text() 16 bytes',    { setup: () => new Blob(['hello, world!!!!!']), fn: async (blob) => await blob.text() });
  b.measure('text() 1KB',         { setup: () => new Blob([KB_STR]), fn: async (blob) => await blob.text() });
  b.measure('text() 64KB',        { setup: () => new Blob([LARGE_STR]), fn: async (blob) => await blob.text() });
  b.measure('arrayBuffer() 1KB',  { setup: () => new Blob([KB_BUF]), fn: async (blob) => await blob.arrayBuffer() });
  b.measure('arrayBuffer() 64KB', { setup: () => new Blob([LARGE_BUF]), fn: async (blob) => await blob.arrayBuffer() });
  b.measure('bytes() 1KB',        { setup: () => new Blob([KB_BUF]), fn: async (blob) => await blob.bytes() });
});
...
bench('File construction', (b) => {
  ...
  b.measure('File properties',    { setup: () => new File(['data'], 'file.txt', { type: 'text/plain' }), fn: (f) => f.name + f.size + f.type });
});
