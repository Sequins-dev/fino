...
const spanRecord: SpanRecord = {
  ...
  endTimeUnixNano: 200,
  attributes: { route: '/bench', tenant: 'acme' },
...
bench('opentelemetry/sdk batching and backpressure', (b) => {
  b.measure('span processor queue pressure', async () => {
    ...
    for (let i = 0; i < 64; i++) {
      processor.onEnd({ ...spanRecord, spanId: String(i).padStart(16, '0') });
    }
  ...
  b.measure('log processor queue pressure', async () => {
    ...
    for (let i = 0; i < 64; i++) {
      processor.onEmit({ ...logRecord, body: `bench log ${i}` });
    }
    await processor.forceFlush();
    await processor.shutdown();
  });
  ...
  b.measure('manual metric reader collect/reset', async () => {
    ...
    for (let i = 0; i < 64; i++) {
      reader.receive([{ ...metricRecord, value: i }]);
      reader.collect();
    }
...
bench('opentelemetry/sdk exporter failure paths', (b) => {
  b.measure('failed span export flush', async () => {
    ...
    for (let i = 0; i < 16; i++) processor.onEnd({ ...spanRecord, spanId: String(i).padStart(16, '0') });
    await processor.forceFlush();
    await processor.shutdown();
  });
  ...
  b.measure('failed log export flush', async () => {
    ...
    for (let i = 0; i < 16; i++) processor.onEmit({ ...logRecord, body: `failed ${i}` });
    await processor.forceFlush();
    await processor.shutdown();
  });
