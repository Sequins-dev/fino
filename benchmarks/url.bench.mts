...
bench('URL setters', (b) => {
  b.measure('pathname',  { setup: () => new URL('https://example.com/old'), fn: (u) => { u.pathname = '/new/path'; } });
  b.measure('hostname',  { setup: () => new URL('https://old.com/path'), fn: (u) => { u.hostname = 'new.com'; } });
  b.measure('search',    { setup: () => new URL('https://example.com/path'), fn: (u) => { u.search = '?a=1&b=2'; } });
});

bench('URLSearchParams', (b) => {
  b.group('construction', (g) => {
    g.measure('from string',  () => new URLSearchParams('a=1&b=2&c=3&d=4&e=5'));
    g.measure('from object',  () => new URLSearchParams({ a: '1', b: '2', c: '3' }));
  ...
  b.group('mutation', (g) => {
    g.measure('append', { setup: () => new URLSearchParams(), fn: (p) => p.append('k', 'v') });
    g.measure('set',    { setup: () => new URLSearchParams('k=old'), fn: (p) => p.set('k', 'new') });
    g.measure('delete', { setup: () => new URLSearchParams('k=v'), fn: (p) => { p.append('k', 'v2'); p.delete('k'); } });
  });
  ...
  b.group('iteration', (g) => {
    g.measure('for-of 5 params', () => { for (const [k, v] of params) { /* noop */ } });
    g.measure('keys',            () => { for (const k of params.keys()) { /* noop */ } });
    g.measure('values',          () => { for (const v of params.values()) { /* noop */ } });
  });
});
