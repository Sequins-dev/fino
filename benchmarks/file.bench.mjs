...
bench('stat', (b) => {
  b.measure('stat file',       async () => { const fs = new DiskFileSystem(loop.current()); await fs.stat('/etc/hosts'); });
  b.measure('stat directory',  async () => { const fs = new DiskFileSystem(loop.current()); await fs.stat('/tmp'); });
  b.measure('lstat file',      async () => { const fs = new DiskFileSystem(loop.current()); await fs.lstat('/etc/hosts'); });
});
...
bench('readFile', (b) => {
  ...
  b.measure('readFile /etc/hosts',   async () => { const fs = new DiskFileSystem(loop.current()); await fs.readFile('/etc/hosts'); });
...
bench('DiskFileSystem construction', (b) => {
  b.measure('new DiskFileSystem', async () => {
    new DiskFileSystem(loop.current());
  });
});
